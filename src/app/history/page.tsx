 ...
 * ------------------------------------------------------------
 * What this page does:
 * 1) Reads entries from localStorage via "@/lib/storage"
 * 2) Shows: entries list + quick stats (7d avg/median) + 30d sparklines (no deps)
 * 3) AI Insights panel:
 *    - API mode: POST /api/insights with { entries }
 *    - Local mode: instant offline summary
 * 4) Tools:
 *    - Reload: re-reads local storage
 *    - Export JSON / CSV
 *    - Import JSON: replaces local store
 *    - Clear data: wipes local store
 *
 * Key rule:
 * - App must stay usable even if AI is slow/offline/broken.
 */
...
function lastNDays(entries: HealthEntry[], n: number) {
...
function spark(values: number[], width = 26) {
  // Tiny ASCII sparkline; fast, no deps.
        ...
        <div style={split}>
          ...
          <section style={card}>
            <div style={sectionHead}>
              ...
              <div style={{ fontSize: 12, opacity: 0.75 }}>Showing newest first (up to 50)</div>
            </div>
                ...
                {entries.slice(0, 50).map((e) => (
                  <div key={e.id} style={row}>
                    ...
                    <div style={metricLine}>
                      ...
                      <Metric label="Notes" value={e.notes ? "✅" : "—"} title={e.notes || ""} />
                    </div>
            ...
            <div style={{ marginTop: 12, fontSize: 12, opacity: 0.7 }}>
              Performance: list is intentionally simple (no chart libs). If we add real charts, we’ll lazy-load them.
            </div>
          </section>
            ...
            <div style={{ display: "grid", gap: 10 }}>
              ...
              <div style={{ display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center" }}>
                ...
                <button
                  ...
                  style={ghostBtn}
                  title="Show raw JSON payload used for AI"
                >
