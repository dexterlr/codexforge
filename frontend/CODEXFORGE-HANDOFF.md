...
The latest family is the controlled end-to-end build/fix workflow release-candidate layer for phases 1242-1257. It keeps `/codexforge-cockpit` as the normal user surface for operator goal, project context, plan summary, file diff, command preview, risk review, approval ticket, apply hold, command hold, evidence, result decision, recovery, audit timeline, operator signoff, and release-candidate review while all real mutation, command execution, provider/model/connector/adapter calls, queue persistence, evidence/result/audit persistence, rollback, retry, recovery, runtime starts, process spawning, port binding, exports, and memory promotion remain blocked until explicit operator approval and future backend-owned guards exist.
