...
The latest completed batch is 1354-1369 - First Tiny Real Controlled Operator Trial. The latest family is the tiny real controlled operator trial layer for phases 1354-1369. It keeps `/codexforge-cockpit` as the normal user surface for the first tiny real controlled trial contract: one sandbox-bounded file write candidate, one allowlisted read-only command candidate, explicit approval ticket, backend execution hold, path guard, command guard, preflight review, evidence/result/audit capture contracts, recovery contract, denied path matrix, operator signoff, first candidate, and release-candidate review while all broad execution, direct frontend file mutation, direct frontend command execution, hidden approvals, approval persistence, queue persistence, lock release, evidence/result/audit persistence, recovery execution, provider/model/connector/adapter calls, runtime starts, port binding, deployment, install, scaffold, secret reads, and memory promotion remain blocked. Latest release candidate: Controlled Tiny Real Operator Trial Release Candidate. Broad execution still blocked. Next likely batch: 1370-1385 - First Real Trial Hardening and Failure Recovery.
