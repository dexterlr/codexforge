...
The latest completed batch is 1370-1385 - First Real Trial Hardening and Failure Recovery. The latest family is the real trial hardening layer for phases 1370-1385. It keeps `/codexforge-cockpit` as the normal user surface for hardening the first tiny real controlled trial: denied path handling, denied command handling, approval expiry handling, dirty workspace handling, preflight failure handling, backend guard mismatch handling, evidence/result/audit capture failure handling, operator stop, manual review, recovery review packet, rollback readiness, retry readiness, and release-candidate review while all broad execution, direct frontend file mutation, direct frontend command execution, hidden approvals, approval persistence, queue persistence, lock release, evidence/result/audit persistence, rollback/retry/recovery execution, provider/model/connector/adapter calls, runtime starts, port binding, deployment, install, scaffold, secret reads, and memory promotion remain blocked. Latest release candidate: Controlled Real Trial Hardening Release Candidate. Broad execution still blocked. Next likely batch: 1386-1401 - Daily-Testable Cockpit MVP Polish.
