...
CodexForge checkpoint documentation now records through phase 1401.

The latest completed batch is 1386-1401 - Daily-Testable Cockpit MVP Polish. The latest family makes `/codexforge-cockpit` the normal user surface with one daily-testable cockpit for Goal, Plan, Files, Commands, Approval, Run State, Evidence, Result, Recovery, Timeline, Safety, Diagnostics, and Daily MVP readiness. Phase pages remain dev test diagnostics only. Latest release candidate: Controlled Daily-Testable Cockpit MVP Release Candidate. Broad execution still blocked: no direct frontend file mutation, direct frontend command execution, hidden approvals, approval persistence, queue persistence, lock release, evidence/result/audit persistence, rollback/retry/recovery execution, provider/model/connector/adapter calls, runtime starts, port binding, deployment, install, scaffold, secret reads, or memory promotion happen from the cockpit. Backend-owned guarded execution and explicit operator approval remain required. Next likely batch: 1402-1417 - Project Context Brain v1.
