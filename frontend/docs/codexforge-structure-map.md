...
CodexForge is a local-first developer workspace checkpointed through detected phase 1849 in the local all-smoke registry. Latest completed batch: 1834-1849 - Strategy Version Review Registry v1. Latest release candidate: Controlled Strategy Version Review Registry Release Candidate. The cockpit now includes safe strategy version review registry previews while strategy version registry remains review-only with synthetic data only. Frontend strategy auto tuning, strategy auto promotion, rule mutation, file writes, version persistence, approval persistence, evidence persistence, broker execution, order placement and dispatch, live market data calls, real P&L calculation, financial advice, personalised recommendations, and buy sell instructions remain blocked. Backend-owned version registry, change workflow, evidence capture, and approval capture remain required, with operator review, risk governor approval, kill switch enforcement, and explicit operator approval still required. Next likely batch: 1850-1865 - Paper Strategy Promotion Gate. This map is for orientation only. It does not rename existing route folders or feature domains.
