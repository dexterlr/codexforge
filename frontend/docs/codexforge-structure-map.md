...
CodexForge is a local-first developer workspace checkpointed through detected phase 1977 in the local all-smoke registry. Latest completed batch: 1962-1977 - Voiceover And Caption Planning Workspace v1. Latest release candidate: Controlled Voiceover And Caption Planning Workspace Release Candidate. The cockpit now includes safe voiceover and caption planning workspace previews below Asset And Shot Planning Workspace, Script And Storyboard Workspace, and Video Creation Domain while voiceover and caption planning remains review-only, video creation remains planning-only, and synthetic data only. Frontend voice generation, voice cloning, audio synthesis, transcription, caption export, subtitle file generation, audio upload/download, final script generation, storyboard image generation, rendering, export, file generation, voice persistence, caption persistence, transcript persistence, audio persistence, asset persistence, rights persistence, prompt persistence, job persistence, approval persistence, provider calls, model calls, connector calls, image generation, video generation, publishing, scheduling, and performance guarantees remain blocked. Backend-owned asset storage, audio storage, render service, export service, provider gateway, rights review, consent review, approval capture, script persistence, storyboard persistence, and caption persistence remain required, with operator review and explicit operator approval still required. Trading workspace remains review-only and no-live-transition. Next likely batch: 1978-1993 - Render Job Boundary v1. This map is for orientation only. It does not rename existing route folders or feature domains.
