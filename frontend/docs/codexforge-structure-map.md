...
CodexForge is a local-first developer workspace checkpointed through detected phase 2153 in the local all-smoke registry. Latest completed batch: 2122-2153 - Artifact Export + Publish Gateway Contract Mega Batch v1. Latest release candidate: Controlled Publish Gateway Contract Release Candidate. The cockpit now includes safe artifact export and publish gateway contract previews below Worker Orchestration Contract while artifact/export/publish contracts remain review-only. No artifact creation from frontend, no artifact persistence from frontend, no export from frontend, no download from frontend, no upload from frontend, no publishing from frontend, no scheduling from frontend, no social API calls from frontend, no token storage from frontend, no account authorization from frontend, no publish persistence from frontend, and no schedule persistence from frontend. Backend-owned artifact storage, export service, publish gateway, account authorization, scheduling gateway, approval capture, rights review, and audit trail remain required. Operator review and explicit operator approval remain required. Video creation workspace remains review-only and planning-only. Trading workspace remains review-only and no-live-transition. Next likely batch: 2154-2185 - Approval Capture + Rights Consent + Audit Ledger Mega Batch v1. This map is for orientation only. It does not rename existing route folders or feature domains.
