...
CodexForge is a local-first developer workspace checkpointed through detected phase 1721 in the local all-smoke registry. Latest completed batch: 1706-1721 - Profit Lockbox + Reinvestment Rules v1. Latest release candidate: Controlled Profit Lockbox Reinvestment Release Candidate. `/codexforge-cockpit` is the normal user surface and now includes safe review-only profit lockbox and reinvestment rule previews; phase pages remain dev test diagnostics only. Profit lockbox and reinvestment rules remain review-only, frontend money movement still blocked, frontend profit withdrawal still blocked, frontend reinvestment execution still blocked, frontend live P&L reads still blocked, frontend broker connection and trade placement still blocked, frontend financial advice still blocked, backend-owned broker boundary remains required, and explicit operator approval remains required. Next likely batch: 1722-1737 - Broker Execution Boundary. This map is for orientation only. It does not rename existing route folders or feature domains.
