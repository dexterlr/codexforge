...
CodexForge is a local-first developer workspace checkpointed through detected phase 1929 in the local all-smoke registry. Latest completed batch: 1914-1929 - Video Creation Domain Boundary v1. Latest release candidate: Controlled Video Creation Domain Boundary Release Candidate. The cockpit now includes safe video creation domain boundary previews below Controlled Paper Trading Workspace while video creation domain remains review-only, planning-only, and synthetic data only. Frontend rendering, export, upload/download, file generation, asset persistence, prompt persistence, job persistence, approval persistence, provider calls, model calls, connector calls, image generation, video generation, voice generation, publishing, scheduling, and performance guarantees remain blocked. Backend-owned asset storage, render service, export service, provider gateway, rights review, and approval capture remain required, with operator review and explicit operator approval still required. Trading workspace remains review-only and no-live-transition. Next likely batch: 1930-1945 - Script And Storyboard Workspace v1. This map is for orientation only. It does not rename existing route folders or feature domains.
