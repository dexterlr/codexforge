...
CodexForge is a local-first developer workspace checkpointed through detected phase 2089 in the local all-smoke registry. Latest completed batch: 2058-2089 - Asset Storage + Audio Storage Contract Mega Batch v1. Latest release candidate: Controlled Audio Storage Contract Release Candidate. The cockpit now includes safe asset storage and audio storage contract previews below Provider Gateway Contract while storage contracts remain review-only. No upload/download from frontend, no media storage from frontend, no asset persistence from frontend, no audio persistence from frontend, no transcript persistence from frontend, no caption persistence from frontend, no consent persistence from frontend, no rights persistence from frontend, no API creation from frontend, no service deployment from frontend, no command execution from frontend, no provider calls from frontend, no model calls from frontend, and no connector calls from frontend. Backend-owned asset storage, audio storage, rights workflow, consent workflow, transcript workflow, caption workflow, access policy, retention policy, audit trail, and approval capture remain required. Operator review and explicit operator approval remain required. Video creation workspace remains review-only and planning-only. Trading workspace remains review-only and no-live-transition. Next likely batch: 2090-2121 - Render Queue + Worker Orchestration Contract Mega Batch v1. This map is for orientation only. It does not rename existing route folders or feature domains.
