...
CodexForge is a local-first developer workspace checkpointed through detected phase 2057 in the local all-smoke registry. Latest completed batch: 2042-2057 - Provider Gateway Contract v1. Latest release candidate: Controlled Provider Gateway Contract Release Candidate. The cockpit now includes safe provider gateway contract previews below Video Backend Service Contract Boundary while provider gateway contract remains review-only, backend service contracts remain review-only, controlled video creation workspace remains review-only, video creation remains planning-only, and synthetic data only. No provider calls from frontend, no model calls from frontend, no connector calls from frontend, no prompt sending from frontend, no credential storage from frontend, no request dispatch from frontend, no response persistence from frontend, no quota mutation from frontend, no audit persistence from frontend, no generation from frontend, no backend implementation from frontend, no API creation from frontend, no service deployment from frontend, and no command execution from frontend. Backend-owned provider gateway remains required, backend-owned credential vault remains required, backend-owned prompt review remains required, backend-owned safety review remains required, backend-owned audit trail remains required, backend-owned approval capture remains required, operator review remains required, and explicit operator approval remains required. Video creation workspace remains review-only and planning-only. Trading workspace remains review-only and no-live-transition. Next likely batch: 2058-2073 - Asset Storage Contract v1. This map is for orientation only. It does not rename existing route folders or feature domains.
