...
CodexForge is a local-first developer workspace checkpointed through detected phase 1865 in the local all-smoke registry. Latest completed batch: 1850-1865 - Paper Strategy Promotion Gate v1. Latest release candidate: Controlled Paper Strategy Promotion Gate Release Candidate. The cockpit now includes safe paper strategy promotion gate previews while paper strategy promotion gate remains review-only with synthetic data only. Frontend strategy auto tuning, strategy auto promotion, rule mutation, file writes, version persistence, approval persistence, evidence persistence, broker execution, order placement and dispatch, paper execution, live execution, live market data calls, real P&L calculation, financial advice, personalised recommendations, and buy sell instructions remain blocked. Backend-owned promotion workflow, version registry, change workflow, evidence capture, and approval capture remain required, with operator review, risk governor approval, kill switch enforcement, and explicit operator approval still required. Next likely batch: 1866-1881 - Paper Trading End-to-End Review Candidate. This map is for orientation only. It does not rename existing route folders or feature domains.
