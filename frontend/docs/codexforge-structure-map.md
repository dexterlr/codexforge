...
CodexForge is a local-first developer workspace checkpointed through detected phase 1961 in the local all-smoke registry. Latest completed batch: 1946-1961 - Asset And Shot Planning Workspace v1. Latest release candidate: Controlled Asset And Shot Planning Workspace Release Candidate. The cockpit now includes safe asset and shot planning workspace previews below Script And Storyboard Workspace and Video Creation Domain while asset and shot planning remains review-only, video creation remains planning-only, and synthetic data only. Frontend upload/download, media storage, final script generation, storyboard image generation, rendering, export, file generation, script persistence, storyboard persistence, asset persistence, rights persistence, prompt persistence, job persistence, approval persistence, provider calls, model calls, connector calls, image generation, video generation, voice generation, publishing, scheduling, and performance guarantees remain blocked. Backend-owned asset storage, render service, export service, provider gateway, rights review, approval capture, script persistence, and storyboard persistence remain required, with operator review and explicit operator approval still required. Trading workspace remains review-only and no-live-transition. Next likely batch: 1962-1977 - Voiceover And Caption Planning Workspace v1. This map is for orientation only. It does not rename existing route folders or feature domains.
