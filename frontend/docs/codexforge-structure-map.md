...
CodexForge is a local-first developer workspace checkpointed through detected phase 1993 in the local all-smoke registry. Latest completed batch: 1978-1993 - Render Job Boundary v1. Latest release candidate: Controlled Render Job Boundary Release Candidate. The cockpit now includes safe render job boundary previews below Voiceover And Caption Planning Workspace, Asset And Shot Planning Workspace, Script And Storyboard Workspace, and Video Creation Domain while render job boundary remains review-only, video creation remains planning-only, and synthetic data only. Frontend rendering, render queue creation, worker dispatch, artifact creation, artifact persistence, export, upload/download, file generation, render job persistence, queue persistence, asset persistence, audio persistence, caption persistence, transcript persistence, rights persistence, prompt persistence, job persistence, approval persistence, provider calls, model calls, connector calls, image generation, video generation, voice generation, publishing, scheduling, and performance guarantees remain blocked. Backend-owned asset storage, audio storage, render service, export service, provider gateway, rights review, consent review, approval capture, script persistence, storyboard persistence, caption persistence, render queue, worker orchestration, and artifact storage remain required, with operator review and explicit operator approval still required. Trading workspace remains review-only and no-live-transition. Next likely batch: 1994-2009 - Video Review And Export Boundary v1. This map is for orientation only. It does not rename existing route folders or feature domains.
