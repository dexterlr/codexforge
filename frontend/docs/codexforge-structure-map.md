...
CodexForge is a local-first developer workspace checkpointed through detected phase 1657 in the local all-smoke registry. Latest completed batch: 1642-1657 - Trading Research Domain Pack v1. Latest release candidate: Controlled Trading Research Domain Release Candidate. `/codexforge-cockpit` is the normal user surface and now includes a safe review-only Trading Research domain pack; phase pages remain dev test diagnostics only. frontend broker connection and trade placement still blocked, frontend live market data calls still blocked, frontend financial advice still blocked, backend-owned broker boundary remains required, and explicit operator approval remains required. Next likely batch: 1658-1673 - Trading Mandate + Risk Governor. This map is for orientation only. It does not rename existing route folders or feature domains.
