...
CodexForge is a local-first developer workspace checkpointed through detected phase 1705 in the local all-smoke registry. Latest completed batch: 1690-1705 - Backtest + Paper Trading Engine v1. Latest release candidate: Controlled Backtest Paper Trading Release Candidate. `/codexforge-cockpit` is the normal user surface and now includes safe review-only backtest and paper trading previews; phase pages remain dev test diagnostics only. frontend backtest execution still blocked, frontend paper trade placement still blocked, frontend executable signals still blocked, frontend broker connection and trade placement still blocked, frontend live market data calls still blocked, frontend financial advice still blocked, backend-owned broker boundary remains required, and explicit operator approval remains required. Next likely batch: 1706-1721 - Profit Lockbox + Reinvestment Rules. This map is for orientation only. It does not rename existing route folders or feature domains.
