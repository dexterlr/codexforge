...
CodexForge is a local-first developer workspace checkpointed through detected phase 1817 in the local all-smoke registry. Latest completed batch: 1802-1817 - Strategy Performance Review Loop v1. Latest release candidate: Controlled Strategy Performance Review Loop Release Candidate. `/codexforge-cockpit` now acts as the one normal user UX with feature-first Trading Workspace, Build Workspace, Approvals, Evidence & Audit, Next Action, and Developer Diagnostics sections; Trading Workspace includes safe broker execution boundary previews, safe paper broker adapter simulator previews, safe paper trading result ledger previews, safe paper trading review dashboard previews, and safe strategy performance review loop previews. Phase pages remain dev test diagnostics only. Main menu hides phase spam, diagnostics remain searchable and directly accessible, no routes were deleted, no smoke coverage was deleted, no execution was enabled, strategy feedback remains review-only and synthetic-only, frontend strategy auto tuning still blocked, frontend strategy auto promotion still blocked, frontend broker execution still blocked, frontend order placement and dispatch still blocked, frontend live market data calls still blocked, frontend real P&L calculation still blocked, frontend financial advice still blocked, frontend personalised recommendations still blocked, frontend buy sell instructions still blocked, frontend evidence persistence still blocked, frontend export/file writes still blocked, backend-owned review workflow remains required, backend-owned evidence capture remains required, operator review remains required, risk governor approval remains required, kill switch enforcement remains required, and explicit operator approval remains required. Next likely batch: 1818-1833 - Strategy Change Control Workflow. This map is for orientation only. It does not rename existing route folders or feature domains.
