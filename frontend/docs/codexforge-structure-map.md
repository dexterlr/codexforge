...
CodexForge is a local-first developer workspace checkpointed through detected phase 1913 in the local all-smoke registry. Latest completed batch: 1898-1913 - Controlled Paper Trading Workspace Release Candidate v1. Latest release candidate: Controlled Paper Trading Workspace Release Candidate. The cockpit now includes safe controlled paper trading workspace release candidate previews below Cockpit Trading Workflow Polish while controlled paper trading workspace remains review-only with synthetic data only. Frontend strategy auto tuning, strategy auto promotion, rule mutation, file writes, version persistence, approval persistence, evidence persistence, queue persistence, worker dispatch, broker execution, order placement and dispatch, paper execution, live execution, live transition, live market data calls, real P&L calculation, financial advice, personalised recommendations, buy sell instructions, and hidden execution affordances remain blocked. Backend-owned paper workflow, promotion workflow, version registry, change workflow, evidence capture, approval capture, execution service, broker adapter, credential vault, and audit trail remain required, with operator review, risk governor approval, kill switch enforcement, and explicit operator approval still required. Next likely batch: 1914-1929 - Video Creation Domain Boundary v1. This map is for orientation only. It does not rename existing route folders or feature domains.
