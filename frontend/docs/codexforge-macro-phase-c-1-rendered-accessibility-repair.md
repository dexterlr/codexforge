...
The required deterministic smoke is `scripts/smoke-codexforge-macro-phase-c-1-rendered-accessibility-repair.ps1`. Its exact 30-path list is retained as the historical C.1 source inventory, and its PowerShell parser loop is bound to the immutable approved creator checkpoint range rather than the later canonical-chat worktree. The current release workflow separately parses every changed or new PowerShell script. The smoke protects provider/runtime/catalog/registry scopes, verifies the catalog digest, preserves historical assertion floors, proves the heading, explanation, and focus-return relationships, and requires aggregate order Macro A → Macro B → Macro C → Macro C.1 → Macro D1 → Macro D2 → canonical Jarvis chat with 75 total, 72 required, and 3 optional entries. The existing `scripts/smoke-codexforge-command-palette.ps1` owns deterministic pre-open focus capture, close-unmount restoration, safe fallback, search focus, focus trapping, and keyboard selection coverage.
