...
Highest detected phase: 1161.

Detection source: `scripts/smoke-codexforge-all.ps1`, parsed by taking the highest `Phase N` entry in the local all-smoke registry. At this checkpoint, all-smoke contains local product phase entries through phase 1161, so the checkpoint is documented as through phase 1161.

Latest detected milestone family: simulated adapter execution dry-run review layer. Phases 1146-1161 add simulated adapter execution preview surfaces after the Controlled Simulated Runtime Release Candidate:
