...
Highest detected phase: 1145.

Detection source: `scripts/smoke-codexforge-all.ps1`, parsed by taking the highest `Phase N` entry in the local all-smoke registry. At this checkpoint, all-smoke contains local product phase entries through phase 1145, so the checkpoint is documented as through phase 1145.

Latest detected milestone family: simulated runtime execution dry-run review layer. Phases 1130-1145 add simulated runtime execution preview surfaces after the Controlled Simulated Command Release Candidate:
