...
Highest detected phase: 1241.

Detection source: `scripts/smoke-codexforge-all.ps1`, parsed by taking the highest `Phase N` entry in the local all-smoke registry. At this checkpoint, all-smoke contains local product phase entries through phase 1241, so the checkpoint is documented as through phase 1241.

Latest detected milestone family: first local change trial layer. Phases 1226-1241 add First Local Change Trial Boundary, Local Change Goal Packet, Local Change Plan Packet, Local Change File Diff Packet, Local Change Command Preview Packet, Local Change Approval Ticket, Local Change Apply Hold, Local Change Command Hold, Local Change Evidence Preview, Local Change Result Preview, Local Change Recovery Preview, Local Change Audit Preview, Local Change Cockpit Trial View, Local Change Denied Path Review, First Approved Local Change Candidate, and Controlled First Local Change Trial Release Candidate:

- These deterministic review/dev surfaces prepare the first approved local project change trial as a cockpit-centered preview-only flow: goal, plan, file diff, command preview, approval ticket, apply hold, command hold, evidence preview, result preview, recovery preview, audit preview, denied path review, candidate, and release-candidate review.
...
- `/codexforge-cockpit` now contains the first approved local change trial preview and remains the preferred normal user surface. Phase pages remain dev/test diagnostics only.
...
Phases 1194-1209 cover the unified cockpit foundation. Phases 1210-1225 cover Cockpit Evidence Result Recovery Boundary, Cockpit Evidence Stream Model, Cockpit File Evidence View, Cockpit Command Evidence View, Cockpit Result Summary Model, Cockpit Result Decision View, Cockpit Recovery Option Model, Cockpit Rollback Preview View, Cockpit Retry Preview View, Cockpit Explain Failure View, Cockpit Audit Trail View, Cockpit Run Timeline View, Cockpit Evidence Export Preview, Cockpit Recovery Safety Gate, First Cockpit Evidence Result Recovery Candidate, and Controlled Cockpit Evidence Result Recovery Release Candidate. Phases 1226-1241 cover First Local Change Trial Boundary through Controlled First Local Change Trial Release Candidate. These surfaces establish `/codexforge-cockpit` as the preferred normal user surface through phase 1241 while keeping phase pages as dev/test diagnostics only. File writes and commands remain blocked until explicit operator approval; no provider, model, connector, runtime, adapter, command, file mutation, approval persistence, evidence persistence, result persistence, audit persistence, export writing, recovery execution, rollback execution, retry execution, queue persistence, scaffold, deployment, package/export, browser credential storage, hidden approval, or automatic memory promotion occurs from the cockpit.
