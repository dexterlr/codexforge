...
Update checkpoint docs to the actual highest detected phase. If phases 794-809 are present and no higher phase exists, document the checkpoint as through phase 809 and describe the backend/local adapter contract layer: backend adapter boundary contract, local bridge adapter boundary contract, file write adapter backend contract, command runner adapter backend contract, local runtime adapter backend contract, evidence store backend contract, result store backend contract, recovery backend contract, packaging backend contract, project scaffold backend contract, adapter backend approval contract, adapter backend audit contract, adapter backend sandbox contract, adapter backend validation contract, adapter backend operator trial contract, and first backend adapter contract candidate. Keep the package review-only, approval-required, backend contract only, not executable from UI, local bridge required, sandbox required, and not adapter-executable until approved backend-owned bounded adapter implementations and evidence exist.
