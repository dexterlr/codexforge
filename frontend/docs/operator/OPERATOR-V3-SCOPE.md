...
- Model router/provider/local bridge release-candidate surfaces cover provider readiness and local model bridge readiness while remaining preview-only, dry-run, and approval-gated. Current checkpoint coverage extends through phase 1577, including the Model Router v2, Provider Approval Gate, and Local Model Bridge v1 layers. `/codexforge-cockpit` is the normal user surface; phase pages remain dev test diagnostics only. These surfaces support build-anything goals across games, apps, websites, dashboards, tools, research, automation, creative, trading, data, documentation, integration, and general project targets while remaining static, preview-only, dry-run-only, approval-gated, denied live execution and denied mutation by default, and blocked from auto-approval, approval decision persistence, provider approval persistence, queue creation, transaction creation, snapshot creation, dry-run execution, execution release, local model calls, model calls, provider calls, connector calls, prompt sending, localhost probing, port binding, credential storage, backend execution, backend adapter execution, domain adapter execution, file writes, file updates, file deletes, file moves, patch applies, apply execution, run execution, arbitrary path crawling, arbitrary local file browsing, auto-open local files, commands, shell escalation, process spawning, environment value display, installs, deploys, runtimes, validation execution, evidence/result/audit persistence, memory persistence from the cockpit, browser storage writes from the cockpit, secret memory from the cockpit, rollback/retry/recovery triggers, packaging, exports, broad execution, direct frontend file mutation, direct frontend command execution, hidden routing, hidden approvals, hidden memory promotion, broad real operator execution, and automatic memory promotion. The latest detected phase is 1577. Latest completed batch: 1562-1577 - Local Model Bridge v1. Latest release candidate: Controlled Local Model Bridge Release Candidate. frontend local model calls still blocked. backend-owned local model bridge remains required. Next likely batch: 1578-1593 - Specialist Worker Registry. Models are workers; CodexForge is the brain with shared memory, knowledge, evidence, result, audit, and approval.
