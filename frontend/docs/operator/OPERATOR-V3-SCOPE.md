...
- Checkpoint through detected phase 1897 in `scripts/smoke-codexforge-all.ps1`.
...
- Model router/provider/local bridge/specialist worker/game server builder/domain pack runner/cockpit domain workspace/trading research/trading mandate/strategy lab/backtest paper trading/profit lockbox reinvestment/cockpit navigation cleanup/broker execution boundary/paper broker adapter simulator/paper trading result ledger/paper trading review dashboard/strategy performance review loop/strategy change control/strategy version registry/paper strategy promotion gate/paper trading end-to-end/cockpit trading workflow polish/controlled paper trading workspace release-candidate/video creation domain boundary/script and storyboard workspace/asset and shot planning workspace/voiceover and caption planning workspace/render job boundary/video review and export boundary/controlled video creation workspace/video backend service contract boundary/provider gateway contract surfaces remain preview-only, dry-run, and approval-gated. Current checkpoint coverage extends through phase 2089, including Controlled Audio Storage Contract Release Candidate. `/codexforge-cockpit` remains the one normal user UX with Start with a goal, Trading Workspace, Build Workspace, Creative Workspace, Approvals, Evidence & Audit, Next Action, and Developer Diagnostics sections; Creative Workspace includes safe asset storage and audio storage contract previews below Provider Gateway Contract. Phase pages remain dev test diagnostics only. No routes were deleted. No smoke coverage was deleted. No execution was enabled. Provider gateway contract remains review-only; controlled video creation workspace remains review-only; video creation remains planning-only; synthetic-only creative data, frontend generation, prompt sending, provider calls, model calls, connector calls, request dispatch, response persistence, quota mutation, audit persistence, credential storage, image generation, video generation, voice generation, final script generation, rendering, render queue creation, worker dispatch, artifact creation, artifact persistence, export, download, upload, publishing, scheduling, file generation, script persistence, storyboard persistence, asset persistence, audio persistence, caption persistence, transcript persistence, rights persistence, prompt persistence, job persistence, render persistence, export persistence, revision persistence, publish persistence, approval persistence, and performance guarantee claims remain blocked. Latest completed batch: 2058-2089 - Asset Storage + Audio Storage Contract Mega Batch v1. Latest release candidate: Controlled Audio Storage Contract Release Candidate. Backend-owned provider gateway, credential vault, prompt review, safety review, audit trail, approval capture, asset storage, audio storage, render service, export service, rights review, consent review, script persistence, storyboard persistence, caption persistence, render queue, worker orchestration, artifact storage, and publish gateway remain required. Operator review and explicit operator approval remain required. Trading workspace remains review-only and no-live-transition. Next likely batch: 2090-2121 - Render Queue + Worker Orchestration Contract Mega Batch v1. Models are workers; CodexForge is the brain with shared memory, knowledge, evidence, result, audit, and approval.
