...
- Checkpoint through detected phase 1129 in `scripts/smoke-codexforge-all.ps1`.
...
- Model router/provider beta release-candidate surfaces cover provider readiness, provider test packets, local model bridge dry-run, provider trial packets, model router trial cockpit, ranking/budget/privacy/shared-context/evidence reviews, first controlled provider trial candidate, Model Router Execution Readiness Candidate, approved provider health check boundary, provider health check request/result packets, local model bridge readiness/context/evidence packets, router trial results, trial summary, regression guard, operator review, first model router beta candidate, and Controlled Model Router Beta Release Candidate while remaining preview-only, dry-run, and approval-gated. The model-routed backend execution review layer covers phases 938-953 through Controlled Backend Model Router Release Candidate. The project-builder MVP review layer covers phases 954-969 through Controlled Project Builder Release Candidate, still static and denied live execution by default. The universal game-builder review layer covers phases 970-985 through Controlled Universal Game Builder Release Candidate, supports any supported game target with safe game adapter families, and remains static, preview-only, approval-gated, and denied live execution by default. The universal project-builder review layer covers phases 986-1001 through Controlled Universal Project Builder Release Candidate, the guided build workflow review layer covers phases 1018-1033 through Controlled Guided Build Workflow Release Candidate, the build plan bundle review layer covers phases 1034-1049 through Controlled Build Plan Bundle Release Candidate, the build plan approval layer covers phases 1050-1065 through Controlled Build Plan Approval Release Candidate, the guarded execution queue preview layer covers phases 1066-1081 through Controlled Guarded Execution Queue Release Candidate, the dry-run execution handoff preview layer covers phases 1082-1097 through Controlled Dry-Run Execution Handoff Release Candidate, the simulated file write dry-run review layer covers phases 1098-1113 through Controlled Simulated File Write Release Candidate, and the simulated command execution dry-run review layer covers phases 1114-1129 through Controlled Simulated Command Release Candidate. These surfaces support build-anything goals across games, apps, websites, dashboards, tools, research, automation, creative, trading, data, documentation, integration, and general project targets while remaining static, preview-only, approval-gated, denied live execution and denied mutation by default, and blocked from auto-approval, approval decision persistence, queue persistence, dry-run execution, lock release, model calls, provider calls, backend adapter execution, domain adapter execution, file writes, file updates, file deletes, file moves, patch applies, arbitrary path crawling, arbitrary local file browsing, auto-open local files, commands, runtimes, validation execution, evidence/result persistence, recovery triggers, packaging, exports, and memory promotion. The latest simulated command phase is 1129. Models are workers; CodexForge is the brain with shared memory, knowledge, evidence, result, audit, and approval.
