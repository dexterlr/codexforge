...
- Checkpoint through detected phase 1401 in `scripts/smoke-codexforge-all.ps1`.
...
- Model router/provider/local bridge/specialist worker/game server builder/domain pack runner/cockpit domain workspace/trading research/trading mandate/strategy lab/backtest paper trading/profit lockbox reinvestment release-candidate surfaces cover provider readiness, local model bridge readiness, specialist worker registry readiness, the first Game Server Builder domain pack, Domain Pack Runner Preview, Cockpit Domain Workspace v1, Trading Research Domain Pack v1, Trading Mandate + Risk Governor v1, Strategy Lab + Signal Engine v1, Backtest + Paper Trading Engine v1, and Profit Lockbox + Reinvestment Rules v1 while remaining preview-only, dry-run, and approval-gated. Current checkpoint coverage extends through phase 1721, including the Model Router v2, Provider Approval Gate, Local Model Bridge v1, Specialist Worker Registry, Game Server Builder, Domain Pack Runner, Cockpit Domain Workspace, Trading Research Domain Pack, Trading Mandate + Risk Governor, Strategy Lab + Signal Engine, Backtest + Paper Trading Engine, and Profit Lockbox + Reinvestment Rules layers. `/codexforge-cockpit` is the normal user surface and now includes safe review-only profit lockbox and reinvestment rule previews; phase pages remain dev test diagnostics only. These surfaces support build-anything goals across games, apps, websites, dashboards, tools, research, automation, creative, trading, data, documentation, integration, and general project targets while remaining static, preview-only, dry-run-only, approval-gated, denied live execution and denied mutation by default, and blocked from auto-approval, approval decision persistence, provider approval persistence, worker decision persistence, queue creation, transaction creation, snapshot creation, dry-run execution, execution release, worker dispatch, domain pack execution, broker connections, live trading, order placement, live market data calls, paper trade placement, backtest execution, money movement, profit withdrawal, reinvestment execution, live P&L reads, automated trading, financial advice, personalised recommendations, buy sell instructions, guaranteed profit claims, local model calls, model calls, provider calls, connector calls, prompt sending, localhost probing, port binding, credential storage, backend execution, backend adapter execution, domain adapter execution, game server starts, Java starts, Docker starts, SteamCMD starts, server jar downloads, plugin installs, mod installs, file writes, generated artifact writes, file updates, file deletes, file moves, patch applies, apply execution, run execution, arbitrary path crawling, arbitrary local file browsing, auto-open local files, commands, shell escalation, process spawning, environment value display, installs, deploys, runtimes, validation execution, evidence/result/audit persistence, memory persistence from the cockpit, browser storage writes from the cockpit, secret memory from the cockpit, rollback/retry/recovery triggers, packaging, exports, broad execution, direct frontend file mutation, direct frontend command execution, hidden routing, hidden approvals, hidden memory promotion, broad real operator execution, and automatic memory promotion. The latest detected phase is 1721. Latest completed batch: 1706-1721 - Profit Lockbox + Reinvestment Rules v1. Latest release candidate: Controlled Profit Lockbox Reinvestment Release Candidate. frontend money movement still blocked. frontend profit withdrawal still blocked. frontend reinvestment execution still blocked. frontend live P&L reads still blocked. frontend broker connection and trade placement still blocked. frontend financial advice still blocked. backend-owned broker boundary remains required. Next likely batch: 1722-1737 - Broker Execution Boundary. Models are workers; CodexForge is the brain with shared memory, knowledge, evidence, result, audit, and approval.
