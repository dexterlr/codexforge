...
- Model router/provider/local bridge/specialist worker/game server builder release-candidate surfaces cover provider readiness, local model bridge readiness, specialist worker registry readiness, and the first Game Server Builder domain pack while remaining preview-only, dry-run, and approval-gated. Current checkpoint coverage extends through phase 1609, including the Model Router v2, Provider Approval Gate, Local Model Bridge v1, Specialist Worker Registry, and Game Server Builder layers. `/codexforge-cockpit` is the normal user surface; phase pages remain dev test diagnostics only. These surfaces support build-anything goals across games, apps, websites, dashboards, tools, research, automation, creative, trading, data, documentation, integration, and general project targets while remaining static, preview-only, dry-run-only, approval-gated, denied live execution and denied mutation by default, and blocked from auto-approval, approval decision persistence, provider approval persistence, worker decision persistence, queue creation, transaction creation, snapshot creation, dry-run execution, execution release, worker dispatch, local model calls, model calls, provider calls, connector calls, prompt sending, localhost probing, port binding, credential storage, backend execution, backend adapter execution, domain adapter execution, game server starts, Java starts, Docker starts, SteamCMD starts, server jar downloads, plugin installs, mod installs, file writes, file updates, file deletes, file moves, patch applies, apply execution, run execution, arbitrary path crawling, arbitrary local file browsing, auto-open local files, commands, shell escalation, process spawning, environment value display, installs, deploys, runtimes, validation execution, evidence/result/audit persistence, memory persistence from the cockpit, browser storage writes from the cockpit, secret memory from the cockpit, rollback/retry/recovery triggers, packaging, exports, broad execution, direct frontend file mutation, direct frontend command execution, hidden routing, hidden approvals, hidden memory promotion, broad real operator execution, and automatic memory promotion. The latest detected phase is 1609. Latest completed batch: 1594-1609 - First Domain Pack Game Server Builder. Latest release candidate: Controlled Game Server Builder Release Candidate. frontend game server execution still blocked. backend-owned game server workflow remains required. Next likely batch: 1610-1625 - Domain Pack Runner Preview. Models are workers; CodexForge is the brain with shared memory, knowledge, evidence, result, audit, and approval.
