...
- Model router/provider/local bridge/specialist worker/game server builder/domain pack runner/cockpit domain workspace/trading research/trading mandate/strategy lab/backtest paper trading/profit lockbox reinvestment/cockpit navigation cleanup/broker execution boundary/paper broker adapter simulator/paper trading result ledger/paper trading review dashboard/strategy performance review loop/strategy change control/strategy version registry/paper strategy promotion gate/paper trading end-to-end/cockpit trading workflow polish/controlled paper trading workspace release-candidate/video creation domain boundary/script and storyboard workspace/asset and shot planning workspace/voiceover and caption planning workspace/render job boundary/video review and export boundary/controlled video creation workspace/video backend service contract boundary/provider gateway contract/asset storage/audio storage/render queue contract/worker orchestration contract surfaces remain preview-only, dry-run, and approval-gated. Current checkpoint coverage extends through phase 2121, including Controlled Worker Orchestration Contract Release Candidate. /codexforge-cockpit remains the one normal user UX and now includes safe render queue and worker orchestration contract previews below Audio Storage Contract. Phase pages remain dev test diagnostics only. No routes were deleted. No smoke coverage was deleted. No execution was enabled. Render and worker contracts remain review-only; synthetic-only creative data, frontend queue creation, job creation, job persistence, worker dispatch, command execution, process spawning, port binding, runtime deployment, service deployment, artifact creation, artifact persistence, telemetry persistence, audit persistence, generation, prompt sending, provider calls, model calls, connector calls, rendering, export, download, upload, publishing, scheduling, file mutation, browser storage writes, and performance guarantee claims remain blocked. Latest completed batch: 2090-2121 - Render Queue + Worker Orchestration Contract Mega Batch v1. Latest release candidate: Controlled Worker Orchestration Contract Release Candidate. Backend-owned render queue, worker orchestration, runtime isolation, sandbox policy, telemetry, failure ledger, artifact storage, approval capture, provider gateway, credential vault, rights review, consent review, and publish gateway remain required. Operator review and explicit operator approval remain required. Trading workspace remains review-only and no-live-transition. Next likely batch: 2122-2153 - Artifact Export + Publish Gateway Contract Mega Batch v1. Models are workers; CodexForge is the brain with shared memory, knowledge, evidence, result, audit, and approval.
