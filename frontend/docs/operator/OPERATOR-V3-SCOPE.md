...
- Model router/provider/local bridge/specialist worker/game server builder/domain pack runner/cockpit domain workspace/trading research/trading mandate/strategy lab/backtest paper trading/profit lockbox reinvestment/cockpit navigation cleanup/broker execution boundary/paper broker adapter simulator/paper trading result ledger/paper trading review dashboard/strategy performance review loop/strategy change control/strategy version registry/paper strategy promotion gate/paper trading end-to-end/cockpit trading workflow polish/controlled paper trading workspace release-candidate/video creation domain boundary/script and storyboard workspace/asset and shot planning workspace/voiceover and caption planning workspace/render job boundary surfaces remain preview-only, dry-run, and approval-gated. Current checkpoint coverage extends through phase 1993, including Render Job Boundary v1. `/codexforge-cockpit` remains the one normal user UX with Start with a goal, Trading Workspace, Build Workspace, Creative Workspace, Approvals, Evidence & Audit, Next Action, and Developer Diagnostics sections; Creative Workspace includes safe video creation domain boundary previews, safe script and storyboard workspace previews, safe asset and shot planning workspace previews, safe voiceover and caption planning workspace previews, and safe render job boundary previews. Phase pages remain dev test diagnostics only. No routes were deleted. No smoke coverage was deleted. No execution was enabled. Render job boundary remains review-only; video creation remains planning-only; synthetic-only creative data, frontend rendering, render queue creation, worker dispatch, artifact creation, artifact persistence, export, upload/download, file generation, render job persistence, queue persistence, asset persistence, audio persistence, caption persistence, transcript persistence, rights persistence, prompt persistence, job persistence, approval persistence, provider calls, model calls, connector calls, image generation, video generation, voice generation, publishing, scheduling, and performance guarantee claims remain blocked. Latest completed batch: 1978-1993 - Render Job Boundary v1. Latest release candidate: Controlled Render Job Boundary Release Candidate. Backend-owned asset storage, audio storage, render service, export service, provider gateway, rights review, consent review, approval capture, script persistence, storyboard persistence, caption persistence, render queue, worker orchestration, and artifact storage remain required. Operator review and explicit operator approval remain required. Trading workspace remains review-only and no-live-transition. Next likely batch: 1994-2009 - Video Review And Export Boundary v1. Models are workers; CodexForge is the brain with shared memory, knowledge, evidence, result, audit, and approval.
