...
- Checkpoint through detected phase 1817 in `scripts/smoke-codexforge-all.ps1`.
...
- Model router/provider/local bridge/specialist worker/game server builder/domain pack runner/cockpit domain workspace/trading research/trading mandate/strategy lab/backtest paper trading/profit lockbox reinvestment/cockpit navigation cleanup/broker execution boundary/paper broker adapter simulator/paper trading result ledger/paper trading review dashboard/strategy performance review loop release-candidate surfaces remain preview-only, dry-run, and approval-gated. Current checkpoint coverage extends through phase 1817, including Strategy Performance Review Loop v1. `/codexforge-cockpit` now acts as the one normal user UX with Start with a goal, Trading Workspace, Build Workspace, Approvals, Evidence & Audit, Next Action, and Developer Diagnostics sections; Trading Workspace includes safe broker execution boundary previews, safe paper broker adapter simulator previews, safe paper trading result ledger previews, safe paper trading review dashboard previews, and safe strategy performance review loop previews. Phase pages remain dev test diagnostics only. No routes were deleted. No smoke coverage was deleted. No execution was enabled. Strategy feedback remains review-only and synthetic-only; frontend strategy auto tuning, strategy auto promotion, broker execution, order placement, order dispatch, live market data calls, real P&L calculation, financial advice, personalised recommendations, buy sell instructions, evidence persistence, export/file writes, money movement, and performance guarantee claims remain blocked. Latest completed batch: 1802-1817 - Strategy Performance Review Loop v1. Latest release candidate: Controlled Strategy Performance Review Loop Release Candidate. Backend-owned review workflow, backend-owned evidence capture, operator review, risk governor approval, kill switch enforcement, and explicit operator approval remain required. Next likely batch: 1818-1833 - Strategy Change Control Workflow. Models are workers; CodexForge is the brain with shared memory, knowledge, evidence, result, audit, and approval.
