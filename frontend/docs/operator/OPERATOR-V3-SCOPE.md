...
- Model router/provider/local bridge/specialist worker/game server builder/domain pack runner/cockpit domain workspace/trading research/trading mandate/strategy lab/backtest paper trading/profit lockbox reinvestment/cockpit navigation cleanup/broker execution boundary/paper broker adapter simulator/paper trading result ledger/paper trading review dashboard/strategy performance review loop/strategy change control/strategy version registry/paper strategy promotion gate/paper trading end-to-end/cockpit trading workflow polish/controlled paper trading workspace release-candidate surfaces remain preview-only, dry-run, and approval-gated. Current checkpoint coverage extends through phase 1913, including Controlled Paper Trading Workspace Release Candidate v1. `/codexforge-cockpit` remains the one normal user UX with Start with a goal, Trading Workspace, Build Workspace, Approvals, Evidence & Audit, Next Action, and Developer Diagnostics sections; Trading Workspace includes safe paper trading end-to-end review previews, safe cockpit trading workflow polish previews, and safe controlled paper trading workspace release candidate previews. Phase pages remain dev test diagnostics only. No routes were deleted. No smoke coverage was deleted. No execution was enabled. Controlled paper trading workspace remains review-only and synthetic-only; frontend strategy auto tuning, strategy auto promotion, rule mutation, file writes, version persistence, approval persistence, evidence persistence, queue persistence, worker dispatch, broker execution, order placement, order dispatch, paper execution, live execution, live transition, live market data calls, real P&L calculation, financial advice, personalised recommendations, buy sell instructions, hidden execution affordances, and performance guarantee claims remain blocked. Latest completed batch: 1898-1913 - Controlled Paper Trading Workspace Release Candidate v1. Latest release candidate: Controlled Paper Trading Workspace Release Candidate. Backend-owned paper workflow, promotion workflow, version registry, change workflow, evidence capture, approval capture, execution service, broker adapter, credential vault, and audit trail remain required. Operator review, risk governor approval, kill switch enforcement, and explicit operator approval remain required. Next likely batch: 1914-1929 - Video Creation Domain Boundary v1. Models are workers; CodexForge is the brain with shared memory, knowledge, evidence, result, audit, and approval.
