...
CodexForge is checkpointed through detected phase 1609 in the local all-smoke registry. The latest completed batch is 1594-1609 - First Domain Pack Game Server Builder. `/codexforge-cockpit` is the normal user surface with Game Server Builder for Server Goal, Server Type, Theme, Minecraft Profile, Plugins, Mods, World Rules, Roles, Permissions, Economy, Quests, Regions, Factions, Files, Commands, Validation, Evidence, Recovery, and Audit. Phase pages remain dev test diagnostics only. Latest release candidate: Controlled Game Server Builder Release Candidate. frontend game server execution still blocked, backend-owned game server workflow remains required, and explicit operator approval remains required. Next likely batch: 1610-1625 - Domain Pack Runner Preview.
