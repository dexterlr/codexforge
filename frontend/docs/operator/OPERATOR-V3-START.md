...
CodexForge is checkpointed through detected phase 1577 in the local all-smoke registry. The latest completed batch is 1562-1577 - Local Model Bridge v1. `/codexforge-cockpit` is the normal user surface with Local Model Bridge v1 for Runtime, Model Registry, Endpoint, Prompt Handoff, Redaction, Capability, Timeout, Cancel, Response, Fallback, Denied, Evidence, Result, and Audit. Phase pages remain dev test diagnostics only. Latest release candidate: Controlled Local Model Bridge Release Candidate. frontend local model calls still blocked, backend-owned local model bridge remains required, and explicit operator approval remains required. Next likely batch: 1578-1593 - Specialist Worker Registry.
