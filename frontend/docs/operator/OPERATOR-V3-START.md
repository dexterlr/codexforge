...
CodexForge is checkpointed through detected phase 1561 in the local all-smoke registry. The latest completed batch is 1546-1561 - Provider Approval Gate. `/codexforge-cockpit` is the normal user surface with Provider Approval for Provider, Model, Prompt, Data Boundary, Privacy, Cost, Capability, Expiry, Replay Protection, Fallback, Denied, Evidence, Result, and Audit. Phase pages remain dev test diagnostics only. Latest release candidate: Controlled Provider Approval Gate Release Candidate. frontend model/provider/connector calls still blocked, backend-owned provider approval remains required, and explicit operator approval remains required. Next likely batch: 1562-1577 - Local Model Bridge v1.
