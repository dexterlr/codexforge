...
CodexForge is checkpointed through detected phase 1529 in the local all-smoke registry. The latest completed batch is 1514-1529 - Release-Grade Audit Trail. `/codexforge-cockpit` is the normal user surface with Release-Grade Audit Trail for Goal, Context, Compiler, Proposal, Approval, Queue, Transaction, Apply, Command, Evidence, Result, Recovery, Memory, Denied Paths, and Operator Timeline. Phase pages remain dev test diagnostics only. Latest release candidate: Controlled Release-Grade Audit Trail Release Candidate. frontend audit persistence still blocked, backend-owned audit capture remains required, and explicit operator approval remains required. Next likely batch: 1530-1545 - Model Router v2.
