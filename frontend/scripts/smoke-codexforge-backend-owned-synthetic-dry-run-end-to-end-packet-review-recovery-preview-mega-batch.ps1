...
Write-Host "=== CodexForge Backend-Owned Synthetic Dry-Run End-to-End Packet Review and Recovery Preview Mega Batch smoke ==="
...
$packetTypesPath = Join-Path $root "src\lib\codexforge\backend-owned-synthetic-dry-run-end-to-end-packet-review-recovery-preview\backend-owned-synthetic-dry-run-end-to-end-packet-review-recovery-preview-types.ts"
$packetCatalogPath = Join-Path $root "src\lib\codexforge\backend-owned-synthetic-dry-run-end-to-end-packet-review-recovery-preview\backend-owned-synthetic-dry-run-end-to-end-packet-review-recovery-preview-catalog.ts"
$packetIndexPath = Join-Path $root "src\lib\codexforge\backend-owned-synthetic-dry-run-end-to-end-packet-review-recovery-preview\index.ts"
...
$frontEndSourceFiles = Get-SourceFiles @(
  ...
  (Join-Path $root "src\lib\codexforge\backend-owned-synthetic-dry-run-end-to-end-packet-review-recovery-preview"),
...
foreach ($needle in @(
  ...
  "5257",
  "Backend-Owned Synthetic Dry-Run End-to-End Packet Review and Recovery Preview"
)) {
...
foreach ($needle in @(
  ...
  "decision state: held / not accepted",
  ...
  "current readiness: end-to-end-packet-review-only / not executable / not persistent",
...
foreach ($needle in @(
  ...
  "End-to-end packet acceptance posture",
  "buildStableEndToEndPacketReviewKey",
  "buildStableEndToEndPacketDecisionReviewKey",
  "buildStableEndToEndPacketStageFailureReviewKey",
  "buildStableEndToEndPacketGateFailureReviewKey",
  "buildStableEndToEndPacketRecoveryPlanKey",
  "buildStableEndToEndPacketRecoveryReadinessChecklistKey",
  "buildStableEndToEndPacketReviewAuditSummaryKey",
  "buildStableEndToEndPacketAcceptancePostureKey",
...
Assert-Contains $athenaPanelSource 'buildScopedItemKey("end-to-end-packet-review-capability"' "AthenaCommandCenterPanel review capability groups use scoped keys"
Assert-Contains $athenaPanelSource 'buildScopedItemKey("end-to-end-packet-review-workspace"' "AthenaCommandCenterPanel review workspace groups use scoped keys"

Assert-NotMatches $athenaPanelSource 'endToEndPacketReviewCapabilityGroups\.map\(\(group,\s*index\)\s*=>\s*\(\s*<span\s+key=\{group\.capabilityFamilyId\}' "AthenaCommandCenterPanel does not use raw repeated capability ids as end-to-end packet review capability keys"
Assert-NotMatches $athenaPanelSource 'endToEndPacketReviewWorkspaceGroups\.map\(\(group,\s*index\)\s*=>\s*\(\s*<span\s+key=\{group\.workspaceTarget\}' "AthenaCommandCenterPanel does not use raw repeated workspace ids as end-to-end packet review workspace keys"
Assert-NotMatches $athenaPanelSource 'endToEndPacketReviewRecords\.map\(\(record\)\s*=>\s*\(\s*<article\s+key=\{record\.(selectedCapabilityFamily\.id|workspaceTarget|label|id)\}' "AthenaCommandCenterPanel review list does not use raw repeated fields as sibling keys"
...
Write-Host "[PASS] Backend-owned synthetic dry-run end-to-end packet review and recovery preview smoke passed."
