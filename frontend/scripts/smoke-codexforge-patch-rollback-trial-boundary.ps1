...
$domain = "src\lib\codexforge\patch-rollback-trial-boundary"
$route = "src\app\patch-rollback-trial"

& (Join-Path $PSScriptRoot "codexforge-local-planning-phase-smoke-helper.ps1") `
  -PhaseName "Phase 276 Patch Rollback Trial Boundary" `
  -ScriptFile "smoke-codexforge-patch-rollback-trial-boundary.ps1" `
  -Domain $domain `
  -Route $route `
  -MainPanel "PatchRollbackTrialBoundaryPanel" `
  -CommandLabel "Go to Patch Rollback Trial Boundary" `
  -Modules @("patch-rollback-trial-boundary-types.ts","patch-rollback-trial-boundary-summary.ts","index.ts") `
  -Components @("PatchRollbackTrialBoundaryPanel.tsx","index.ts") `
  -Exports @("buildPatchRollbackTrialBoundaryStableKey","buildPatchRollbackTrial","buildPatchRollbackTrials","buildPatchRollbackTrialBoundary","buildPatchRollbackTrialBoundaryModel","summarizePatchRollbackTrialBoundary","PATCH_ROLLBACK_TRIAL_BOUNDARY_LANGUAGE") `
  -PlainEnglish @("Patch rollback trial boundary","Rollback is not performed from this page","Approved local boundary is required before rollback","Rollback does not delete unrelated files","Rollback trial identity","Source patch apply result","Rollback reason","Affected files summary","Allowed rollback scope","Denied rollback scope","Recovery checklist","Required confirmation copy","Audit handoff","Blocked reasons","advanced rollback details collapsed/secondary","no automatic local action","no raw fetch from arbitrary UI","no command execution","no shell command execution","no test execution from UI","no git command execution from UI","no runCommand brokerExecution or local executor API calls from UI","no direct Jarvisd call from arbitrary UI","no Jarvisd capability execution from UI","no daemon process creation from frontend","no browser-stored signing secrets","no session token localStorage storage","no arbitrary local file browsing","no arbitrary file read/open","no auto-open local files","no file mutation","no file write","no patch apply behavior","no rollback behavior","no file deletion","no secret value display","no automatic provider send","no provider APIs are called","no GitHub API calls from UI","no localStorage API key storage","no process.env printing","no direct appendEvent call from UI","no direct saveBrainGraph call from UI","no direct graph mutation from UI","no memory auto-promotion","no process kill/restart/shutdown from UI","no package install behavior","no weakened safe path checks","no removed server-only boundaries") `
  -ExtraRoutes @("/patch-apply-result","/file-operation-recovery","/review-inbox","/jarvisd-audit-ingestion","/codebase-change-plan")
...
foreach ($needle in @(
  ...
  Assert-Contains $source $needle "patch rollback trial includes $needle"
}
...
$blockedPatterns = @{
  ...
  "no rollback behavior" = "rollbackPerformedFromPageAllowed:\s*true|rollbackAllowedFromUi:\s*true|rollbackFiles\s*\(|performRollback\s*\("
...
Write-Host "[OK] CodexForge Patch Rollback Trial Boundary smoke passed."
