...
$sharedBoundaryPath = Join-RepoPath "src" "lib" "codexforge" "controlled-provider-dry-run-candidate-map"
...
foreach ($needle in @(
  "ControlledProviderDryRunCandidatePageClientShell",
  "ControlledProviderDryRunCandidateRoutePanel",
...
Assert-NotMatches $source $unsafeApiNamePattern "unsafe camelCase controlled provider dry run API names"
Write-Host ("[OK] " + $SmokeName + " static controlled provider dry run candidate smoke passed.")
