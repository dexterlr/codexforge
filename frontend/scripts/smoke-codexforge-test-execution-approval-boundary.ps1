...
$domain = "src\lib\codexforge\test-execution-approval-boundary"
$route = "src\app\test-execution-approval"

& (Join-Path $PSScriptRoot "codexforge-local-planning-phase-smoke-helper.ps1") `
  -PhaseName "Test Execution Approval Boundary" `
  -ScriptFile "smoke-codexforge-test-execution-approval-boundary.ps1" `
  -Domain $domain `
  -Route $route `
  -MainPanel "TestExecutionApprovalBoundaryPanel" `
  -CommandLabel "Go to Test Execution Approval Boundary" `
  -Modules @("test-execution-approval-boundary-types.ts","test-execution-approval-boundary-summary.ts","index.ts") `
  -Components @("TestExecutionApprovalBoundaryPanel.tsx","index.ts") `
  -Exports @("buildTestExecutionApprovalBoundaryStableKey","buildTestExecutionApproval","buildTestExecutionApprovals","buildTestExecutionApprovalBoundary","buildTestExecutionApprovalBoundaryModel","summarizeTestExecutionApprovalBoundary","TEST_EXECUTION_APPROVAL_BOUNDARY_LANGUAGE") `
  -PlainEnglish @("Test execution approval boundary","Tests are not executed from this page","Future execution remains behind approved local boundary","Approval is required before any test run","Execution request identity","Selected test command","Working directory scope","Workspace trust status","Command approval status","Allowed scope","Denied scope","Timeout policy","Approval copy","Blocked reasons","advanced execution details collapsed/secondary","approved local boundary required","tests are not run automatically","no test execution from UI","nothing reads arbitrary files automatically","nothing executes from arbitrary UI","no automatic local action","no raw fetch from arbitrary UI","no command execution","no shell command execution","no arbitrary local file browsing","no arbitrary file read/open","no file mutation","no file deletion","no patch apply behavior","no package install behavior","no direct Jarvisd call from arbitrary UI","no Jarvisd capability execution from arbitrary UI","no provider APIs are called","no automatic provider send","no secrets displayed","no secrets exported","no secrets included","no localStorage API key storage","no process.env printing","no direct appendEvent call from UI","no direct saveBrainGraph call from UI","no direct graph mutation from UI","no memory auto-promotion") `
  -ExtraRoutes @("/test-command-planner","/local-command-approval","/workspace-trust-policy","/jarvisd-permissions","/test-result-summary")
...
foreach ($needle in @(
  ...
  Assert-Contains $source $needle "test execution approval boundary includes $needle"
}
...
$blockedPatterns = @{
  ...
  "no direct Jarvisd call from arbitrary UI" = "jarvisdDirectCallAllowedFromUi:\s*true|jarvisdCapabilityExecutionAllowedFromUi:\s*true|callJarvisd\s*\(|jarvisdClient\s*\.|executeJarvisdCapability\s*\("
...
Write-Host "[OK] CodexForge Test Execution Approval Boundary smoke passed."
