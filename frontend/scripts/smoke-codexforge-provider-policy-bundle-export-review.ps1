...
foreach ($needle in @(
  ...
  Assert-Contains $source $needle "provider policy bundle export review includes $needle"
}
...
$blockedPatterns = @{
  ...
  "no API key export" = "apiKeyExportAllowed:\s*true|policyBundleSecretsExportAllowed:\s*true|exportApiKey\s*\("
  "no secret export" = "secretExportAllowed:\s*true|policyBundleSecretsExportAllowed:\s*true|exportSecret\s*\("
  "no provider credentials export" = "providerCredentialsExportAllowed:\s*true|providerCredentialsIncludedAllowed:\s*true|exportProviderCredentials\s*\("
...
Write-Host "[OK] CodexForge Provider Policy Bundle Export Review smoke passed."
