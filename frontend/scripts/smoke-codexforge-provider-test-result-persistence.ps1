...
$domain = "src\lib\codexforge\provider-test-result-persistence"
$route = "src\app\provider-test-result-persistence"

& (Join-Path $PSScriptRoot "codexforge-local-planning-phase-smoke-helper.ps1") `
  -PhaseName "Phase 292 Provider Test Result Persistence" `
  -ScriptFile "smoke-codexforge-provider-test-result-persistence.ps1" `
  -Domain $domain `
  -Route $route `
  -MainPanel "ProviderTestResultPersistencePanel" `
  -CommandLabel "Go to Provider Test Result Persistence" `
  -Modules @("provider-test-result-persistence-types.ts","provider-test-result-persistence-summary.ts","index.ts") `
  -Components @("ProviderTestResultPersistencePanel.tsx","index.ts") `
  -Exports @("buildProviderTestResultPersistenceStableKey","buildProviderTestResultPersistenceRecord","buildProviderTestResultPersistenceRecords","buildProviderTestResultPersistenceBoundary","buildProviderTestResultPersistenceModel","summarizeProviderTestResultPersistence","PROVIDER_TEST_RESULT_PERSISTENCE_LANGUAGE") `
  -PlainEnglish @("Provider test result persistence","Provider test results are reviewed before use","Raw responses stay secondary","Memory is not auto-promoted","API keys and secrets are not persisted","Review inbox handoff","Persistence identity","Source live test trial","Provider/model summary","Result status: passed, failed, blocked, timed out, needs review","Response summary","Redaction status","Cost/latency summary","Retention policy","Calibration route","Blocked reasons","provider tests require explicit approval","advanced result details collapsed/secondary","server-only path boundary markers remain intact","no automatic local action","no raw fetch from arbitrary UI","no command execution","no shell command execution","no test execution from UI","no git command execution from UI","no direct Jarvisd call from arbitrary UI","no Jarvisd capability execution from UI","no daemon process creation from frontend","no browser-stored signing secrets","no session token localStorage storage","no arbitrary local file browsing","no arbitrary path crawling","no arbitrary file read/open","no auto-open local files","no file mutation","no file write","no patch apply behavior","no file deletion","no secret value display","no automatic provider send","no provider APIs are called","no Anthropic API calls","no OpenAI-compatible API calls","no API request sent","no prompt/file sending without approval","no auto-spend tokens","no auto-route live provider traffic","no silent provider registry mutation","no localStorage API key storage","no process.env printing","no direct appendEvent call from UI","no direct saveBrainGraph call from UI","no direct graph mutation from UI","no memory auto-promotion","no process kill/restart/shutdown from UI","no package install behavior") `
  -ExtraRoutes @("/provider-test-results","/review-inbox","/provider-cost-latency-calibration","/anthropic-live-test-trial","/multi-provider-live-test-trial")
...
foreach ($needle in @(
  ...
  Assert-Contains $source $needle "provider test result persistence includes $needle"
}
...
Write-Host "[OK] CodexForge Provider Test Result Persistence smoke passed."
