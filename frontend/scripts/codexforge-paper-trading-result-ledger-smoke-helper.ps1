...
$sourceParts = @()
foreach ($scanRoot in @("src\lib\codexforge\paper-trading-result-ledger", $Domain, $Route, (Join-Path "scripts" $ScriptFile))) {
...
Assert-Contains $source "PaperTradingResultLedgerRoutePanel" "main route panel"
Assert-Contains $source "PaperTradingResultLedgerCockpitSummaryPanel" "cockpit paper trading result ledger summary panel"
Assert-Contains $cockpitPanel "PaperTradingResultLedgerCockpitSummaryPanel" "cockpit navigation cleanup ledger summary"
Assert-Contains $unifiedCockpit "PaperTradingResultLedgerCockpitSummaryPanel" "unified cockpit ledger summary"
...
Write-Host "[OK] $SmokeName static paper trading result ledger smoke passed."
