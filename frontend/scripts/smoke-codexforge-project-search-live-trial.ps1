...
$domain = "src\lib\codexforge\project-search-live-trial"
$route = "src\app\project-search-live-trial"

& (Join-Path $PSScriptRoot "codexforge-local-planning-phase-smoke-helper.ps1") `
  -PhaseName "Phase 279 Project Search Live Trial" `
  -ScriptFile "smoke-codexforge-project-search-live-trial.ps1" `
  -Domain $domain `
  -Route $route `
  -MainPanel "ProjectSearchLiveTrialPanel" `
  -CommandLabel "Go to Project Search Live Trial" `
  -Modules @("project-search-live-trial-types.ts","project-search-live-trial-summary.ts","index.ts") `
  -Components @("ProjectSearchLiveTrialPanel.tsx","index.ts") `
  -Exports @("buildProjectSearchLiveTrialStableKey","buildProjectSearchLiveTrial","buildProjectSearchLiveTrials","buildProjectSearchLiveTrialBoundary","buildProjectSearchLiveTrialModel","summarizeProjectSearchLiveTrial","PROJECT_SEARCH_LIVE_TRIAL_LANGUAGE") `
  -PlainEnglish @("Project search live trial","Search only uses approved bounded indexed workspace data","Arbitrary local browsing is not allowed","Sensitive matches stay redacted until review","Search trial identity","Source index trial","Query summary","Approved search scope","Excluded paths summary","Result preview summary","Sensitive match handling","Redaction status","File preview bridge route","Audit handoff","Server search route","Blocked reasons","approved bounded indexed workspace data required","server-only path boundary markers remain intact","no automatic local action","no raw fetch from arbitrary UI","no command execution","no shell command execution","no test execution from UI","no git command execution from UI","no direct Jarvisd call from arbitrary UI","no Jarvisd capability execution from UI","no daemon process creation from frontend","no browser-stored signing secrets","no session token localStorage storage","no arbitrary local file browsing","no arbitrary local browsing","no arbitrary path crawling","no arbitrary file read/open","no auto-open local files","no file mutation","no file write","no patch apply behavior","no file deletion","no secret value display","no automatic provider send","no provider APIs are called","no GitHub API calls from UI","no localStorage API key storage","no process.env printing","no audit log mutation from UI","no direct appendEvent call from UI","no direct saveBrainGraph call from UI","no direct graph mutation from UI","no memory auto-promotion","no process kill/restart/shutdown from UI","no package install behavior","no weakened safe path checks","no removed server-only boundaries") `
  -ExtraRoutes @("/project-indexer-live-trial","/project-file-search","/jarvisd-file-preview-bridge","/review-inbox","/project-risk-scan-live-trial")
...
foreach ($needle in @(
  ...
  Assert-Contains $source $needle "project search live trial includes $needle"
}
...
$blockedPatterns = @{
  ...
  "no arbitrary local browsing or path crawling" = "showOpenFilePicker|browseLocalFiles\s*\(|arbitraryLocalBrowsingAllowed:\s*true|arbitraryPathCrawlingAllowed:\s*true|input\s+type=.*file"
...
$safePathSource = (
  ...
  Get-Content -Raw "src\app\api\codexforge\project\read\route.ts"
)
foreach ($marker in @("import `"server-only`";","resolveCodexForgeProjectPath","isAbsolutePathInsideBase","Path traversal guard","file size cap","binary guard","max result cap","no command execution","no writes")) {
...
Write-Host "[OK] CodexForge Project Search Live Trial smoke passed."
