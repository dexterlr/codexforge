...
$domain = "src\lib\codexforge\branch-tag-release-handoff"
$route = "src\app\branch-tag-release-handoff"

& (Join-Path $PSScriptRoot "codexforge-local-planning-phase-smoke-helper.ps1") `
  -PhaseName "Branch Tag Release Handoff" `
  -ScriptFile "smoke-codexforge-branch-tag-release-handoff.ps1" `
  -Domain $domain `
  -Route $route `
  -MainPanel "BranchTagReleaseHandoffPanel" `
  -CommandLabel "Go to Branch Tag Release Handoff" `
  -Modules @("branch-tag-release-handoff-types.ts","branch-tag-release-handoff-summary.ts","index.ts") `
  -Components @("BranchTagReleaseHandoffPanel.tsx","index.ts") `
  -Exports @("buildBranchTagReleaseHandoffStableKey","buildBranchTagReleaseHandoff","buildBranchTagReleaseHandoffs","buildBranchTagReleaseHandoffBoundary","buildBranchTagReleaseHandoffModel","summarizeBranchTagReleaseHandoff","BRANCH_TAG_RELEASE_HANDOFF_LANGUAGE") `
  -PlainEnglish @("Branch tag release handoff","Branches and tags are not created from this page","Push and tag commands are manual only","Secrets are not included in release copy","Release handoff identity","Source commit approval","Target branch summary","Tag naming recommendation","Validation summary","Risk/secrets status","Push policy","Rollback note","Release notes route","Blocked reasons","advanced handoff details collapsed/secondary","approved local boundary required","release actions are not run from arbitrary UI","git commands are not run from arbitrary UI","no git command execution from UI","nothing executes from arbitrary UI","no automatic local action","no raw fetch from arbitrary UI","no command execution","no shell command execution","no branch creation","no tag creation","no PR creation","no release publishing","no push behavior","no merge behavior","no arbitrary local file browsing","no arbitrary file read/open","no file mutation","no file deletion","no patch apply behavior","no package install behavior","no provider APIs are called","no GitHub API calls from UI","no automatic provider send","no secrets displayed","no secrets exported","no secrets included","no localStorage API key storage","no process.env printing","no direct appendEvent call from UI","no direct saveBrainGraph call from UI","no direct graph mutation from UI","no memory auto-promotion") `
  -ExtraRoutes @("/git-commit-approval","/git-status-review","/test-result-summary","/project-risk-secrets-scan","/release-notes-draft-builder")
...
foreach ($needle in @(
  ...
  Assert-Contains $source $needle "branch tag release handoff includes $needle"
}
...
Write-Host "[OK] CodexForge Branch Tag Release Handoff smoke passed."
