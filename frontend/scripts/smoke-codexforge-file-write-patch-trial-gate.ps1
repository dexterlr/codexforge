...
$domain = "src\lib\codexforge\file-write-patch-trial-gate"
$route = "src\app\file-write-patch-trial"

& (Join-Path $PSScriptRoot "codexforge-local-planning-phase-smoke-helper.ps1") `
  -PhaseName "File Write Patch Trial Gate" `
  -ScriptFile "smoke-codexforge-file-write-patch-trial-gate.ps1" `
  -Domain $domain `
  -Route $route `
  -MainPanel "FileWritePatchTrialGatePanel" `
  -CommandLabel "Go to File Write Patch Trial" `
  -Modules @("file-write-patch-trial-gate-types.ts","file-write-patch-trial-gate-summary.ts","index.ts") `
  -Components @("FileWritePatchTrialGatePanel.tsx","index.ts") `
  -Exports @("buildFileWritePatchTrialGateStableKey","buildFileWritePatchTrial","buildFileWritePatchTrials","buildFileWritePatchTrialBoundary","buildFileWritePatchTrialGateModel","summarizeFileWritePatchTrialGate","FILE_WRITE_PATCH_TRIAL_GATE_LANGUAGE") `
  -PlainEnglish @("File write patch trial gate","Write and patch operations are not performed from this page","Approved local boundary is required before mutation","Delete operations require separate explicit review","Allowed write scope","Required confirmation copy","Trial gate identity","Dry-run dependency","Patch preview dependency","Approval boundary dependency","Denied write scope","Expected changed files","Rollback plan","Blocked reasons","advanced trial details collapsed/secondary","no automatic local action","no raw fetch from arbitrary UI","no command execution","no shell command execution","no direct Jarvisd call from arbitrary UI","no Jarvisd capability execution from UI","no daemon process creation from frontend","no browser-stored signing secrets","no session token localStorage storage","no arbitrary local file browsing","no arbitrary file read/open","no auto-open local files","no file mutation","no file write","no file deletion","no patch apply behavior","no secret value display","no automatic provider send","no provider APIs are called","no GitHub API calls from UI","no localStorage API key storage","no process.env printing","no audit log mutation from UI","no direct appendEvent call from UI","no direct saveBrainGraph call from UI","no direct graph mutation from UI","no memory auto-promotion","no package install behavior") `
  -ExtraRoutes @("/file-operation-dry-run","/patch-preview-workbench","/patch-apply-approval","/local-file-approval","/file-operation-result")
...
foreach ($needle in @(
  ...
  Assert-Contains $source $needle "file write patch trial includes $needle"
}
...
Write-Host "[OK] CodexForge File Write Patch Trial Gate smoke passed."
