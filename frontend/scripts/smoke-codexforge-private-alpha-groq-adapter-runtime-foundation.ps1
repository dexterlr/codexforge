...
async function main() {
  ...
  assert(
    JSON.stringify(
      privateAlpha.PRIVATE_ALPHA_EXECUTION_ERROR_CODES.slice(0, originalOllamaErrorCodes.length)
    ) === JSON.stringify(originalOllamaErrorCodes),
    "Original Ollama error-code ordering stays exact."
  );
  assert(
    JSON.stringify(
      privateAlpha.PRIVATE_ALPHA_EXECUTION_ERROR_CODES.slice(originalOllamaErrorCodes.length)
    ) === JSON.stringify(groqErrorCodes),
    "Groq error codes are appended in the required order."
  );
