...
$domain = "src\lib\codexforge\secrets-token-storage-regression-sweep"
$route = "src\app\secrets-token-storage-regression-sweep"
$protectedRoutes = @(
  "/daily-operator-home",
  "/global-review-inbox",
  "/approval-queue",
...
& (Join-Path $PSScriptRoot "codexforge-local-project-knowledge-smoke-helper.ps1") `
  -PhaseName "Phase 381 Secrets / Token Storage Regression Sweep" `
  -ScriptFile "smoke-codexforge-secrets-token-storage-regression-sweep.ps1" `
  -Domain $domain `
  -Route $route `
  -MainPanel "SecretsTokenStorageRegressionSweepPanel" `
  -CommandLabel "Go to Secrets Token Storage Sweep" `
  -Modules @("secrets-token-storage-regression-sweep-types.ts","secrets-token-storage-regression-sweep-summary.ts","index.ts") `
  -Components @("SecretsTokenStorageRegressionSweepPanel.tsx","index.ts") `
  -Exports @("buildSecretsTokenStorageRegressionSweepStableKey","buildSecretsTokenStorageRegressionSweep","buildSecretsTokenStorageRegressionSweeps","buildSecretsTokenStorageRegressionSweepBoundary","buildSecretsTokenStorageRegressionSweepModel","summarizeSecretsTokenStorageRegressionSweep","SECRETS_TOKEN_STORAGE_REGRESSION_SWEEP_LANGUAGE") `
  -PhaseMarkers $phaseMarkers `
  -PlainEnglish @(@("Secrets sweep identity","Browser storage policy","Connector token policy","Provider key policy","Regression findings preview","Blocked risks","Next recommended route","advanced secrets details collapsed/secondary") + $dailyCockpitSafetyMarkers) `
  -ExtraRoutes @("/local-first-privacy-audit","/cross-loop-search-review","/dashboard-density-navigation-polish","/credentials")
...
$blockedPatterns = @{
  ...
  "no token API key secret or env exposure" = "tokenStorageAllowed:\s*true|browserTokenStorageAllowed:\s*true|localStorageTokenStorageAllowed:\s*true|sessionStorageTokenStorageAllowed:\s*true|localStorageApiKeyStorageAllowed:\s*true|sessionStorageApiKeyStorageAllowed:\s*true|connectorTokenDisplayAllowed:\s*true|providerKeyDisplayAllowed:\s*true|localStorage\.setItem|sessionStorage\.setItem|processEnvDisplayAllowed:\s*true|process\.env\.[A-Za-z0-9_]+|secretsDisplayedAllowed:\s*true|sk-[A-Za-z0-9]{20,}|AIza[0-9A-Za-z_-]{20,}"
...
Write-Host "[OK] CodexForge Secrets / Token Storage Regression Sweep smoke passed."
