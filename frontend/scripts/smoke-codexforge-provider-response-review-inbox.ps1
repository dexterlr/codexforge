...
$domain = "src\lib\codexforge\provider-response-review-inbox"
$route = "src\app\provider-response-review-inbox"
$newRoutes = @(
...
& (Join-Path $PSScriptRoot "codexforge-local-project-knowledge-smoke-helper.ps1") `
  -PhaseName "Phase 419 Provider Response Review Inbox" `
  -ScriptFile "smoke-codexforge-provider-response-review-inbox.ps1" `
  -Domain $domain `
  -Route $route `
  -MainPanel "ProviderResponseReviewInboxPanel" `
  -CommandLabel "Go to Provider Response Review Inbox" `
  -Modules @("provider-response-review-inbox-types.ts","provider-response-review-inbox-summary.ts","index.ts") `
  -Components @("ProviderResponseReviewInboxPanel.tsx","index.ts") `
  -Exports @("buildProviderResponseReviewInboxStableKey","buildProviderResponseReviewInbox","buildProviderResponseReviewInboxes","buildProviderResponseReviewInboxBoundary","buildProviderResponseReviewInboxModel","summarizeProviderResponseReviewInbox","PROVIDER_RESPONSE_REVIEW_INBOX_LANGUAGE") `
  -PhaseMarkers $phaseMarkers `
  -PlainEnglish @(@("provider response inbox identity","Redaction and privacy rules","Acceptance/rejection criteria","Denied response actions","Blocked response risks","Provider safety regression route","First controlled trial route","Next recommended action") + $providerReviewSafetyMarkers) `
  -ExtraRoutes @("/provider-safety-regression-review","/first-controlled-provider-trial","/provider-test-results","/provider-cost-rate-limit-review")
...
Write-Host "[OK] CodexForge Provider Response Review Inbox smoke passed."
