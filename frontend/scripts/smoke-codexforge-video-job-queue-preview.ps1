...
Write-Host "=== Video Job Queue Preview smoke ==="
$domain = "src\lib\codexforge\video-job-queue-preview"; $components = Join-Path $domain "components"; $route = "src\app\video-jobs"
Assert-DirectoryExists $domain; Assert-DirectoryExists $components
foreach ($file in @("video-job-queue-types.ts","video-job-request.ts","video-job-review.ts","video-job-status.ts","video-job-resource-estimate.ts","video-job-artifact-plan.ts","video-job-queue-summary.ts","index.ts")) { Assert-FileExists (Join-Path $domain $file) }
foreach ($file in @("VideoJobQueuePreviewPanel.tsx","VideoJobRequestPanel.tsx","VideoJobReviewPanel.tsx","VideoJobStatusPanel.tsx","VideoJobResourceEstimatePanel.tsx","VideoJobArtifactPlanPanel.tsx","VideoJobQueueSummaryPanel.tsx","VideoJobQueueSafetyStrip.tsx","VideoJobQueueEmptyState.tsx","index.ts")) { Assert-FileExists (Join-Path $components $file) }
...
foreach ($export in @("buildVideoJobRequest","buildDefaultVideoJobRequests","buildVideoJobReview","buildVideoJobStatus","buildVideoJobResourceEstimate","buildVideoJobArtifactPlan","buildVideoJobQueueSummary","summarizeVideoJobQueue")) { Assert-Contains $source $export "expected export $export" }
foreach ($needle in @("Video job queue","Prepare video jobs safely before anything renders.","Review video job","prompt","workflowType","provider","localCloudPosture","resolutionTarget","durationTarget","draft","final","estimatedGpuTimePosture","artifactDestination","approvalRequired","noAutoRunGuarantee","draft","needs-review","approved-preview-only","queued-planned","blocked","running-future","complete-supplied","failed-supplied","local draft comes first")) { Assert-Contains $source $needle "plain English/source includes $needle" }
Assert-Contains $routeSource "VideoJobQueuePreviewPanel" "route imports/renders main panel"; Assert-Contains $routeSource "CodexForgeAppShell" "route uses home-grade/unified shell marker"
Assert-Contains $commands "Go to Video Jobs" "command palette includes Go to Video Jobs"; Assert-Contains $nav "/video-jobs" "navigation includes /video-jobs"
if (([regex]::Matches($allSmoke, "smoke-codexforge-video-job-queue-preview\.ps1")).Count -ne 1) { throw "[FAIL] managed smoke suite includes phase exactly once" }
...
Write-Host "[OK] Video Job Queue Preview smoke passed."
