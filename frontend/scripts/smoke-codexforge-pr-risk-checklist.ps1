...
$domain = "src\lib\codexforge\pr-risk-checklist"
$route = "src\app\pr-risk-checklist"

& (Join-Path $PSScriptRoot "codexforge-local-planning-phase-smoke-helper.ps1") `
  -PhaseName "PR Risk Checklist" `
  -ScriptFile "smoke-codexforge-pr-risk-checklist.ps1" `
  -Domain $domain `
  -Route $route `
  -MainPanel "PrRiskChecklistPanel" `
  -CommandLabel "Go to PR Risk Checklist" `
  -Modules @("pr-risk-checklist-types.ts","pr-risk-checklist-summary.ts","index.ts") `
  -Components @("PrRiskChecklistPanel.tsx","index.ts") `
  -Exports @("buildPrRiskChecklistStableKey","buildPrRiskCheck","buildPrRiskChecks","buildPrRiskChecklistBoundary","buildPrRiskChecklistModel","summarizePrRiskChecklist","PR_RISK_CHECKLIST_LANGUAGE") `
  -PlainEnglish @("PR risk checklist","PR risk review does not approve merges automatically","Suspected secrets are redacted","High-risk changes require explicit review","Risk checklist identity","Source PR prep","Changed areas","Risk categories","Test and validation coverage","Secrets scan status","Migration config env risk","UX/navigation risk","Release notes route","Blocked reasons","advanced checklist details collapsed/secondary","approved local boundary required","release actions are not run from arbitrary UI","git commands are not run from arbitrary UI","no git command execution from UI","nothing executes from arbitrary UI","no automatic local action","no raw fetch from arbitrary UI","no command execution","no shell command execution","no branch creation","no tag creation","no PR creation","no release publishing","no push behavior","no merge behavior","no arbitrary local file browsing","no arbitrary file read/open","no file mutation","no file deletion","no patch apply behavior","no package install behavior","no provider APIs are called","no GitHub API calls from UI","no automatic provider send","no secrets displayed","no secrets exported","no secrets included","no localStorage API key storage","no process.env printing","no direct appendEvent call from UI","no direct saveBrainGraph call from UI","no direct graph mutation from UI","no memory auto-promotion") `
  -ExtraRoutes @("/pull-request-prep-review","/branch-tag-release-handoff","/test-result-summary","/project-risk-secrets-scan","/release-notes-draft-builder")
...
foreach ($needle in @(
  ...
  Assert-Contains $source $needle "PR risk checklist includes $needle"
}
...
Write-Host "[OK] CodexForge PR Risk Checklist smoke passed."
