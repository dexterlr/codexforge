...
$domain = "src\lib\codexforge\project-indexer-live-trial"
$route = "src\app\project-indexer-live-trial"

& (Join-Path $PSScriptRoot "codexforge-local-planning-phase-smoke-helper.ps1") `
  -PhaseName "Phase 278 Project Indexer Live Trial" `
  -ScriptFile "smoke-codexforge-project-indexer-live-trial.ps1" `
  -Domain $domain `
  -Route $route `
  -MainPanel "ProjectIndexerLiveTrialPanel" `
  -CommandLabel "Go to Project Indexer Live Trial" `
  -Modules @("project-indexer-live-trial-types.ts","project-indexer-live-trial-summary.ts","index.ts") `
  -Components @("ProjectIndexerLiveTrialPanel.tsx","index.ts") `
  -Exports @("buildProjectIndexerLiveTrialStableKey","buildProjectIndexerLiveTrial","buildProjectIndexerLiveTrials","buildProjectIndexerLiveTrialBoundary","buildProjectIndexerLiveTrialModel","summarizeProjectIndexerLiveTrial","PROJECT_INDEXER_LIVE_TRIAL_LANGUAGE") `
  -PlainEnglish @("Project indexer live trial","Live trial uses only approved bounded workspace data","Arbitrary local file crawling is not allowed","Secrets are not read or displayed","Live trial identity","Trusted workspace dependency","Approved bounded root summary","Index scope summary","Excluded paths summary","File type coverage","Sync/result status","Privacy/secrets redaction status","Audit handoff","Recovery route","Server snapshot route","Blocked reasons","approved bounded workspace data required","server-only path boundary markers remain intact","no automatic local action","no raw fetch from arbitrary UI","no command execution","no shell command execution","no test execution from UI","no git command execution from UI","no direct Jarvisd call from arbitrary UI","no Jarvisd capability execution from UI","no daemon process creation from frontend","no browser-stored signing secrets","no session token localStorage storage","no arbitrary local file browsing","no arbitrary path crawling","no arbitrary file read/open","no auto-open local files","no file mutation","no file write","no patch apply behavior","no file deletion","no secret value display","no automatic provider send","no provider APIs are called","no GitHub API calls from UI","no localStorage API key storage","no process.env printing","no audit log mutation from UI","no direct appendEvent call from UI","no direct saveBrainGraph call from UI","no direct graph mutation from UI","no memory auto-promotion","no process kill/restart/shutdown from UI","no package install behavior","no weakened safe path checks","no removed server-only boundaries") `
  -ExtraRoutes @("/safe-project-indexer","/workspace-trust-policy","/jarvisd-index-sync","/project-search-live-trial","/jarvisd-recovery-console")
...
foreach ($needle in @(
  ...
  Assert-Contains $source $needle "project indexer live trial includes $needle"
}
...
$blockedPatterns = @{
  ...
  "no arbitrary local file browsing or path crawling" = "showOpenFilePicker|browseLocalFiles\s*\(|arbitraryLocalBrowsingAllowed:\s*true|arbitraryLocalFileCrawlingAllowed:\s*true|arbitraryPathCrawlingAllowed:\s*true|input\s+type=.*file"
...
Write-Host "[OK] CodexForge Project Indexer Live Trial smoke passed."
