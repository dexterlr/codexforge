...
$domain = "src\lib\codexforge\automation-dry-run-trial-review"
$route = "src\app\automation-dry-run-trial-review"
$newRoutes = @(
...
& (Join-Path $PSScriptRoot "codexforge-local-project-knowledge-smoke-helper.ps1") `
  -PhaseName "Phase 438 Automation Dry-Run Trial Review" `
  -ScriptFile "smoke-codexforge-automation-dry-run-trial-review.ps1" `
  -Domain $domain `
  -Route $route `
  -MainPanel "AutomationDryRunTrialReviewPanel" `
  -CommandLabel "Go to Automation Dry-Run Trial Review" `
  -Modules @("automation-dry-run-trial-review-types.ts","automation-dry-run-trial-review-summary.ts","index.ts") `
  -Components @("AutomationDryRunTrialReviewPanel.tsx","index.ts") `
  -Exports @("buildAutomationDryRunTrialReviewStableKey","buildAutomationDryRunTrialReview","buildAutomationDryRunTrialReviews","buildAutomationDryRunTrialReviewBoundary","buildAutomationDryRunTrialReviewModel","summarizeAutomationDryRunTrialReview","AUTOMATION_DRY_RUN_TRIAL_REVIEW_LANGUAGE") `
  -PhaseMarkers $phaseMarkers `
  -PlainEnglish @(@("automation dry-run identity","approval gate checklist","denied automation actions","connector/provider boundary notes","blocked dry-run risks","approval queue route","schedule safety route","next recommended action") + $automationReadinessSafetyMarkers) `
  -ExtraRoutes @("/automation-approval-queue-review","/automation-schedule-safety-review","/automation-permission-readiness-audit","/connector-integration-release-candidate")
...
Write-Host "[OK] CodexForge Automation Dry-Run Trial Review smoke passed."
