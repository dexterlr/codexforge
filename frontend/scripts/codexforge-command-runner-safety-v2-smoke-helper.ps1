...
$sourceParts = @()
foreach ($scanRoot in @($Domain, $Route, "src\lib\codexforge\command-runner-safety-v2", "src\lib\codexforge\unified-cockpit")) {
...
foreach ($safetyMarker in @(
  ...
  "frontend command execution still blocked",
  "backend-owned command execution remains required"
)) {
...
Write-Host "[OK] $SmokeName static command runner safety v2 smoke passed."
