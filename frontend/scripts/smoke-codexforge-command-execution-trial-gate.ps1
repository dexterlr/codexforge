...
$domain = "src\lib\codexforge\command-execution-trial-gate"
$route = "src\app\command-execution-trial"

& (Join-Path $PSScriptRoot "codexforge-local-planning-phase-smoke-helper.ps1") `
  -PhaseName "Command Execution Trial Gate" `
  -ScriptFile "smoke-codexforge-command-execution-trial-gate.ps1" `
  -Domain $domain `
  -Route $route `
  -MainPanel "CommandExecutionTrialGatePanel" `
  -CommandLabel "Go to Command Execution Trial" `
  -Modules @("command-execution-trial-gate-types.ts","command-execution-trial-gate-summary.ts","index.ts") `
  -Components @("CommandExecutionTrialGatePanel.tsx","index.ts") `
  -Exports @("buildCommandExecutionTrialGateStableKey","buildCommandExecutionTrial","buildCommandExecutionTrials","buildCommandExecutionTrialBoundary","buildCommandExecutionTrialGateModel","summarizeCommandExecutionTrialGate","COMMAND_EXECUTION_TRIAL_GATE_LANGUAGE") `
  -PlainEnglish @("Command execution trial gate","Commands are not executed from this page","Approved local boundary is required before execution","Env values and secrets are not exposed","Timeout policy","Required confirmation copy","Execution trial identity","Dry-run dependency","Command approval boundary dependency","Workspace trust status","Allowed command scope","Denied command scope","Expected output shape","Blocked reasons","advanced trial details collapsed/secondary","no automatic local action","no raw fetch from arbitrary UI","no command execution","no shell command execution","no runCommand brokerExecution or local executor API calls from UI","no direct Jarvisd call from arbitrary UI","no Jarvisd capability execution from UI","no daemon process creation from frontend","no browser-stored signing secrets","no session token localStorage storage","no arbitrary local file browsing","no arbitrary file read/open","no auto-open local files","no file mutation","no file write","no file deletion","no patch apply behavior","no secret value display","no automatic provider send","no provider APIs are called","no GitHub API calls from UI","no localStorage API key storage","no process.env printing","no audit log mutation from UI","no direct appendEvent call from UI","no direct saveBrainGraph call from UI","no direct graph mutation from UI","no memory auto-promotion","no process kill/restart/shutdown from UI","no package install behavior") `
  -ExtraRoutes @("/command-dry-run","/local-command-approval","/workspace-trust-policy","/jarvisd-runtime-enforcement","/command-result-capture")
...
foreach ($needle in @(
  ...
  Assert-Contains $source $needle "command execution trial includes $needle"
}
...
$blockedPatterns = @{
  ...
  "no command execution" = "commandsExecutedFromPageAllowed:\s*true|commandExecutionAllowedFromUi:\s*true|shellExecutionAllowedFromUi:\s*true|child_process|execSync|spawn\s*\(|runCommand\s*\(|brokerExecution\s*\(|localExecutorApiCallAllowedFromUi:\s*true|Start-Process|Invoke-Expression"
...
Write-Host "[OK] CodexForge Command Execution Trial Gate smoke passed."
