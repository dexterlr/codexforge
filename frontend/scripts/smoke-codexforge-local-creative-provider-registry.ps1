...
Write-Host "=== Local Creative Provider Registry smoke ==="
$domain = "src\lib\codexforge\local-creative-provider-registry"; $components = Join-Path $domain "components"; $route = "src\app\local-creative"
Assert-DirectoryExists $domain; Assert-DirectoryExists $components
foreach ($file in @("local-creative-provider-types.ts","local-creative-provider.ts","local-creative-provider-kind.ts","local-creative-capability.ts","local-creative-cost-profile.ts","local-creative-routing-profile.ts","local-creative-provider-summary.ts","index.ts")) { Assert-FileExists (Join-Path $domain $file) }
foreach ($file in @("LocalCreativeProviderRegistryPanel.tsx","LocalCreativeProviderPanel.tsx","LocalCreativeProviderKindPanel.tsx","LocalCreativeCapabilityPanel.tsx","LocalCreativeCostProfilePanel.tsx","LocalCreativeRoutingProfilePanel.tsx","LocalCreativeProviderSummaryPanel.tsx","LocalCreativeProviderSafetyStrip.tsx","LocalCreativeProviderEmptyState.tsx","index.ts")) { Assert-FileExists (Join-Path $components $file) }
...
foreach ($export in @("buildLocalCreativeProvider","buildDefaultLocalCreativeProviders","buildLocalCreativeProviderKind","buildLocalCreativeCapability","buildLocalCreativeCostProfile","buildLocalCreativeRoutingProfile","buildLocalCreativeProviderSummary","summarizeLocalCreativeProviders")) { Assert-Contains $source $export "expected export $export" }
foreach ($needle in @("Local creative providers","Use your workstation first for creative drafts, keyframes, and video jobs.","Review local providers","comfyui-local","automatic1111-local","forge-local","invoke-local","blender-local","ffmpeg-local","local-upscaler","local-interpolator","custom-local-http","manual-workflow","image generation","image upscale","video draft","video upscale","frame interpolation","keyframe generation","storyboard assets","prompt planning","batch render","artifact gallery","save cloud credits","Manual setup")) { Assert-Contains $source $needle "plain English/source includes $needle" }
Assert-Contains $routeSource "LocalCreativeProviderRegistryPanel" "route imports/renders main panel"; Assert-Contains $routeSource "CodexForgeAppShell" "route uses home-grade/unified shell marker"
Assert-Contains $commands "Go to Local Creative" "command palette includes Go to Local Creative"; Assert-Contains $nav "/local-creative" "navigation includes /local-creative"
if (([regex]::Matches($allSmoke, "smoke-codexforge-local-creative-provider-registry\.ps1")).Count -ne 1) { throw "[FAIL] managed smoke suite includes phase exactly once" }
...
Write-Host "[OK] Local Creative Provider Registry smoke passed."
