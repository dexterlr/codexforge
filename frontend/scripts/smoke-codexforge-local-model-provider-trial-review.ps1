...
$domain = "src\lib\codexforge\local-model-provider-trial-review"
$route = "src\app\local-model-provider-trial-review"
$newRoutes = @(
...
& (Join-Path $PSScriptRoot "codexforge-local-project-knowledge-smoke-helper.ps1") `
  -PhaseName "Phase 413 Local Model Provider Trial Review" `
  -ScriptFile "smoke-codexforge-local-model-provider-trial-review.ps1" `
  -Domain $domain `
  -Route $route `
  -MainPanel "LocalModelProviderTrialReviewPanel" `
  -CommandLabel "Go to Local Model Provider Trial Review" `
  -Modules @("local-model-provider-trial-review-types.ts","local-model-provider-trial-review-summary.ts","index.ts") `
  -Components @("LocalModelProviderTrialReviewPanel.tsx","index.ts") `
  -Exports @("buildLocalModelProviderTrialReviewStableKey","buildLocalModelProviderTrialReview","buildLocalModelProviderTrialReviews","buildLocalModelProviderTrialReviewBoundary","buildLocalModelProviderTrialReviewModel","summarizeLocalModelProviderTrialReview","LOCAL_MODEL_PROVIDER_TRIAL_REVIEW_LANGUAGE") `
  -PhaseMarkers $phaseMarkers `
  -PlainEnglish @(@("local model provider trial identity","denied local model actions","manual validation checklist","safety/credential boundaries","blocked local model trial risks","provider integration route","beta hardening route","next recommended action") + $localModelSafetyMarkers) `
  -ExtraRoutes @("/controlled-provider-integration-plan","/beta-hardening-final-pass","/local-bridge-readiness-audit","/local-provider-probes")
...
$blockedPatterns = @{
  ...
  "no local model local traffic bridge service tool or probe calls" = "localModelCallsAllowedFromUi:\s*true|localModelTrafficAllowedFromUi:\s*true|localBridgeEndpointCallsAllowedFromUi:\s*true|localServiceCallsAllowedFromUi:\s*true|localToolLaunchingAllowedFromUi:\s*true|localProbeExecutionAllowedFromUi:\s*true|callLocalModel\s*\(|runLocalModel\s*\(|routeLocalModelTraffic\s*\(|callLocalBridge\s*\(|callLocalService\s*\(|launchLocalTool\s*\(|runLocalProbe\s*\("
  "no endpoint token local credential storage" = "endpointStorageAllowed:\s*true|tokenStorageAllowed:\s*true|localCredentialStorageAllowed:\s*true|storeEndpoint\s*\(|storeToken\s*\(|storeLocalCredential\s*\(|localStorage\.setItem|sessionStorage\.setItem"
...
Write-Host "[OK] CodexForge Local Model Provider Trial Review smoke passed."
