...
$domain = "src\lib\codexforge\patch-preview-workbench-live-context-integration"
$route = "src\app\patch-preview-live-context"

& (Join-Path $PSScriptRoot "codexforge-local-planning-phase-smoke-helper.ps1") `
  -PhaseName "Phase 285 Patch Preview Workbench Live Context Integration" `
  -ScriptFile "smoke-codexforge-patch-preview-workbench-live-context-integration.ps1" `
  -Domain $domain `
  -Route $route `
  -MainPanel "PatchPreviewWorkbenchLiveContextIntegrationPanel" `
  -CommandLabel "Go to Patch Preview Live Context" `
  -Modules @("patch-preview-workbench-live-context-integration-types.ts","patch-preview-workbench-live-context-integration-summary.ts","index.ts") `
  -Components @("PatchPreviewWorkbenchLiveContextIntegrationPanel.tsx","index.ts") `
  -Exports @("buildPatchPreviewWorkbenchLiveContextIntegrationStableKey","buildPatchPreviewWorkbenchLiveContextIntegration","buildPatchPreviewWorkbenchLiveContextIntegrations","buildPatchPreviewWorkbenchLiveContextBoundary","buildPatchPreviewWorkbenchLiveContextIntegrationModel","summarizePatchPreviewWorkbenchLiveContextIntegration","PATCH_PREVIEW_WORKBENCH_LIVE_CONTEXT_INTEGRATION_LANGUAGE") `
  -PlainEnglish @("Patch preview workbench live context integration","Preview still does not apply patches","Raw diffs stay secondary","Suspected secrets are redacted","Dependency risk context","Rollback note","Integration identity","Source change plan live context","Source project intelligence result","Affected files summary","Redaction status","Hunk/context summary","Approval route","Blocked reasons","live context does not mutate files","advanced patch context details collapsed/secondary","server-only path boundary markers remain intact","no automatic local action","no raw fetch from arbitrary UI","no command execution","no shell command execution","no test execution from UI","no git command execution from UI","no direct Jarvisd call from arbitrary UI","no Jarvisd capability execution from UI","no daemon process creation from frontend","no browser-stored signing secrets","no session token localStorage storage","no arbitrary local file browsing","no arbitrary path crawling","no arbitrary file read/open","no auto-open local files","no file mutation","no file write","no patch apply behavior","no file deletion","no secret value display","no automatic provider send","no provider APIs are called","no GitHub API calls from UI","no localStorage API key storage","no process.env printing","no audit log mutation from UI","no direct appendEvent call from UI","no direct saveBrainGraph call from UI","no direct graph mutation from UI","no memory auto-promotion","no process kill/restart/shutdown from UI","no package install behavior") `
  -ExtraRoutes @("/change-plan-live-context","/project-intelligence-result","/patch-preview-workbench","/patch-apply-approval","/project-intelligence-recovery")
...
foreach ($needle in @(
  ...
  Assert-Contains $source $needle "patch preview live context includes $needle"
}
...
$blockedPatterns = @{
  ...
  "no patch apply behavior" = "previewAppliesPatchesAllowed:\s*true|patchApplyAllowedFromUi:\s*true|applyPatch\s*\(|applyDiff\s*\("
  "no raw diffs primary" = "rawDiffsPrimaryAllowed:\s*true"
...
Write-Host "[OK] CodexForge Patch Preview Workbench Live Context Integration smoke passed."
