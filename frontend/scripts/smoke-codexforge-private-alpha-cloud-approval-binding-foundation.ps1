...
Assert-NotMatches $storeSource 'inspectPrivateAlphaProviderRuntime|createPrivateAlphaGroqProviderAdapter|routeCodexForgeModel' "Store remains disconnected from the runtime resolver and routing"
...
async function main() {
  ...
  assert(
    ...
    "Groq approval statement explicitly says cloud execution remains disabled"
  );
  const groqApprovalEvent = groqApproved.auditEvents[groqApproved.auditEvents.length - 1];
  assert(
    groqApprovalEvent.summary ===
      "Manual approval recorded for exact Groq Cloud model scope openai/gpt-oss-20b. Cloud execution remains disabled." &&
      !groqApprovalEvent.summary.includes("GROQ_API_KEY"),
    "Groq approval audit event names the exact approved model safely"
  );
