...
$sourceParts = @()
foreach ($scanRoot in @("src\lib\codexforge\video-backend-service-contract-boundary", $Domain, $Route, (Join-Path "scripts" $ScriptFile))) {
...
Assert-Contains $source "VideoBackendServiceContractBoundaryRoutePanel" "main route panel"
Assert-Contains $source "VideoBackendServiceContractBoundaryCockpitSummaryPanel" "cockpit video backend service contract summary panel"
...
Write-Host ("[OK] " + $SmokeName + " static video backend service contract boundary smoke passed.")
