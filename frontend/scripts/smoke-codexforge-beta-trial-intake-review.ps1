...
$domain = "src\lib\codexforge\beta-trial-intake-review"
$route = "src\app\beta-trial-intake-review"
$protectedRoutes = @(
...
& (Join-Path $PSScriptRoot "codexforge-local-project-knowledge-smoke-helper.ps1") `
  -PhaseName "Phase 384 Beta Trial Intake Review" `
  -ScriptFile "smoke-codexforge-beta-trial-intake-review.ps1" `
  -Domain $domain `
  -Route $route `
  -MainPanel "BetaTrialIntakeReviewPanel" `
  -CommandLabel "Go to Beta Trial Intake Review" `
  -Modules @("beta-trial-intake-review-types.ts","beta-trial-intake-review-summary.ts","index.ts") `
  -Components @("BetaTrialIntakeReviewPanel.tsx","index.ts") `
  -Exports @("buildBetaTrialIntakeReviewStableKey","buildBetaTrialIntakeReview","buildBetaTrialIntakeReviews","buildBetaTrialIntakeReviewBoundary","buildBetaTrialIntakeReviewModel","summarizeBetaTrialIntakeReview","BETA_TRIAL_INTAKE_REVIEW_LANGUAGE") `
  -PhaseMarkers $phaseMarkers `
  -PlainEnglish @(@("Beta intake identity","Trial scope summary","Approval gates","Onboarding readiness","Feedback inbox route","Blocked intake reasons","Next recommended action","advanced intake details collapsed/secondary") + $betaReadinessSafetyMarkers) `
  -ExtraRoutes @("/foundation-beta-candidate","/beta-feedback-inbox","/full-smoke-suite-stability-pass","/local-first-privacy-audit")
...
Write-Host "[OK] CodexForge Beta Trial Intake Review smoke passed."
