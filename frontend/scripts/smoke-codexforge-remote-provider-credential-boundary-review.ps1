...
$domain = "src\lib\codexforge\remote-provider-credential-boundary-review"
$route = "src\app\remote-provider-credential-boundary-review"
$newRoutes = @(
...
& (Join-Path $PSScriptRoot "codexforge-local-project-knowledge-smoke-helper.ps1") `
  -PhaseName "Phase 415 Remote Provider Credential Boundary Review" `
  -ScriptFile "smoke-codexforge-remote-provider-credential-boundary-review.ps1" `
  -Domain $domain `
  -Route $route `
  -MainPanel "RemoteProviderCredentialBoundaryReviewPanel" `
  -CommandLabel "Go to Remote Provider Credential Boundary Review" `
  -Modules @("remote-provider-credential-boundary-review-types.ts","remote-provider-credential-boundary-review-summary.ts","index.ts") `
  -Components @("RemoteProviderCredentialBoundaryReviewPanel.tsx","index.ts") `
  -Exports @("buildRemoteProviderCredentialBoundaryReviewStableKey","buildRemoteProviderCredentialBoundaryReview","buildRemoteProviderCredentialBoundaryReviews","buildRemoteProviderCredentialBoundaryReviewBoundary","buildRemoteProviderCredentialBoundaryReviewModel","summarizeRemoteProviderCredentialBoundaryReview","REMOTE_PROVIDER_CREDENTIAL_BOUNDARY_REVIEW_LANGUAGE") `
  -PhaseMarkers $phaseMarkers `
  -PlainEnglish @(@("credential boundary identity","redaction rules","manual validation checklist","approval gates","blocked credential risks","provider failover route","multi-provider routing route","next recommended action") + $credentialSafetyMarkers) `
  -ExtraRoutes @("/openai-compatible-provider-trial-review","/provider-failover-policy-review","/multi-provider-routing-release-candidate","/secrets-token-storage-regression-sweep")
...
foreach ($marker in @($phaseMarkers + $credentialSafetyMarkers + @("no Date.now","no Date.now for deterministic layout/ids","no Math.random","no obvious duplicate React key patterns"))) {
...
Write-Host "[OK] CodexForge Remote Provider Credential Boundary Review smoke passed."
