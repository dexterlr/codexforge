...
$domain = "src\lib\codexforge\expert-mode-fast-path-review"
$route = "src\app\expert-mode-fast-path-review"
$protectedRoutes = @(
  "/daily-operator-home",
  "/global-review-inbox",
  "/approval-queue",
...
& (Join-Path $PSScriptRoot "codexforge-local-project-knowledge-smoke-helper.ps1") `
  -PhaseName "Phase 377 Expert Mode Fast Path Review" `
  -ScriptFile "smoke-codexforge-expert-mode-fast-path-review.ps1" `
  -Domain $domain `
  -Route $route `
  -MainPanel "ExpertModeFastPathReviewPanel" `
  -CommandLabel "Go to Expert Mode Fast Path" `
  -Modules @("expert-mode-fast-path-review-types.ts","expert-mode-fast-path-review-summary.ts","index.ts") `
  -Components @("ExpertModeFastPathReviewPanel.tsx","index.ts") `
  -Exports @("buildExpertModeFastPathReviewStableKey","buildExpertModeFastPathReview","buildExpertModeFastPathReviews","buildExpertModeFastPathReviewBoundary","buildExpertModeFastPathReviewModel","summarizeExpertModeFastPathReview","EXPERT_MODE_FAST_PATH_REVIEW_LANGUAGE") `
  -PhaseMarkers $phaseMarkers `
  -PlainEnglish @(@("Expert fast path identity","Required approval gates","Skipped clutter summary","Boundaries that cannot be bypassed","Novice mode route","Daily home route","Next recommended action","advanced expert details collapsed/secondary") + $dailyCockpitSafetyMarkers) `
  -ExtraRoutes @("/daily-operator-home","/novice-mode-guided-flow-polish","/safety-boundary-matrix-finalization","/failure-recovery-playbook-finalization")
...
$blockedPatterns = @{
  ...
  "no approval recovery or rollback automation" = "approvalAutomationAllowedFromUi:\s*true|recoveryAutomationAllowedFromUi:\s*true|rollbackExecutionAllowedFromUi:\s*true|runRecovery\s*\(|executeRecovery\s*\(|runRollback\s*\(|rollbackFiles\s*\("
...
Write-Host "[OK] CodexForge Expert Mode Fast Path Review smoke passed."
