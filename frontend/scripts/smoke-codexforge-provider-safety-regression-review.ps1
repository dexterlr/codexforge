...
$domain = "src\lib\codexforge\provider-safety-regression-review"
$route = "src\app\provider-safety-regression-review"
$newRoutes = @(
...
& (Join-Path $PSScriptRoot "codexforge-local-project-knowledge-smoke-helper.ps1") `
  -PhaseName "Phase 421 Provider Safety Regression Review" `
  -ScriptFile "smoke-codexforge-provider-safety-regression-review.ps1" `
  -Domain $domain `
  -Route $route `
  -MainPanel "ProviderSafetyRegressionReviewPanel" `
  -CommandLabel "Go to Provider Safety Regression Review" `
  -Modules @("provider-safety-regression-review-types.ts","provider-safety-regression-review-summary.ts","index.ts") `
  -Components @("ProviderSafetyRegressionReviewPanel.tsx","index.ts") `
  -Exports @("buildProviderSafetyRegressionReviewStableKey","buildProviderSafetyRegressionReview","buildProviderSafetyRegressionReviews","buildProviderSafetyRegressionReviewBoundary","buildProviderSafetyRegressionReviewModel","summarizeProviderSafetyRegressionReview","PROVIDER_SAFETY_REGRESSION_REVIEW_LANGUAGE") `
  -PhaseMarkers $phaseMarkers `
  -PlainEnglish @(@("provider safety regression identity","Privacy and credential checks","Approval regression checks","Denied regression shortcuts","Unresolved provider safety blockers","Multi-provider routing route","Cost/rate-limit route","Next recommended action") + $providerReviewSafetyMarkers) `
  -ExtraRoutes @("/multi-provider-routing-release-candidate","/provider-cost-rate-limit-review","/provider-response-review-inbox","/remote-provider-credential-boundary-review")
...
Write-Host "[OK] CodexForge Provider Safety Regression Review smoke passed."
