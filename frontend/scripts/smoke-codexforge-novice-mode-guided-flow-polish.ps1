...
& (Join-Path $PSScriptRoot "codexforge-local-project-knowledge-smoke-helper.ps1") `
  -PhaseName "Phase 376 Novice Mode Guided Flow Polish" `
  -ScriptFile "smoke-codexforge-novice-mode-guided-flow-polish.ps1" `
  -Domain $domain `
  -Route $route `
  -MainPanel "NoviceModeGuidedFlowPolishPanel" `
  -CommandLabel "Go to Novice Mode Guided Flow" `
  -Modules @("novice-mode-guided-flow-polish-types.ts","novice-mode-guided-flow-polish-summary.ts","index.ts") `
  -Components @("NoviceModeGuidedFlowPolishPanel.tsx","index.ts") `
  -Exports @("buildNoviceModeGuidedFlowPolishStableKey","buildNoviceModeGuidedFlowPolish","buildNoviceModeGuidedFlowPolishes","buildNoviceModeGuidedFlowPolishBoundary","buildNoviceModeGuidedFlowPolishModel","summarizeNoviceModeGuidedFlowPolish","NOVICE_MODE_GUIDED_FLOW_POLISH_LANGUAGE") `
  -PhaseMarkers $phaseMarkers `
  -PlainEnglish @(@("Novice flow identity","Approval checkpoints","Blocked action explanations","Recovery handoff","Daily home route","Expert fast path route","Next recommended action","advanced novice flow details collapsed/secondary") + $dailyCockpitSafetyMarkers) `
  -ExtraRoutes @("/daily-operator-home","/safety-boundary-matrix-finalization","/failure-recovery-playbook-finalization","/expert-mode-fast-path-review")
...
Write-Host "[OK] CodexForge Novice Mode Guided Flow Polish smoke passed."
