...
$domain = "src\lib\codexforge\coding-loop-mvp-audit"
$route = "src\app\coding-loop-mvp-audit"

& (Join-Path $PSScriptRoot "codexforge-local-planning-phase-smoke-helper.ps1") `
  -PhaseName "Phase 277 Coding Loop MVP Audit" `
  -ScriptFile "smoke-codexforge-coding-loop-mvp-audit.ps1" `
  -Domain $domain `
  -Route $route `
  -MainPanel "CodingLoopMvpAuditPanel" `
  -CommandLabel "Go to Coding Loop MVP Audit" `
  -Modules @("coding-loop-mvp-audit-types.ts","coding-loop-mvp-audit-summary.ts","index.ts") `
  -Components @("CodingLoopMvpAuditPanel.tsx","index.ts") `
  -Exports @("buildCodingLoopMvpAuditStableKey","buildCodingLoopMvpAudit","buildCodingLoopMvpAudits","buildCodingLoopMvpAuditBoundary","buildCodingLoopMvpAuditModel","summarizeCodingLoopMvpAudit","CODING_LOOP_MVP_AUDIT_LANGUAGE") `
  -PlainEnglish @("Coding loop MVP audit","Coding MVP audit does not execute commands","Audit does not apply patches or create commits","Secrets are not inspected or displayed","Audit identity","Covered coding loop surfaces","Project intelligence readiness","Change plan readiness","Patch preview readiness","File operation readiness","Test operation readiness","Git operation readiness","Patch apply/rollback readiness","Review inbox readiness","Known gaps","Release decision: ready, ready with fixes, blocked","Next recommended route","advanced audit details collapsed/secondary","no automatic local action","no raw fetch from arbitrary UI","no command execution","no shell command execution","no test execution from UI","no git command execution from UI","no runCommand brokerExecution or local executor API calls from UI","no direct Jarvisd call from arbitrary UI","no Jarvisd capability execution from UI","no daemon process creation from frontend","no browser-stored signing secrets","no session token localStorage storage","no arbitrary local file browsing","no arbitrary file read/open","no auto-open local files","no file mutation","no file write","no patch apply behavior","no rollback behavior","no file deletion","no secret value display","no automatic provider send","no provider APIs are called","no GitHub API calls from UI","no localStorage API key storage","no process.env printing","no direct appendEvent call from UI","no direct saveBrainGraph call from UI","no direct graph mutation from UI","no memory auto-promotion","no process kill/restart/shutdown from UI","no package install behavior","no weakened safe path checks","no removed server-only boundaries") `
  -ExtraRoutes @("/codebase-change-plan","/patch-preview-workbench","/patch-apply-trial","/review-inbox","/release-smoke")
...
foreach ($needle in @(
  ...
  Assert-Contains $source $needle "coding loop MVP audit includes $needle"
}
...
$blockedPatterns = @{
  ...
  "no command execution" = "commandsExecutedFromAuditAllowed:\s*true|commandExecutionAllowedFromUi:\s*true|shellExecutionAllowedFromUi:\s*true|child_process|execSync|spawn\s*\(|runCommand\s*\(|brokerExecution\s*\(|localExecutorApiCallAllowedFromUi:\s*true|Start-Process|Invoke-Expression"
  "no test execution from UI" = "testExecutionFromUiAllowed:\s*true|runTests\s*\(|executeTests\s*\("
  "no git command execution from UI" = "gitCommandExecutionAllowedFromUi:\s*true|commitCreationAllowedFromAudit:\s*true|gitCommand\s*\(|runGit\s*\(|createCommit\s*\(|createPullRequest\s*\("
  ...
  "no patch apply behavior" = "patchApplyAllowedFromAudit:\s*true|patchApplyAllowedFromUi:\s*true|applyPatch\s*\(|applyDiff\s*\("
  "no rollback behavior" = "rollbackAllowedFromUi:\s*true|rollbackFiles\s*\(|performRollback\s*\("
  "no secret value display" = "secretValuesInspectedOrDisplayedAllowed:\s*true|secretValuesDisplayedAllowed:\s*true|displaySecrets\s*\(|exportSecrets\s*\(|rawSecretDisplayAllowed:\s*true"
...
Write-Host "[OK] CodexForge Coding Loop MVP Audit smoke passed."
