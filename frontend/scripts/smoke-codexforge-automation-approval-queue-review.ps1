...
$domain = "src\lib\codexforge\automation-approval-queue-review"
$route = "src\app\automation-approval-queue-review"
$newRoutes = @(
...
& (Join-Path $PSScriptRoot "codexforge-local-project-knowledge-smoke-helper.ps1") `
  -PhaseName "Phase 439 Automation Approval Queue Review" `
  -ScriptFile "smoke-codexforge-automation-approval-queue-review.ps1" `
  -Domain $domain `
  -Route $route `
  -MainPanel "AutomationApprovalQueueReviewPanel" `
  -CommandLabel "Go to Automation Approval Queue Review" `
  -Modules @("automation-approval-queue-review-types.ts","automation-approval-queue-review-summary.ts","index.ts") `
  -Components @("AutomationApprovalQueueReviewPanel.tsx","index.ts") `
  -Exports @("buildAutomationApprovalQueueReviewStableKey","buildAutomationApprovalQueueReview","buildAutomationApprovalQueueReviews","buildAutomationApprovalQueueReviewBoundary","buildAutomationApprovalQueueReviewModel","summarizeAutomationApprovalQueueReview","AUTOMATION_APPROVAL_QUEUE_REVIEW_LANGUAGE") `
  -PhaseMarkers $phaseMarkers `
  -PlainEnglish @(@("automation approval queue identity","operator decision checklist","denied approval shortcuts","retention and audit notes","blocked approval queue risks","schedule safety route","automation release candidate route","next recommended action") + $automationReadinessSafetyMarkers) `
  -ExtraRoutes @("/automation-dry-run-trial-review","/automation-schedule-safety-review","/automation-integration-release-candidate","/approval-queue")
...
Write-Host "[OK] CodexForge Automation Approval Queue Review smoke passed."
