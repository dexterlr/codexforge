...
$sourceParts = @()
foreach ($scanRoot in @("src\lib\codexforge\render-job-boundary", $Domain, $Route, (Join-Path "scripts" $ScriptFile))) {
...
Assert-Contains $source "RenderJobBoundaryRoutePanel" "main route panel"
Assert-Contains $source "RenderJobBoundaryCockpitSummaryPanel" "cockpit render job boundary summary panel"
...
Write-Host ("[OK] " + $SmokeName + " static render job boundary smoke passed.")
