...
$domain = "src\lib\codexforge\connector-evidence-handoff-review"
$route = "src\app\connector-evidence-handoff-review"
$newRoutes = @(
...
& (Join-Path $PSScriptRoot "codexforge-local-project-knowledge-smoke-helper.ps1") `
  -PhaseName "Phase 436 Connector Evidence Handoff Review" `
  -ScriptFile "smoke-codexforge-connector-evidence-handoff-review.ps1" `
  -Domain $domain `
  -Route $route `
  -MainPanel "ConnectorEvidenceHandoffReviewPanel" `
  -CommandLabel "Go to Connector Evidence Handoff Review" `
  -Modules @("connector-evidence-handoff-review-types.ts","connector-evidence-handoff-review-summary.ts","index.ts") `
  -Components @("ConnectorEvidenceHandoffReviewPanel.tsx","index.ts") `
  -Exports @("buildConnectorEvidenceHandoffReviewStableKey","buildConnectorEvidenceHandoffReview","buildConnectorEvidenceHandoffReviews","buildConnectorEvidenceHandoffReviewBoundary","buildConnectorEvidenceHandoffReviewModel","summarizeConnectorEvidenceHandoffReview","CONNECTOR_EVIDENCE_HANDOFF_REVIEW_LANGUAGE") `
  -PhaseMarkers $phaseMarkers `
  -PlainEnglish @(@("connector evidence handoff identity","review-before-use policy","denied handoff actions","memory and research boundary notes","blocked evidence handoff risks","connector release candidate route","automation dry-run route","next recommended action") + $connectorReadinessSafetyMarkers) `
  -ExtraRoutes @("/connector-data-redaction-trial-review","/connector-integration-release-candidate","/automation-permission-readiness-audit","/research-evidence-inbox")
...
Write-Host "[OK] CodexForge Connector Evidence Handoff Review smoke passed."
