...
$domain = "src\lib\codexforge\openai-compatible-live-test-trial"
$route = "src\app\openai-compatible-live-test-trial"

& (Join-Path $PSScriptRoot "codexforge-local-planning-phase-smoke-helper.ps1") `
  -PhaseName "Phase 289 OpenAI-Compatible Live Test Trial" `
  -ScriptFile "smoke-codexforge-openai-compatible-live-test-trial.ps1" `
  -Domain $domain `
  -Route $route `
  -MainPanel "OpenAiCompatibleLiveTestTrialPanel" `
  -CommandLabel "Go to OpenAI Compatible Live Test Trial" `
  -Modules @("openai-compatible-live-test-trial-types.ts","openai-compatible-live-test-trial-summary.ts","index.ts") `
  -Components @("OpenAiCompatibleLiveTestTrialPanel.tsx","index.ts") `
  -Exports @("buildOpenAiCompatibleLiveTestTrialStableKey","buildOpenAiCompatibleLiveTestTrial","buildOpenAiCompatibleLiveTestTrials","buildOpenAiCompatibleLiveTestTrialBoundary","buildOpenAiCompatibleLiveTestTrialModel","summarizeOpenAiCompatibleLiveTestTrial","OPENAI_COMPATIBLE_LIVE_TEST_TRIAL_LANGUAGE") `
  -PlainEnglish @("OpenAI compatible live test trial","OpenAI compatible tests require explicit approval","No API request is sent from this page","API keys and secrets are never displayed","Estimated cost token guardrail","Expected response shape","Test trial identity","Provider profile","Base URL / endpoint summary","Model summary","Payload privacy classification","Approval status","Result capture route","Blocked reasons","provider tests require explicit approval","advanced trial details collapsed/secondary","server-only path boundary markers remain intact","no automatic local action","no raw fetch from arbitrary UI","no command execution","no shell command execution","no test execution from UI","no git command execution from UI","no direct Jarvisd call from arbitrary UI","no Jarvisd capability execution from UI","no daemon process creation from frontend","no browser-stored signing secrets","no session token localStorage storage","no arbitrary local file browsing","no arbitrary path crawling","no arbitrary file read/open","no auto-open local files","no file mutation","no file write","no patch apply behavior","no file deletion","no secret value display","no automatic provider send","no provider APIs are called","no OpenAI-compatible API calls","no API request sent","no prompt/file sending without approval","no auto-spend tokens","no localStorage API key storage","no process.env printing","no direct appendEvent call from UI","no direct saveBrainGraph call from UI","no direct graph mutation from UI","no memory auto-promotion","no process kill/restart/shutdown from UI","no package install behavior") `
  -ExtraRoutes @("/provider-live-test-runner-boundary","/provider-live-test-gate","/openai-compatible-live-test","/provider-policy-bundle","/provider-test-results")
...
foreach ($needle in @(
  ...
  Assert-Contains $source $needle "OpenAI compatible live test trial includes $needle"
}
...
$blockedPatterns = @{
  ...
  "no provider API calls" = "providerApiCallsAllowedFromUi:\s*true|api\.openai|api\.anthropic|generativelanguage|sendToProvider\s*\(|callProviderApi\s*\("
  "no OpenAI-compatible API calls" = "openAiCompatibleApiCallsAllowedFromUi:\s*true|apiRequestSentFromPageAllowed:\s*true|callOpenAiCompatible\s*\(|openAiCompatibleRequest\s*\("
...
Write-Host "[OK] CodexForge OpenAI-Compatible Live Test Trial smoke passed."
