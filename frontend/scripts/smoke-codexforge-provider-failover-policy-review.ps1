...
$domain = "src\lib\codexforge\provider-failover-policy-review"
$route = "src\app\provider-failover-policy-review"
$newRoutes = @(
...
& (Join-Path $PSScriptRoot "codexforge-local-project-knowledge-smoke-helper.ps1") `
  -PhaseName "Phase 416 Provider Failover Policy Review" `
  -ScriptFile "smoke-codexforge-provider-failover-policy-review.ps1" `
  -Domain $domain `
  -Route $route `
  -MainPanel "ProviderFailoverPolicyReviewPanel" `
  -CommandLabel "Go to Provider Failover Policy Review" `
  -Modules @("provider-failover-policy-review-types.ts","provider-failover-policy-review-summary.ts","index.ts") `
  -Components @("ProviderFailoverPolicyReviewPanel.tsx","index.ts") `
  -Exports @("buildProviderFailoverPolicyReviewStableKey","buildProviderFailoverPolicyReview","buildProviderFailoverPolicyReviews","buildProviderFailoverPolicyReviewBoundary","buildProviderFailoverPolicyReviewModel","summarizeProviderFailoverPolicyReview","PROVIDER_FAILOVER_POLICY_REVIEW_LANGUAGE") `
  -PhaseMarkers $phaseMarkers `
  -PlainEnglish @(@("failover policy identity","retry/backoff review","denied failover shortcuts","validation evidence requirements","blocked failover risks","multi-provider routing route","credential boundary route","next recommended action") + $failoverSafetyMarkers) `
  -ExtraRoutes @("/remote-provider-credential-boundary-review","/multi-provider-routing-release-candidate","/openai-compatible-provider-trial-review","/provider-routing-readiness-audit")
...
foreach ($marker in @($phaseMarkers + $failoverSafetyMarkers + @("no Date.now","no Date.now for deterministic layout/ids","no Math.random","no obvious duplicate React key patterns"))) {
...
Write-Host "[OK] CodexForge Provider Failover Policy Review smoke passed."
