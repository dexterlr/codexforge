...
$domain = "src\lib\codexforge\git-diff-review-surface"
$route = "src\app\git-diff-review"

& (Join-Path $PSScriptRoot "codexforge-local-planning-phase-smoke-helper.ps1") `
  -PhaseName "Git Diff Review Surface" `
  -ScriptFile "smoke-codexforge-git-diff-review-surface.ps1" `
  -Domain $domain `
  -Route $route `
  -MainPanel "GitDiffReviewSurfacePanel" `
  -CommandLabel "Go to Git Diff Review Surface" `
  -Modules @("git-diff-review-surface-types.ts","git-diff-review-surface-summary.ts","index.ts") `
  -Components @("GitDiffReviewSurfacePanel.tsx","index.ts") `
  -Exports @("buildGitDiffReviewSurfaceStableKey","buildGitDiffReview","buildGitDiffReviews","buildGitDiffReviewSurfaceBoundary","buildGitDiffReviewSurfaceModel","summarizeGitDiffReviewSurface","GIT_DIFF_REVIEW_SURFACE_LANGUAGE") `
  -PlainEnglish @("Git diff review surface","Raw diffs stay secondary","Git diff is not run from this page","Suspected secrets are redacted","Diff review identity","Source git status review","Changed files summary","Diff summary","Risk/secrets status","Test result dependency","Patch result dependency","Review checklist","Commit message route","Blocked reasons","advanced diff details collapsed/secondary","approved local boundary required","git commands are not run from arbitrary UI","no git command execution from UI","nothing executes from arbitrary UI","no automatic local action","no raw fetch from arbitrary UI","no command execution","no shell command execution","no commit creation","no push behavior","no arbitrary local file browsing","no arbitrary file read/open","no file mutation","no file deletion","no patch apply behavior","no package install behavior","no provider APIs are called","no automatic provider send","no secrets displayed","no secrets exported","no secrets included","no localStorage API key storage","no process.env printing","no direct appendEvent call from UI","no direct saveBrainGraph call from UI","no direct graph mutation from UI","no memory auto-promotion") `
  -ExtraRoutes @("/git-status-review","/project-risk-secrets-scan","/patch-result-capture","/test-result-summary","/commit-message-builder")
...
foreach ($needle in @(
  ...
  Assert-Contains $source $needle "git diff review includes $needle"
}
...
$blockedPatterns = @{
  ...
  "no git command execution from UI" = "gitCommandExecutionAllowedFromUi:\s*true|gitDiffRunsFromPageAllowed:\s*true|runGitCommand\s*\(|executeGitCommand\s*\("
...
Write-Host "[OK] CodexForge Git Diff Review Surface smoke passed."
