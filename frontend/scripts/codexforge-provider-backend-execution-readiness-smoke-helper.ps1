...
function Join-RepoPath {
  ...
  $path = $safeSegments[0]
  for ($index = 1; $index -lt $safeSegments.Count; $index++) { $path = Join-Path $path $safeSegments[$index] }
  return $path
}
function Normalize-SmokeScriptFile { param([string]$Value) $scriptFileName = Split-Path -Leaf $Value.Trim().Replace("/", "\") if ([string]::IsNullOrWhiteSpace($scriptFileName)) { throw "[FAIL] Missing smoke script file name" } if ($scriptFileName -notlike "smoke-codexforge-*.ps1") { throw ("[FAIL] Unexpected smoke script file name: " + $scriptFileName) } return $scriptFileName }
function Normalize-RouteSlug { param([string]$Value, [string]$FallbackHref) $candidate = $Value; if ([string]::IsNullOrWhiteSpace($candidate)) { $candidate = $FallbackHref } $candidate = $candidate.Trim().Replace("\", "/").Trim("/"); if ($candidate.StartsWith("src/lib/codexforge/")) { $candidate = $candidate.Substring("src/lib/codexforge/".Length) } if ($candidate.StartsWith("src/app/")) { $candidate = $candidate.Substring("src/app/".Length) } return $candidate.Trim("/") }
...
$sharedBoundaryPath = Join-RepoPath "src" "lib" "codexforge" "provider-backend-execution-readiness-map"
...
foreach ($path in @($domainPath, $routePath, $routePagePath, $routeClientPath, $scriptPath, $sharedBoundaryPath)) { if (-not (Test-Path $path)) { throw ("[FAIL] Missing path: " + $path) } Write-Host ("[PASS] path exists " + $path) }
$sourceParts = @()
foreach ($scanRoot in @($sharedBoundaryPath, $domainPath, $routePath, $scriptPath)) { if (Test-Path $scanRoot -PathType Leaf) { $sourceParts += Get-Content -Raw $scanRoot } else { $sourceParts += Get-ChildItem -Recurse -File $scanRoot | ForEach-Object { Get-Content -Raw $_.FullName } } }
...
foreach ($needle in @("ProviderBackendExecutionReadinessPageClientShell", "ProviderBackendExecutionReadinessRoutePanel", "Provider Backend Execution Readiness", "Provider Backend Execution Readiness Map", "Provider Execution Prerequisite Matrix", "Provider Backend Execution Contract", "Provider Server Runtime Boundary", "Provider Credential Injection Readiness", "Provider Token Redaction Readiness", "Provider Approval Enforcement Readiness", "Provider Audit Persistence Readiness", "Provider Prompt Boundary Readiness", "Provider Response Capture Readiness", "Provider SDK Isolation Readiness", "Provider Network Egress Readiness", "Provider Rate Limit Enforcement Readiness", "Provider Timeout Enforcement Readiness", "Provider Retry Fallback Readiness", "Provider Cost Guard Readiness", "Provider Safety Guard Readiness", "Provider Privacy Guard Readiness", "Provider Observability Readiness", "Provider Dry Run To Real Bridge", "Disabled Provider Backend Execution Lane", "Provider Backend Execution Cockpit Readiness Rail", "Provider Backend Execution State", "Provider Backend Execution Recovery", "Provider Backend Execution Acceptance Criteria", "Provider Backend Execution Fixture Safety Guard", "Provider Backend Execution Prompt Transmission Blocker", "Provider Backend Execution Credential Token Blocker", "Provider Backend Execution Streaming Blocker", "Provider Backend Execution Completion Candidate", "Review-only provider backend execution readiness", "Synthetic provider backend execution readiness data only", "No live provider execution", "No provider calls", "No model calls", "No prompt sending", "No credential storage", "No token storage", "No streaming", "No frontend persistence", "No browser storage writes", "No connector calls", "No upload", "No download", "No render", "No export", "No publish", "No schedule", "No queue dispatch", "No worker dispatch", "No database writes", "No command execution", "No service creation", "No API creation from frontend", "No route handlers for live provider execution", "Backend-owned provider adapter remains required", "Explicit operator approval required", "Audit trail required", "next batch: 2506-2537 - First Real Provider Call Guard Mega Batch v1", $RouteHref, $CommandLabel, $Title, "disabled")) { Assert-Contains $source $needle ("source marker " + $needle) }
...
$networkOrStoragePattern = @(("fetch" + "("), "XMLHttpRequest", "EventSource", "WebSocket", ("local" + "Storage"), ("session" + "Storage"), ("document" + "." + "cookie")) | ForEach-Object { [regex]::Escape($_) }
Assert-NotMatches $source ($networkOrStoragePattern -join "|") "network or browser storage APIs"
$nondeterministicPattern = @(("Math" + "." + "random"), ("Date" + "." + "now"), ("crypto" + "." + "randomUUID")) | ForEach-Object { [regex]::Escape($_) }
...
Assert-NotMatches $source $unsafeApiNamePattern "unsafe camelCase provider backend execution API names"
Write-Host ("[OK] " + $SmokeName + " static provider backend execution readiness smoke passed.")
