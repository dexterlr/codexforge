...
$domain = "src\lib\codexforge\global-review-inbox-consolidation"
$route = "src\app\global-review-inbox"
$protectedRoutes = @(
  "/daily-operator-home",
  "/global-review-inbox",
  "/approval-queue",
...
& (Join-Path $PSScriptRoot "codexforge-local-project-knowledge-smoke-helper.ps1") `
  -PhaseName "Phase 371 Global Review Inbox Consolidation" `
  -ScriptFile "smoke-codexforge-global-review-inbox-consolidation.ps1" `
  -Domain $domain `
  -Route $route `
  -MainPanel "GlobalReviewInboxConsolidationPanel" `
  -CommandLabel "Go to Global Review Inbox" `
  -Modules @("global-review-inbox-consolidation-types.ts","global-review-inbox-consolidation-summary.ts","index.ts") `
  -Components @("GlobalReviewInboxConsolidationPanel.tsx","index.ts") `
  -Exports @("buildGlobalReviewInboxConsolidationStableKey","buildGlobalReviewInboxConsolidation","buildGlobalReviewInboxConsolidations","buildGlobalReviewInboxConsolidationBoundary","buildGlobalReviewInboxConsolidationModel","summarizeGlobalReviewInboxConsolidation","GLOBAL_REVIEW_INBOX_CONSOLIDATION_LANGUAGE") `
  -PhaseMarkers $phaseMarkers `
  -PlainEnglish @(@("Global inbox identity","Source loop summary","Risk/severity summary","Privacy/redaction status","Approval queue route","Result history route","Next recommended action","advanced inbox details collapsed/secondary") + $dailyCockpitSafetyMarkers) `
  -ExtraRoutes @("/daily-operator-home","/approval-queue","/result-history","/cross-loop-safety-audit-inbox")
...
$blockedPatterns = @{
  ...
  "no approval automation" = "approvalAutomationAllowedFromUi:\s*true|automaticApprovalAllowedFromUi:\s*true"
...
Write-Host "[OK] CodexForge Global Review Inbox Consolidation smoke passed."
