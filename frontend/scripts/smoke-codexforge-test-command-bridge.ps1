...
$domain = "src\lib\codexforge\test-command-bridge"
$route = "src\app\test-command-bridge"

& (Join-Path $PSScriptRoot "codexforge-local-planning-phase-smoke-helper.ps1") `
  -PhaseName "Test Command Bridge" `
  -ScriptFile "smoke-codexforge-test-command-bridge.ps1" `
  -Domain $domain `
  -Route $route `
  -MainPanel "TestCommandBridgePanel" `
  -CommandLabel "Go to Test Command Bridge" `
  -Modules @("test-command-bridge-types.ts","test-command-bridge-summary.ts","index.ts") `
  -Components @("TestCommandBridgePanel.tsx","index.ts") `
  -Exports @("buildTestCommandBridgeStableKey","buildTestCommandBridge","buildTestCommandBridges","buildTestCommandBridgeBoundary","buildTestCommandBridgeModel","summarizeTestCommandBridge","TEST_COMMAND_BRIDGE_LANGUAGE") `
  -PlainEnglish @("Test command bridge","Test commands are not run from this page","Env values and secrets are never displayed","Execution remains behind approved local boundary","Recommended command summary","Audit handoff","Bridge identity","Source test command planner","Source command dry-run bridge","Workspace trust dependency","Permission enforcement dependency","Working directory scope","Expected duration/risk","Environment/secrets safety note","Blocked reasons","advanced bridge details collapsed/secondary","test bridges do not run commands","approved local boundary required","tests are not run automatically","no test execution from UI","no giant raw test output above fold","no automatic local action","no raw fetch from arbitrary UI","no command execution","no shell command execution","no runCommand brokerExecution or local executor API calls from UI","no direct Jarvisd call from arbitrary UI","no Jarvisd capability execution from UI","no daemon process creation from frontend","no browser-stored signing secrets","no session token localStorage storage","no arbitrary local file browsing","no arbitrary file read/open","no auto-open local files","no file mutation","no file write","no file deletion","no patch apply behavior","no secret value display","no automatic provider send","no provider APIs are called","no GitHub API calls from UI","no localStorage API key storage","no process.env printing","no audit log mutation from UI","no direct appendEvent call from UI","no direct saveBrainGraph call from UI","no direct graph mutation from UI","no memory auto-promotion","no process kill/restart/shutdown from UI","no package install behavior","no weakened safe path checks","no removed server-only boundaries") `
  -ExtraRoutes @("/test-command-planner","/command-dry-run","/workspace-trust-policy","/jarvisd-runtime-enforcement","/jarvisd-audit-ingestion","/test-execution-trial")
...
foreach ($needle in @(
  ...
  Assert-Contains $source $needle "test command bridge includes $needle"
}
...
$blockedPatterns = @{
  ...
  "no test execution from UI" = "testCommandsRunFromPageAllowed:\s*true|testExecutionFromUiAllowed:\s*true|testsRunFromPageAllowed:\s*true|testsExecutedFromPageAllowed:\s*true|runTests\s*\(|executeTests\s*\("
...
Write-Host "[OK] CodexForge Test Command Bridge smoke passed."
