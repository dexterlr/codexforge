...
$domain = "src\lib\codexforge\connector-integration-release-candidate"
$route = "src\app\connector-integration-release-candidate"
$newRoutes = @(
...
& (Join-Path $PSScriptRoot "codexforge-local-project-knowledge-smoke-helper.ps1") `
  -PhaseName "Phase 437 Connector Integration Release Candidate" `
  -ScriptFile "smoke-codexforge-connector-integration-release-candidate.ps1" `
  -Domain $domain `
  -Route $route `
  -MainPanel "ConnectorIntegrationReleaseCandidatePanel" `
  -CommandLabel "Go to Connector Integration Release Candidate" `
  -Modules @("connector-integration-release-candidate-types.ts","connector-integration-release-candidate-summary.ts","index.ts") `
  -Components @("ConnectorIntegrationReleaseCandidatePanel.tsx","index.ts") `
  -Exports @("buildConnectorIntegrationReleaseCandidateStableKey","buildConnectorIntegrationReleaseCandidate","buildConnectorIntegrationReleaseCandidates","buildConnectorIntegrationReleaseCandidateBoundary","buildConnectorIntegrationReleaseCandidateModel","summarizeConnectorIntegrationReleaseCandidate","CONNECTOR_INTEGRATION_RELEASE_CANDIDATE_LANGUAGE") `
  -PhaseMarkers $phaseMarkers `
  -PlainEnglish @(@("connector integration candidate identity","permission boundary status","redaction boundary status","denied connector paths","blocked integration risks","automation dry-run route","automation approval queue route","next recommended action") + $connectorReadinessSafetyMarkers) `
  -ExtraRoutes @("/connector-live-permission-trial-review","/connector-data-redaction-trial-review","/connector-evidence-handoff-review","/approval-queue","/automation-permission-readiness-audit")
...
Write-Host "[OK] CodexForge Connector Integration Release Candidate smoke passed."
