...
$domain = "src\lib\codexforge\jarvisd-safe-file-search-bridge"
$route = "src\app\jarvisd-file-search-bridge"

& (Join-Path $PSScriptRoot "codexforge-local-planning-phase-smoke-helper.ps1") `
  -PhaseName "Jarvisd Safe File Search Bridge" `
  -ScriptFile "smoke-codexforge-jarvisd-safe-file-search-bridge.ps1" `
  -Domain $domain `
  -Route $route `
  -MainPanel "JarvisdSafeFileSearchBridgePanel" `
  -CommandLabel "Go to Jarvisd File Search Bridge" `
  -Modules @("jarvisd-safe-file-search-bridge-types.ts","jarvisd-safe-file-search-bridge-summary.ts","index.ts") `
  -Components @("JarvisdSafeFileSearchBridgePanel.tsx","index.ts") `
  -Exports @("buildJarvisdSafeFileSearchBridgeStableKey","buildJarvisdSafeFileSearchBridge","buildJarvisdSafeFileSearchBridges","buildJarvisdSafeFileSearchBridgeBoundary","buildJarvisdSafeFileSearchBridgeModel","summarizeJarvisdSafeFileSearchBridge","JARVISD_SAFE_FILE_SEARCH_BRIDGE_LANGUAGE") `
  -PlainEnglish @("Jarvisd safe file search bridge","Search only uses approved indexed workspace data","Arbitrary local browsing is not allowed","Sensitive matches stay redacted until review","Excluded paths summary","Result capture route","File bridges are reviewed before use","Approved local boundary required","Search bridge identity","Workspace trust dependency","Index sync dependency","Query summary","Allowed search scope","Sensitive match handling","Redaction status","Audit handoff","Blocked reasons","no automatic local action","no raw fetch from arbitrary UI","no live handshake from arbitrary UI","no daemon process creation from frontend","no browser-stored signing secrets","no session token localStorage storage","no audit log mutation from UI","no direct appendEvent call from UI","no Jarvisd capability execution from UI","no command execution","no arbitrary local file browsing","no arbitrary file read/open","no auto-open local files","no live search from arbitrary UI","no arbitrary path crawling","no file mutation","no file deletion","no secret value display","no automatic provider send","no provider APIs are called","no GitHub API calls from UI","no localStorage API key storage","no process.env printing","no direct saveBrainGraph call from UI","no direct graph mutation from UI","no memory auto-promotion","no package install behavior") `
  -ExtraRoutes @("/jarvisd-index-sync","/workspace-trust-policy","/safe-project-indexer","/jarvisd-secrets-redaction","/review-inbox","/jarvisd-audit-ingestion")
...
foreach ($needle in @(
  ...
  Assert-Contains $source $needle "safe search bridge includes $needle"
}
...
$blockedPatterns = @{
  ...
  "no live search from arbitrary UI" = "liveSearchAllowedFromUi:\s*true|runLiveSearch\s*\(|executeSearch\s*\(|searchFiles\s*\("
...
Write-Host "[OK] CodexForge Jarvisd Safe File Search Bridge smoke passed."
