...
$domain = "src\lib\codexforge\safety-boundary-matrix-finalization"
$route = "src\app\safety-boundary-matrix-finalization"
$protectedRoutes = @(
  "/daily-operator-home",
  "/global-review-inbox",
  "/approval-queue",
...
& (Join-Path $PSScriptRoot "codexforge-local-project-knowledge-smoke-helper.ps1") `
  -PhaseName "Phase 374 Safety Boundary Matrix Finalization" `
  -ScriptFile "smoke-codexforge-safety-boundary-matrix-finalization.ps1" `
  -Domain $domain `
  -Route $route `
  -MainPanel "SafetyBoundaryMatrixFinalizationPanel" `
  -CommandLabel "Go to Safety Boundary Matrix" `
  -Modules @("safety-boundary-matrix-finalization-types.ts","safety-boundary-matrix-finalization-summary.ts","index.ts") `
  -Components @("SafetyBoundaryMatrixFinalizationPanel.tsx","index.ts") `
  -Exports @("buildSafetyBoundaryMatrixFinalizationStableKey","buildSafetyBoundaryMatrixFinalization","buildSafetyBoundaryMatrixFinalizations","buildSafetyBoundaryMatrixFinalizationBoundary","buildSafetyBoundaryMatrixFinalizationModel","summarizeSafetyBoundaryMatrixFinalization","SAFETY_BOUNDARY_MATRIX_FINALIZATION_LANGUAGE") `
  -PhaseMarkers $phaseMarkers `
  -PlainEnglish @(@("Matrix identity","Execution boundaries","Provider boundaries","Connector boundaries","Local file boundaries","Automation boundaries","Unresolved gaps","Recovery playbook route","Expert/novice mode routes","advanced matrix details collapsed/secondary") + $dailyCockpitSafetyMarkers) `
  -ExtraRoutes @("/daily-operator-home","/failure-recovery-playbook-finalization","/novice-mode-guided-flow-polish","/expert-mode-fast-path-review")
...
$blockedPatterns = @{
  "no action execution from UI" = "actionsExecutedFromUi:\s*true|executeAction\s*\(|runAction\s*\("
  ...
  "no approval recovery or rollback automation" = "approvalAutomationAllowedFromUi:\s*true|recoveryAutomationAllowedFromUi:\s*true|rollbackExecutionAllowedFromUi:\s*true|approveAction\s*\(|grantApproval\s*\(|runRecovery\s*\(|executeRecovery\s*\(|runRollback\s*\(|rollbackFiles\s*\("
...
Write-Host "[OK] CodexForge Safety Boundary Matrix Finalization smoke passed."
