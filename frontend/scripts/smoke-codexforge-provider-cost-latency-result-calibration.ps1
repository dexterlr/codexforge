...
$domain = "src\lib\codexforge\provider-cost-latency-result-calibration"
$route = "src\app\provider-cost-latency-calibration"

& (Join-Path $PSScriptRoot "codexforge-local-planning-phase-smoke-helper.ps1") `
  -PhaseName "Phase 293 Provider Cost Latency Result Calibration" `
  -ScriptFile "smoke-codexforge-provider-cost-latency-result-calibration.ps1" `
  -Domain $domain `
  -Route $route `
  -MainPanel "ProviderCostLatencyResultCalibrationPanel" `
  -CommandLabel "Go to Provider Cost Latency Calibration" `
  -Modules @("provider-cost-latency-result-calibration-types.ts","provider-cost-latency-result-calibration-summary.ts","index.ts") `
  -Components @("ProviderCostLatencyResultCalibrationPanel.tsx","index.ts") `
  -Exports @("buildProviderCostLatencyResultCalibrationStableKey","buildProviderCostLatencyResultCalibrationRecord","buildProviderCostLatencyResultCalibrationRecords","buildProviderCostLatencyResultCalibrationBoundary","buildProviderCostLatencyResultCalibrationModel","summarizeProviderCostLatencyResultCalibration","PROVIDER_COST_LATENCY_RESULT_CALIBRATION_LANGUAGE") `
  -PlainEnglish @("Provider cost latency result calibration","Calibration does not auto-update routing","No tokens are spent from this page","Provider policy changes require explicit review","Budget guardrail status","Router recommendation route","Calibration identity","Source provider test results","Provider/model summary","Token/cost estimate summary","Latency summary","Reliability signal","Policy impact note","Blocked reasons","provider tests require explicit approval","advanced metric details collapsed/secondary","server-only path boundary markers remain intact","no automatic local action","no raw fetch from arbitrary UI","no command execution","no shell command execution","no test execution from UI","no git command execution from UI","no direct Jarvisd call from arbitrary UI","no Jarvisd capability execution from UI","no daemon process creation from frontend","no browser-stored signing secrets","no session token localStorage storage","no arbitrary local file browsing","no arbitrary path crawling","no arbitrary file read/open","no auto-open local files","no file mutation","no file write","no patch apply behavior","no file deletion","no secret value display","no automatic provider send","no provider APIs are called","no Anthropic API calls","no OpenAI-compatible API calls","no API request sent","no prompt/file sending without approval","no auto-spend tokens","no auto-route live provider traffic","no silent provider registry mutation","no localStorage API key storage","no process.env printing","no direct appendEvent call from UI","no direct saveBrainGraph call from UI","no direct graph mutation from UI","no memory auto-promotion","no process kill/restart/shutdown from UI","no package install behavior") `
  -ExtraRoutes @("/provider-test-result-persistence","/provider-test-results","/provider-budget-guardrails","/router-recommendation-review","/provider-policy-bundle")
...
foreach ($needle in @(
  ...
  Assert-Contains $source $needle "provider cost latency result calibration includes $needle"
}
...
Write-Host "[OK] CodexForge Provider Cost Latency Result Calibration smoke passed."
