...
$domain = "src\lib\codexforge\live-workflow-regression-matrix"
$route = "src\app\live-workflow-regression-matrix"
$newRoutes = @(
...
& (Join-Path $PSScriptRoot "codexforge-local-project-knowledge-smoke-helper.ps1") `
  -PhaseName "Phase 455 Live Workflow Regression Matrix" `
  -ScriptFile "smoke-codexforge-live-workflow-regression-matrix.ps1" `
  -Domain $domain `
  -Route $route `
  -MainPanel "LiveWorkflowRegressionMatrixPanel" `
  -CommandLabel "Go to Live Workflow Regression Matrix" `
  -Modules @("live-workflow-regression-matrix-types.ts","live-workflow-regression-matrix-summary.ts","index.ts") `
  -Components @("LiveWorkflowRegressionMatrixPanel.tsx","index.ts") `
  -Exports @("buildLiveWorkflowRegressionMatrixStableKey","buildLiveWorkflowRegressionMatrix","buildLiveWorkflowRegressionMatrixReviews","buildLiveWorkflowRegressionMatrixBoundary","buildLiveWorkflowRegressionMatrixModel","summarizeLiveWorkflowRegressionMatrix","LIVE_WORKFLOW_REGRESSION_MATRIX_LANGUAGE") `
  -PhaseMarkers $phaseMarkers `
  -PlainEnglish $plainEnglish `
  -ExtraRoutes @("/first-controlled-live-workflow-replay","/live-workflow-failure-patch-review","/live-workflow-hardening-pass","/controlled-live-workflow-release-candidate")
...
Write-Host "[OK] CodexForge Live workflow regression matrix smoke passed."
