...
$domain = "src\lib\codexforge\provider-permission-presets"
$route = "src\app\provider-permission-presets"
$newRoutes = @(
...
& (Join-Path $PSScriptRoot "codexforge-local-project-knowledge-smoke-helper.ps1") `
  -PhaseName "Phase 431 Provider Permission Presets" `
  -ScriptFile "smoke-codexforge-provider-permission-presets.ps1" `
  -Domain $domain `
  -Route $route `
  -MainPanel "ProviderPermissionPresetsPanel" `
  -CommandLabel "Go to Provider Permission Presets" `
  -Modules @("provider-permission-presets-types.ts","provider-permission-presets-summary.ts","index.ts") `
  -Components @("ProviderPermissionPresetsPanel.tsx","index.ts") `
  -Exports @("buildProviderPermissionPresetsStableKey","buildProviderPermissionPreset","buildProviderPermissionPresets","buildProviderPermissionPresetsBoundary","buildProviderPermissionPresetsModel","summarizeProviderPermissionPresets","PROVIDER_PERMISSION_PRESETS_LANGUAGE") `
  -PhaseMarkers $phaseMarkers `
  -PlainEnglish @(@("provider permission preset identity","Denied permission shortcuts","Approval gate checklist","Privacy and credential notes","Blocked preset risks","Audit trail route","Hardening pass route","Next recommended action") + $providerReviewSafetyMarkers) `
  -ExtraRoutes @("/provider-selection-ux-polish","/provider-audit-trail-review","/provider-integration-hardening-pass","/connector-permission-readiness-audit")
...
$blockedPatterns = @{
  "no provider permission preset persistence or grants" = "providerPermissionPresetPersistenceAllowedFromUi:\s*true|providerPermissionGrantAllowedFromUi:\s*true|permissionGrantPersistenceAllowedFromUi:\s*true|persistProviderPermission\s*\(|saveProviderPreset\s*\(|grantProviderPermission\s*\(|grantPermission\s*\("
  "no provider selection persistence switching or configuration changes" = "providerSelectionPersistenceAllowedFromUi:\s*true|liveProviderSwitchingAllowedFromUi:\s*true|providerConfigurationChangesAllowedFromUi:\s*true|providerSettingsPersistenceAllowedFromUi:\s*true|persistProviderSelection\s*\(|saveProviderSelection\s*\(|switchProvider\s*\(|selectLiveProvider\s*\(|changeProviderConfiguration\s*\(|saveProviderSettings\s*\("
...
Write-Host "[OK] CodexForge Provider Permission Presets smoke passed."
