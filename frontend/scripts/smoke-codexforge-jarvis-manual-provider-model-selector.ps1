...
Assert-True ($changedPaths.Count -eq $allowedChangedFiles.Count) "Git scope contains exactly the twelve allowed Slice J files"
foreach ($path in $changedPaths) {
  Assert-True ($allowedChangedFiles -contains $path) "Git scope stays within the allowed Slice J files: $path"
}
...
$approvedCloudWindow = Get-Window $panelSource 'data-codexforge-private-alpha-cloud-execution="disabled"' 1400
...
Assert-Contains $panelSource 'const canCreateBoundRun = selectedTarget !== null && actionInFlight === null;' "Create is disabled without a valid model"
...
Assert-Contains $panelSource 'disabled={selectedProviderId === "groq-cloud"}' "Groq disables the capability select"
...
Assert-Contains $panelSource 'data-codexforge-private-alpha-cloud-boundary="approval-only"' "Cloud warning marker exists"
Assert-Contains $panelSource 'Creating the request does' "Cloud warning states creation makes no Groq call"
Assert-Contains $panelSource 'Approving the request does not contact' "Cloud warning states approval makes no Groq call"
Assert-Contains $panelSource 'Cloud execution is currently disabled.' "Cloud warning states cloud execution remains disabled"
...
Assert-Contains $panelSource '(cloudApprovalOnlyRun && !cloudDataTransferAcknowledged)' "Cloud approval requires both checkboxes"
...
Assert-Contains $panelSource 'No data has been sent to Groq.' "Groq awaiting-approval messaging says no data was sent"
Assert-Contains $panelSource 'No prompt was sent to Groq' "Groq approved messaging says no prompt was sent"
Assert-Contains $panelSource 'data-codexforge-private-alpha-cloud-execution="disabled"' "Cloud execution-disabled marker exists"
Assert-NotMatches $approvedCloudWindow 'type="checkbox"' "Approved Groq UI renders no execution checkbox"
Assert-NotMatches $approvedCloudWindow 'Execute once on local Ollama' "Approved Groq UI renders no execute button"
Assert-NotMatches $approvedCloudWindow 'executePrivateAlphaRun|handleExecute' "Approved Groq UI does not call executePrivateAlphaRun"
...
$longestNewPathLength = ($allowedChangedFiles | ForEach-Object { $_.Length } | Measure-Object -Maximum).Maximum
