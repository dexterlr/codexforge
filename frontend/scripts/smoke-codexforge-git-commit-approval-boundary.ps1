...
$domain = "src\lib\codexforge\git-commit-approval-boundary"
$route = "src\app\git-commit-approval"

& (Join-Path $PSScriptRoot "codexforge-local-planning-phase-smoke-helper.ps1") `
  -PhaseName "Git Commit Approval Boundary" `
  -ScriptFile "smoke-codexforge-git-commit-approval-boundary.ps1" `
  -Domain $domain `
  -Route $route `
  -MainPanel "GitCommitApprovalBoundaryPanel" `
  -CommandLabel "Go to Git Commit Approval Boundary" `
  -Modules @("git-commit-approval-boundary-types.ts","git-commit-approval-boundary-summary.ts","index.ts") `
  -Components @("GitCommitApprovalBoundaryPanel.tsx","index.ts") `
  -Exports @("buildGitCommitApprovalBoundaryStableKey","buildGitCommitApproval","buildGitCommitApprovals","buildGitCommitApprovalBoundary","buildGitCommitApprovalBoundaryModel","summarizeGitCommitApprovalBoundary","GIT_COMMIT_APPROVAL_BOUNDARY_LANGUAGE") `
  -PlainEnglish @("Git commit approval boundary","Commits are not created from this page","Commit creation requires explicit approval","Future git execution remains behind approved local boundary","Commit approval identity","Selected commit message","Git status dependency","Git diff dependency","Test result dependency","Risk/secrets status","Allowed scope","Denied scope","Approval copy","Blocked reasons","advanced approval details collapsed/secondary","approved local boundary required","git commands are not run from arbitrary UI","no git command execution from UI","nothing executes from arbitrary UI","no automatic local action","no raw fetch from arbitrary UI","no command execution","no shell command execution","no commit creation","no push behavior","no arbitrary local file browsing","no arbitrary file read/open","no file mutation","no file deletion","no patch apply behavior","no package install behavior","no provider APIs are called","no automatic provider send","no secrets displayed","no secrets exported","no secrets included","no localStorage API key storage","no process.env printing","no direct appendEvent call from UI","no direct saveBrainGraph call from UI","no direct graph mutation from UI","no memory auto-promotion") `
  -ExtraRoutes @("/commit-message-builder","/git-status-review","/git-diff-review","/test-result-summary","/local-command-approval")
...
foreach ($needle in @(
  ...
  Assert-Contains $source $needle "git commit approval includes $needle"
}
...
Write-Host "[OK] CodexForge Git Commit Approval Boundary smoke passed."
