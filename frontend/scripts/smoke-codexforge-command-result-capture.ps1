...
$domain = "src\lib\codexforge\command-result-capture"
$route = "src\app\command-result-capture"

& (Join-Path $PSScriptRoot "codexforge-local-planning-phase-smoke-helper.ps1") `
  -PhaseName "Command Result Capture" `
  -ScriptFile "smoke-codexforge-command-result-capture.ps1" `
  -Domain $domain `
  -Route $route `
  -MainPanel "CommandResultCapturePanel" `
  -CommandLabel "Go to Command Result Capture" `
  -Modules @("command-result-capture-types.ts","command-result-capture-summary.ts","index.ts") `
  -Components @("CommandResultCapturePanel.tsx","index.ts") `
  -Exports @("buildCommandResultCaptureStableKey","buildCommandResult","buildCommandResults","buildCommandResultCaptureBoundary","buildCommandResultCaptureModel","summarizeCommandResultCapture","COMMAND_RESULT_CAPTURE_LANGUAGE") `
  -PlainEnglish @("Command result capture","Results are reviewed before promotion","Raw output stays secondary","Memory is not auto-promoted","Output summary","Review inbox handoff","Result identity","Source execution trial","Command status: passed, failed, blocked, timed out, needs review","Command summary","Environment/secrets redaction status","Affected files indicator","Recovery route","Blocked reasons","advanced output details collapsed/secondary","no automatic local action","no raw fetch from arbitrary UI","no command execution","no shell command execution","no runCommand brokerExecution or local executor API calls from UI","no direct Jarvisd call from arbitrary UI","no Jarvisd capability execution from UI","no daemon process creation from frontend","no browser-stored signing secrets","no session token localStorage storage","no arbitrary local file browsing","no arbitrary file read/open","no auto-open local files","no file mutation","no file write","no file deletion","no patch apply behavior","no secret value display","no automatic provider send","no provider APIs are called","no GitHub API calls from UI","no localStorage API key storage","no process.env printing","no audit log mutation from UI","no direct appendEvent call from UI","no direct saveBrainGraph call from UI","no direct graph mutation from UI","no memory auto-promotion","no process kill/restart/shutdown from UI","no package install behavior") `
  -ExtraRoutes @("/command-execution-trial","/review-inbox","/command-failure-recovery","/run-history","/jarvisd-audit-ingestion")
...
foreach ($needle in @(
  ...
  Assert-Contains $source $needle "command result capture includes $needle"
}
...
Write-Host "[OK] CodexForge Command Result Capture smoke passed."
