...
Write-Host "=== CodexForge Backend-Owned Minimal Manual-Gated Synthetic Dry-Run Audit and Approval Join Review and Recovery Preview Mega Batch smoke ==="
...
$reviewModuleTypesPath = Join-Path $root "src\lib\codexforge\min-synth-audit-join-review\min-synth-audit-join-review-types.ts"
$reviewModuleCatalogPath = Join-Path $root "src\lib\codexforge\min-synth-audit-join-review\min-synth-audit-join-review-catalog.ts"
$reviewModuleIndexPath = Join-Path $root "src\lib\codexforge\min-synth-audit-join-review\index.ts"
...
foreach ($needle in @(
  ...
  "5577",
  "Backend-Owned Minimal Manual-Gated Synthetic Dry-Run Audit and Approval Join Review and Recovery Preview"
)) {
...
foreach ($needle in @(
  ...
  "Athena can review the backend-owned minimal manual-gated synthetic dry-run audit and approval join MVP",
...
foreach ($needle in @(
  ...
  "Athena can now review the backend-owned minimal manual-gated synthetic dry-run audit and approval join MVP",
  "minimal synthetic audit and approval join review is preview-only",
  "server-only synthetic audit and approval join helper exists",
  "synthetic audit and approval join is produced in memory only",
  "no frontend request is created",
...
Assert-Contains $serverHelperNormalized "joinMinimalManualGatedSyntheticDryRunAuditAndApprovalMvp" "server-only audit and approval join helper marker still exists"
Assert-Contains $serverHelperNormalized "deterministic synthetic audit and approval join only" "deterministic synthetic audit and approval join marker still exists"
Assert-Contains $serverHelperNormalized "synthetic audit and approval join is produced in memory only" "in-memory only join marker still exists"

Assert-Contains $allSmokeNormalized "smoke-codexforge-backend-owned-minimal-manual-gated-synthetic-dry-run-audit-approval-join-review-recovery-preview-mega-batch.ps1" "scripts/smoke-codexforge-all.ps1 references this smoke"
Assert-Contains $checkpointNormalized "Highest detected phase: 5577" "checkpoint current doc reports Highest detected phase: 5577"
Assert-Contains $checkpointNormalized "Latest completed batch: 5546-5577 - Backend-Owned Minimal Manual-Gated Synthetic Dry-Run Audit and Approval Join Review and Recovery Preview" "checkpoint current doc reports latest completed batch"
Assert-Contains $checkpointNormalized "Next likely batch: 5578-5609 - Backend-Owned Minimal Manual-Gated Synthetic Dry-Run End-to-End Packet MVP" "checkpoint current doc reports next likely batch"
...
Assert-Contains $athenaPanelSource '"synthetic-audit-approval-join-review-capability"' "AthenaCommandCenterPanel scopes synthetic audit approval join review capability keys"
Assert-Contains $athenaPanelSource '"synthetic-audit-approval-join-review-workspace"' "AthenaCommandCenterPanel scopes synthetic audit approval join review workspace keys"
...
Write-Host "[PASS] Backend-owned minimal manual-gated synthetic dry-run audit and approval join review and recovery preview smoke passed"
