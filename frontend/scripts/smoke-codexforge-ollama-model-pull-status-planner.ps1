...
$domain = "src\lib\codexforge\ollama-model-pull-status-planner"
$route = "src\app\ollama-model-planner"

& (Join-Path $PSScriptRoot "codexforge-local-planning-phase-smoke-helper.ps1") `
  -PhaseName "Ollama Model Pull/Status Planner" `
  -ScriptFile "smoke-codexforge-ollama-model-pull-status-planner.ps1" `
  -Domain $domain `
  -Route $route `
  -MainPanel "OllamaModelPullStatusPlannerPanel" `
  -CommandLabel "Go to Ollama Model Planner" `
  -Modules @("ollama-model-pull-status-planner-types.ts","ollama-model-pull-status-planner-summary.ts","index.ts") `
  -Components @("OllamaModelPullStatusPlannerPanel.tsx","index.ts") `
  -Exports @("buildOllamaModelPlannerStableKey","buildOllamaModelPullStatusPlan","buildOllamaModelPullStatusPlans","buildOllamaModelPullStatusBoundary","buildOllamaModelPullStatusPlannerModel","summarizeOllamaModelPullStatusPlanner") `
  -PlainEnglish @("Ollama model pull and status planner","Model pulls are manual","Future approved local daemon only","Manual command handoff","Do not download models","No live pull button","local-only","readiness-gated","planning only unless approved local boundary exists","no localStorage API key storage","no arbitrary local file browsing","no model download/install behavior","no automatic provider send","no shell command execution") `
  -ExtraRoutes @("/local-model-manager","/provider-adapters","/local-machine","/task-router","/token-router")
...
Assert-Contains $index './ollama-model-pull-status-planner-types' "index exports Ollama planner types"
Assert-Contains $index './ollama-model-pull-status-planner-summary' "index exports Ollama planner builders"
...
Write-Host "[OK] CodexForge Ollama Model Pull/Status Planner smoke passed."
