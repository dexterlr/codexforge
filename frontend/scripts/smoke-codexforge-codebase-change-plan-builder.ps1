...
$domain = "src\lib\codexforge\codebase-change-plan-builder"
$route = "src\app\codebase-change-plan"

& (Join-Path $PSScriptRoot "codexforge-local-planning-phase-smoke-helper.ps1") `
  -PhaseName "Codebase Change Plan Builder" `
  -ScriptFile "smoke-codexforge-codebase-change-plan-builder.ps1" `
  -Domain $domain `
  -Route $route `
  -MainPanel "CodebaseChangePlanBuilderPanel" `
  -CommandLabel "Go to Codebase Change Plan Builder" `
  -Modules @("codebase-change-plan-builder-types.ts","codebase-change-plan-builder-summary.ts","index.ts") `
  -Components @("CodebaseChangePlanBuilderPanel.tsx","index.ts") `
  -Exports @("buildCodebaseChangePlanBuilderStableKey","buildCodebaseChangePlan","buildCodebaseChangePlans","buildCodebaseChangePlanBuilderBoundary","buildCodebaseChangePlanBuilderModel","summarizeCodebaseChangePlanBuilder","CODEBASE_CHANGE_PLAN_BUILDER_LANGUAGE") `
  -PlainEnglish @("Codebase change plan builder","Change plans do not modify files","Arbitrary local browsing is not allowed","Secrets stay redacted","Change request summary","Affected area","Project index dependency","Dependency/risk signal summary","Proposed file targets","Non-goals","Risk level","Approval requirement","Patch preview route","Blocked reasons","approved local boundary required","nothing reads arbitrary files automatically","nothing executes from arbitrary UI","no automatic local action","no raw fetch from arbitrary UI","no command execution","no shell command execution","no arbitrary local file browsing","no arbitrary file read/open","no file mutation","no file deletion","no patch apply behavior","no package install behavior","no provider APIs are called","no automatic provider send","no secrets displayed","no secrets exported","no secrets included","no localStorage API key storage","no process.env printing","no direct appendEvent call from UI","no direct saveBrainGraph call from UI","no direct graph mutation from UI","no memory auto-promotion") `
  -ExtraRoutes @("/safe-project-indexer","/project-dependency-map","/project-risk-secrets-scan","/local-file-approval","/patch-preview-workbench")
...
foreach ($needle in @(
  ...
  Assert-Contains $source $needle "codebase change plan builder includes $needle"
}
...
$blockedPatterns = @{
  ...
  "no file mutation" = "changePlansModifyFilesAllowed:\s*true|fileMutationAllowedFromUi:\s*true|writeFile\s*\(|mutateFiles\s*\(|applyPatchAutomatically"
  ...
  "no live patch generation from UI" = "livePatchGenerationAllowedFromUi:\s*true|generatePatch\s*\("
...
Write-Host "[OK] CodexForge Codebase Change Plan Builder smoke passed."
