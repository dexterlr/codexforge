...
$domain = "src\lib\codexforge\jarvisd-permission-runtime-enforcement"
$route = "src\app\jarvisd-runtime-enforcement"

& (Join-Path $PSScriptRoot "codexforge-local-planning-phase-smoke-helper.ps1") `
  -PhaseName "Jarvisd Permission Runtime Enforcement" `
  -ScriptFile "smoke-codexforge-jarvisd-permission-runtime-enforcement.ps1" `
  -Domain $domain `
  -Route $route `
  -MainPanel "JarvisdPermissionRuntimeEnforcementPanel" `
  -CommandLabel "Go to Jarvisd Runtime Enforcement" `
  -Modules @("jarvisd-permission-runtime-enforcement-types.ts","jarvisd-permission-runtime-enforcement-summary.ts","index.ts") `
  -Components @("JarvisdPermissionRuntimeEnforcementPanel.tsx","index.ts") `
  -Exports @("buildJarvisdPermissionRuntimeEnforcementStableKey","buildJarvisdPermissionRuntimeEnforcementReview","buildJarvisdPermissionRuntimeEnforcementReviews","buildJarvisdPermissionRuntimeEnforcementBoundary","buildJarvisdPermissionRuntimeEnforcementModel","summarizeJarvisdPermissionRuntimeEnforcement","JARVISD_PERMISSION_RUNTIME_ENFORCEMENT_LANGUAGE") `
  -PlainEnglish @("Jarvisd permission runtime enforcement","Enforcement does not grant permissions automatically","Capabilities are not executed from this page","Expired sessions stay blocked","Allowed runtime scope","Audit handoff","Runtime controls are reviewed before use","Approved local boundary required","Secrets and signing material are never displayed or stored in browser storage","Enforcement identity","Session consent dependency","Permission boundary dependency","Requested capability","Denied runtime scope","Enforcement decision","Expiry/revocation status","Blocked reasons","no automatic local action","no raw fetch from arbitrary UI","no live handshake from arbitrary UI","no daemon process creation from frontend","no browser-stored signing secrets","no session token localStorage storage","no settings auto-import","no audit log mutation from UI","no direct appendEvent call from UI","no Jarvisd capability execution from UI","no command execution","no arbitrary local file browsing","no arbitrary file read/open","no file mutation","no file deletion","no provider APIs are called","no GitHub API calls from UI","no localStorage API key storage","no process.env printing","no direct saveBrainGraph call from UI","no direct graph mutation from UI","no memory auto-promotion","no package install behavior") `
  -ExtraRoutes @("/jarvisd-session-consent","/jarvisd-permissions","/jarvisd-signed-request","/jarvisd-audit-ingestion","/jarvisd-execution-registry")
...
foreach ($needle in @(
  ...
  Assert-Contains $source $needle "runtime enforcement includes $needle"
}
...
$blockedPatterns = @{
  ...
  "no capability execution" = "capabilityExecutionAllowedFromUi:\s*true|executeJarvisdCapability\s*\(|runJarvisdCapability\s*\("
  "no expired session execution" = "expiredSessionExecutionAllowed:\s*true|runExpiredSession\s*\("
...
Write-Host "[OK] CodexForge Jarvisd Permission Runtime Enforcement smoke passed."
