...
$domain = "src\lib\codexforge\codexforge-extension-architecture-decision"
$route = "src\app\extension-architecture-decision"

& (Join-Path $PSScriptRoot "codexforge-local-planning-phase-smoke-helper.ps1") `
  -PhaseName "Phase 317 CodexForge Extension Architecture Decision" `
  -ScriptFile "smoke-codexforge-extension-architecture-decision.ps1" `
  -Domain $domain `
  -Route $route `
  -MainPanel "CodexForgeExtensionArchitectureDecisionPanel" `
  -CommandLabel "Go to CodexForge Extension Architecture Decision" `
  -Modules @("codexforge-extension-architecture-decision-types.ts","codexforge-extension-architecture-decision-summary.ts","index.ts") `
  -Components @("CodexForgeExtensionArchitectureDecisionPanel.tsx","index.ts") `
  -Exports @("buildCodexForgeExtensionArchitectureDecisionStableKey","buildCodexForgeExtensionArchitectureDecision","buildCodexForgeExtensionArchitectureDecisions","buildCodexForgeExtensionArchitectureDecisionBoundary","buildCodexForgeExtensionArchitectureDecisionModel","summarizeCodexForgeExtensionArchitectureDecision","CODEXFORGE_EXTENSION_ARCHITECTURE_DECISION_LANGUAGE") `
  -PlainEnglish @("CodexForge extension architecture decision","Extension architecture does not enable plugins automatically","Runtime execution remains behind approved boundaries","Third-party adoption requires license security review","Third-party adoption requires license/security review","Staged rollout recommendation","Release decision","Architecture decision identity","Source comparison surfaces","Recommended extension shape","Permission model","Audit/recovery model","Memory/RAG policy","Non-goals","Next recommended route","Proceed","Proceed with fixes","Blocked","no automatic provider calls","no provider API calls","no automatic provider send","no prompt/file sending without approval","no auto-spend tokens","no auto-route live provider traffic","no API key export","no secret export","no localStorage API key storage","no process.env printing","no API keys or secrets displayed","no plugin execution","no tool execution","no agent execution","no MCP runtime","no MCP tool calls","no memory/RAG ingestion","no Brain graph mutation","no appendEvent/saveBrainGraph calls from UI","no Ruflo/Odysseus vendoring","no Ruflo/Odysseus runtime integration","no Ruflo/Odysseus dependency references","future adoption requires license/security review","no ComfyUI job submission","no ComfyUI request sent from UI","no arbitrary local endpoint calls from UI","no uncontrolled polling loops","no render job start/cancel/hold/retry behavior","no command execution","no shell command execution","no git command execution from UI","no test execution from UI","no Jarvisd capability execution from UI","no daemon process creation from frontend","no browser-stored signing secrets","no session token localStorage storage","no arbitrary local file browsing","no arbitrary path crawling","no arbitrary file read/open","no auto-open local files","no file mutation","no file write","no patch apply behavior","no file deletion","no artifact deletion","no memory auto-promotion","no process kill/restart/shutdown from UI","no package install behavior","server-only path boundary markers remain intact","advanced decision details collapsed/secondary","plain English","no duplicate route chip cloud","hero title does not vertically wrap","no giant raw JSON above fold","advanced details collapsed/secondary","no unsafe execution buttons","no real video generation","no image generation","no upscale execution","no frame interpolation execution","no ComfyUI workflow run","no job queue execution","no hardware/system command","no prompt payload sent to providers","no cloud provider API calls","no password storage","no API key localStorage","no raw secret display","no process.env value printed in UI","no hardcoded API keys","no direct appendEvent call from UI","no direct saveBrainGraph call from UI","no direct graph mutation from UI","no direct apply-diff call from UI","no direct write-file call from UI","no direct run-command call from UI","no broker-execution call except blocked-policy text","no Math.random","no Date.now for deterministic layout/ids","no d3-force","no mojibake","no obvious duplicate React key patterns") `
  -ExtraRoutes @("/agent-plugin-registry-comparison","/mcp-tool-boundary-comparison","/agent-memory-rag-pattern-review","/jarvisd-runtime-enforcement","/provider-governance-release-candidate")
...
$blockedPatterns = @{
  ...
  "no plugin execution" = "pluginExecutionAllowedFromUi:\s*true|pluginRuntimeCreated:\s*true|extensionArchitectureEnablesPluginsAutomatically:\s*true|executePlugin\s*\(|runPlugin\s*\("
  ...
  "no MCP runtime" = "mcpRuntimeCreated:\s*true|mcpToolsEnabled:\s*true|mcpServerCreated:\s*true|mcpClientCreated:\s*true|createMcpServer\s*\(|createMcpClient\s*\("
...
Write-Host "[OK] CodexForge Extension Architecture Decision smoke passed."
