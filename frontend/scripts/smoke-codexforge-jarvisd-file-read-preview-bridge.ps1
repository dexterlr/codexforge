...
$domain = "src\lib\codexforge\jarvisd-file-read-preview-bridge"
$route = "src\app\jarvisd-file-preview-bridge"

& (Join-Path $PSScriptRoot "codexforge-local-planning-phase-smoke-helper.ps1") `
  -PhaseName "Jarvisd File Read Preview Bridge" `
  -ScriptFile "smoke-codexforge-jarvisd-file-read-preview-bridge.ps1" `
  -Domain $domain `
  -Route $route `
  -MainPanel "JarvisdFileReadPreviewBridgePanel" `
  -CommandLabel "Go to Jarvisd File Preview Bridge" `
  -Modules @("jarvisd-file-read-preview-bridge-types.ts","jarvisd-file-read-preview-bridge-summary.ts","index.ts") `
  -Components @("JarvisdFileReadPreviewBridgePanel.tsx","index.ts") `
  -Exports @("buildJarvisdFileReadPreviewBridgeStableKey","buildJarvisdFileReadPreviewBridge","buildJarvisdFileReadPreviewBridges","buildJarvisdFileReadPreviewBridgeBoundary","buildJarvisdFileReadPreviewBridgeModel","summarizeJarvisdFileReadPreviewBridge","JARVISD_FILE_READ_PREVIEW_BRIDGE_LANGUAGE") `
  -PlainEnglish @("Jarvisd file read preview bridge","File previews require approved scope","Arbitrary local browsing is not allowed","Secret values stay redacted","Allowed path scope summary","Audit handoff","File bridges are reviewed before use","Approved local boundary required","Preview bridge identity","Source workspace trust","File operation approval dependency","Permission runtime enforcement dependency","Signed request dependency","Denied path scope summary","Redaction status","Preview result status","Blocked reasons","no automatic local action","no raw fetch from arbitrary UI","no live handshake from arbitrary UI","no daemon process creation from frontend","no browser-stored signing secrets","no session token localStorage storage","no audit log mutation from UI","no direct appendEvent call from UI","no Jarvisd capability execution from UI","no command execution","no arbitrary local file browsing","no arbitrary file read/open","no auto-open local files","no live search from arbitrary UI","no arbitrary path crawling","no file mutation","no file deletion","no secret value display","no automatic provider send","no provider APIs are called","no GitHub API calls from UI","no localStorage API key storage","no process.env printing","no direct saveBrainGraph call from UI","no direct graph mutation from UI","no memory auto-promotion","no package install behavior") `
  -ExtraRoutes @("/workspace-trust-policy","/local-file-approval","/jarvisd-runtime-enforcement","/jarvisd-signed-request","/jarvisd-secrets-redaction","/jarvisd-audit-ingestion")
...
foreach ($needle in @(
  ...
  Assert-Contains $source $needle "file preview bridge includes $needle"
}
...
$blockedPatterns = @{
  ...
  "no arbitrary file browsing or read open" = "arbitraryLocalBrowsingAllowed:\s*true|arbitraryFileReadOpenAllowed:\s*true|directFileReadAllowedFromUi:\s*true|showOpenFilePicker|browseLocalFiles\s*\(|readFile\s*\(|openFile\s*\("
  ...
  "no secret value display" = "secretValuesDisplayedAllowed:\s*true|secretsDisplayedAllowed:\s*true|displaySecrets\s*\(|exportSecrets\s*\(|includeSecretFields:\s*true|rawSecretDisplayAllowed:\s*true"
  "no automatic provider send or provider APIs" = "automaticProviderSendAllowed:\s*true|providerApiCallsAllowedFromUi:\s*true|sendToProvider\s*\(|api\.openai|api\.anthropic|generativelanguage|cloudProviderCall\s*\("
...
Write-Host "[OK] CodexForge Jarvisd File Read Preview Bridge smoke passed."
