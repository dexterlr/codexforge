...
$domain = "src\lib\codexforge\project-file-search-preview"
$route = "src\app\project-file-search"

& (Join-Path $PSScriptRoot "codexforge-local-planning-phase-smoke-helper.ps1") `
  -PhaseName "Project File Search Preview" `
  -ScriptFile "smoke-codexforge-project-file-search-preview.ps1" `
  -Domain $domain `
  -Route $route `
  -MainPanel "ProjectFileSearchPreviewPanel" `
  -CommandLabel "Go to Project File Search Preview" `
  -Modules @("project-file-search-preview-types.ts","project-file-search-preview-summary.ts","index.ts") `
  -Components @("ProjectFileSearchPreviewPanel.tsx","index.ts") `
  -Exports @("buildProjectFileSearchPreviewStableKey","buildProjectFileSearchPreview","buildProjectFileSearchPreviews","buildProjectFileSearchPreviewBoundary","buildProjectFileSearchPreviewModel","summarizeProjectFileSearchPreview","PROJECT_FILE_SEARCH_PREVIEW_LANGUAGE") `
  -PlainEnglish @("Project file search preview","Search only uses approved indexed workspace data","Arbitrary local browsing is not allowed","Sensitive matches stay redacted until review","Search scope summary","Trusted workspace dependency","Query summary","Result preview summary","Excluded paths","Sensitive match handling","File operation approval route","Indexer route","Audit note","Blocked reasons","approved local boundary required","nothing reads arbitrary files automatically","nothing executes from arbitrary UI","no automatic local action","no raw fetch from arbitrary UI","no command execution","no shell command execution","no arbitrary local file browsing","no arbitrary file read/open","no file mutation","no file deletion","no package install behavior","no provider APIs are called","no automatic provider send","no secrets displayed","no secrets exported","no secrets included","no localStorage API key storage","no process.env printing") `
  -ExtraRoutes @("/safe-project-indexer","/local-file-approval","/workspace-trust-policy","/project-dependency-map","/project-risk-secrets-scan")
...
foreach ($needle in @(
  ...
  Assert-Contains $source $needle "project file search preview includes $needle"
}
...
Write-Host "[OK] CodexForge Project File Search Preview smoke passed."
