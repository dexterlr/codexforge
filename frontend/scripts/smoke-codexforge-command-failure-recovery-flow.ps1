...
$domain = "src\lib\codexforge\command-failure-recovery-flow"
$route = "src\app\command-failure-recovery"

& (Join-Path $PSScriptRoot "codexforge-local-planning-phase-smoke-helper.ps1") `
  -PhaseName "Command Failure Recovery Flow" `
  -ScriptFile "smoke-codexforge-command-failure-recovery-flow.ps1" `
  -Domain $domain `
  -Route $route `
  -MainPanel "CommandFailureRecoveryFlowPanel" `
  -CommandLabel "Go to Command Failure Recovery" `
  -Modules @("command-failure-recovery-flow-types.ts","command-failure-recovery-flow-summary.ts","index.ts") `
  -Components @("CommandFailureRecoveryFlowPanel.tsx","index.ts") `
  -Exports @("buildCommandFailureRecoveryFlowStableKey","buildCommandFailureRecovery","buildCommandFailureRecoveries","buildCommandFailureRecoveryBoundary","buildCommandFailureRecoveryFlowModel","summarizeCommandFailureRecoveryFlow","COMMAND_FAILURE_RECOVERY_FLOW_LANGUAGE") `
  -PlainEnglish @("Command failure recovery flow","Retry is never automatic","Recovery does not execute commands","No local process is killed or restarted from this page","Safe recovery checklist","Blocked retry reasons","Recovery identity","Source command result","Failure category","Likely cause","Retry eligibility","Affected scope","Next recommended route","Audit handoff","advanced recovery details collapsed/secondary","no automatic local action","no raw fetch from arbitrary UI","no command execution","no shell command execution","no runCommand brokerExecution or local executor API calls from UI","no direct Jarvisd call from arbitrary UI","no Jarvisd capability execution from UI","no daemon process creation from frontend","no browser-stored signing secrets","no session token localStorage storage","no arbitrary local file browsing","no arbitrary file read/open","no auto-open local files","no file mutation","no file write","no file deletion","no patch apply behavior","no secret value display","no automatic provider send","no provider APIs are called","no GitHub API calls from UI","no localStorage API key storage","no process.env printing","no audit log mutation from UI","no direct appendEvent call from UI","no direct saveBrainGraph call from UI","no direct graph mutation from UI","no memory auto-promotion","no process kill/restart/shutdown from UI","no package install behavior") `
  -ExtraRoutes @("/command-result-capture","/review-inbox","/command-dry-run","/command-execution-trial","/jarvisd-audit-ingestion")
...
foreach ($needle in @(
  ...
  Assert-Contains $source $needle "command failure recovery includes $needle"
}

$deterministicSource = $source.Replace("no Math.random", "").Replace("no Date.now for deterministic layout/ids", "").Replace("no Date.now", "")
$blockedPatterns = @{
  "no automatic retry" = "retryAutomaticAllowed:\s*true|autoRetry\s*\(|retryCommand\s*\(|retryOperation\s*\("
  ...
  "no command execution" = "recoveryExecutesCommandsAllowed:\s*true|commandExecutionAllowedFromUi:\s*true|shellExecutionAllowedFromUi:\s*true|child_process|execSync|spawn\s*\(|runCommand\s*\(|brokerExecution\s*\(|localExecutorApiCallAllowedFromUi:\s*true|Start-Process|Invoke-Expression"
...
Write-Host "[OK] CodexForge Command Failure Recovery Flow smoke passed."
