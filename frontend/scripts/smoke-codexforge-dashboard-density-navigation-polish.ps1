...
$domain = "src\lib\codexforge\dashboard-density-navigation-polish"
$route = "src\app\dashboard-density-navigation-polish"
$protectedRoutes = @(
  "/daily-operator-home",
  "/global-review-inbox",
  "/approval-queue",
...
& (Join-Path $PSScriptRoot "codexforge-local-project-knowledge-smoke-helper.ps1") `
  -PhaseName "Phase 378 Dashboard Density / Navigation Polish" `
  -ScriptFile "smoke-codexforge-dashboard-density-navigation-polish.ps1" `
  -Domain $domain `
  -Route $route `
  -MainPanel "DashboardDensityNavigationPolishPanel" `
  -CommandLabel "Go to Dashboard Density Navigation Polish" `
  -Modules @("dashboard-density-navigation-polish-types.ts","dashboard-density-navigation-polish-summary.ts","index.ts") `
  -Components @("DashboardDensityNavigationPolishPanel.tsx","index.ts") `
  -Exports @("buildDashboardDensityNavigationPolishStableKey","buildDashboardDensityNavigationPolish","buildDashboardDensityNavigationPolishReviews","buildDashboardDensityNavigationPolishBoundary","buildDashboardDensityNavigationPolishModel","summarizeDashboardDensityNavigationPolish","DASHBOARD_DENSITY_NAVIGATION_POLISH_LANGUAGE") `
  -PhaseMarkers $phaseMarkers `
  -PlainEnglish @(@("Dashboard polish identity","Current navigation groups","Density risks","Novice mode adjustments","Cross-loop search route","Privacy audit route","Blocked reasons","advanced density details collapsed/secondary") + $dailyCockpitSafetyMarkers) `
  -ExtraRoutes @("/daily-operator-home","/novice-mode-guided-flow-polish","/expert-mode-fast-path-review","/cross-loop-search-review","/local-first-privacy-audit")
...
$blockedPatterns = @{
  "no action approval execution or route execution from UI" = "actionsApprovedFromUi:\s*true|actionsExecutedFromUi:\s*true|routeExecutionAllowedFromUi:\s*true|approveAction\s*\(|grantApproval\s*\(|executeAction\s*\(|runAction\s*\(|executeRoute\s*\(|runRoute\s*\("
...
Write-Host "[OK] CodexForge Dashboard Density / Navigation Polish smoke passed."
