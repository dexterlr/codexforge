...
$domain = "src\lib\codexforge\agent-plugin-registry-comparison"
$route = "src\app\agent-plugin-registry-comparison"

& (Join-Path $PSScriptRoot "codexforge-local-planning-phase-smoke-helper.ps1") `
  -PhaseName "Phase 314 Agent Plugin Registry Comparison" `
  -ScriptFile "smoke-codexforge-agent-plugin-registry-comparison.ps1" `
  -Domain $domain `
  -Route $route `
  -MainPanel "AgentPluginRegistryComparisonPanel" `
  -CommandLabel "Go to Agent Plugin Registry Comparison" `
  -Modules @("agent-plugin-registry-comparison-types.ts","agent-plugin-registry-comparison-summary.ts","index.ts") `
  -Components @("AgentPluginRegistryComparisonPanel.tsx","index.ts") `
  -Exports @("buildAgentPluginRegistryComparisonStableKey","buildAgentPluginRegistryComparison","buildAgentPluginRegistryComparisons","buildAgentPluginRegistryComparisonBoundary","buildAgentPluginRegistryComparisonModel","summarizeAgentPluginRegistryComparison","AGENT_PLUGIN_REGISTRY_COMPARISON_LANGUAGE") `
  -PlainEnglish @("Agent plugin registry comparison","Plugin comparison does not execute plugins","Third-party code is not vendored or copied","Future plugin adoption requires license security review","Future plugin adoption requires license/security review","Plugin manifest lessons","MCP boundary route","Comparison identity","Source reference spikes","CodexForge registry requirements","Permission/scope lessons","Audit/recovery requirements","Sandboxing/non-goals","Risks/gaps","Next recommended route","no automatic provider calls","no provider API calls","no automatic provider send","no prompt/file sending without approval","no auto-spend tokens","no auto-route live provider traffic","no API key export","no secret export","no localStorage API key storage","no process.env printing","no API keys or secrets displayed","no plugin execution","no tool execution","no agent execution","no MCP runtime","no MCP tool calls","no memory/RAG ingestion","no Brain graph mutation","no appendEvent/saveBrainGraph calls from UI","no Ruflo/Odysseus vendoring","no Ruflo/Odysseus runtime integration","no Ruflo/Odysseus dependency references","no ComfyUI job submission","no ComfyUI request sent from UI","no arbitrary local endpoint calls from UI","no uncontrolled polling loops","no render job start/cancel/hold/retry behavior","no command execution","no shell command execution","no git command execution from UI","no test execution from UI","no Jarvisd capability execution from UI","no daemon process creation from frontend","no browser-stored signing secrets","no session token localStorage storage","no arbitrary local file browsing","no arbitrary path crawling","no arbitrary file read/open","no auto-open local files","no file mutation","no file write","no patch apply behavior","no file deletion","no artifact deletion","no memory auto-promotion","no process kill/restart/shutdown from UI","no package install behavior","server-only path boundary markers remain intact","advanced comparison details collapsed/secondary","plain English","no duplicate route chip cloud","hero title does not vertically wrap","no giant raw JSON above fold","advanced details collapsed/secondary","no unsafe execution buttons","no real video generation","no image generation","no upscale execution","no frame interpolation execution","no ComfyUI workflow run","no job queue execution","no hardware/system command","no prompt payload sent to providers","no cloud provider API calls","no password storage","no API key localStorage","no raw secret display","no process.env value printed in UI","no hardcoded API keys","no direct appendEvent call from UI","no direct saveBrainGraph call from UI","no direct graph mutation from UI","no direct apply-diff call from UI","no direct write-file call from UI","no direct run-command call from UI","no broker-execution call except blocked-policy text","no Math.random","no Date.now for deterministic layout/ids","no d3-force","no mojibake","no obvious duplicate React key patterns") `
  -ExtraRoutes @("/ruflo-reference-architecture","/odysseus-reference-architecture","/mcp-tool-boundary-comparison","/jarvisd-execution-registry","/jarvisd-runtime-enforcement")
...
$blockedPatterns = @{
  ...
  "no plugin execution" = "pluginExecutionAllowedFromUi:\s*true|pluginComparisonExecutesPlugins:\s*true|executePlugin\s*\(|runPlugin\s*\("
  ...
  "no Ruflo/Odysseus vendoring" = "rufloCodeVendoredOrCopied:\s*true|odysseusCodeVendoredOrCopied:\s*true|thirdPartyCodeVendoredOrCopied:\s*true|vendor[/\\](ruflo|odysseus)|third_party[/\\](ruflo|odysseus)"
  "no Ruflo/Odysseus runtime integration" = "rufloRuntimeIntegrationAdded:\s*true|odysseusRuntimeIntegrationAdded:\s*true|executeRuflo\s*\(|executeOdysseus\s*\(|connectRuflo\s*\(|connectOdysseus\s*\("
...
Write-Host "[OK] CodexForge Agent Plugin Registry Comparison smoke passed."
