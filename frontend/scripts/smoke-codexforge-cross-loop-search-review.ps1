...
$domain = "src\lib\codexforge\cross-loop-search-review"
$route = "src\app\cross-loop-search-review"
$protectedRoutes = @(
  "/daily-operator-home",
  "/global-review-inbox",
  "/approval-queue",
...
& (Join-Path $PSScriptRoot "codexforge-local-project-knowledge-smoke-helper.ps1") `
  -PhaseName "Phase 379 Cross-Loop Search Review" `
  -ScriptFile "smoke-codexforge-cross-loop-search-review.ps1" `
  -Domain $domain `
  -Route $route `
  -MainPanel "CrossLoopSearchReviewPanel" `
  -CommandLabel "Go to Cross-Loop Search Review" `
  -Modules @("cross-loop-search-review-types.ts","cross-loop-search-review-summary.ts","index.ts") `
  -Components @("CrossLoopSearchReviewPanel.tsx","index.ts") `
  -Exports @("buildCrossLoopSearchReviewStableKey","buildCrossLoopSearchReview","buildCrossLoopSearchReviews","buildCrossLoopSearchReviewBoundary","buildCrossLoopSearchReviewModel","summarizeCrossLoopSearchReview","CROSS_LOOP_SEARCH_REVIEW_LANGUAGE") `
  -PhaseMarkers $phaseMarkers `
  -PlainEnglish @(@("Search review identity","Allowed search scope","Denied search scope","Privacy/redaction rules","Local-first privacy audit route","Secrets regression route","Blocked reasons","Private connector/project details stay redacted","advanced search details collapsed/secondary") + $dailyCockpitSafetyMarkers) `
  -ExtraRoutes @("/dashboard-density-navigation-polish","/local-first-privacy-audit","/secrets-token-storage-regression-sweep","/global-review-inbox")
...
$blockedPatterns = @{
  ...
  "no search execution or source reading from UI" = "searchExecutionAllowedFromUi:\s*true|searchSourceReadingAllowedFromUi:\s*true|runSearch\s*\(|executeSearch\s*\(|callSearchProvider\s*\(|readSearchSource\s*\("
  ...
  "no provider connector web or local bridge calls" = "providerApiCallsAllowedFromUi:\s*true|connectorApiCallsAllowedFromUi:\s*true|webSearchProviderCallsAllowedFromUi:\s*true|localBridgeEndpointCallsAllowedFromUi:\s*true|fetch\s*\(|XMLHttpRequest|axios|callProviderApi\s*\(|callConnectorApi\s*\(|readConnectorData\s*\(|syncConnectorData\s*\(|callLocalBridge\s*\("
...
Write-Host "[OK] CodexForge Cross-Loop Search Review smoke passed."
