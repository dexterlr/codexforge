...
$domain = "src\lib\codexforge\patch-apply-result-capture"
$route = "src\app\patch-apply-result"

& (Join-Path $PSScriptRoot "codexforge-local-planning-phase-smoke-helper.ps1") `
  -PhaseName "Phase 275 Patch Apply Result Capture" `
  -ScriptFile "smoke-codexforge-patch-apply-result-capture.ps1" `
  -Domain $domain `
  -Route $route `
  -MainPanel "PatchApplyResultCapturePanel" `
  -CommandLabel "Go to Patch Apply Result Capture" `
  -Modules @("patch-apply-result-capture-types.ts","patch-apply-result-capture-summary.ts","index.ts") `
  -Components @("PatchApplyResultCapturePanel.tsx","index.ts") `
  -Exports @("buildPatchApplyResultCaptureStableKey","buildPatchApplyResultCapture","buildPatchApplyResultCaptures","buildPatchApplyResultCaptureBoundary","buildPatchApplyResultCaptureModel","summarizePatchApplyResultCapture","PATCH_APPLY_RESULT_CAPTURE_LANGUAGE") `
  -PlainEnglish @("Patch apply result capture","Results are reviewed before promotion","Failed patch applies are retained for recovery","Memory is not auto-promoted","Result identity","Source patch apply trial","Apply status: passed, failed, blocked, needs review","Affected files summary","Hunk result summary","Validation/test summary","Risk/secrets follow-up","Rollback trial route","Review inbox handoff","Blocked reasons","advanced result details collapsed/secondary","no automatic local action","no raw fetch from arbitrary UI","no command execution","no shell command execution","no test execution from UI","no git command execution from UI","no runCommand brokerExecution or local executor API calls from UI","no direct Jarvisd call from arbitrary UI","no Jarvisd capability execution from UI","no daemon process creation from frontend","no browser-stored signing secrets","no session token localStorage storage","no arbitrary local file browsing","no arbitrary file read/open","no auto-open local files","no file mutation","no file write","no patch apply behavior","no rollback behavior","no file deletion","no secret value display","no automatic provider send","no provider APIs are called","no GitHub API calls from UI","no localStorage API key storage","no process.env printing","no direct appendEvent call from UI","no direct saveBrainGraph call from UI","no direct graph mutation from UI","no memory auto-promotion","no process kill/restart/shutdown from UI","no package install behavior","no weakened safe path checks","no removed server-only boundaries") `
  -ExtraRoutes @("/patch-apply-trial","/patch-rollback-trial","/review-inbox","/test-result-capture","/git-commit-result")
...
foreach ($needle in @(
  ...
  Assert-Contains $source $needle "patch apply result includes $needle"
}
...
Write-Host "[OK] CodexForge Patch Apply Result Capture smoke passed."
