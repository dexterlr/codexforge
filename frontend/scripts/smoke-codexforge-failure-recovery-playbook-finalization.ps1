...
$domain = "src\lib\codexforge\failure-recovery-playbook-finalization"
$route = "src\app\failure-recovery-playbook-finalization"
$protectedRoutes = @(
  "/daily-operator-home",
  "/approval-queue",
  "/result-history",
...
& (Join-Path $PSScriptRoot "codexforge-local-project-knowledge-smoke-helper.ps1") `
  -PhaseName "Phase 375 Failure Recovery Playbook Finalization" `
  -ScriptFile "smoke-codexforge-failure-recovery-playbook-finalization.ps1" `
  -Domain $domain `
  -Route $route `
  -MainPanel "FailureRecoveryPlaybookFinalizationPanel" `
  -CommandLabel "Go to Failure Recovery Playbook" `
  -Modules @("failure-recovery-playbook-finalization-types.ts","failure-recovery-playbook-finalization-summary.ts","index.ts") `
  -Components @("FailureRecoveryPlaybookFinalizationPanel.tsx","index.ts") `
  -Exports @("buildFailureRecoveryPlaybookFinalizationStableKey","buildFailureRecoveryPlaybookFinalization","buildFailureRecoveryPlaybookFinalizations","buildFailureRecoveryPlaybookFinalizationBoundary","buildFailureRecoveryPlaybookFinalizationModel","summarizeFailureRecoveryPlaybookFinalization","FAILURE_RECOVERY_PLAYBOOK_FINALIZATION_LANGUAGE") `
  -PhaseMarkers $phaseMarkers `
  -PlainEnglish @(@("Recovery playbook identity","Source safety matrix","Recommended manual recovery steps","Validation evidence required","Escalation guidance","Novice mode route","Expert fast path route","Blocked reasons","advanced recovery details collapsed/secondary") + $dailyCockpitSafetyMarkers) `
  -ExtraRoutes @("/safety-boundary-matrix-finalization","/novice-mode-guided-flow-polish","/expert-mode-fast-path-review","/result-history")
...
Write-Host "[OK] CodexForge Failure Recovery Playbook Finalization smoke passed."
