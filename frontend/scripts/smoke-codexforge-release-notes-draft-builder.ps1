...
$domain = "src\lib\codexforge\release-notes-draft-builder"
$route = "src\app\release-notes-draft-builder"

& (Join-Path $PSScriptRoot "codexforge-local-planning-phase-smoke-helper.ps1") `
  -PhaseName "Release Notes Draft Builder" `
  -ScriptFile "smoke-codexforge-release-notes-draft-builder.ps1" `
  -Domain $domain `
  -Route $route `
  -MainPanel "ReleaseNotesDraftBuilderPanel" `
  -CommandLabel "Go to Release Notes Draft Builder" `
  -Modules @("release-notes-draft-builder-types.ts","release-notes-draft-builder-summary.ts","index.ts") `
  -Components @("ReleaseNotesDraftBuilderPanel.tsx","index.ts") `
  -Exports @("buildReleaseNotesDraftBuilderStableKey","buildReleaseNotesDraft","buildReleaseNotesDrafts","buildReleaseNotesDraftBuilderBoundary","buildReleaseNotesDraftBuilderModel","summarizeReleaseNotesDraftBuilder","RELEASE_NOTES_DRAFT_BUILDER_LANGUAGE") `
  -PlainEnglish @("Release notes draft builder","Release notes are drafts only","No release is published from this page","Secrets are not included in release notes","Release notes identity","Source branch/tag handoff","PR prep dependency","User-facing summary","Technical summary","Validation summary","Known gaps","Risk notes","Copyable release notes draft","Next recommended route","Blocked reasons","advanced draft details collapsed/secondary","approved local boundary required","release actions are not run from arbitrary UI","git commands are not run from arbitrary UI","no git command execution from UI","nothing executes from arbitrary UI","no automatic local action","no raw fetch from arbitrary UI","no command execution","no shell command execution","no branch creation","no tag creation","no PR creation","no release publishing","no push behavior","no merge behavior","no arbitrary local file browsing","no arbitrary file read/open","no file mutation","no file deletion","no patch apply behavior","no package install behavior","no provider APIs are called","no GitHub API calls from UI","no automatic provider send","no secrets displayed","no secrets exported","no secrets included","no localStorage API key storage","no process.env printing","no direct appendEvent call from UI","no direct saveBrainGraph call from UI","no direct graph mutation from UI","no memory auto-promotion") `
  -ExtraRoutes @("/branch-tag-release-handoff","/pull-request-prep-review","/pr-risk-checklist","/test-result-summary","/git-diff-review")
...
foreach ($needle in @(
  ...
  Assert-Contains $source $needle "release notes draft builder includes $needle"
}
...
Write-Host "[OK] CodexForge Release Notes Draft Builder smoke passed."
