...
$domain = "src\lib\codexforge\project-dependency-map"
$route = "src\app\project-dependency-map"

& (Join-Path $PSScriptRoot "codexforge-local-planning-phase-smoke-helper.ps1") `
  -PhaseName "Project Dependency Map" `
  -ScriptFile "smoke-codexforge-project-dependency-map.ps1" `
  -Domain $domain `
  -Route $route `
  -MainPanel "ProjectDependencyMapPanel" `
  -CommandLabel "Go to Project Dependency Map" `
  -Modules @("project-dependency-map-types.ts","project-dependency-map-summary.ts","index.ts") `
  -Components @("ProjectDependencyMapPanel.tsx","index.ts") `
  -Exports @("buildProjectDependencyMapStableKey","buildProjectDependencyMap","buildProjectDependencyMaps","buildProjectDependencyMapBoundary","buildProjectDependencyMapModel","summarizeProjectDependencyMap","PROJECT_DEPENDENCY_MAP_LANGUAGE") `
  -PlainEnglish @("Project dependency map","Dependency map does not install packages","No commands are run from this page","Approved indexed metadata","Project summary","Dependency source","Dependency group","Internal module relationship","External package summary","Risk note","Stale/unknown marker","Command approval route","Risk scanner route","Blocked reasons","approved local boundary required","nothing reads arbitrary files automatically","nothing executes from arbitrary UI","no automatic local action","no raw fetch from arbitrary UI","no command execution","no shell command execution","no arbitrary local file browsing","no arbitrary file read/open","no file mutation","no file deletion","no package install behavior","no provider APIs are called","no automatic provider send","no secrets displayed","no secrets exported","no secrets included","no localStorage API key storage","no process.env printing") `
  -ExtraRoutes @("/safe-project-indexer","/project-file-search","/local-command-approval","/project-risk-secrets-scan","/files")
...
foreach ($needle in @(
  ...
  Assert-Contains $source $needle "project dependency map includes $needle"
}
...
Write-Host "[OK] CodexForge Project Dependency Map smoke passed."
