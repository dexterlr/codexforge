...
$domain = "src\lib\codexforge\daily-operator-home-polish"
$route = "src\app\daily-operator-home"
$protectedRoutes = @(
...
& (Join-Path $PSScriptRoot "codexforge-local-project-knowledge-smoke-helper.ps1") `
  -PhaseName "Phase 370 Daily Operator Home Polish" `
  -ScriptFile "smoke-codexforge-daily-operator-home-polish.ps1" `
  -Domain $domain `
  -Route $route `
  -MainPanel "DailyOperatorHomePolishPanel" `
  -CommandLabel "Go to Daily Operator Home" `
  -Modules @("daily-operator-home-polish-types.ts","daily-operator-home-polish-summary.ts","index.ts") `
  -Components @("DailyOperatorHomePolishPanel.tsx","index.ts") `
  -Exports @("buildDailyOperatorHomePolishStableKey","buildDailyOperatorHomePolish","buildDailyOperatorHomePolishes","buildDailyOperatorHomePolishBoundary","buildDailyOperatorHomePolishModel","summarizeDailyOperatorHomePolish","DAILY_OPERATOR_HOME_POLISH_LANGUAGE") `
  -PhaseMarkers $phaseMarkers `
  -PlainEnglish @(@("Daily home identity","Loop readiness summary","Blocked work summary","Approval queue route","Global review inbox route","Result history route","Next recommended action","advanced daily home details collapsed/secondary") + $dailyCockpitSafetyMarkers) `
  -ExtraRoutes @("/global-review-inbox","/approval-queue","/result-history","/unified-workspace-real-world-trial-report")
...
$blockedPatterns = @{
  ...
  "no approval automation" = "approvalAutomationAllowedFromUi:\s*true|automaticApprovalAllowedFromUi:\s*true|approveAction\s*\(|grantApproval\s*\("
...
Write-Host "[OK] CodexForge Daily Operator Home Polish smoke passed."
