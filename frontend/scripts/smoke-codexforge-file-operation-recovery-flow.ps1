...
$domain = "src\lib\codexforge\file-operation-recovery-flow"
$route = "src\app\file-operation-recovery"

& (Join-Path $PSScriptRoot "codexforge-local-planning-phase-smoke-helper.ps1") `
  -PhaseName "File Operation Recovery Flow" `
  -ScriptFile "smoke-codexforge-file-operation-recovery-flow.ps1" `
  -Domain $domain `
  -Route $route `
  -MainPanel "FileOperationRecoveryFlowPanel" `
  -CommandLabel "Go to File Operation Recovery" `
  -Modules @("file-operation-recovery-flow-types.ts","file-operation-recovery-flow-summary.ts","index.ts") `
  -Components @("FileOperationRecoveryFlowPanel.tsx","index.ts") `
  -Exports @("buildFileOperationRecoveryFlowStableKey","buildFileOperationRecovery","buildFileOperationRecoveries","buildFileOperationRecoveryBoundary","buildFileOperationRecoveryFlowModel","summarizeFileOperationRecoveryFlow","FILE_OPERATION_RECOVERY_FLOW_LANGUAGE") `
  -PlainEnglish @("File operation recovery flow","Retry is never automatic","Recovery does not mutate files","Rollback requires explicit approved boundary","Safe recovery checklist","Blocked retry reasons","Recovery identity","Source result","Failure category","Affected scope","Rollback recommendation","Retry eligibility","Next recommended route","Audit handoff","advanced recovery details collapsed/secondary","no automatic local action","no raw fetch from arbitrary UI","no command execution","no shell command execution","no direct Jarvisd call from arbitrary UI","no Jarvisd capability execution from UI","no daemon process creation from frontend","no browser-stored signing secrets","no session token localStorage storage","no arbitrary local file browsing","no arbitrary file read/open","no auto-open local files","no file mutation","no file write","no file deletion","no patch apply behavior","no secret value display","no automatic provider send","no provider APIs are called","no GitHub API calls from UI","no localStorage API key storage","no process.env printing","no audit log mutation from UI","no direct appendEvent call from UI","no direct saveBrainGraph call from UI","no direct graph mutation from UI","no memory auto-promotion","no package install behavior") `
  -ExtraRoutes @("/file-operation-result","/review-inbox","/local-file-approval","/patch-apply-approval","/jarvisd-audit-ingestion")
...
foreach ($needle in @(
  ...
  Assert-Contains $source $needle "file operation recovery includes $needle"
}

$deterministicSource = $source.Replace("no Math.random", "").Replace("no Date.now for deterministic layout/ids", "").Replace("no Date.now", "")
$blockedPatterns = @{
  "no automatic retry" = "retryAutomaticAllowed:\s*true|autoRetry\s*\(|retryOperation\s*\("
  ...
  "no file mutation" = "fileMutationAllowedFromUi:\s*true|recoveryMutationAllowedFromUi:\s*true|writeFile\s*\(|mutateFiles\s*\("
...
Write-Host "[OK] CodexForge File Operation Recovery Flow smoke passed."
