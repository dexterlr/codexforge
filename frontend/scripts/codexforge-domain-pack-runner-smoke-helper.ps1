...
$sourceParts = @()
foreach ($scanRoot in @($Domain, $Route, "src\lib\codexforge\domain-pack-runner", "src\lib\codexforge\unified-cockpit")) {
...
foreach ($safetyMarker in @(
  ...
  "frontend domain pack execution still blocked",
  "backend-owned domain pack runner remains required"
)) {
...
Write-Host "[OK] $SmokeName static domain pack runner smoke passed."
