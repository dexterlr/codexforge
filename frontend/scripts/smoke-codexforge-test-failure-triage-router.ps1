...
$domain = "src\lib\codexforge\test-failure-triage-router"
$route = "src\app\test-failure-triage"

& (Join-Path $PSScriptRoot "codexforge-local-planning-phase-smoke-helper.ps1") `
  -PhaseName "Test Failure Triage Router" `
  -ScriptFile "smoke-codexforge-test-failure-triage-router.ps1" `
  -Domain $domain `
  -Route $route `
  -MainPanel "TestFailureTriageRouterPanel" `
  -CommandLabel "Go to Test Failure Triage Router" `
  -Modules @("test-failure-triage-router-types.ts","test-failure-triage-router-summary.ts","index.ts") `
  -Components @("TestFailureTriageRouterPanel.tsx","index.ts") `
  -Exports @("buildTestFailureTriageRouterStableKey","buildTestFailureTriage","buildTestFailureTriages","buildTestFailureTriageRouterBoundary","buildTestFailureTriageRouterModel","summarizeTestFailureTriageRouter","TEST_FAILURE_TRIAGE_ROUTER_LANGUAGE") `
  -PlainEnglish @("Test failure triage router","Triage does not auto-fix failures","Retry is never automatic","No commands are run automatically","Failure identity","Source test result","Likely cause category","Severity","Safe next action","Recommended route","Retry eligibility","Blocked retry reasons","Patch planning route","Recovery handoff","advanced triage details collapsed/secondary","approved local boundary required","tests are not run automatically","no test execution from UI","nothing reads arbitrary files automatically","nothing executes from arbitrary UI","no automatic local action","no raw fetch from arbitrary UI","no command execution","no shell command execution","no arbitrary local file browsing","no arbitrary file read/open","no file mutation","no file deletion","no patch apply behavior","no package install behavior","no provider APIs are called","no automatic provider send","no secrets displayed","no secrets exported","no secrets included","no localStorage API key storage","no process.env printing","no direct appendEvent call from UI","no direct saveBrainGraph call from UI","no direct graph mutation from UI","no memory auto-promotion") `
  -ExtraRoutes @("/test-result-summary","/codebase-change-plan","/test-command-planner","/recovery","/review-inbox")
...
foreach ($needle in @(
  ...
  Assert-Contains $source $needle "test failure triage router includes $needle"
}

$deterministicSource = $source.Replace("no Math.random", "").Replace("no Date.now for deterministic layout/ids", "").Replace("no Date.now", "")
$blockedPatterns = @{
  "no automatic local action" = "automaticLocalActionAllowed:\s*true|localActionsWithoutReviewAllowed:\s*true|executeLocalAction\s*\(|runLocalAction\s*\(|retryAutomaticAllowed:\s*true|triageAutoFixAllowed:\s*true"
...
Write-Host "[OK] CodexForge Test Failure Triage Router smoke passed."
