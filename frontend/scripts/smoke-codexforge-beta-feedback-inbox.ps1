...
$domain = "src\lib\codexforge\beta-feedback-inbox"
$route = "src\app\beta-feedback-inbox"
$protectedRoutes = @(
...
& (Join-Path $PSScriptRoot "codexforge-local-project-knowledge-smoke-helper.ps1") `
  -PhaseName "Phase 385 Beta Feedback Inbox" `
  -ScriptFile "smoke-codexforge-beta-feedback-inbox.ps1" `
  -Domain $domain `
  -Route $route `
  -MainPanel "BetaFeedbackInboxPanel" `
  -CommandLabel "Go to Beta Feedback Inbox" `
  -Modules @("beta-feedback-inbox-types.ts","beta-feedback-inbox-summary.ts","index.ts") `
  -Components @("BetaFeedbackInboxPanel.tsx","index.ts") `
  -Exports @("buildBetaFeedbackInboxStableKey","buildBetaFeedbackInboxReview","buildBetaFeedbackInboxReviews","buildBetaFeedbackInboxBoundary","buildBetaFeedbackInboxModel","summarizeBetaFeedbackInbox","BETA_FEEDBACK_INBOX_LANGUAGE") `
  -PhaseMarkers $phaseMarkers `
  -PlainEnglish @(@("Feedback inbox identity","Severity and confidence summary","Privacy/redaction status","Linked beta trial scope","Blocked feedback items","Next recommended action","advanced feedback details collapsed/secondary") + $betaReadinessSafetyMarkers) `
  -ExtraRoutes @("/beta-trial-intake-review","/foundation-beta-candidate","/review-inbox","/local-first-privacy-audit")
...
Write-Host "[OK] CodexForge Beta Feedback Inbox smoke passed."
