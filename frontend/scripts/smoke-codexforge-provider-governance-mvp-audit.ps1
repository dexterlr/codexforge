...
foreach ($needle in @(
  ...
  Assert-Contains $source $needle "provider governance MVP audit includes $needle"
}
...
$blockedPatterns = @{
  ...
  "no auto-route live provider traffic" = "autoRouteLiveProviderTrafficAllowed:\s*true|governanceAuditEnablesLiveRoutingAllowed:\s*true|routeLiveTraffic\s*\("
  ...
  "no API keys or secrets displayed" = "apiKeysDisplayedAllowed:\s*true|secretValuesDisplayedAllowed:\s*true|secretsDisplayedAllowed:\s*true|displaySecrets\s*\(|secretsInspectionAllowedFromUi:\s*true|sk-[A-Za-z0-9]{20,}|AIza[0-9A-Za-z_-]{20,}"
...
Write-Host "[OK] CodexForge Provider Governance MVP Audit smoke passed."
