...
$domain = "src\lib\codexforge\local-model-runtime-boundary-review"
$route = "src\app\local-model-runtime-boundary-review"
$newRoutes = @(
...
& (Join-Path $PSScriptRoot "codexforge-local-project-knowledge-smoke-helper.ps1") `
  -PhaseName "Phase 422 Local Model Runtime Boundary Review" `
  -ScriptFile "smoke-codexforge-local-model-runtime-boundary-review.ps1" `
  -Domain $domain `
  -Route $route `
  -MainPanel "LocalModelRuntimeBoundaryReviewPanel" `
  -CommandLabel "Go to Local Model Runtime Boundary Review" `
  -Modules @("local-model-runtime-boundary-review-types.ts","local-model-runtime-boundary-review-summary.ts","index.ts") `
  -Components @("LocalModelRuntimeBoundaryReviewPanel.tsx","index.ts") `
  -Exports @("buildLocalModelRuntimeBoundaryReviewStableKey","buildLocalModelRuntimeBoundaryReview","buildLocalModelRuntimeBoundaryReviews","buildLocalModelRuntimeBoundaryReviewBoundary","buildLocalModelRuntimeBoundaryReviewModel","summarizeLocalModelRuntimeBoundaryReview","LOCAL_MODEL_RUNTIME_BOUNDARY_REVIEW_LANGUAGE") `
  -PhaseMarkers $phaseMarkers `
  -PlainEnglish @(@("local model runtime boundary identity","Denied runtime actions","Endpoint privacy rules","Manual validation checklist","Blocked runtime risks","Local model output inbox route","Local model failover route","Next recommended action") + $localModelSafetyMarkers) `
  -ExtraRoutes @("/local-model-output-review-inbox","/local-model-failover-review","/local-model-provider-trial-review","/local-bridge-readiness-audit")
...
$blockedPatterns = @{
  ...
  "no local model calls bridge calls endpoint probes tool launching live tests or traffic routing" = "localModelCallsAllowedFromUi:\s*true|localBridgeEndpointCallsAllowedFromUi:\s*true|localEndpointProbeExecutionAllowedFromUi:\s*true|localToolLaunchingAllowedFromUi:\s*true|localModelLiveConnectionTestsAllowedFromUi:\s*true|localModelTrafficRoutingAllowedFromUi:\s*true|callLocalModel\s*\(|runLocalModel\s*\(|callLocalBridge\s*\(|probeLocalEndpoint\s*\(|testLocalModelConnection\s*\(|launchLocalTool\s*\(|routeLocalModelTraffic\s*\("
  "no runtime switching model retry prompt send output persistence or ingestion" = "runtimeSwitchingAllowedFromUi:\s*true|modelRetryCallsAllowedFromUi:\s*true|promptSendingAllowedFromUi:\s*true|modelOutputStorageAllowedFromUi:\s*true|modelOutputIngestionAllowedFromUi:\s*true|switchRuntime\s*\(|retryModelCall\s*\(|sendPrompt\s*\(|storeModelOutput\s*\(|persistModelOutput\s*\(|ingestModelOutput\s*\("
  ...
  "no credential token endpoint output or browser storage" = "credentialStorageAllowed:\s*true|tokenStorageAllowed:\s*true|endpointStorageAllowed:\s*true|outputStorageAllowed:\s*true|localStorageTokenStorageAllowed:\s*true|sessionStorageTokenStorageAllowed:\s*true|localStorageApiKeyStorageAllowed:\s*true|sessionStorageApiKeyStorageAllowed:\s*true|storeCredential\s*\(|storeEndpoint\s*\(|storeToken\s*\(|localStorage\.setItem|sessionStorage\.setItem"
  "no local bridge connector web search or GitHub API calls" = "connectorApiCallsAllowedFromUi:\s*true|webSearchProviderCallsAllowedFromUi:\s*true|githubApiCallsAllowedFromUi:\s*true|callConnectorApi\s*\(|callSearchProvider\s*\(|callGithubApi\s*\(|fetch\s*\(|XMLHttpRequest|axios"
  "no prompt file project connector provider model endpoint or output data sends" = "promptFileProjectConnectorProviderModelEndpointOutputDataAutoSendAllowed:\s*true|sendPrompt\s*\(|sendFiles\s*\(|sendProjectData\s*\(|sendConnectorData\s*\(|sendProviderData\s*\(|sendModelData\s*\(|sendEndpointData\s*\(|sendOutputData\s*\("
  ...
  "no env secret display or example real keys endpoints" = "processEnvDisplayAllowed:\s*true|process\.env\.[A-Za-z0-9_]+|secretsDisplayedAllowed:\s*true|sk-[A-Za-z0-9]{20,}|AIza[0-9A-Za-z_-]{20,}|xox[baprs]-[A-Za-z0-9-]{20,}|https?://(?:localhost|127\.0\.0\.1|0\.0\.0\.0|[a-z0-9.-]+\.[a-z]{2,})(?::\d+)?"
...
Write-Host "[OK] CodexForge Local Model Runtime Boundary Review smoke passed."
