...
$sharedBoundaryPath = Join-RepoPath "src" "lib" "codexforge" "provider-mock-result-harness-map"
...
foreach ($needle in @(
  "ProviderMockResultHarnessPageClientShell",
  "ProviderMockResultHarnessPanel",
...
Assert-NotMatches $source $unsafeApiNamePattern "unsafe camelCase provider mock result API names"
Write-Host ("[OK] " + $SmokeName + " static provider mock result harness smoke passed.")
