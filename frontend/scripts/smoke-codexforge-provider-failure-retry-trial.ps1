...
foreach ($needle in @(
  ...
  Assert-Contains $source $needle "provider failure retry trial includes $needle"
}
...
$blockedPatterns = @{
  ...
  "no provider retry from UI" = "providerRetryAllowedFromUi:\s*true|retryAutomaticAllowed:\s*true|retryProvider\s*\(|sendRetryRequest\s*\("
...
Write-Host "[OK] CodexForge Provider Failure Retry Trial smoke passed."
