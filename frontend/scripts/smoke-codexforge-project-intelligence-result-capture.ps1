...
$domain = "src\lib\codexforge\project-intelligence-result-capture"
$route = "src\app\project-intelligence-result"

& (Join-Path $PSScriptRoot "codexforge-local-planning-phase-smoke-helper.ps1") `
  -PhaseName "Phase 282 Project Intelligence Result Capture" `
  -ScriptFile "smoke-codexforge-project-intelligence-result-capture.ps1" `
  -Domain $domain `
  -Route $route `
  -MainPanel "ProjectIntelligenceResultCapturePanel" `
  -CommandLabel "Go to Project Intelligence Result Capture" `
  -Modules @("project-intelligence-result-capture-types.ts","project-intelligence-result-capture-summary.ts","index.ts") `
  -Components @("ProjectIntelligenceResultCapturePanel.tsx","index.ts") `
  -Exports @("buildProjectIntelligenceResultCaptureStableKey","buildProjectIntelligenceResult","buildProjectIntelligenceResults","buildProjectIntelligenceResultCaptureBoundary","buildProjectIntelligenceResultCaptureModel","summarizeProjectIntelligenceResultCapture","PROJECT_INTELLIGENCE_RESULT_CAPTURE_LANGUAGE") `
  -PlainEnglish @("Project intelligence result capture","Results are reviewed before use","Secret values remain redacted","Memory is not auto-promoted","Findings summary","Review inbox handoff","Result identity","Source index/search/dependency/risk trials","Result status: passed, failed, blocked, needs review","Workspace scope summary","Redaction status","Risk/secrets follow-up","Recovery route","Next recommended route","Blocked reasons","project intelligence result capture language","live context does not mutate files","secret values are redacted and never displayed","advanced result details collapsed/secondary","server-only path boundary markers remain intact","no automatic local action","no raw fetch from arbitrary UI","no command execution","no shell command execution","no test execution from UI","no git command execution from UI","no direct Jarvisd call from arbitrary UI","no Jarvisd capability execution from UI","no daemon process creation from frontend","no browser-stored signing secrets","no session token localStorage storage","no arbitrary local file browsing","no arbitrary path crawling","no arbitrary file read/open","no auto-open local files","no file mutation","no file write","no patch apply behavior","no file deletion","no secret value display","no automatic provider send","no provider APIs are called","no GitHub API calls from UI","no localStorage API key storage","no process.env printing","no audit log mutation from UI","no direct appendEvent call from UI","no direct saveBrainGraph call from UI","no direct graph mutation from UI","no memory auto-promotion","no process kill/restart/shutdown from UI","no package install behavior") `
  -ExtraRoutes @("/project-indexer-live-trial","/project-search-live-trial","/project-dependency-live-trial","/project-risk-scan-live-trial","/project-intelligence-recovery")
...
foreach ($needle in @(
  ...
  Assert-Contains $source $needle "project intelligence result includes $needle"
}
...
Write-Host "[OK] CodexForge Project Intelligence Result Capture smoke passed."
