...
$domain = "src\lib\codexforge\project-dependency-map-live-trial"
$route = "src\app\project-dependency-live-trial"

& (Join-Path $PSScriptRoot "codexforge-local-planning-phase-smoke-helper.ps1") `
  -PhaseName "Phase 280 Project Dependency Map Live Trial" `
  -ScriptFile "smoke-codexforge-project-dependency-map-live-trial.ps1" `
  -Domain $domain `
  -Route $route `
  -MainPanel "ProjectDependencyMapLiveTrialPanel" `
  -CommandLabel "Go to Project Dependency Live Trial" `
  -Modules @("project-dependency-map-live-trial-types.ts","project-dependency-map-live-trial-summary.ts","index.ts") `
  -Components @("ProjectDependencyMapLiveTrialPanel.tsx","index.ts") `
  -Exports @("buildProjectDependencyMapLiveTrialStableKey","buildProjectDependencyMapLiveTrial","buildProjectDependencyMapLiveTrials","buildProjectDependencyMapLiveTrialBoundary","buildProjectDependencyMapLiveTrialModel","summarizeProjectDependencyMapLiveTrial","PROJECT_DEPENDENCY_MAP_LIVE_TRIAL_LANGUAGE") `
  -PlainEnglish @("Project dependency map live trial","Dependency map does not install packages","No commands are run from this page","Approved indexed metadata","Dependency trial identity","Source index trial","Dependency source summary","Internal module relationship summary","External package summary","Stale/unknown markers","Risk note","Command dry-run route","Risk scan route","Audit handoff","Approved metadata route","Blocked reasons","no automatic local action","no raw fetch from arbitrary UI","no command execution","no shell command execution","no test execution from UI","no git command execution from UI","no direct Jarvisd call from arbitrary UI","no Jarvisd capability execution from UI","no daemon process creation from frontend","no browser-stored signing secrets","no session token localStorage storage","no arbitrary local file browsing","no arbitrary path crawling","no arbitrary file read/open","no auto-open local files","no file mutation","no file write","no patch apply behavior","no file deletion","no secret value display","no automatic provider send","no provider APIs are called","no GitHub API calls from UI","no localStorage API key storage","no process.env printing","no audit log mutation from UI","no direct appendEvent call from UI","no direct saveBrainGraph call from UI","no direct graph mutation from UI","no memory auto-promotion","no process kill/restart/shutdown from UI","no package install behavior","no weakened safe path checks","no removed server-only boundaries") `
  -ExtraRoutes @("/project-indexer-live-trial","/project-dependency-map","/command-dry-run","/project-risk-scan-live-trial","/review-inbox")
...
foreach ($needle in @(
  ...
  Assert-Contains $source $needle "project dependency map live trial includes $needle"
}
...
Write-Host "[OK] CodexForge Project Dependency Map Live Trial smoke passed."
