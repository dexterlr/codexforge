...
$domain = "src\lib\codexforge\result-history-consolidation"
$route = "src\app\result-history"
$protectedRoutes = @(
  "/daily-operator-home",
  "/global-review-inbox",
  "/approval-queue",
...
& (Join-Path $PSScriptRoot "codexforge-local-project-knowledge-smoke-helper.ps1") `
  -PhaseName "Phase 373 Result History Consolidation" `
  -ScriptFile "smoke-codexforge-result-history-consolidation.ps1" `
  -Domain $domain `
  -Route $route `
  -MainPanel "ResultHistoryConsolidationPanel" `
  -CommandLabel "Go to Result History" `
  -Modules @("result-history-consolidation-types.ts","result-history-consolidation-summary.ts","index.ts") `
  -Components @("ResultHistoryConsolidationPanel.tsx","index.ts") `
  -Exports @("buildResultHistoryConsolidationStableKey","buildResultHistoryConsolidation","buildResultHistoryConsolidations","buildResultHistoryConsolidationBoundary","buildResultHistoryConsolidationModel","summarizeResultHistoryConsolidation","RESULT_HISTORY_CONSOLIDATION_LANGUAGE") `
  -PhaseMarkers $phaseMarkers `
  -PlainEnglish @(@("Result history identity","Validation outcomes","Safety audit summary","Linked review items","Daily home route","Global inbox route","Next recommended action","advanced history details collapsed/secondary") + $dailyCockpitSafetyMarkers) `
  -ExtraRoutes @("/daily-operator-home","/global-review-inbox","/approval-queue","/cross-loop-result-handoff-review")
...
$blockedPatterns = @{
  "no replay or action execution from UI" = "actionsReplayedFromUi:\s*true|actionsExecutedFromUi:\s*true|replayAction\s*\(|replayResult\s*\(|executeAction\s*\(|runAction\s*\("
  "no workflow execution or replay" = "workflowExecutionAllowedFromUi:\s*true|workflowReplayAllowedFromUi:\s*true|runWorkflow\s*\(|executeWorkflow\s*\(|replayWorkflow\s*\("
...
Write-Host "[OK] CodexForge Result History Consolidation smoke passed."
