...
$domain = "src\lib\codexforge\agent-memory-rag-pattern-review"
$route = "src\app\agent-memory-rag-pattern-review"

& (Join-Path $PSScriptRoot "codexforge-local-planning-phase-smoke-helper.ps1") `
  -PhaseName "Phase 316 Agent Memory RAG Pattern Review" `
  -ScriptFile "smoke-codexforge-agent-memory-rag-pattern-review.ps1" `
  -Domain $domain `
  -Route $route `
  -MainPanel "AgentMemoryRagPatternReviewPanel" `
  -CommandLabel "Go to Agent Memory RAG Pattern Review" `
  -Modules @("agent-memory-rag-pattern-review-types.ts","agent-memory-rag-pattern-review-summary.ts","index.ts") `
  -Components @("AgentMemoryRagPatternReviewPanel.tsx","index.ts") `
  -Exports @("buildAgentMemoryRagPatternReviewStableKey","buildAgentMemoryRagPatternReview","buildAgentMemoryRagPatternReviews","buildAgentMemoryRagPatternReviewBoundary","buildAgentMemoryRagPatternReviewModel","summarizeAgentMemoryRagPatternReview","AGENT_MEMORY_RAG_PATTERN_REVIEW_LANGUAGE") `
  -PlainEnglish @("Agent memory RAG pattern review","Memory and RAG are not ingested automatically","Memory promotion requires explicit review","Secret values are redacted before review","Data source boundaries","Promotion policy","Review identity","Source reference spikes","Memory/RAG pattern lessons","Redaction/privacy rules","Review inbox dependency","Denied ingestion scope","Extension architecture route","Blocked reasons","no automatic provider calls","no provider API calls","no automatic provider send","no prompt/file sending without approval","no auto-spend tokens","no auto-route live provider traffic","no API key export","no secret export","no localStorage API key storage","no process.env printing","no API keys or secrets displayed","no plugin execution","no tool execution","no agent execution","no MCP runtime","no MCP tool calls","no memory/RAG ingestion","no Brain graph mutation","no appendEvent/saveBrainGraph calls from UI","no Ruflo/Odysseus vendoring","no Ruflo/Odysseus runtime integration","no Ruflo/Odysseus dependency references","future adoption requires license/security review","no ComfyUI job submission","no ComfyUI request sent from UI","no arbitrary local endpoint calls from UI","no uncontrolled polling loops","no render job start/cancel/hold/retry behavior","no command execution","no shell command execution","no git command execution from UI","no test execution from UI","no Jarvisd capability execution from UI","no daemon process creation from frontend","no browser-stored signing secrets","no session token localStorage storage","no arbitrary local file browsing","no arbitrary path crawling","no arbitrary file read/open","no auto-open local files","no file mutation","no file write","no patch apply behavior","no file deletion","no artifact deletion","no memory auto-promotion","no process kill/restart/shutdown from UI","no package install behavior","server-only path boundary markers remain intact","advanced memory/RAG details collapsed/secondary","plain English","no duplicate route chip cloud","hero title does not vertically wrap","no giant raw JSON above fold","advanced details collapsed/secondary","no unsafe execution buttons","no real video generation","no image generation","no upscale execution","no frame interpolation execution","no ComfyUI workflow run","no job queue execution","no hardware/system command","no prompt payload sent to providers","no cloud provider API calls","no password storage","no API key localStorage","no raw secret display","no process.env value printed in UI","no hardcoded API keys","no direct appendEvent call from UI","no direct saveBrainGraph call from UI","no direct graph mutation from UI","no direct apply-diff call from UI","no direct write-file call from UI","no direct run-command call from UI","no broker-execution call except blocked-policy text","no Math.random","no Date.now for deterministic layout/ids","no d3-force","no mojibake","no obvious duplicate React key patterns") `
  -ExtraRoutes @("/ruflo-reference-architecture","/odysseus-reference-architecture","/mcp-tool-boundary-comparison","/review-inbox","/extension-architecture-decision")
...
Assert-NotMatches $packageSource '"ruflo"|"@ruflo/|"odysseus"|"@odysseus/|"mcp"|"@modelcontextprotocol/|"pinecone"|"weaviate"|"chroma"|"qdrant"|"milvus"|"pgvector"' "no Ruflo/Odysseus/MCP/vector dependency references in package manifest"
...
$blockedPatterns = @{
  ...
  "no memory/RAG ingestion" = "memoryRagAutoIngestionAllowed:\s*true|memoryIngestionAllowedFromUi:\s*true|ragIngestionAllowedFromUi:\s*true|memoryRagIngestionAllowedFromUi:\s*true|ingestMemory\s*\(|ingestRag\s*\(|indexRag\s*\("
  ...
  "no vector database runtime" = "pinecone|weaviate|chroma|qdrant|milvus|pgvector"
...
Write-Host "[OK] CodexForge Agent Memory RAG Pattern Review smoke passed."
