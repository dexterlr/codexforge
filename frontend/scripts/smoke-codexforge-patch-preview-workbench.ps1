...
$domain = "src\lib\codexforge\patch-preview-workbench"
...
& (Join-Path $PSScriptRoot "codexforge-local-planning-phase-smoke-helper.ps1") `
  ...
  -Domain $domain `
  -Route $route `
  -MainPanel "PatchPreviewWorkbenchPanel" `
  -CommandLabel "Go to Patch Preview Workbench" `
  -Modules @("patch-preview-workbench-types.ts","patch-preview-workbench-summary.ts","index.ts") `
  -Components @("PatchPreviewWorkbenchPanel.tsx","index.ts") `
  -Exports @("buildPatchPreviewWorkbenchStableKey","buildPatchPreview","buildPatchPreviews","buildPatchPreviewWorkbenchBoundary","buildPatchPreviewWorkbenchModel","summarizePatchPreviewWorkbench","PATCH_PREVIEW_WORKBENCH_LANGUAGE") `
  -PlainEnglish @("Patch preview workbench","Preview does not apply patches","Raw diffs stay secondary","Suspected secrets are redacted","Patch identity","Source change plan","Affected files summary","Hunk summary","Risk/secrets scan status","Test plan summary","Approval copy","Apply boundary route","Rollback note","Blocked reasons","advanced diff details collapsed/secondary","approved local boundary required","nothing reads arbitrary files automatically","nothing executes from arbitrary UI","no automatic local action","no raw fetch from arbitrary UI","no command execution","no shell command execution","no arbitrary local file browsing","no arbitrary file read/open","no file mutation","no file deletion","no patch apply behavior","no package install behavior","no provider APIs are called","no automatic provider send","no secrets displayed","no secrets exported","no secrets included","no localStorage API key storage","no process.env printing","no direct appendEvent call from UI","no direct saveBrainGraph call from UI","no direct graph mutation from UI","no memory auto-promotion") `
  -ExtraRoutes @("/codebase-change-plan","/project-risk-secrets-scan","/local-file-approval","/local-command-approval","/patch-apply-approval")
...
foreach ($needle in @(
  ...
  Assert-Contains $source $needle "patch preview workbench includes $needle"
}
...
Write-Host "[OK] CodexForge Patch Preview Workbench smoke passed."
