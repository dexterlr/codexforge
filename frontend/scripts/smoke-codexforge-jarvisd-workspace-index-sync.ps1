...
$domain = "src\lib\codexforge\jarvisd-workspace-index-sync"
$route = "src\app\jarvisd-index-sync"

& (Join-Path $PSScriptRoot "codexforge-local-planning-phase-smoke-helper.ps1") `
  -PhaseName "Jarvisd Workspace Index Sync" `
  -ScriptFile "smoke-codexforge-jarvisd-workspace-index-sync.ps1" `
  -Domain $domain `
  -Route $route `
  -MainPanel "JarvisdWorkspaceIndexSyncPanel" `
  -CommandLabel "Go to Jarvisd Index Sync" `
  -Modules @("jarvisd-workspace-index-sync-types.ts","jarvisd-workspace-index-sync-summary.ts","index.ts") `
  -Components @("JarvisdWorkspaceIndexSyncPanel.tsx","index.ts") `
  -Exports @("buildJarvisdWorkspaceIndexSyncStableKey","buildJarvisdWorkspaceIndexSync","buildJarvisdWorkspaceIndexSyncs","buildJarvisdWorkspaceIndexSyncBoundary","buildJarvisdWorkspaceIndexSyncModel","summarizeJarvisdWorkspaceIndexSync","JARVISD_WORKSPACE_INDEX_SYNC_LANGUAGE") `
  -PlainEnglish @("Jarvisd workspace index sync","Index sync remains behind approved local boundary","Arbitrary local file crawling is not allowed","Secrets are not read or displayed","Allowed roots summary","Recovery route","Approved local boundary required","Index sync identity","Trusted workspace dependency","Denied roots summary","Excluded paths summary","File type policy","Sync status","Privacy/secrets policy","Audit handoff","Blocked reasons","no automatic local action","no raw fetch from arbitrary UI","no live handshake from arbitrary UI","no daemon process creation from frontend","no browser-stored signing secrets","no session token localStorage storage","no audit log mutation from UI","no direct appendEvent call from UI","no Jarvisd capability execution from UI","no command execution","no arbitrary local file browsing","no arbitrary file read/open","no auto-open local files","no live search from arbitrary UI","no arbitrary path crawling","no file mutation","no file deletion","no secret value display","no automatic provider send","no provider APIs are called","no GitHub API calls from UI","no localStorage API key storage","no process.env printing","no direct saveBrainGraph call from UI","no direct graph mutation from UI","no memory auto-promotion","no package install behavior") `
  -ExtraRoutes @("/workspace-trust-policy","/safe-project-indexer","/jarvisd-file-search-bridge","/jarvisd-secrets-redaction","/jarvisd-recovery-console","/jarvisd-audit-ingestion")
...
foreach ($needle in @(
  ...
  Assert-Contains $source $needle "workspace index sync includes $needle"
}
...
$blockedPatterns = @{
  ...
  "no arbitrary file or path crawling" = "arbitraryLocalFileCrawlingAllowed:\s*true|arbitraryPathCrawlingAllowed:\s*true|crawlArbitraryPaths\s*\(|crawlPath\s*\(|crawlFiles\s*\("
  ...
  "no secret value display or secret read" = "secretValuesDisplayedAllowed:\s*true|secretsReadAllowed:\s*true|secretsDisplayedAllowed:\s*true|displaySecrets\s*\(|exportSecrets\s*\(|includeSecretFields:\s*true|rawSecretDisplayAllowed:\s*true"
...
Write-Host "[OK] CodexForge Jarvisd Workspace Index Sync smoke passed."
