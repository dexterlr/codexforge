...
$domain = "src\lib\codexforge\automation-schedule-safety-review"
$route = "src\app\automation-schedule-safety-review"
$newRoutes = @(
...
& (Join-Path $PSScriptRoot "codexforge-local-project-knowledge-smoke-helper.ps1") `
  -PhaseName "Phase 440 Automation Schedule Safety Review" `
  -ScriptFile "smoke-codexforge-automation-schedule-safety-review.ps1" `
  -Domain $domain `
  -Route $route `
  -MainPanel "AutomationScheduleSafetyReviewPanel" `
  -CommandLabel "Go to Automation Schedule Safety Review" `
  -Modules @("automation-schedule-safety-review-types.ts","automation-schedule-safety-review-summary.ts","index.ts") `
  -Components @("AutomationScheduleSafetyReviewPanel.tsx","index.ts") `
  -Exports @("buildAutomationScheduleSafetyReviewStableKey","buildAutomationScheduleSafetyReview","buildAutomationScheduleSafetyReviews","buildAutomationScheduleSafetyReviewBoundary","buildAutomationScheduleSafetyReviewModel","summarizeAutomationScheduleSafetyReview","AUTOMATION_SCHEDULE_SAFETY_REVIEW_LANGUAGE") `
  -PhaseMarkers $phaseMarkers `
  -PlainEnglish @(@("automation schedule safety identity","quiet-hours and notification checklist","denied schedule shortcuts","connector/provider timing boundary notes","blocked schedule risks","automation release candidate route","dry-run route","next recommended action") + $automationReadinessSafetyMarkers) `
  -ExtraRoutes @("/automation-dry-run-trial-review","/automation-approval-queue-review","/automation-integration-release-candidate","/scheduled-research-check-boundary")
...
Write-Host "[OK] CodexForge Automation Schedule Safety Review smoke passed."
