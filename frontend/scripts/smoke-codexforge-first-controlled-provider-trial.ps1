...
$domain = "src\lib\codexforge\first-controlled-provider-trial"
$route = "src\app\first-controlled-provider-trial"
$newRoutes = @(
...
& (Join-Path $PSScriptRoot "codexforge-local-project-knowledge-smoke-helper.ps1") `
  -PhaseName "Phase 418 First Controlled Provider Trial" `
  -ScriptFile "smoke-codexforge-first-controlled-provider-trial.ps1" `
  -Domain $domain `
  -Route $route `
  -MainPanel "FirstControlledProviderTrialPanel" `
  -CommandLabel "Go to First Controlled Provider Trial" `
  -Modules @("first-controlled-provider-trial-types.ts","first-controlled-provider-trial-summary.ts","index.ts") `
  -Components @("FirstControlledProviderTrialPanel.tsx","index.ts") `
  -Exports @("buildFirstControlledProviderTrialStableKey","buildFirstControlledProviderTrial","buildFirstControlledProviderTrials","buildFirstControlledProviderTrialBoundary","buildFirstControlledProviderTrialModel","summarizeFirstControlledProviderTrial","FIRST_CONTROLLED_PROVIDER_TRIAL_LANGUAGE") `
  -PhaseMarkers $phaseMarkers `
  -PlainEnglish @(@("first controlled provider trial identity","Prompt safety checklist","Approval gates","Validation evidence requirements","Denied trial actions","Blocked trial risks","Provider response inbox route","Cost and rate-limit route","Next recommended action") + $providerReviewSafetyMarkers) `
  -ExtraRoutes @("/provider-response-review-inbox","/provider-cost-rate-limit-review","/openai-compatible-provider-trial-review","/multi-provider-routing-release-candidate")
...
Write-Host "[OK] CodexForge First Controlled Provider Trial smoke passed."
