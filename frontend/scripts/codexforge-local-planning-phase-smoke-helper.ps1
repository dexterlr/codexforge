...
foreach ($marker in $requiredMarkers + @("no Date.now")) {
...
Assert-NotMatches $deterministicSource "nvidia-smi|Get-CimInstance|Get-WmiObject|wmic|child_process|execSync|spawn\s*\(" "no hardware/system command"
Assert-NotMatches $deterministicSource "generateVideo\s*\(|createVideo\s*\(|generateImage\s*\(|createImage\s*\(|runUpscale\s*\(|executeUpscale\s*\(|runInterpolation\s*\(|interpolateFrames\s*\(|runWorkflow\s*\(|executeWorkflow\s*\(|queue_prompt" "no media generation, upscale, interpolation, or ComfyUI workflow run"
Assert-NotMatches $deterministicSource "startJob\s*\(|runJob\s*\(|enqueueJob\s*\(|dequeueJob\s*\(|pauseQueue\s*\(|resumeQueue\s*\(|cancelJob\s*\(" "no job queue execution"
