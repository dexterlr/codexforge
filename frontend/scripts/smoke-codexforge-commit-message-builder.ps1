...
$domain = "src\lib\codexforge\commit-message-builder"
$route = "src\app\commit-message-builder"

& (Join-Path $PSScriptRoot "codexforge-local-planning-phase-smoke-helper.ps1") `
  -PhaseName "Commit Message Builder" `
  -ScriptFile "smoke-codexforge-commit-message-builder.ps1" `
  -Domain $domain `
  -Route $route `
  -MainPanel "CommitMessageBuilderPanel" `
  -CommandLabel "Go to Commit Message Builder" `
  -Modules @("commit-message-builder-types.ts","commit-message-builder-summary.ts","index.ts") `
  -Components @("CommitMessageBuilderPanel.tsx","index.ts") `
  -Exports @("buildCommitMessageBuilderStableKey","buildCommitMessageDraft","buildCommitMessageDrafts","buildCommitMessageBuilderBoundary","buildCommitMessageBuilderModel","summarizeCommitMessageBuilder","COMMIT_MESSAGE_BUILDER_LANGUAGE") `
  -PlainEnglish @("Commit message builder","Commit messages are drafts only","No commit is created from this page","Secrets are not included in commit copy","Commit draft identity","Source diff review","Change summary","Validation summary","Risk/secrets summary","Suggested commit subject","Suggested commit body bullets","Approval copy","Commit approval route","Blocked reasons","advanced commit details collapsed/secondary","approved local boundary required","git commands are not run from arbitrary UI","no git command execution from UI","nothing executes from arbitrary UI","no automatic local action","no raw fetch from arbitrary UI","no command execution","no shell command execution","no commit creation","no push behavior","no arbitrary local file browsing","no arbitrary file read/open","no file mutation","no file deletion","no patch apply behavior","no package install behavior","no provider APIs are called","no automatic provider send","no secrets displayed","no secrets exported","no secrets included","no localStorage API key storage","no process.env printing","no direct appendEvent call from UI","no direct saveBrainGraph call from UI","no direct graph mutation from UI","no memory auto-promotion") `
  -ExtraRoutes @("/git-diff-review","/test-result-summary","/patch-result-capture","/git-commit-approval")
...
foreach ($needle in @(
  ...
  Assert-Contains $source $needle "commit message builder includes $needle"
}
...
Write-Host "[OK] CodexForge Commit Message Builder smoke passed."
