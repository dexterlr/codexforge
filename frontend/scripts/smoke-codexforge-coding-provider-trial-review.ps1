...
$domain = "src\lib\codexforge\coding-provider-trial-review"
$route = "src\app\coding-provider-trial-review"
$newRoutes = @(
...
$providerTrialSafetyMarkers = @(
  ...
  "no coding provider calls",
  "no coding workflow execution",
  "no patch apply behavior",
  "no code apply from UI",
...
& (Join-Path $PSScriptRoot "codexforge-local-project-knowledge-smoke-helper.ps1") `
  -PhaseName "Phase 428 Coding Provider Trial Review" `
  -ScriptFile "smoke-codexforge-coding-provider-trial-review.ps1" `
  -Domain $domain `
  -Route $route `
  -MainPanel "CodingProviderTrialReviewPanel" `
  -CommandLabel "Go to Coding Provider Trial Review" `
  -Modules @("coding-provider-trial-review-types.ts","coding-provider-trial-review-summary.ts","index.ts") `
  -Components @("CodingProviderTrialReviewPanel.tsx","index.ts") `
  -Exports @("buildCodingProviderTrialReviewStableKey","buildCodingProviderTrialReview","buildCodingProviderTrialReviews","buildCodingProviderTrialReviewBoundary","buildCodingProviderTrialReviewModel","summarizeCodingProviderTrialReview","CODING_PROVIDER_TRIAL_REVIEW_LANGUAGE") `
  -PhaseMarkers $phaseMarkers `
  -PlainEnglish @(@("coding provider trial identity","Validation requirement checklist","Approval gate checklist","Denied coding provider actions","Blocked coding trial risks","Creative provider trial route","Cross-provider comparison route","Next recommended action") + $providerTrialSafetyMarkers) `
  -ExtraRoutes @("/creative-provider-trial-review","/research-provider-trial-review","/cross-provider-result-comparison","/coding-workflow-readiness-audit")
...
Assert-NotMatches $source "codingWorkflowExecutionAllowedFromUi:\s*true|patchApplyAllowedFromUi:\s*true|codeApplyAllowedFromUi:\s*true|applyPatch\s*\(|applyDiff\s*\(|applyCode\s*\(|executeWorkflow\s*\(|runWorkflow\s*\(" "no coding workflow execution or patch apply behavior"
Assert-NotMatches $source "commandExecutionAllowedFromUi:\s*true|shellExecutionAllowedFromUi:\s*true|gitCommandExecutionAllowedFromUi:\s*true|testExecutionFromUiAllowed:\s*true|buildExecutionFromUiAllowed:\s*true|smokeExecutionFromUiAllowed:\s*true|runCommand\s*\(|runGit\s*\(|runTests\s*\(|runBuild\s*\(|runSmoke\s*\(|child_process|execSync|spawn\s*\(" "no command git shell test build or smoke execution"
Assert-NotMatches $source "codingProviderCallsAllowedFromUi:\s*true|providerApiCallsAllowedFromUi:\s*true|providerConnectionTestsAllowedFromUi:\s*true|liveProviderTrafficAllowedFromUi:\s*true|providerTrafficRoutingAllowedFromUi:\s*true|promptSendingAllowedFromUi:\s*true|callProviderApi\s*\(|callCodingProvider\s*\(|connectProvider\s*\(|testProviderConnection\s*\(|routeLiveProviderTraffic\s*\(|sendPrompt\s*\(" "no provider calls connection tests traffic routing or prompt sending"
...
Assert-NotMatches $source "readFile\s*\(|openFile\s*\(|writeFile\s*\(|exportFile\s*\(|downloadFile\s*\(|deleteFile\s*\(" "no arbitrary file read open write export or delete behavior"
...
Write-Host "[OK] CodexForge Coding Provider Trial Review smoke passed."
