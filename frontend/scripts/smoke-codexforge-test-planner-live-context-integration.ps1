...
$domain = "src\lib\codexforge\test-planner-live-context-integration"
$route = "src\app\test-planner-live-context"

& (Join-Path $PSScriptRoot "codexforge-local-planning-phase-smoke-helper.ps1") `
  -PhaseName "Phase 286 Test Planner Live Context Integration" `
  -ScriptFile "smoke-codexforge-test-planner-live-context-integration.ps1" `
  -Domain $domain `
  -Route $route `
  -MainPanel "TestPlannerLiveContextIntegrationPanel" `
  -CommandLabel "Go to Test Planner Live Context" `
  -Modules @("test-planner-live-context-integration-types.ts","test-planner-live-context-integration-summary.ts","index.ts") `
  -Components @("TestPlannerLiveContextIntegrationPanel.tsx","index.ts") `
  -Exports @("buildTestPlannerLiveContextIntegrationStableKey","buildTestPlannerLiveContextIntegration","buildTestPlannerLiveContextIntegrations","buildTestPlannerLiveContextBoundary","buildTestPlannerLiveContextIntegrationModel","summarizeTestPlannerLiveContextIntegration","TEST_PLANNER_LIVE_CONTEXT_INTEGRATION_LANGUAGE") `
  -PlainEnglish @("Test planner live context integration","Live context improves test planning but does not run tests","Commands are not executed from this page","Env values and secrets are never displayed","Recommended test scope","Execution trial route","Integration identity","Source project intelligence result","Source patch preview live context","Affected files/modules summary","Recommended command summary","Risk/secrets redaction status","Confidence/coverage note","Blocked reasons","live context does not execute commands","advanced context details collapsed/secondary","server-only path boundary markers remain intact","no automatic local action","no raw fetch from arbitrary UI","no command execution","no shell command execution","no test execution from UI","no git command execution from UI","no direct Jarvisd call from arbitrary UI","no Jarvisd capability execution from UI","no daemon process creation from frontend","no browser-stored signing secrets","no session token localStorage storage","no arbitrary local file browsing","no arbitrary path crawling","no arbitrary file read/open","no auto-open local files","no file mutation","no file write","no patch apply behavior","no file deletion","no secret value display","no automatic provider send","no provider APIs are called","no OpenAI-compatible API calls","no localStorage API key storage","no process.env printing","no direct appendEvent call from UI","no direct saveBrainGraph call from UI","no direct graph mutation from UI","no memory auto-promotion","no process kill/restart/shutdown from UI","no package install behavior") `
  -ExtraRoutes @("/project-intelligence-result","/patch-preview-live-context","/test-command-planner","/test-execution-trial","/test-result-capture")
...
foreach ($needle in @(
  ...
  Assert-Contains $source $needle "test planner live context includes $needle"
}
...
$blockedPatterns = @{
  ...
  "no command execution" = "commandsExecutedFromPageAllowed:\s*true|commandExecutionAllowedFromUi:\s*true|shellExecutionAllowedFromUi:\s*true|child_process|execSync|spawn\s*\(|runCommand\s*\(|brokerExecution\s*\(|Start-Process|Invoke-Expression"
  ...
  "no test execution from UI" = "liveContextRunsTestsAllowed:\s*true|testExecutionFromUiAllowed:\s*true|runTests\s*\(|executeTests\s*\("
  ...
  "no automatic provider send" = "automaticProviderSendAllowed:\s*true|promptOrFileAutoSendAllowed:\s*true|sendContextToProvider\s*\(|sendFindingsToProvider\s*\("
  ...
  "no Jarvisd capability execution from UI" = "jarvisdCapabilityExecutionAllowedFromUi:\s*true|executeJarvisdCapability\s*\(|runJarvisdCapability\s*\("
...
Write-Host "[OK] CodexForge Test Planner Live Context Integration smoke passed."
