...
CodexForge checkpoint documentation now records through phase 1561.

The latest completed batch is 1546-1561 - Provider Approval Gate. The latest family adds deterministic provider approval gate boundary, provider request packet, provider identity preview, prompt payload approval preview, data boundary approval preview, privacy class approval preview, cost class approval preview, capability justification approval preview, expiry replay protection preview, provider fallback approval preview, provider denial route preview, provider evidence capture preview, provider result capture preview, cockpit provider approval summary, first provider approval gate candidate, and controlled provider approval gate release candidate surfaces. Latest release candidate: Controlled Provider Approval Gate Release Candidate. The cockpit is the normal user surface and now includes Provider Approval with Provider, Model, Prompt, Data Boundary, Privacy, Cost, Capability, Expiry, Replay Protection, Fallback, Denied, Evidence, Result, and Audit. Phase pages remain dev test diagnostics only. frontend model/provider/connector calls still blocked: no model calls, provider calls, connector calls, prompt sending, credential storage, hidden routing, hidden approvals, provider approval persistence, queue persistence, transaction persistence, evidence/result/audit persistence, file writes, diff application, commands, recovery execution, process spawning, port binding, installs, deploys, or runtime starts happen from the cockpit. backend-owned provider approval remains required. Explicit operator approval remains required. Next likely batch: 1562-1577 - Local Model Bridge v1.
