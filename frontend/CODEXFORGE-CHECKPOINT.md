...
CodexForge checkpoint documentation now records through phase 1481.

The latest completed batch is 1466-1481 - Backend Execution Queue v1. The latest family adds deterministic backend execution queue boundary, queue item model preview, approval state preview, preflight state preview, apply state preview, run state preview, evidence capture state preview, result capture state preview, audit capture state preview, blocked denied state preview, failed canceled state preview, manual review state preview, recovery state preview, cockpit queue summary, first backend execution queue candidate, and controlled backend execution queue release candidate surfaces. Latest release candidate: Controlled Backend Execution Queue Release Candidate. The cockpit is the normal user surface and now includes Backend Execution Queue v1 with Queue Item, Approval, Preflight, Apply, Run, Evidence, Result, Audit, Blocked, Denied, Failed, Canceled, Manual Review, Recovery, and Transitions. Phase pages remain dev test diagnostics only. Broad execution still blocked and frontend queue persistence still blocked: no model calls, provider calls, connector calls, command execution, file writes, diff application, hidden approval, approval persistence, queue creation, queue persistence, execution release, evidence/result/audit persistence, memory persistence from the cockpit, browser storage writes from the cockpit, secret memory from the cockpit, rollback/retry/recovery execution, local runtime start, port binding, deploy, install, scaffold, environment value display, or memory promotion happens from the cockpit. Backend-owned queue state, backend-owned guarded execution, and explicit operator approval remain required. Next likely batch: 1482-1497 - Apply Run Transaction Boundary.
