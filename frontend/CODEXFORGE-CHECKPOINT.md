...
CodexForge checkpoint documentation now records through phase 1433.

The latest completed batch is 1418-1433 - Goal Compiler v1. The latest family adds a deterministic Goal Compiler v1 layer for raw goal intake, goal domain classification, task type classification, target artifact preview, context requirements, file impact expectations, command expectations, risk level, approval scope, evidence requirements, done criteria, recovery implications, model/tool routing hints, and cockpit goal summary. Latest release candidate: Controlled Goal Compiler Release Candidate. The cockpit is the normal user surface and now includes Goal, Domain, Task, Target, Context, Files, Commands, Risk, Approval, Evidence, Done, Recovery, and Model Tool Hints. Phase pages remain dev test diagnostics only. Broad execution still blocked: no model calls, provider calls, connector calls, goal execution, direct frontend file mutation, direct frontend command execution, hidden approval, approval persistence, queue persistence, evidence/result/audit persistence, rollback/retry/recovery execution, local runtime start, port binding, deploy, install, scaffold, environment value display, or memory promotion happens from the cockpit. Backend-owned goal compilation, backend-owned guarded execution, and explicit operator approval remain required. Next likely batch: 1434-1449 - Plan Diff Command Composer v1.
