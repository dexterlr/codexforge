...
CodexForge checkpoint documentation now records through phase 1609.

The latest completed batch is 1594-1609 - First Domain Pack Game Server Builder. The latest family adds deterministic game server builder domain boundary, game server goal intake preview, server type classifier preview, theme lore pack preview, Minecraft server profile preview, plugin mod intent preview, world rules config preview, roles permissions economy preview, quest region faction preview, server file plan preview, server command plan preview, server validation evidence preview, server recovery audit preview, cockpit game server builder summary, first game server builder candidate, and controlled game server builder release candidate surfaces. Latest release candidate: Controlled Game Server Builder Release Candidate. The cockpit is the normal user surface and now includes Game Server Builder with Server Goal, Server Type, Theme, Minecraft Profile, Plugins, Mods, World Rules, Roles, Permissions, Economy, Quests, Regions, Factions, Files, Commands, Validation, Evidence, Recovery, and Audit. Phase pages remain dev test diagnostics only. frontend game server execution still blocked: no server starts, plugin installs, mod installs, server jar downloads, Java starts, Docker starts, SteamCMD starts, port binding, command execution, model calls, local model calls, provider calls, connector calls, prompt sending, file writes, diff application, queue creation, transaction creation, snapshot creation, approval persistence, evidence/result/audit persistence, memory promotion, rollback/retry/recovery execution, process spawning, deployment, browser storage writes, credential storage, secret reads, API key reads, or localhost probing happen from the cockpit. backend-owned game server workflow remains required. Explicit operator approval remains required. Next likely batch: 1610-1625 - Domain Pack Runner Preview.
