...
CodexForge checkpoint documentation now records through phase 1497.

The latest completed batch is 1482-1497 - Apply Run Transaction Boundary. The latest family adds deterministic apply run transaction boundary, transaction intent packet, transaction preflight preview, workspace snapshot preview, guarded apply transaction step, guarded run transaction step, transaction validation step, transaction evidence capture preview, transaction result capture preview, transaction audit capture preview, transaction rollback readiness preview, transaction retry readiness preview, transaction recovery decision preview, cockpit transaction summary, first apply run transaction candidate, and controlled apply run transaction release candidate surfaces. Latest release candidate: Controlled Apply Run Transaction Release Candidate. The cockpit is the normal user surface and now includes Apply Run Transaction Boundary with Transaction, Intent, Preflight, Snapshot, Apply, Run, Validate, Evidence, Result, Audit, Rollback, Retry, Recovery, and Denied Paths. Phase pages remain dev test diagnostics only. Broad execution still blocked and frontend transaction persistence still blocked: no model calls, provider calls, connector calls, command execution, file writes, diff application, hidden approval, approval persistence, queue creation, transaction creation, snapshot creation, transaction persistence, evidence/result/audit persistence, memory persistence from the cockpit, browser storage writes from the cockpit, secret memory from the cockpit, rollback/retry/recovery execution, local runtime start, port binding, deploy, install, scaffold, environment value display, or memory promotion happens from the cockpit. Backend-owned transaction state, backend-owned guarded execution, and explicit operator approval remain required. Next likely batch: 1498-1513 - Command Runner Safety v2.
