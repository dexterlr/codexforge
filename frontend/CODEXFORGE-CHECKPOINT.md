...
CodexForge checkpoint documentation now records through phase 1577.

The latest completed batch is 1562-1577 - Local Model Bridge v1. The latest family adds deterministic local model bridge boundary, local runtime identity preview, local model registry preview, local endpoint boundary preview, local prompt handoff preview, local context redaction preview, local capability fit preview, local timeout cancellation preview, local response capture preview, local fallback route preview, local denial route preview, local evidence capture preview, local result audit preview, cockpit local model bridge summary, first local model bridge candidate, and controlled local model bridge release candidate surfaces. Latest release candidate: Controlled Local Model Bridge Release Candidate. The cockpit is the normal user surface and now includes Local Model Bridge v1 with Runtime, Model Registry, Endpoint, Prompt Handoff, Redaction, Capability, Timeout, Cancel, Response, Fallback, Denied, Evidence, Result, and Audit. Phase pages remain dev test diagnostics only. frontend local model calls still blocked: no local model calls, model calls, provider calls, connector calls, runtime starts, localhost probing, port binding, prompt sending, secret/API key reads, credential storage, hidden local model routing, provider approval persistence, response persistence, queue persistence, transaction persistence, evidence/result/audit persistence, file writes, diff application, commands, recovery execution, process spawning, installs, deploys, or memory promotion happen from the cockpit. backend-owned local model bridge remains required. Explicit operator approval remains required. Next likely batch: 1578-1593 - Specialist Worker Registry.
