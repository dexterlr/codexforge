...
CodexForge checkpoint documentation now records through phase 1465.

The latest completed batch is 1450-1465 - Evidence Memory v1. The latest family adds a deterministic Evidence Memory v1 layer for evidence memory boundary, goal memory snapshot, context memory snapshot, plan memory snapshot, diff memory snapshot, command memory snapshot, approval memory snapshot, evidence memory packet, result memory packet, recovery memory packet, audit memory packet, denied memory boundary, memory promotion review, cockpit evidence memory summary, first evidence memory candidate, and controlled evidence memory release candidate. Latest release candidate: Controlled Evidence Memory Release Candidate. The cockpit is the normal user surface and now includes Evidence Memory v1 with Remembered Goal, Remembered Context, Remembered Plan, Remembered Files, Remembered Commands, Approval Memory, Evidence Memory, Result Memory, Recovery Memory, Audit Memory, Memory Review, and Denied Memory. Phase pages remain dev test diagnostics only. Broad execution still blocked and automatic memory promotion still blocked: no model calls, provider calls, connector calls, command execution, file writes, diff application, hidden approval, approval persistence, queue persistence, evidence/result/audit persistence, memory persistence from the cockpit, browser storage writes from the cockpit, secret memory from the cockpit, rollback/retry/recovery execution, local runtime start, port binding, deploy, install, scaffold, environment value display, or memory promotion happens from the cockpit. Backend-owned evidence memory, backend-owned guarded execution, and explicit operator approval remain required. Next likely batch: 1466-1481 - Backend Execution Queue v1.
