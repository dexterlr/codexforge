...
CodexForge checkpoint documentation now records through phase 1385.

The latest completed batch is 1370-1385 - First Real Trial Hardening and Failure Recovery. The latest family is the real trial hardening layer for phases 1370-1385. It covers Real Trial Hardening Boundary, Denied Path Failure Handling, Denied Command Failure Handling, Approval Expiry Failure Handling, Dirty Workspace Failure Handling, Preflight Failure Handling, Backend Guard Mismatch Handling, Evidence Capture Failure Handling, Result Capture Failure Handling, Audit Capture Failure Handling, Operator Stop Handling, Manual Review Handling, Recovery Review Packet, Rollback Readiness Preview, Retry Readiness Preview, and Controlled Real Trial Hardening Release Candidate surfaces. Latest release candidate: Controlled Real Trial Hardening Release Candidate. These surfaces remain static, deterministic, cockpit-centered, hardening-only, tiny-real-only, explicit-approval-required, and backend-owned: the cockpit cannot directly mutate files, cannot directly run commands, cannot persist approvals, cannot persist queues, cannot persist evidence/results/audit logs, cannot release execution, cannot execute rollback/retry/recovery, cannot call models/providers/connectors/adapters, cannot start runtimes, cannot bind ports, cannot deploy, cannot install, cannot scaffold, cannot read secrets, and cannot promote memory. Broad execution still blocked. Next likely batch: 1386-1401 - Daily-Testable Cockpit MVP Polish.
