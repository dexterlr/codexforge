...
CodexForge checkpoint documentation now records through phase 1257.

The latest family is the controlled end-to-end build/fix workflow release-candidate layer for phases 1242-1257. It covers the workflow boundary, goal intake, project context, plan summary, file diff, command preview, risk review, approval ticket, apply hold, command hold, evidence packet, result decision, recovery packet, audit timeline, First End-to-End Build Fix Candidate, and Controlled End-to-End Build Fix Workflow Release Candidate surfaces. These surfaces remain static, deterministic, preview-only, and approval-gated; they do not call models, send prompts, write files, run commands, persist approvals, create queues, persist evidence/results/audit logs, execute rollback/retry/recovery, start runtimes, spawn processes, bind ports, call providers/connectors/adapters, or promote memory.
