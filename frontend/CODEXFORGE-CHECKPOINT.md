...
CodexForge checkpoint documentation now records through phase 1417.

The latest completed batch is 1402-1417 - Project Context Brain v1. The latest family adds a deterministic Project Context Brain v1 layer for workspace identity, project map, framework/runtime hints, package/tooling hints, important files, command candidates, risk zones, denied context boundaries, evidence needs, result expectations, recovery implications, context confidence, and cockpit project context summary. Latest release candidate: Controlled Project Context Brain Release Candidate. The cockpit is the normal user surface and now includes Project, Stack, Files, Commands, Risks, Evidence, Result, Recovery, and Confidence context. Phase pages remain dev test diagnostics only. Broad execution still blocked: no arbitrary cockpit file crawling, secret reads, runtime probes, direct frontend file mutation, direct frontend command execution, hidden approval, approval persistence, queue persistence, evidence/result/audit persistence, rollback/retry/recovery execution, provider/model/connector/adapter call, runtime start, port binding, deploy, install, scaffold, environment value display, or memory promotion happens from the cockpit. Backend-owned context inspection, backend-owned guarded execution, and explicit operator approval remain required. Next likely batch: 1418-1433 - Goal Compiler v1.
