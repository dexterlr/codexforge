...
CodexForge checkpoint documentation now records through phase 1449.

The latest completed batch is 1434-1449 - Plan Diff Command Composer v1. The latest family adds a deterministic Plan Diff Command Composer v1 layer for plan steps, file impact plan, diff preview, command preview, risk review, approval packet, execution hold, evidence expectations, result expectations, recovery plan, audit timeline, model/tool handoff, denied composer boundaries, and cockpit work proposal summary. Latest release candidate: Controlled Plan Diff Command Composer Release Candidate. The cockpit is the normal user surface and now includes Plan, Files, Diff, Commands, Risks, Approval, Hold, Evidence, Result, Recovery, Timeline, and Model Tool Handoff. Phase pages remain dev test diagnostics only. Broad execution still blocked: no model calls, provider calls, connector calls, plan execution, direct frontend file mutation, direct frontend diff application, direct frontend command execution, hidden approval, approval persistence, queue persistence, evidence/result/audit persistence, rollback/retry/recovery execution, local runtime start, port binding, deploy, install, scaffold, environment value display, or memory promotion happens from the cockpit. Backend-owned work proposal execution, backend-owned guarded execution, and explicit operator approval remain required. Next likely batch: 1450-1465 - Evidence Memory v1.
