...
The latest completed batch is 1386-1401 - Daily-Testable Cockpit MVP Polish. The latest family makes `/codexforge-cockpit` the normal user surface with one daily-testable cockpit for Goal, Plan, Files, Commands, Approval, Run State, Evidence, Result, Recovery, Timeline, Safety, Diagnostics, and Daily MVP readiness. Latest release candidate: Controlled Daily-Testable Cockpit MVP Release Candidate. Phase pages remain dev test diagnostics only. Broad execution still blocked: no direct frontend file mutation, direct frontend command execution, hidden approval, approval persistence, queue persistence, lock release, evidence/result/audit persistence, rollback/retry/recovery execution, provider/model/connector/adapter call, runtime start, port binding, deploy, install, scaffold, secret read, or memory promotion happens from the cockpit. Backend-owned guarded execution and explicit operator approval remain required. Next likely batch: 1402-1417 - Project Context Brain v1.
