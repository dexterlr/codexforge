...
CodexForge is checkpointed through phase 1609, detected from the local `scripts/smoke-codexforge-all.ps1` registry by taking the highest `Phase N` entry. The latest completed batch is 1594-1609 - First Domain Pack Game Server Builder. The latest detected milestone family adds deterministic game server builder domain boundary, game server goal intake preview, server type classifier preview, theme lore pack preview, Minecraft server profile preview, plugin mod intent preview, world rules config preview, roles permissions economy preview, quest region faction preview, server file plan preview, server command plan preview, server validation evidence preview, server recovery audit preview, cockpit game server builder summary, first game server builder candidate, and controlled game server builder release candidate surfaces while `/codexforge-cockpit` remains the normal user surface and phase pages remain dev test diagnostics only. frontend game server execution still blocked: no server starts, plugin installs, mod installs, server jar downloads, Java starts, Docker starts, SteamCMD starts, port binding, command execution, model calls, local model calls, provider calls, connector calls, prompt sending, file writes, diff application, queue creation, transaction creation, snapshot creation, approval persistence, evidence/result/audit persistence, memory promotion, rollback/retry/recovery execution, process spawning, deployment, browser storage writes, credential storage, secret reads, API key reads, or localhost probing occur from the cockpit. backend-owned game server workflow remains required. Explicit operator approval remains required.
...
- Highest detected phase: 1609.
- Checkpoint date: 2026-06-23.
- Latest completed batch: 1594-1609 - First Domain Pack Game Server Builder.
- Latest detected milestone family: First Domain Pack Game Server Builder.
- Latest release candidate: Controlled Game Server Builder Release Candidate.
- Next likely batch: 1610-1625 - Domain Pack Runner Preview.
...
- Unified cockpit: `/codexforge-cockpit` is the preferred normal user surface for goal, plan, work proposal, file-write diff preview, command preview, approval queue, execution state, evidence, result, recovery, timeline, model/tool handoff, safety coach, project context brain summary, Provider Approval, and dev/test diagnostics.
...
- Current checkpoint: documentation and hygiene alignment through detected phase 1609.
- Next milestone: 1610-1625 - Domain Pack Runner Preview. Keep Game Server Builder preview-only unless explicitly approved, and run any backend-owned domain pack workflow only after explicit approval, worker routing review, local bridge/provider approval review, sandbox enforcement, audit, evidence/result links, rollback plan, validation packet, denial behavior, and safety smoke evidence exist.
