...
CodexForge is checkpointed through phase 1225, detected from the local `scripts/smoke-codexforge-all.ps1` registry by taking the highest `Phase N` entry. The latest detected milestone family is the cockpit evidence/result/recovery release-candidate layer: Cockpit Evidence Result Recovery Boundary, Cockpit Evidence Stream Model, Cockpit File Evidence View, Cockpit Command Evidence View, Cockpit Result Summary Model, Cockpit Result Decision View, Cockpit Recovery Option Model, Cockpit Rollback Preview View, Cockpit Retry Preview View, Cockpit Explain Failure View, Cockpit Audit Trail View, Cockpit Run Timeline View, Cockpit Evidence Export Preview, Cockpit Recovery Safety Gate, First Cockpit Evidence Result Recovery Candidate, and Controlled Cockpit Evidence Result Recovery Release Candidate. These surfaces extend `/codexforge-cockpit` as the preferred normal user review surface while phase pages remain dev/test surfaces only, without allowing UI command execution, shell execution, model calls, provider calls, connector calls, file writes, runtime execution, adapter execution, approval persistence, evidence/result persistence, audit persistence, export writing, rollback/retry/recovery execution, environment value reads, process spawning, or hidden mutation.
...
- Latest detected milestone family: Cockpit evidence/result/recovery release-candidate layer.
...
- Current checkpoint: documentation and hygiene alignment through detected phase 1225.
...
Phases 1194-1209 cover the unified cockpit foundation and establish `/codexforge-cockpit` as the preferred normal user surface. Phases 1210-1225 add cockpit evidence/result/recovery preview surfaces: evidence stream, file evidence, command evidence, result summary, result decision, recovery options, rollback preview, retry preview, explain failure, audit trail, run timeline, evidence export preview, recovery safety gate, first candidate, and controlled release candidate. These surfaces keep `/codexforge-cockpit` current through phase 1225 while phase pages remain dev/test diagnostics only. File writes and commands remain blocked until explicit operator approval; no provider, model, connector, runtime, adapter, command, file mutation, approval persistence, evidence persistence, result persistence, audit persistence, export writing, recovery execution, rollback execution, retry execution, queue persistence, scaffold, deployment, package/export, browser credential storage, hidden approval, or automatic memory promotion occurs from the cockpit.
