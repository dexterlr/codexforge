...
CodexForge is checkpointed through phase 1433, detected from the local `scripts/smoke-codexforge-all.ps1` registry by taking the highest `Phase N` entry. The latest completed batch is 1418-1433 - Goal Compiler v1. The latest detected milestone family adds deterministic raw goal intake, goal domain classification, task type classification, target artifact preview, context requirements, file impact expectations, command expectations, risk level, approval scope, evidence requirements, done criteria, recovery implications, model/tool routing hints, and cockpit goal summary surfaces while `/codexforge-cockpit` remains the normal user surface and phase pages remain dev test diagnostics only. Broad execution still blocked: no model calls, provider calls, connector calls, goal execution, arbitrary cockpit file crawling, secret reads, runtime probes, direct frontend file mutation, direct frontend command execution, hidden approvals, approval persistence, queue persistence, evidence/result/audit persistence, rollback/retry/recovery execution, runtime starts, process spawning, port binding, deployment, install, scaffold, environment value display, or automatic memory promotion occurs from the cockpit. Backend-owned goal compilation, backend-owned guarded execution, and explicit operator approval remain required.
...
- Highest detected phase: 1433.
- Checkpoint date: 2026-06-22.
- Latest completed batch: 1418-1433 - Goal Compiler v1.
- Latest detected milestone family: project context brain v1.
- Latest release candidate: Controlled Goal Compiler Release Candidate.
- Next likely batch: 1434-1449 - Plan Diff Command Composer v1.
...
- Unified cockpit: `/codexforge-cockpit` is the preferred normal user surface for goal, plan, file-write diff preview, command preview, approval queue, execution state, evidence, result, recovery, safety coach, project context brain summary, and dev/test diagnostics.
...
- Current checkpoint: documentation and hygiene alignment through detected phase 1353.
- Next milestone: keep model/provider/router work preview-only unless explicitly approved, and choose any first bounded backend/local implementation proposal only after explicit approval, backend contract review, local bridge contract review, sandbox enforcement, audit, evidence/result links, rollback plan, validation packet, denial behavior, and safety smoke evidence exist.
