...
CodexForge is checkpointed through phase 953, detected from the local `scripts/smoke-codexforge-all.ps1` registry by taking the highest `Phase N` entry. The latest detected milestone family is the model-routed backend execution review layer: backend execution router integration boundary, file write/command runner/local runtime/evidence store/result store/recovery/packaging/project scaffold previews, model-routed approval/audit/sandbox/validation/operator-trial packets, first model-routed backend execution candidate, and Controlled Backend Model Router Release Candidate surfaces.
