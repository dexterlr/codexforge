...
CodexForge checkpoint documentation now records through phase 1945.

The latest completed batch is 1930-1945 - Script And Storyboard Workspace v1. The latest family adds Script And Storyboard Workspace Boundary, Script Brief Intake Preview, Hook And Opening Beat Preview, Scene Beat Outline Preview, Storyboard Card Grid Preview, Shot Intent Note Preview, Visual Reference Planning Preview, B-Roll And Asset Note Preview, Caption And Supers Note Preview, Review Comments Lane Preview, Brand And Rights Note Preview, Model Generation Blocked Boundary Preview, Storyboard Export Blocked Boundary Preview, Cockpit Script And Storyboard Summary, First Script And Storyboard Workspace Candidate, and Controlled Script And Storyboard Workspace Release Candidate surfaces. Latest release candidate: Controlled Script And Storyboard Workspace Release Candidate. `/codexforge-cockpit` now acts as the one normal user UX and includes safe script and storyboard workspace previews grouped inside Creative Workspace below Video Creation Domain. Script and storyboard workspace remains review-only. Video creation remains planning-only. Synthetic data only. Frontend final script generation still blocked. Frontend storyboard image generation still blocked. Frontend rendering still blocked. Frontend export still blocked. Frontend upload/download still blocked. Frontend file generation still blocked. Frontend script persistence still blocked. Frontend storyboard persistence still blocked. Frontend asset persistence still blocked. Frontend prompt persistence still blocked. Frontend job persistence still blocked. Frontend approval persistence still blocked. Frontend provider calls still blocked. Frontend model calls still blocked. Frontend connector calls still blocked. Frontend image generation still blocked. Frontend video generation still blocked. Frontend voice generation still blocked. Frontend publishing still blocked. Frontend scheduling still blocked. Backend-owned asset storage remains required. Backend-owned render service remains required. Backend-owned export service remains required. Backend-owned provider gateway remains required. Backend-owned rights review remains required. Backend-owned approval capture remains required. Backend-owned script persistence remains required. Backend-owned storyboard persistence remains required. Operator review remains required. Explicit operator approval remains required. Trading workspace remains review-only and no-live-transition. Main menu hides phase spam. Diagnostics remain searchable and directly accessible. No routes were deleted. No smoke coverage was deleted. No execution was enabled. Next likely batch: 1946-1961 - Asset And Shot Planning Workspace v1.
