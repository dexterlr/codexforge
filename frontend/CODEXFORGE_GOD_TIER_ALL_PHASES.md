...
The latest family is the backend guarded apply/run preview release-candidate layer for phases 1322-1337. It adds a cockpit-centered, preview-only backend-owned guarded apply/run contract for approval handoff, guarded apply, guarded run, combined apply/run, path guard, command guard, approval enforcement, evidence, result, recovery, audit, queue state, denied path review, go/no-go review, first candidate, and controlled release candidate without model calls, file writes, apply execution, command execution, backend execution, approval persistence, evidence/result/audit persistence, recovery execution, lock release, queue creation, runtime starts, adapter execution, exports, real operator trial execution, or memory promotion.
