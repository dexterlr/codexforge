...
The latest completed batch is 1370-1385 - First Real Trial Hardening and Failure Recovery. The latest family is the real trial hardening layer for phases 1370-1385. It adds cockpit-centered hardening for the first tiny real controlled trial: hardening boundary, denied path handling, denied command handling, approval expiry handling, dirty workspace handling, preflight failure handling, backend guard mismatch handling, evidence/result/audit capture failure handling, operator stop, manual review, recovery review packet, rollback readiness, retry readiness, and controlled hardening release candidate. Latest release candidate: Controlled Real Trial Hardening Release Candidate. Broad execution still blocked: no direct frontend file mutation, direct frontend command execution, hidden approval, approval persistence, queue persistence, lock release, evidence/result/audit persistence, rollback/retry/recovery execution, provider/model/connector/adapter call, runtime start, port binding, deploy, install, scaffold, secret read, or memory promotion happens from the cockpit. Next likely batch: 1386-1401 - Daily-Testable Cockpit MVP Polish.
