...
The latest family is the backend approval handoff release-candidate layer for phases 1306-1321. It adds a cockpit-centered, preview-only backend-owned approval handoff contract for approval ticket, file write handoff, command handoff, evidence handoff, result handoff, recovery handoff, audit handoff, queue handoff, denied path matrix, operator signoff, go/no-go review, security review, failure review, first candidate, and controlled release candidate without model calls, file writes, command execution, backend execution, approval persistence, evidence/result/audit persistence, recovery execution, lock release, queue creation, runtime starts, adapter execution, exports, real operator trial execution, or memory promotion.
