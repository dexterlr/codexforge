...
The latest completed batch is 1354-1369 - First Tiny Real Controlled Operator Trial. The latest family is the tiny real controlled operator trial layer for phases 1354-1369. It adds a cockpit-centered tiny-real-only bridge from guarded apply/run dry-run preview to the first controlled operator trial contract: trial boundary, sandbox file write candidate, allowlisted command candidate, approval ticket, backend execution hold, path guard, command guard, preflight review, evidence capture contract, result capture contract, audit capture contract, recovery contract, denied path matrix, operator signoff, first candidate, and controlled release candidate. Latest release candidate: Controlled Tiny Real Operator Trial Release Candidate. Broad execution still blocked: no direct frontend file mutation, direct frontend command execution, hidden approval, approval persistence, queue persistence, lock release, evidence/result/audit persistence, recovery execution, provider/model/connector/adapter call, runtime start, port binding, deploy, install, scaffold, secret read, or memory promotion happens from the cockpit. Next likely batch: 1370-1385 - First Real Trial Hardening and Failure Recovery.
