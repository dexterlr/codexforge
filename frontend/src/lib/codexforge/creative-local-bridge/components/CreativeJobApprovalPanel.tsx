...
export function CreativeJobApprovalPanel({ approval }: { approval: CreativeJobApprovalPacket }) {
  ...
  return (
    <section style={card} data-codexforge-creative-job-approval-panel="CreativeJobApprovalPanel renders approval packet defaults approved false">
      ...
      <div style={grid}>
        {rows.map(([label, value]) => <div key={label} style={row}><span>{label}</span><strong>{value}</strong></div>)}
      </div>
    </section>
  );
}

const card: CSSProperties = { border: "1px solid rgba(251,113,133,0.22)", background: "rgba(38,10,24,0.75)", borderRadius: 8, padding: 16, display: "grid", gap: 10 };
...
const row: CSSProperties = { border: "1px solid rgba(251,113,133,0.16)", borderRadius: 8, padding: 9, display: "grid", gap: 4, overflowWrap: "anywhere" };
