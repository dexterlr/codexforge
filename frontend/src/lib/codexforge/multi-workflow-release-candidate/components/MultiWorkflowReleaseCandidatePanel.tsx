...
export function MultiWorkflowReleaseCandidatePanel() {
  ...
  return (
    ...
    >
      ...
      <PreviewFoundationCard title="Plain-English multi-workflow release candidate">
        <PreviewFoundationCopy>{model.summary}</PreviewFoundationCopy>
        <PreviewFoundationCopy>
          This page reviews multi-workflow release candidate identity, trial plan status, trial review status, regression
          status, safety approval readiness status, denied release actions, unresolved release blockers, controlled live
          signoff route, Daily Beta release candidate route, and next recommended action. It does not approve release,
          execute workflows, or persist settings.
        </PreviewFoundationCopy>
      </PreviewFoundationCard>
      <section id="multi-workflow-release-candidate" style={previewStyles.grid}>
        {model.candidates.map((candidate) => (
          <PreviewFoundationCard
            key={buildMultiWorkflowReleaseCandidateStableKey("multi-workflow-rc-card", candidate.id)}
            title={candidate.multiWorkflowReleaseCandidateIdentity}
          >
            <PreviewFoundationPillList
              items={[
                `Status: ${candidate.status}`,
                `Trial plan status: ${candidate.trialPlanStatus.join("; ")}`,
                `Trial review status: ${candidate.trialReviewStatus.join("; ")}`,
                ...
                `Denied release actions: ${candidate.deniedReleaseActions.join("; ")}`,
                `Unresolved release blockers: ${candidate.unresolvedReleaseBlockers.join("; ")}`,
                candidate.controlledLiveSignoffRoute,
                candidate.dailyBetaReleaseCandidateRoute,
      ...
      <PreviewFoundationDetail summary="Advanced release candidate details">
        ...
        />
        <PreviewFoundationCopy>
          Advanced release candidate details stay collapsed or secondary. This route never approves release, goes live,
          executes workflows, persists release settings, calls providers, calls local models, calls connectors, creates
          automations, writes files, or mutates memory.
        </PreviewFoundationCopy>
      </PreviewFoundationDetail>
    </div>
  );
