...
import { summarizeApplyMutationFirewall, type ApplyMutationFirewall } from "../index";

type Props = {
  firewall: ApplyMutationFirewall;
};

export function ApplyMutationFirewallPanel({ firewall }: Props) {
  return (
    <section style={card} data-codexforge-apply-mutation-firewall="ApplyMutationFirewallPanel renders actual mutation remains blocked">
      <span style={eyebrow}>Mutation Firewall</span>
      <h3 style={title}>Actual mutation remains blocked</h3>
      <ul style={list}>{summarizeApplyMutationFirewall(firewall).map((item) => <li key={item}>{item}</li>)}</ul>
      <div style={chips}>{firewall.blockedTools.map((tool) => <span key={tool} style={chip}>{tool}</span>)}</div>
    </section>
  );
}

const card: CSSProperties = { border: "1px solid rgba(248,113,113,0.24)", background: "rgba(127,29,29,0.14)", borderRadius: 8, padding: 12, display: "grid", gap: 8, minWidth: 0 };
