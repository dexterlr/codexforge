...
const ITEMS: readonly Omit<WorkflowEntrypoint, "id">[] = [
  ...
  { label: "Patch Preview v1", route: "/ai", readiness: "needs-review", missingPieces: ["Current file verification", "Preview package normalization", "Apply gate link"], safetyBoundary: "Preview-only; no direct apply-diff call from UI.", nextImplementationPhase: "Phase 57 Real Patch Preview v1", recommendedValidation: "Patch Preview smoke and git diff --check." },
...
export function summarizeWorkflowEntrypoints(model: Pick<WorkflowEntrypoints, "items" | "readyCount">): string[] {
  return [
    `${model.items.length} workflow entrypoints are defined including File Reader v1 functional/read-only, Patch Preview v1, and Validation Runner v1.`,
    `${model.readyCount} workflows are ready; the rest remain review-required or blocked until approval boundaries exist.`,
    "Phase 57 Real Patch Preview v1 is the recommended next functional workflow.",
  ];
}
