...
const ITEMS: readonly Omit<SharedReadinessItem, "id">[] = [
  ...
  { label: "Jarvis Chat", sourceRoute: "/jarvis", readinessLevel: "ready", blockerCount: 0, warningCount: 0, nextAction: "Start or reopen a bounded local conversation.", validationCommand: "npm run smoke:codexforge:server", reviewRequired: true, routeHref: "/jarvis" },
  ...
  { label: "Guarded Apply Candidate Implementation Plan", sourceRoute: "/guarded-apply-candidate", readinessLevel: "needs-review", blockerCount: 3, warningCount: 1, nextAction: "Clear blocker gaps before implementation; Guarded Apply Candidate MVP only if gaps are cleared.", validationCommand: "powershell -ExecutionPolicy Bypass -File .\\scripts\\smoke-codexforge-guarded-apply-candidate.ps1", reviewRequired: true, routeHref: "/guarded-apply-candidate" },
