...
const ACTIONS: readonly Omit<SharedNextAction, "id">[] = [
  ...
  { label: "Real Apply Guard Review", reason: "Review the actual guarded apply path before any apply automation increase.", priority: 71, routeHref: "/apply-guard-review", copyPrompt: "Open Real Apply Guard Review. Check approval, diff/path boundaries, rollback, command/write separation, validation, and go/no-go. Recommend Guarded Apply Candidate Implementation only if go/no-go permits." },
  { label: "Guarded Apply Candidate Implementation Plan", reason: "Clear blocker gaps before first guarded apply candidate implementation.", priority: 71.5, routeHref: "/guarded-apply-candidate", copyPrompt: "Open Guarded Apply Candidate. Keep one file, one diff, one approval, one rollback, one validation checklist, and execution allowed false. Recommend Guarded Apply Candidate MVP only if gaps are cleared." },
