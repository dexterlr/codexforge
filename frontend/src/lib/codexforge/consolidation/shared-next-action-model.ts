...
const ACTIONS: readonly Omit<SharedNextAction, "id">[] = [
  ...
  { label: "Inspect Files", reason: "File workflow before abstract dashboards.", priority: 50, routeHref: "/files", copyPrompt: "Prepare Phase 56 Real Local Project Reader. Keep file workflow read-only until approval boundaries exist." },
  ...
  { label: "Phase 56 Real Local Project Reader", reason: "If clean, move to the real file workflow.", priority: 70, routeHref: "/files", copyPrompt: "Proceed to Phase 56 Real Local Project Reader: build read-only local file discovery, project tree, file preview, and safe context handoff." },
];
...
export function summarizeSharedNextActionModel(model: Pick<SharedNextActionModel, "selected" | "candidates">): string[] {
  return [
    ...
    "If clean, recommend commit checkpoint or Phase 56 Real Local Project Reader.",
  ];
}
