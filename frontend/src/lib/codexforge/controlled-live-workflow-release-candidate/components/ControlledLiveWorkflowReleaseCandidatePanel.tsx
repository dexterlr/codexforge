    ...
    >
      <PreviewFoundationHero
        ...
        links={[
          ...
          { href: "/live-workflow-operator-feedback-review", label: "Feedback review" },
          { href: "/first-controlled-live-workflow-replay", label: "Replay review" },
        ]}
      />
      <PreviewFoundationSafetyStrip items={model.reviewLanguage} />
      <PreviewFoundationCard title="Plain-English Controlled live workflow release candidate">
        <PreviewFoundationCopy>{model.summary}</PreviewFoundationCopy>
        <PreviewFoundationCopy>
          This page reviews controlled live workflow candidate identity, evidence status, result review status, feedback status, approval gate status, denied release paths, unresolved release blockers, replay route, regression matrix route, and next recommended action. It does not go live, execute workflows, call providers, call local models, call connectors, create automations, approve actions, store outputs, mutate files, or mutate memory.
        </PreviewFoundationCopy>
      </PreviewFoundationCard>
      <section id="controlled-live-workflow-release-candidate" style={previewStyles.grid}>
        {model.reviews.map((review) => (
          <PreviewFoundationCard
            key={buildControlledLiveWorkflowReleaseCandidateStableKey("controlled-live-workflow-release-candidate-card", review.id)}
            title={review.controlledLiveWorkflowCandidateIdentity}
          >
            <PreviewFoundationPillList
              items={[
                `Status: ${review.status}`,
                `Evidence status: ${review.evidenceStatus.join("; ")}`,
                `Result review status: ${review.resultReviewStatus.join("; ")}`,
                `Feedback status: ${review.feedbackStatus.join("; ")}`,
                `Approval gate status: ${review.approvalGateStatus.join("; ")}`,
                `Denied release paths: ${review.deniedReleasePaths.join("; ")}`,
                `Unresolved release blockers: ${review.unresolvedReleaseBlockers.join("; ")}`,
                review.replayRoute,
      ...
      <PreviewFoundationDetail summary="Advanced release candidate details">
        ...
        <PreviewFoundationPillList items={model.reviews.map((review) => review.advancedReleaseCandidateDetails)} />
        <PreviewFoundationCopy>
          Advanced details stay collapsed or secondary. Controlled live workflow release candidate remains separate from workflow execution, live workflow launch, provider calls, local model calls, connector calls, automation creation, approval persistence, output storage, evidence/result/feedback ingestion, file mutation, memory mutation, tools, agents, plugins, and MCP runtimes.
        </PreviewFoundationCopy>
      </PreviewFoundationDetail>
    </div>
  );
