...
import type { RegressionPreviewHandoff } from "../regression-triage-types";

export function RegressionPreviewHandoffPanel({
  handoff,
  onCopyPrompt,
  onUsePrompt,
}: {
  handoff: RegressionPreviewHandoff;
  onCopyPrompt?: (prompt: string) => void;
  onUsePrompt?: (prompt: string) => void;
}) {
  return (
    <section data-codexforge-regression-preview-handoff-panel="RegressionPreviewHandoffPanel renders preview diff only no file writes without approval no command execution without approval" style={card}>
      <h3 style={title}>Safe Patch Preview handoff</h3>
      <p style={body}>Copy triage prompt only. This panel does not auto-run, auto-fix, auto-rollback, or write files.</p>
      <pre style={preview}>{handoff.prompt}</pre>
      <div style={actions}>
        <button type="button" style={button} onClick={() => onUsePrompt?.(handoff.prompt)}>
          Use triage prompt
        </button>
        <button type="button" style={secondary} onClick={() => onCopyPrompt?.(handoff.prompt)}>
          Copy triage prompt
        </button>
      </div>
    </section>
...
const preview: CSSProperties = { margin: 0, whiteSpace: "pre-wrap", maxHeight: 220, overflow: "auto", border: "1px solid rgba(148,163,184,0.14)", borderRadius: 8, padding: 10, fontSize: 11, lineHeight: 1.45 };
