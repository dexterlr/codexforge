  ...
  return (
    <section style={vrCard} data-codexforge-validation-output-capture-panel="ValidationOutputCapturePanel renders output capture supports stdout excerpt supports stderr excerpt caps output length manual paste supported no auto-persistence">
      ...
      <textarea
        ...
        rows={7}
        value={manualOutput}
        onChange={(event) => onManualOutputChange(event.target.value)}
        placeholder="Paste validation output to continue"
        aria-label="Paste validation output to continue"
        data-codexforge-validation-output-overflow="overflowX auto safe wrapping no command execution no file writes"
      />
      <ul style={vrList}>
