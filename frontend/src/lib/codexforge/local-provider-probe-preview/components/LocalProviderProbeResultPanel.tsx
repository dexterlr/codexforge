...
import type { LocalProviderProbeResult } from "../local-provider-probe-types";
export function LocalProviderProbeResultPanel({ results }: { results: LocalProviderProbeResult[] }) { return <article style={card}><span style={tag}>Results</span><h2 style={title}>Metadata-only meanings</h2>{results.map((result) => <p key={result.id} style={copy}><strong>{result.targetId}</strong>: {result.status}. {result.meaning}</p>)}</article>; }
