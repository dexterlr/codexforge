...
export function LocalProviderProbeSafetyPanel({ safety }: { safety: LocalProviderProbeSafety }) { return <article style={card}><span style={tag}>Safety</span><h2 style={title}>Probe boundary</h2><ul style={list}>{safety.rules.map((rule) => <li key={rule}>{rule}</li>)}</ul><p style={copy}>Blocked: {safety.blocked.join(", ")}.</p></article>; }
