...
export const panel: CSSProperties = {
  ...
  background: "linear-gradient(145deg, rgba(8,18,34,0.94), rgba(3,7,18,0.76))",
  borderRadius: 8,
  padding: 16,
  display: "grid",
  gap: 12,
...
export const codeBlock: CSSProperties = { margin: 0, whiteSpace: "pre-wrap", overflowWrap: "anywhere", color: "#dbeafe", fontSize: 12, lineHeight: 1.5, maxHeight: 360, overflow: "auto", fontFamily: "var(--font-geist-mono), ui-monospace, SFMono-Regular, monospace" };

export function postureColor(posture: string): string {
  if (posture === "ready" || posture === "info") return "#86efac";
  if (posture === "blocked" || posture === "blocker") return "#fca5a5";
  if (posture === "risk") return "#fdba74";
  if (posture === "warning" || posture === "review") return "#fde68a";
  return "#cbd5e1";
}
