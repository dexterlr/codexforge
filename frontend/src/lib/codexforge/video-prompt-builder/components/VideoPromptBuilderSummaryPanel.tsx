...
import { summarizeVideoPromptBuilder } from "../video-prompt-builder-summary";

export function VideoPromptBuilderSummaryPanel({ summary }: { summary: VideoPromptBuilderSummary }) {
  return <article style={card}><h2 style={title}>Builder summary</h2><p style={copy}>{summary.summary}</p><p style={copy}>{summarizeVideoPromptBuilder(summary)}</p></article>;
}
