...
const COMMON_DENIED_ACTIONS = [
  "No route deletion, smoke deletion, hidden execution, broker connection, trade placement, money movement, model call, provider call, connector call, prompt sending, command execution, file mutation, approval persistence, evidence persistence, audit persistence, queue creation, transaction creation, worker dispatch, runtime start, process spawn, port bind, install, deploy, localhost probing, credential storage, or browser storage write from the cockpit.",
] as const;
...
const TRADING_WORKSPACE_HUB = createSection({
  ...
  humanReadableSummary:
    "Trading Workspace groups Trading Research, Mandate / Risk Governor, Strategy Lab / Signal Engine, Backtest / Paper Trading, Profit Lockbox / Reinvestment Rules, Broker Execution Boundary previews, and Paper Broker Adapter Simulator previews behind one user-facing section.",
  featureLabels: ["Trading Research", "Mandate / Risk Governor", "Strategy Lab / Signal Engine", "Backtest / Paper Trading", "Profit Lockbox / Reinvestment Rules", "Broker Execution Boundary", "Paper Broker Adapter Simulator"],
  plannedInputs: ["Research notes", "Risk governor posture", "Strategy hypothesis", "Backtest readiness", "Profit lockbox rules", "Broker boundary status", "Paper broker simulator status"],
  plannedOutputs: ["Trading Workspace", "Grouped trading diagnostics", "Broker boundary summary", "Paper broker simulator summary", "No live trading", "No financial advice"],
  checklistPrefix: "trading-workspace-hub",
  checklistSummary:
    "Trading workspace hub preview groups trading research mandate risk governor strategy lab backtest paper trading profit lockbox broker execution boundary and paper broker adapter simulator into one user-facing trading workspace.",
...
const PHASE_ROUTE_GROUPING = createSection({
  ...
  featureLabels: ["Core Foundations", "Build Workspace", "Trading Research", "Mandate Risk Governor", "Strategy Lab Signal Engine", "Backtest Paper Trading", "Profit Lockbox Reinvestment", "Broker Execution Boundary", "Paper Broker Adapter Simulator", "Cockpit UX Diagnostics"],
...
const COCKPIT_STATUS_SUMMARY = createSection({
  ...
  humanReadableSummary:
    "Cockpit Status Summary shows current checkpoint through phase 1769, latest batch, latest release candidate, smoke posture, UX readiness, and blocked execution boundaries.",
...
const QUICK_ACTIONS: readonly CockpitNavigationCleanupItem[] = [
  ...
  { id: "open-trading-workspace", label: "Open Trading Workspace", detail: "Review trading research, risk, strategy, backtest, paper trading, profit lockbox, broker boundary, and paper broker simulator sections.", state: "user-facing" },
