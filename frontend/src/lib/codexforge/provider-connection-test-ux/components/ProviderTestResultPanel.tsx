...
import type { ProviderTestResult } from "../provider-connection-test-types";
export function ProviderTestResultPanel({ results }: { results: ProviderTestResult[] }) { return <article style={card}><span style={tag}>Results</span><h2 style={title}>No live call results</h2>{results.map((result) => <p key={result.id} style={copy}><strong>{result.planId}</strong>: {result.status}. {result.resultText}</p>)}</article>; }
