...
export function ProviderTestPlanPanel({ plans }: { plans: ProviderTestPlan[] }) { return <article style={card}><span style={tag}>Plans</span><h2 style={title}>Provider test plan</h2>{plans.map((plan) => <p key={plan.id} style={copy}><strong>{plan.provider}</strong>: {plan.scope}, {plan.status}. {plan.plainEnglish}</p>)}</article>; }
