...
const ACTIONS: readonly ProductReadinessNextAction[] = [
  ...
  { id: "review-local-bridge-health", label: "review Local Bridge Health", reason: "Local Bridge Health Check v1 is preview/manual-only and identifies missing config before future execution.", priority: 138, route: "/local-bridge-health", copyPrompt: "Review Local Bridge Health Check v1. Keep health checks preview-only/manual-only/future-guarded: no command execution, no local HTTP calls by default, no file writes, no render execution, and no local app launch. Next action can recommend Future Guarded Health Probe or Creative Execution Sandbox." },
] as const;
