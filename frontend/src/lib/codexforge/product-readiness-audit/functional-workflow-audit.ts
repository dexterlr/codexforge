...
const WORKFLOWS: readonly Omit<ProductFunctionalWorkflowItem, "id">[] = [
  { label: "project file read", status: "preview-only", missingPieces: ["clear read-only project file browser contract"], safetyBlockers: ["no source file mutation from UI"], nextFunctionalPhase: "Make Files real read-only workflow", routeEntryPoints: ["/files"], validationNeeded: ["Files smoke", "read-only safety assertions"] },
...
export function summarizeFunctionalWorkflowAudit(audit: Pick<ProductFunctionalWorkflowAudit, "items" | "functionalCount" | "blockedCount">): string[] {
  return [
    ...
    "Real workflow before abstract features is the next product rule.",
  ];
}
