...
export function selectGlobalActivityNextAction(events: readonly GlobalActivityEvent[]): GlobalActivityNextAction {
  ...
  if (ranked.some((event) => event.type === "brain.continuityReviewed" || event.source === "brain-continuity")) {
    return action("review Brain continuity", "Review Brain Continuity Dashboard posture without persistence.", "/brain-continuity", 94);
  }
