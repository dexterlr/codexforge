...
export function StoryboardSafetyPanel({ safety }: { safety: StoryboardSafety }) { return <article style={card}><h2 style={title}>Safety</h2><p style={copy}>{safety.plainEnglish}</p><p style={copy}>Nothing generated yet: yes. Workflow run allowed: no. Provider calls allowed: no. Approval required later: yes.</p></article>; }
