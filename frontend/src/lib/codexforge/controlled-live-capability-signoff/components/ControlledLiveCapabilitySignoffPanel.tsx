...
export function ControlledLiveCapabilitySignoffPanel() {
  ...
  return (
    ...
    >
      ...
      <PreviewFoundationCard title="Plain-English controlled live capability signoff">
        <PreviewFoundationCopy>{model.summary}</PreviewFoundationCopy>
        <PreviewFoundationCopy>
          This page reviews controlled live capability signoff identity, live capability groups,
          provider/local/connector/automation readiness checklist, approval/evidence/result/recovery readiness
          checklist, release readiness checklist, denied signoff actions, unresolved live capability blockers, Daily Beta
          release candidate route, Daily Beta controlled operator trial route, and next recommended action. It does not
          sign off automatically, go live, or execute workflows.
        </PreviewFoundationCopy>
      </PreviewFoundationCard>
      <section id="controlled-live-capability-signoff" style={previewStyles.grid}>
        {model.signoffs.map((signoff) => (
          <PreviewFoundationCard
            key={buildControlledLiveCapabilitySignoffStableKey("controlled-live-signoff-card", signoff.id)}
            title={signoff.controlledLiveCapabilitySignoffIdentity}
          >
            <PreviewFoundationPillList
              items={[
                `Status: ${signoff.status}`,
                `Live capability groups: ${signoff.liveCapabilityGroups.join("; ")}`,
                `Provider/local/connector/automation readiness checklist: ${signoff.providerLocalConnectorAutomationReadinessChecklist.join("; ")}`,
                `Approval/evidence/result/recovery readiness checklist: ${signoff.approvalEvidenceResultRecoveryReadinessChecklist.join("; ")}`,
                `Release readiness checklist: ${signoff.releaseReadinessChecklist.join("; ")}`,
                `Denied signoff actions: ${signoff.deniedSignoffActions.join("; ")}`,
                `Unresolved live capability blockers: ${signoff.unresolvedLiveCapabilityBlockers.join("; ")}`,
                signoff.dailyBetaReleaseCandidateRoute,
                signoff.dailyBetaControlledOperatorTrialRoute,
                signoff.nextRecommendedAction,
              ]}
            />
          </PreviewFoundationCard>
        ))}
      ...
      <PreviewFoundationDetail summary="Advanced signoff details">
        ...
        <PreviewFoundationPillList items={model.signoffs.map((signoff) => signoff.advancedSignoffDetails)} />
        <PreviewFoundationCopy>
          Advanced signoff details stay collapsed or secondary. This route never signs off live capability
          automatically, goes live, executes workflows, persists approval decisions, calls providers, calls local
          models, calls connectors, creates automations, writes files, or mutates memory.
        </PreviewFoundationCopy>
      </PreviewFoundationDetail>
    </div>
  );
