...
export function DryRunInputPanel({ input, validation }: Props) {
  return (
    <section style={card} data-codexforge-apply-diff-dry-run-input="DryRunInputPanel renders">
      <div style={row}>
        <span style={eyebrow}>Dry Run Input</span>
      ...
      <dl style={facts}>
        ...
        <dt>Approval packet</dt>
        <dd>{input.approvalPacketId}</dd>
        <dt>Patch source</dt>
        <dd>{input.patchSourceState}</dd>
        <dt>Current files</dt>
      ...
      <ul style={list}>{summarizeApplyDiffDryRunInput(input).map((item) => <li key={item}>{item}</li>)}</ul>
...
const facts: CSSProperties = { display: "grid", gridTemplateColumns: "126px minmax(0, 1fr)", gap: "4px 8px", margin: 0, fontSize: 12, color: "#dbeafe" };
