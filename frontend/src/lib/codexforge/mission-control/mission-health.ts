...
export function buildMissionHealthReport(): MissionHealthReport {
  const dimensions: MissionHealthDimension[] = [
    ...
    buildHealthDimension("product-simplification", "Product Simplification readiness", "ready", "Real Workflow Wizard v1", "Product Simplification is the UX/product layer for /start, plain-English intent choices, compact safety badges, collapsed advanced details, and a next action recommending Real Workflow Wizard v1; no command execution, no file writes, and no Brain graph mutation."),
    ...
    buildHealthDimension("creative-execution-sandbox", "Creative Execution Sandbox readiness", "preview-only", "Run sandbox simulation", "Creative Execution Sandbox readiness is simulation-only for creative executor request, sandbox run simulation, fake output lifecycle, cancellation simulation, artifact capture placeholders, simulated logs, verification summary, and review board handoff; no real execution, no render execution, no command execution, no local HTTP calls, no provider calls, no file writes, and preserve latest-message authority."),
    buildHealthDimension("real-creative-executor-readiness", "Real Creative Executor Readiness Audit readiness", "preview-only", "Review creative readiness", "Real Creative Executor Readiness Audit readiness is audit-only for local bridge profiles, adapter allowlists, path boundaries, artifact output, dry-run evidence, approval, kill-switch posture, scorecard, and Phase 71 Future Guarded Health Probe packet; execution allowed false, no real execution, no render execution, no command execution, no local HTTP calls, no provider calls, no file writes, and preserve latest-message authority."),
    buildHealthDimension("future-guarded-health-probe", "Future Guarded Health Probe readiness", "preview-only", "Review guarded health probe", "Future Guarded Health Probe readiness is metadata/manual-first/request-ready for bridge health profile, explicit approval, allowlist, policy, preflight, guarded bridge, metadata-only result capture, and readiness update; real probes disabled by default, no creative job execution, no render execution, no arbitrary command, no arbitrary endpoint, no file writes, and preserve latest-message authority."),
    buildHealthDimension("real-creative-executor-mvp-design", "Real Creative Executor MVP Design readiness", "preview-only", "Review MVP candidate", "Real Creative Executor MVP Design readiness is design-only for choosing artifact-capture-only as the first safe MVP candidate, approval packet, output boundary, kill-switch posture, artifact review loop, and user-friendly flow; execution allowed false, no real execution, no render execution, no command execution, no local HTTP calls, no file writes, and preserve latest-message authority."),
    buildHealthDimension("local-bridge-health", "Local Bridge Health readiness", "preview-only", "Review local bridge health", "Local Bridge Health readiness is preview/manual-only/future-guarded for Blender, ComfyUI, Unreal, ffmpeg, artifact output root, local renderer, custom service, and manual export configuration; probes blocked by default, no command execution, no local HTTP calls by default, no file writes, and no render execution."),
    ...
    buildHealthDimension("task-activation", "Reviewed Task Activation readiness", "needs-review", "Activate reviewed task", "Accepted suggestions can become activation requests, policy-reviewed active plan previews, and visible copy-only Jarvis handoffs without hidden injection, auto-run, or file mutation."),
    ...
    buildHealthDimension("real-apply-guard-review", "Real Apply Guard Review readiness", "needs-review", "Review apply guard", "Real Apply Guard Review audits approval packet enforcement, diff boundaries, path boundaries, rollback confidence, command/write separation, validation requirement, failure routing, and go/no-go before any apply automation increase; executionAllowed false, no auto-apply, no auto-run, no direct UI apply/write/run, and preserve latest-message authority."),
    buildHealthDimension("guarded-apply-candidate", "Guarded Apply Candidate readiness", "needs-review", "Clear blocker gaps before implementation", "Guarded Apply Candidate Implementation Plan is a safety/reliability planning layer for one file, one diff, one explicit approval packet, one guarded apply request, rollback, separate validation, result handoff, and execution allowed false; clear blocker gaps before implementation."),
