...
export function buildMissionSurfaceRegistry(): MissionSurfaceRegistry {
  const surfaces: MissionSurface[] = [
    ...
    buildMissionSurface({
      id: "patch-preview",
      label: "Patch Preview and Grounded Fix Recommendation",
      href: "/files",
      status: "preview-ready",
      role: "Safe patch preview before any file mutation path, plus grounded fix recommendations that route edits to preview-only handoff.",
      riskPosture: "preview-only",
      nextAction: "Review grounded fix recommendation.",
      readiness: "preview-only",
      relatedPhase: "Safe Patch Preview and Grounded Fix Recommendation",
    }),
    buildMissionSurface({
      id: "chat",
