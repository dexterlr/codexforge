...
export function buildMissionSurfaceRegistry(): MissionSurfaceRegistry {
  const surfaces: MissionSurface[] = [
    ...
    buildMissionSurface({
      id: "files",
      label: "Project Reader + Real Patch Preview",
      href: "/files",
      status: "online",
      role: "Real Local Project Reader readiness plus Real Patch Preview readiness for file tree, read-only file preview, metadata, purpose, risk, deterministic search, preview-only unified diff artifacts, risk/tests/rollback, and copy-only Patch Application Gate handoff.",
      riskPosture: "preview-only",
      nextAction: "Prepare preview diff.",
      readiness: "preview-only",
      relatedPhase: "Phase 56 Real Local Project Reader and Phase 57 Real Patch Preview v1",
    }),
    buildMissionSurface({
      id: "capabilities",
      label: "Capabilities",
    ...
    buildMissionSurface({
      id: "patch-preview",
      label: "Real Patch Preview, Regression Fix Queue, Patch Preview Queue, Preview Diff Composer, Patch Application Gate, Apply-Diff Dry Run, Apply-Diff Execution Gate, and Grounded Fix Recommendation",
      href: "/files",
      status: "preview-ready",
      role: "Real Patch Preview v1 functional/preview-only before any file mutation path, grounded fix recommendations that route edits to preview-only handoff, queued patch preview items, Preview Diff Composer pseudo-diff packages, Patch Application Gate approval packets, Apply-Diff Dry Run result ledgers, and Apply-Diff Execution Gate request readiness.",
      riskPosture: "preview-only",
      nextAction: "Prepare preview diff.",
      readiness: "preview-only",
