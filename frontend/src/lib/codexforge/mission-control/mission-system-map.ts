...
export function buildMissionSystemMap(): MissionSystemMap {
  const nodes: MissionSystemNode[] = [
    ...
    { id: "snapshot-restore", label: "Snapshot Restore Gate", detail: "Preview-only restore approval request" },
    { id: "brain-continuity", label: "Brain Continuity Dashboard", detail: "Read-only continuity posture and next safe action" },
  ...
  const edges: MissionSystemEdge[] = [
    ...
    buildMissionSystemEdge("runtime-replay", "snapshot-restore", "Replay evidence can inform restore request preview", "preview-only"),
    buildMissionSystemEdge("brain-governance", "snapshot-restore", "Governance review gates restore approval preview", "readonly"),
    buildMissionSystemEdge("brain-continuity", "snapshot-restore", "Continuity posture informs restore review without execution", "readonly"),
    ...
    buildMissionSystemEdge("operator-home", "chat", "Home routes reviewed handoffs to Jarvis Chat", "readonly"),
    ...
    buildMissionSystemEdge("stabilization", "chat", "Stabilization evidence can be copied visibly into /jarvis", "readonly"),
    ...
    buildMissionSystemEdge("tasks", "chat", "Reviewed Task Activation hands off visibly to /jarvis", "preview-only"),
