...
import {
  buildDailyBetaOneControlledRolloutPlanModel,
  buildDailyBetaOneControlledRolloutPlanStableKey,
} from "@/lib/codexforge/daily-beta-1-controlled-rollout-plan";
...
const DAILY_BETA_ONE_CONTROLLED_ROLLOUT_PLAN_MARKERS = [
...
export function DailyBetaOneControlledRolloutPlanPanel() {
  const model = buildDailyBetaOneControlledRolloutPlanModel();

  return (
    <div
      style={previewStyles.shell}
      data-codexforge-daily-beta-1-controlled-rollout-plan={`${DAILY_BETA_ONE_CONTROLLED_ROLLOUT_PLAN_MARKERS} buildDailyBetaOneControlledRolloutPlanStableKey DailyBetaOneControlledRolloutPlanPanel route imports/renders main panel`}
    >
      <PreviewFoundationHero
        phase="Phase 519"
        title="Daily Beta 1 rollout"
        subtitle="Daily Beta 1 controlled rollout plan reviews rollout steps without executing rollout. Rollout actions require explicit operator approval, and unapproved rollout paths remain blocked."
        primary={{ href: "#daily-beta-1-controlled-rollout-plan", label: "Review rollout plan" }}
        links={[
      ...
      />
      <PreviewFoundationSafetyStrip items={model.rolloutPlanLanguage} />
      <PreviewFoundationCard title="Plain-English Daily Beta 1 controlled rollout plan">
        <PreviewFoundationCopy>{model.summary}</PreviewFoundationCopy>
        <PreviewFoundationCopy>
          This page reviews Daily Beta 1 rollout plan identity, rollout stage groups, operator cohort checklist,
          approval gate checklist, rollback checklist, monitoring/review checklist, denied rollout actions, unresolved
          rollout blockers, Daily Beta 1 rollout review route, Daily Beta 1 feedback inbox route, and next recommended
          action. It does not execute rollout, launch Daily Beta 1, send notifications, create automations, or schedule
          tasks.
        </PreviewFoundationCopy>
      </PreviewFoundationCard>
      <section id="daily-beta-1-controlled-rollout-plan" style={previewStyles.grid}>
        {model.plans.map((plan) => (
          <PreviewFoundationCard
            key={buildDailyBetaOneControlledRolloutPlanStableKey("daily-beta-1-rollout-plan-card", plan.id)}
            title={plan.dailyBetaOneRolloutPlanIdentity}
          >
            <PreviewFoundationPillList
              items={[
                `Status: ${plan.status}`,
                `Rollout stage groups: ${plan.rolloutStageGroups.join("; ")}`,
                `Operator cohort checklist: ${plan.operatorCohortChecklist.join("; ")}`,
                `Approval gate checklist: ${plan.approvalGateChecklist.join("; ")}`,
                `Rollback checklist: ${plan.rollbackChecklist.join("; ")}`,
                `Monitoring/review checklist: ${plan.monitoringReviewChecklist.join("; ")}`,
                `Denied rollout actions: ${plan.deniedRolloutActions.join("; ")}`,
                `Unresolved rollout blockers: ${plan.unresolvedRolloutBlockers.join("; ")}`,
                plan.dailyBetaOneRolloutReviewRoute,
                plan.dailyBetaOneFeedbackInboxRoute,
      ...
      <PreviewFoundationDetail summary="Advanced rollout plan details">
        <PreviewFoundationPillList items={model.advancedDetails} />
        <PreviewFoundationPillList items={model.plans.map((plan) => plan.advancedRolloutPlanDetails)} />
        <PreviewFoundationCopy>
          Advanced rollout plan details stay collapsed or secondary. This route never executes rollout, launches Daily
          Beta 1, sends notifications, creates automations, schedules tasks, creates background jobs, calls providers,
          calls connectors, stores rollout decisions, or stores outputs.
        </PreviewFoundationCopy>
      </PreviewFoundationDetail>
    </div>
  );
