...
import type { CodingFlowPrimaryActionFix } from "../coding-flow-ux-fix-types";

export function CodingFlowPrimaryActionFixPanel({ fixes }: { fixes: CodingFlowPrimaryActionFix[] }) {
  return (
    <section style={panel} data-codexforge-coding-flow-primary-action-fix-panel="CodingFlowPrimaryActionFixPanel renders primary action includes Continue code flow Copy UX fix checklist no auto-apply no auto-run">
      <h2 style={title}>Primary actions</h2>
      <div style={grid}>
        {fixes.map((fix) => (
          <article key={fix.actionId} style={itemStyle}>
            <span style={small}>{fix.route}</span>
            <strong>{fix.label}</strong>
            <span style={small}>{fix.reason}</span>
          </article>
        ))}
      </div>
    </section>
