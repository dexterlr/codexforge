...
import {
  buildCodexForgeDailyBetaOneCandidateModel,
  buildCodexForgeDailyBetaOneCandidateStableKey,
} from "@/lib/codexforge/codexforge-daily-beta-1-candidate";
...
const CODEXFORGE_DAILY_BETA_ONE_CANDIDATE_MARKERS = [
...
export function CodexForgeDailyBetaOneCandidatePanel() {
  const model = buildCodexForgeDailyBetaOneCandidateModel();

  return (
    <div
      style={previewStyles.shell}
      data-codexforge-daily-beta-1-candidate={`${CODEXFORGE_DAILY_BETA_ONE_CANDIDATE_MARKERS} buildCodexForgeDailyBetaOneCandidateStableKey CodexForgeDailyBetaOneCandidatePanel route imports/renders main panel`}
    >
      <PreviewFoundationHero
        phase="Phase 518"
        title="Daily Beta 1"
        subtitle="CodexForge Daily Beta 1 candidate summarizes named Daily Beta 1 readiness without going live. Daily Beta 1 release requires explicit operator approval, and unresolved Daily Beta 1 blockers stay blocked."
        primary={{ href: "#codexforge-daily-beta-1-candidate", label: "Review candidate" }}
        links={[
      ...
      />
      <PreviewFoundationSafetyStrip items={model.dailyBetaOneCandidateLanguage} />
      <PreviewFoundationCard title="Plain-English CodexForge Daily Beta 1 candidate">
        <PreviewFoundationCopy>{model.summary}</PreviewFoundationCopy>
        <PreviewFoundationCopy>
          This page reviews Daily Beta 1 candidate identity, hardening/docs/onboarding/signoff status, multi-workflow
          status, controlled live status, Daily Beta readiness checklist, denied Daily Beta 1 actions, unresolved Daily
          Beta 1 blockers, Daily Beta 1 rollout plan route, Daily Beta 1 rollout review route, and next recommended
          action. It does not go live, launch Daily Beta 1, execute workflows, or persist release settings.
        </PreviewFoundationCopy>
      </PreviewFoundationCard>
      <section id="codexforge-daily-beta-1-candidate" style={previewStyles.grid}>
        {model.candidates.map((candidate) => (
          <PreviewFoundationCard
            key={buildCodexForgeDailyBetaOneCandidateStableKey("daily-beta-1-candidate-card", candidate.id)}
            title={candidate.dailyBetaOneCandidateIdentity}
          >
            <PreviewFoundationPillList
              items={[
                `Status: ${candidate.status}`,
                `Hardening/docs/onboarding/signoff status: ${candidate.hardeningDocsOnboardingSignoffStatus.join("; ")}`,
                `Multi-workflow status: ${candidate.multiWorkflowStatus.join("; ")}`,
                `Controlled live status: ${candidate.controlledLiveStatus.join("; ")}`,
                `Daily Beta readiness checklist: ${candidate.dailyBetaReadinessChecklist.join("; ")}`,
                `Denied Daily Beta 1 actions: ${candidate.deniedDailyBetaOneActions.join("; ")}`,
                `Unresolved Daily Beta 1 blockers: ${candidate.unresolvedDailyBetaOneBlockers.join("; ")}`,
                candidate.dailyBetaOneRolloutPlanRoute,
                candidate.dailyBetaOneRolloutReviewRoute,
      ...
      <PreviewFoundationDetail summary="Advanced Daily Beta 1 candidate details">
        <PreviewFoundationPillList items={model.advancedDetails} />
        <PreviewFoundationPillList
          items={model.candidates.map((candidate) => candidate.advancedDailyBetaOneCandidateDetails)}
        />
        <PreviewFoundationCopy>
          Advanced Daily Beta 1 candidate details stay collapsed or secondary. This route never goes live, launches
          Daily Beta 1, executes workflows, persists release settings, calls providers, calls local models, calls
          connectors, creates automations, writes files, mutates memory, or stores outputs.
        </PreviewFoundationCopy>
      </PreviewFoundationDetail>
    </div>
  );
