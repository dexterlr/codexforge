...
export function LiveWorkflowRegressionMatrixPanel() {
  ...
  return (
    ...
    >
      ...
      <PreviewFoundationCard title="Plain-English Live workflow regression matrix">
        <PreviewFoundationCopy>{model.summary}</PreviewFoundationCopy>
        <PreviewFoundationCopy>
          This page reviews live workflow regression identity, regression groups, approval safety provider local connector automation checks, blocked regression gaps, denied regression actions, failure patch route, hardening pass route, and next recommended action. It does not run tests, execute workflows, apply patches, write files, call providers, call local models, call connectors, create automations, mutate files, or mutate memory.
        </PreviewFoundationCopy>
      </PreviewFoundationCard>
      <section id="live-workflow-regression-matrix" style={previewStyles.grid}>
        {model.reviews.map((review) => (
          <PreviewFoundationCard
            key={buildLiveWorkflowRegressionMatrixStableKey("live-workflow-regression-matrix-card", review.id)}
            title={review.liveWorkflowRegressionIdentity}
          >
            <PreviewFoundationPillList
              items={[
                `Status: ${review.status}`,
                `Regression groups: ${review.regressionGroups.join("; ")}`,
                `Approval safety provider local connector automation checks: ${review.approvalSafetyProviderLocalConnectorAutomationChecks.join("; ")}`,
                `Blocked regression gaps: ${review.blockedRegressionGaps.join("; ")}`,
