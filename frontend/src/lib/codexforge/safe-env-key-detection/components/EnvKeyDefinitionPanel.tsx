...
export function EnvKeyDefinitionPanel({ definitions }: { definitions: EnvKeyDefinition[] }) { return <article style={card}><span style={tag}>Definitions</span><h2 style={title}>Provider env names</h2>{definitions.map((definition) => <p key={definition.id} style={copy}><strong>{definition.keyName}</strong> - {definition.provider} {definition.kind}</p>)}</article>; }
