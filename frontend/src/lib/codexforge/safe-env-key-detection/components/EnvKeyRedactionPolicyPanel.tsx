...
export function EnvKeyRedactionPolicyPanel({ policy }: { policy: EnvKeyRedactionPolicy }) { return <article style={card}><span style={tag}>Redaction policy</span><h2 style={title}>Never show secrets</h2><ul style={list}>{policy.rules.map((rule) => <li key={rule}>{rule}</li>)}</ul><p style={copy}>Blocked: {policy.blocked.join(", ")}.</p></article>; }
