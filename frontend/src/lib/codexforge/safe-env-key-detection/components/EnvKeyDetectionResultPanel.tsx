...
import type { EnvKeyDetectionResult } from "../safe-env-key-types";
export function EnvKeyDetectionResultPanel({ results }: { results: EnvKeyDetectionResult[] }) { return <article style={card}><span style={tag}>Safe result labels</span><h2 style={title}>Detection status</h2>{results.map((result) => <p key={result.id} style={copy}><strong>{result.keyName}</strong>: {result.state}. Value shown: {result.valueVisible ? "yes" : "no"}.</p>)}</article>; }
