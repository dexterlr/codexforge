    ...
    >
      <PreviewFoundationHero
        ...
        links={[
          ...
          { href: "/multi-workflow-release-candidate", label: "Multi-workflow RC" },
        ]}
      />
      <PreviewFoundationSafetyStrip items={model.dailyBetaReleaseCandidateLanguage} />
      <PreviewFoundationCard title="Plain-English CodexForge Daily Beta release candidate">
        <PreviewFoundationCopy>{model.summary}</PreviewFoundationCopy>
        <PreviewFoundationCopy>
          This page reviews Daily beta release candidate identity, multi-workflow status, controlled live signoff status,
          daily workflow readiness status, approval/safety readiness status, denied Daily Beta release actions,
          unresolved Daily Beta blockers, Daily Beta controlled operator trial route, Daily Beta feedback review route,
          and next recommended action. It does not go live, execute workflows, launch Daily Beta, or persist release
          settings.
        </PreviewFoundationCopy>
      </PreviewFoundationCard>
      <section id="codexforge-daily-beta-release-candidate" style={previewStyles.grid}>
        {model.candidates.map((candidate) => (
          <PreviewFoundationCard
            key={buildCodexForgeDailyBetaReleaseCandidateStableKey("daily-beta-rc-card", candidate.id)}
            title={candidate.dailyBetaReleaseCandidateIdentity}
          >
            <PreviewFoundationPillList
              items={[
                `Status: ${candidate.status}`,
                `Multi-workflow status: ${candidate.multiWorkflowStatus.join("; ")}`,
                `Controlled live signoff status: ${candidate.controlledLiveSignoffStatus.join("; ")}`,
                `Daily workflow readiness status: ${candidate.dailyWorkflowReadinessStatus.join("; ")}`,
                `Approval/safety readiness status: ${candidate.approvalSafetyReadinessStatus.join("; ")}`,
                `Denied Daily Beta release actions: ${candidate.deniedDailyBetaReleaseActions.join("; ")}`,
                `Unresolved Daily Beta blockers: ${candidate.unresolvedDailyBetaBlockers.join("; ")}`,
                candidate.dailyBetaControlledOperatorTrialRoute,
                candidate.dailyBetaFeedbackReviewRoute,
      ...
      <PreviewFoundationDetail summary="Advanced Daily Beta release candidate details">
        <PreviewFoundationPillList items={model.advancedDetails} />
        <PreviewFoundationPillList
          items={model.candidates.map((candidate) => candidate.advancedDailyBetaReleaseCandidateDetails)}
        />
        <PreviewFoundationCopy>
          Advanced Daily Beta release candidate details stay collapsed or secondary. This route never goes live, launches
          Daily Beta, executes workflows, persists release settings, calls providers, calls local models, calls
          connectors, creates automations, writes files, or mutates memory.
        </PreviewFoundationCopy>
      </PreviewFoundationDetail>
    </div>
  );
