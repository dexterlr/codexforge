...
import {
  buildPaperTradingEndToEndReviewRouteModel,
  buildPaperTradingEndToEndReviewStableKey,
  type PaperTradingEndToEndReviewItem,
  type PaperTradingEndToEndReviewRouteSlug,
  type PaperTradingEndToEndReviewSection,
  type PaperTradingEndToEndReviewState,
} from "../paper-trading-end-to-end-review-model";

export function PaperTradingEndToEndReviewPageClientShell({
  routeSlug,
}: {
  routeSlug: PaperTradingEndToEndReviewRouteSlug;
}) {
  const model = buildPaperTradingEndToEndReviewRouteModel(routeSlug);
  ...
  return (
    ...
    >
      <PaperTradingEndToEndReviewRoutePanel routeSlug={routeSlug} />
    </CodexForgeAppShell>
  );
}

export function PaperTradingEndToEndReviewCockpitSummaryPanel() {
  const model = buildPaperTradingEndToEndReviewRouteModel("cockpit-paper-trading-end-to-end-summary");
  const review = model.paperTradingEndToEndReview;
  const cockpitCards: readonly { section: PaperTradingEndToEndReviewSection; stateLabel: string }[] = [
    { section: review.paperTradingEndToEndBoundary, stateLabel: "Boundary" },
    { section: review.researchToMandateTrace, stateLabel: "Research" },
    { section: review.mandateToStrategyTrace, stateLabel: "Mandate" },
    { section: review.strategyToPaperAdapterTrace, stateLabel: "Adapter" },
    { section: review.paperAdapterToLedgerTrace, stateLabel: "Ledger" },
    { section: review.ledgerToReviewDashboardTrace, stateLabel: "Dashboard" },
    { section: review.reviewDashboardToChangeControlTrace, stateLabel: "Change" },
    { section: review.changeControlToVersionRegistryTrace, stateLabel: "Version" },
    { section: review.versionRegistryToPromotionGateTrace, stateLabel: "Promotion" },
    { section: review.promotionGateToPaperReviewTrace, stateLabel: "Paper Review" },
    { section: review.endToEndBlockerMap, stateLabel: "Blockers" },
    { section: review.operatorEndToEndReview, stateLabel: "Operator" },
    { section: review.noExecutionBridgeBoundary, stateLabel: "No Execution" },
  ];

  return (
    <section
      style={cockpitPanel}
      aria-label="Paper Trading End-to-End Review"
      data-codexforge-paper-trading-end-to-end-review={model.cockpitMarkers.join(" | ")}
    >
      <header style={hero}>
        <div style={eyebrowRow}>
          <span style={phaseBadge}>Paper Trading End-to-End Review</span>
          <span style={surfaceBadge}>Trading Workspace</span>
          <span style={approvalBadge}>Review-only synthetic candidate</span>
        </div>
        <h2 style={title}>Paper Trading End-to-End Review</h2>
        <p style={summary}>
          Safe review-only paper trading end-to-end review preview tying research to mandate, mandate to strategy,
          strategy to paper adapter, paper adapter to ledger, ledger to review dashboard, review dashboard to change
          control, change control to version registry, version registry to promotion gate, promotion gate to paper review,
          blocker map, operator review, no execution bridge, and denied paths. Synthetic data only. No financial advice
          from the cockpit, no personalised recommendations from the cockpit, no buy sell instructions from the cockpit,
          no strategy auto promotion from the cockpit, no strategy auto tuning from the cockpit, no automatic rule
          mutation from the cockpit, no frontend file mutation, no frontend approval persistence, no frontend version
          persistence, no frontend evidence persistence, no real P&amp;L analysis from the cockpit, no live market data
          calls from the cockpit, no order placement from the cockpit, no order dispatch from the cockpit, no broker
          execution from the cockpit, no paper execution from the cockpit, no live execution from the cockpit, no money
          movement from the cockpit, no trading automation from the cockpit, and no performance guarantees.
        </p>
      </header>

      <section style={summaryGrid} aria-label="Paper trading end-to-end cockpit summary">
        {review.cockpitSummary.map((item, index) => (
          <CheckRow
            key={buildPaperTradingEndToEndReviewStableKey([
              "paper-trading-end-to-end-cockpit-summary",
              String(index),
              item.id,
            ])}
            item={item}
      ...
      <section style={sectionGrid} aria-label="Paper trading end-to-end cockpit cards">
        {cockpitCards.map((card, index) => (
          <SummaryCard
            key={buildPaperTradingEndToEndReviewStableKey([
              "paper-trading-end-to-end-cockpit-card",
              String(index),
      ...
      <section style={noticeBand} aria-label="Paper trading end-to-end safety limits">
        {review.explicitSafetyLimits.map((limit, index) => (
          <span
            key={buildPaperTradingEndToEndReviewStableKey([
              "paper-trading-end-to-end-cockpit-limit",
              String(index),
              limit,
            ])}
            style={dangerChip}
      ...
      <a style={safeLink} href="/cockpit-paper-trading-end-to-end-summary">
        Review Paper Trading End-to-End Summary
      </a>
    </section>
  );
}

export function PaperTradingEndToEndReviewRoutePanel({
  routeSlug,
  embedded = false,
}: {
  routeSlug: PaperTradingEndToEndReviewRouteSlug;
  ...
  const model = buildPaperTradingEndToEndReviewRouteModel(routeSlug);
  const review = model.paperTradingEndToEndReview;
  const titleText = embedded ? "Paper Trading End-to-End Review" : model.route.title;

  return (
    <section
      style={embedded ? embeddedPage : page}
      data-codexforge-paper-trading-end-to-end-review-route={model.route.markerPhrases.join(" | ")}
    >
      <header style={hero}>
        <div style={eyebrowRow}>
          <span style={phaseBadge}>{embedded ? "Paper Trading End-to-End Review" : model.route.phase}</span>
        ...
        <p style={bodyText}>
          Paper Trading End-to-End Review v1 is review-only from the frontend. This is not financial advice,
          personalised recommendation, buy sell instruction, automated strategy optimisation, strategy auto-promotion,
          automatic rule mutation, live trading, paper order placement, paper execution, live execution, broker execution,
          real P&amp;L analysis, live market data, money movement, file mutation from the frontend, approval persistence
          from the frontend, evidence persistence from the frontend, or strategy version persistence from the frontend.
        </p>
        <p style={bodyText}>
      ...
      <section style={markerBand} aria-label="Paper trading end-to-end page markers">
        {model.route.markerPhrases.map((marker, index) => (
          <span
            key={buildPaperTradingEndToEndReviewStableKey([
              "paper-trading-end-to-end-route-marker",
              model.route.slug,
              String(index),
              marker,
            ])}
      ...
      <section style={identityBand} aria-label="Paper trading end-to-end review model fields">
        <div>
          <p style={panelEyebrow}>Paper trading end-to-end review model</p>
          <h2 style={sectionTitle}>paperTradingEndToEndReviewId: {review.paperTradingEndToEndReviewId}</h2>
        </div>
        <p style={bodyText}>paperTradingEndToEndReviewKind: {review.paperTradingEndToEndReviewKind}</p>
        <div style={chipRow}>
          {[
            ...
            <span
              key={buildPaperTradingEndToEndReviewStableKey([
                "paper-trading-end-to-end-field",
                String(index),
                field,
              ])}
              style={chip}
      ...
      <section style={sectionGrid} aria-label="Paper trading end-to-end review route sections">
        {model.sections.map((section, index) => (
          <SectionCard
            key={buildPaperTradingEndToEndReviewStableKey([
              "paper-trading-end-to-end-section",
              model.route.slug,
              String(index),
              section.sectionId,
            ])}
      ...
      <section style={splitBand} aria-label="Paper trading end-to-end summary and safety limits">
        <article style={panel}>
          ...
          <div style={checklistGrid}>
            {review.cockpitSummary.map((item, index) => (
              <CheckRow
                key={buildPaperTradingEndToEndReviewStableKey([
                  "paper-trading-end-to-end-route-summary",
                  String(index),
                  item.id,
                ])}
                item={item}
        ...
        <article style={panel}>
          ...
          <div style={chipRow}>
            {review.explicitSafetyLimits.map((limit, index) => (
              <span
                key={buildPaperTradingEndToEndReviewStableKey([
                  "paper-trading-end-to-end-route-limit",
                  String(index),
                  limit,
                ])}
                style={dangerChip}
      ...
      <section style={continuityBand} aria-label="Paper trading end-to-end continuity">
        <div style={panelHeader}>
          <div>
            <p style={panelEyebrow}>Continuity</p>
            <h3 style={sectionTitle}>
              Paper Trading End-to-End Review Research To Mandate Trace Mandate To Strategy Trace Strategy To Paper
              Adapter Trace Paper Adapter To Ledger Trace Ledger To Review Dashboard Trace Review Dashboard To Change
              Control Trace Change Control To Version Registry Trace Version Registry To Promotion Gate Trace Promotion
              Gate To Paper Review Trace End-to-End Blocker Map Operator End-to-End Review No Execution Bridge Boundary
            </h3>
          </div>
          <span style={stateStyle("backend-owned")}>Backend-owned paper workflow required</span>
        </div>
        <p style={bodyText}>
          Controlled paper trading end-to-end review release candidate prepares CodexForge for backend-owned paper trading
          end-to-end workflows without frontend mutation, version persistence, approval persistence, evidence persistence,
          ...
          instructions, or execution. Paper trading end-to-end review remains review-only. Synthetic data only.
        </p>
      </section>
      ...
      <details style={diagnosticsDrawer} open={!embedded}>
        ...
        <p style={bodyText}>
          Normal users continue to work from /codexforge-cockpit. Paper trading end-to-end review phase pages remain dev
        ...
        <div style={routeGrid}>
          {model.diagnosticRoutes.map((route, index) => (
            <a
              key={buildPaperTradingEndToEndReviewStableKey([
                "paper-trading-end-to-end-diagnostic-route",
                String(index),
                route.slug,
              ])}
              style={routeLink}
...
function SummaryCard({ section, stateLabel }: { section: PaperTradingEndToEndReviewSection; stateLabel: string }) {
  return (
    <article style={panel}>
      <div style={panelHeader}>
        <div>
      ...
      <div style={chipRow}>
        {section.plannedOutputs.slice(0, 4).map((output, index) => (
          <span
            key={buildPaperTradingEndToEndReviewStableKey([
              "paper-trading-end-to-end-summary-output",
              section.sectionId,
              String(index),
              output,
            ])}
...
function SectionCard({ section }: { section: PaperTradingEndToEndReviewSection }) {
  return (
    <article style={panel}>
      <div style={panelHeader}>
        <div>
      ...
      <div style={chipRow}>
        {section.deniedActions.map((action, index) => (
          <span
            key={buildPaperTradingEndToEndReviewStableKey([
              "paper-trading-end-to-end-denied-action",
              section.sectionId,
              String(index),
              action,
            ])}
      ...
      <div style={checklistGrid}>
        {section.checklist.map((item, index) => (
          <CheckRow
            key={buildPaperTradingEndToEndReviewStableKey([
              "paper-trading-end-to-end-section-check",
              section.sectionId,
              String(index),
              item.id,
            ])}
  ...
  return (
    <div style={fieldRow}>
      ...
      {values.map((value, index) => (
        <span
          key={buildPaperTradingEndToEndReviewStableKey([
            "paper-trading-end-to-end-field-list",
            label,
            String(index),
            value,
          ])}
...
function CheckRow({ item }: { item: PaperTradingEndToEndReviewItem }) {
  return (
    <div style={checkRow}>
...
function formatState(state: PaperTradingEndToEndReviewState): string {
...
function stateStyle(state: PaperTradingEndToEndReviewState): CSSProperties {
  return {
    ...stateBadge,
    ...(state === "blocked"
      ? blockedBadge
...
function smallStateStyle(state: PaperTradingEndToEndReviewState): CSSProperties {
  return {
    ...smallStateBadge,
    ...(state === "blocked"
      ? blockedBadge
