...
export const JARVIS_UNIFIED_PRODUCT_SURFACES = {
  home: {
    ...
    badge: "Start here",
    summary:
      "Athena is the main Jarvis control layer and helps you plan, route, review, and safely hand off AI work across CodexForge. Athena can now review the backend-owned minimal manual-gated text model adapter MVP. minimal text adapter review is preview-only. server-only text adapter helper exists. text adapter output is deterministic fixture output only. text adapter is not provider-capable yet. redacted prompt envelope is preview-only. prompt transmission state is not sent. no frontend request is created. no API route is created. no model calls yet. no prompt sending. no provider SDKs imported. queue, worker, and job execution remain blocked. result, audit, and approval persistence remain blocked. text adapter result capture MVP comes next.",
    summaryMarkers: [
  ...
  jarvis: {
    ...
    summary:
      "Athena Command Center is your upper Jarvis layer for preview-only planning, routing, review, and safe handoff across CodexForge. Athena can review the backend-owned minimal manual-gated text model adapter MVP. minimal text adapter review is preview-only. server-only text adapter helper exists. text adapter output is deterministic fixture output only. text adapter is not provider-capable yet. redacted prompt envelope is preview-only. prompt transmission state is not sent. no frontend request is created. no API route is created. no prompt sending. no model calls yet. no provider SDKs imported. no provider execution. no queue dispatch. no worker dispatch. no job execution. no result persistence. no audit persistence. no approval persistence. no database write. no file write. approval fixture is preview-only. manual confirmation fixture is preview-only. approval token is not issued. approval lease is not created. text adapter result capture MVP comes next. current readiness: minimal-text-adapter-review-only / backend-only / fixture-only / not provider-capable / not persistent. acceptance state: not accepted for live provider execution / text adapter fixture MVP accepted only. recovery is manual review only. retry disabled. fallback disabled.",
    summaryMarkers: [
