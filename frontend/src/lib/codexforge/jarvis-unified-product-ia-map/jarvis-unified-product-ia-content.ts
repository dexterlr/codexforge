...
export const JARVIS_UNIFIED_PRODUCT_SURFACES = {
  home: {
    ...
    badge: "Start here",
    summary:
      "Athena is the main Jarvis control layer and helps you plan, route, review, and safely hand off AI work across CodexForge. Athena can now preview the backend-owned minimal manual-gated text model adapter audit and approval join MVP. minimal text adapter audit and approval join MVP is backend-only. server-only text adapter audit and approval join helper exists. text adapter audit and approval join is produced in memory only. deterministic text adapter audit and approval join only. redacted prompt envelope is preview-only. prompt transmission state is not sent. no frontend request is created. no API route is created. no model calls yet. no prompt sending. no provider SDKs imported. queue, worker, and job execution remain blocked. result, audit, and approval persistence remain blocked. text adapter audit and approval join review and recovery preview comes next.",
    summaryMarkers: [
  ...
  jarvis: {
    ...
    summary:
      "Athena Command Center is your upper Jarvis layer for preview-only planning, routing, review, and safe handoff across CodexForge. Athena can preview the backend-owned minimal manual-gated text model adapter audit and approval join MVP. minimal text adapter audit and approval join MVP is backend-only. server-only text adapter audit and approval join helper exists. text adapter audit and approval join is produced in memory only. deterministic text adapter audit and approval join only. text adapter result capture is not persistent. redacted prompt envelope is preview-only. prompt transmission state is not sent. no frontend request is created. no API route is created. no prompt sending. no model calls yet. no provider SDKs imported. no provider execution. no queue dispatch. no worker dispatch. no job execution. no result persistence. no audit persistence. no approval persistence. no database write. no file write. approval fixture is preview-only. manual confirmation fixture is preview-only. approval token is not issued. approval lease is not created. current readiness: minimal-text-adapter-audit-approval-join-mvp-only / backend-only / fixture-only / in-memory-only / not persistent. text adapter audit and approval join review and recovery preview comes next.",
    summaryMarkers: [
      ...
      "text adapter audit and approval join is produced in memory only",
      "deterministic text adapter audit and approval join only",
      "current readiness: minimal-text-adapter-audit-approval-join-mvp-only / backend-only / fixture-only / in-memory-only / not persistent",
