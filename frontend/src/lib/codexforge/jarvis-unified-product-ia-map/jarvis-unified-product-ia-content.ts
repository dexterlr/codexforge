...
export const JARVIS_UNIFIED_PRODUCT_SURFACES = {
  home: {
    ...
    badge: "Start here",
    summary:
      "Athena is the main Jarvis control layer and helps you plan, route, review, and safely hand off AI work across CodexForge. Athena can now review the backend-owned minimal manual-gated provider adapter dry-run execution MVP. provider adapter dry-run execution review is preview-only. server-only provider dry-run execution helper exists. provider dry-run execution is deterministic fixture-only. dry-run fixture response is produced in memory only. live provider execution is blocked. credential reference is opaque label only. credential value is not present. credential value is not read. env vars are not read. provider key is not read. selected provider slot is preview-only. backup provider slot is preview-only. local/private alternative is preview-only. no frontend request is created. no API route is created. no model calls yet. no prompt sending. no provider SDKs imported. queue, worker, and job execution remain blocked. result, audit, and approval persistence remain blocked. provider adapter dry-run result capture MVP comes next.",
    summaryMarkers: [
  ...
  jarvis: {
    ...
    summary:
      "Athena Command Center is your upper Jarvis layer for preview-only planning, routing, review, and safe handoff across CodexForge. Athena can review the backend-owned minimal manual-gated provider adapter dry-run execution MVP. provider adapter dry-run execution review is preview-only. server-only provider dry-run execution helper exists. provider dry-run execution is deterministic fixture-only. dry-run fixture response is produced in memory only. live provider execution is blocked. credential reference is opaque label only. credential value is not present. credential value is not read. env vars are not read. provider key is not read. selected provider slot is preview-only. backup provider slot is preview-only. local/private alternative is preview-only. provider adapter dry-run execution is not live provider execution. no frontend request is created. no API route is created. no prompt sending. no model calls yet. no provider SDKs imported. no live provider execution. no queue dispatch. no worker dispatch. no job execution. no result persistence. no audit persistence. no approval persistence. no database write. no file write. approval fixture is preview-only. manual confirmation fixture is preview-only. approval token is not issued. approval lease is not created. current readiness: minimal-provider-dry-run-execution-review-only / backend-only / dry-run-fixture-only / credential-reference-only / not-live-provider-executing / not persistent. acceptance state: not accepted for live provider execution / provider dry-run execution fixture MVP accepted only. recovery is manual review only. retry disabled. fallback disabled. provider adapter dry-run result capture MVP comes next.",
    summaryMarkers: [
