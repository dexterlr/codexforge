...
export const ATHENA_PRODUCT_UX_POLISH_MODEL = {
  ...
  cockpitSummary:
    "Athena is the main Jarvis control layer. Athena is the main chat control layer. Athena can draft structured commands, prepare preview-only approval packets, review provider routing posture, inspect backend-owned run admission and dry-run runner layers, preview backend-owned synthetic dry-run result capture contracts, preview backend-owned synthetic dry-run audit and approval join contracts, preview backend-owned synthetic dry-run audit and approval join reviews, preview backend-owned synthetic dry-run end-to-end packet contracts, preview backend-owned synthetic dry-run end-to-end packet reviews, and now preview backend-owned synthetic dry-run manual approval handoff contracts, handoff packets, request/response contracts, approval scopes, handoff gates, readiness matrices, and evidence summaries. Athena can preview backend-owned synthetic dry-run manual approval handoff contracts. manual approval handoff contract is preview-only. handoff state: draft / preview-only. handoff request is not created. handoff invocation is not invoked. handoff response is not received. handoff error is not received. operator approval state: not requested. manual confirmation state: not captured. approval decision state: not evaluated. approval token is not issued. approval lease is not created. approval reference is not persisted. audit reference is not persisted. result reference is not persisted. evidence packet is preview-only. database write is not implemented. file write is not implemented. No prompt sending. No model calls yet. No provider SDKs imported. manual approval handoff review and recovery preview comes next. Provider execution is blocked. Queue dispatch is blocked. Worker dispatch is blocked. Job execution is blocked. Jarvis is the operating system / safety control plane, and specialist plugin pages stay preview-only tools Athena can route to and later control with approvals and backend gates.",
  ...
  nextOperatorActions: [
    ...
    "Review the backend-owned synthetic dry-run manual approval handoff contract, manual approval handoff packet, manual approval handoff request/response/error contracts, manual approval scope records, manual approval handoff gates, manual approval handoff readiness matrix, and manual approval handoff evidence summary before the backend-owned synthetic dry-run manual approval handoff review and recovery preview batch.",
  ],
...
export const ATHENA_CONTROL_PLANE_IDENTITY = {
  ...
  mission:
    "Athena is the main chat control layer above all specialist Jarvis workspaces and approval-gated product lanes. Athena is the main Jarvis control layer. Athena can draft structured commands from natural requests, prepare preview-only approval drafts, review routed work, preview AI model provider slots, compare capability families, review backend-owned run admission and dry-run runner layers, preview backend-owned synthetic dry-run runner skeletons, inspect backend-owned synthetic dry-run result capture contracts, inspect backend-owned synthetic dry-run audit and approval join contracts, inspect backend-owned synthetic dry-run audit and approval join reviews, inspect backend-owned synthetic dry-run end-to-end packet reviews, and now inspect backend-owned synthetic dry-run manual approval handoff contracts, manual approval handoff packets, request/response contracts, scope records, gate records, readiness matrices, evidence summaries, preview cross-workspace run timelines, show static audit memory previews, and safely hand off work across CodexForge.",
  operatorPromise:
    "Ask Athena to plan, route, review, and safely hand off work across CodexForge.",
  posture:
    "Athena conversational command composer, AI model provider registry, capability matrix, server-only model adapter contracts, backend-owned run admission previews, backend-owned dry-run runner previews, backend-owned synthetic dry-run runner skeleton previews, backend-owned synthetic dry-run result capture contract previews, backend-owned synthetic dry-run result capture review previews, backend-owned synthetic dry-run audit and approval join contract previews, backend-owned synthetic dry-run audit and approval join review previews, backend-owned synthetic dry-run end-to-end packet contract previews, synthetic end-to-end stage contract previews, synthetic end-to-end lineage previews, end-to-end packet request/response previews, end-to-end packet gate previews, end-to-end packet readiness previews, end-to-end packet acceptance posture previews, backend-owned synthetic dry-run end-to-end packet review previews, and backend-owned synthetic dry-run manual approval handoff contract previews remain preview-only. Command composer is preview-only. Approval drafts are preview-only. Provider slots are registry-only. Routing is preview-only. Provider selection is static preview only. Run admission is preview-only. Dry-run runner review is preview-only. manual approval handoff contract is preview-only. handoff state is draft / preview-only. handoff request is not created. handoff invocation is not invoked. handoff response is not received. handoff error is not received. operator approval state is not requested. manual confirmation state is not captured. approval decision state is not evaluated. acceptance state is not accepted / preview-only. Recovery is manual review only. Execution is blocked by default. Plugin registry is inert. Command router is preview-only. Chat input remains inert/local only and executes nothing.",
...
export const ATHENA_CHAT_PLACEHOLDER_MODEL = {
  ...
  helperText:
    "Chat input remains inert/local only. No prompt sending. No model calls yet. No provider SDKs imported. No frontend fetch/network call. Server-only adapter contracts are preview-only. Backend-owned run admission and dry-run runner layers are preview-only. Backend-owned synthetic dry-run manual approval handoff contract is preview-only. handoff state: draft / preview-only. handoff request is not created. handoff invocation is not invoked. handoff response is not received. handoff error is not received. operator approval state: not requested. manual confirmation state: not captured. approval decision state: not evaluated. approval token is not issued. approval lease is not created. approval reference is not persisted. audit reference is not persisted. result reference is not persisted. evidence packet is preview-only. database write is not implemented. file write is not implemented. current readiness: manual-approval-handoff-contract-only / not approved / not executable / not persistent. No browser storage. No persistent memory. No plugin execution from chat yet. Approval-gated handoffs only. Backend-only execution required. Audit required. Kill switch required. No queue dispatch. No worker dispatch. No job execution. No result persistence. manual approval handoff review and recovery preview comes next.",
  executionPosture:
    "Execution remains approval-gated, blocked by default, and backend-only. Command composer is preview-only. Approval drafts are preview-only. Adapter contracts are preview-only. Backend-owned run admission and dry-run runner layers are preview-only. Backend-owned synthetic dry-run runner skeleton is preview-only. Backend-owned synthetic dry-run end-to-end packet contract is preview-only. Backend-owned synthetic dry-run end-to-end packet review is preview-only. Backend-owned synthetic dry-run manual approval handoff contract is preview-only. Manual approval handoff packet is preview-only. Manual approval handoff request/response contract is preview-only. Manual approval scope is preview-only / not approved. Manual approval handoff gates are preview-only / blocked. Manual approval handoff readiness matrix is preview-only. Manual approval handoff evidence summary is preview-only. Audit joins are not persisted. Approval joins are not persisted. Result references are not persisted. Current readiness is manual-approval-handoff-contract-only / not approved / not executable / not persistent. Timeline is preview-only. Audit memory is static preview only. No autonomous execution.",
...
export const ATHENA_FUTURE_CAPABILITIES = [
  {
    id: "backend-owned-synthetic-dry-run-manual-approval-handoff-review-recovery-preview-next",
    label:
      "Backend-owned synthetic dry-run manual approval handoff review and recovery preview comes next",
    summary:
      "The next likely batch is 5290-5321 - Backend-Owned Synthetic Dry-Run Manual Approval Handoff Review and Recovery Preview.",
  },
  {
...
export const ATHENA_COMMAND_CENTER_MODEL = {
  batch:
    ATHENA_BACKEND_OWNED_SYNTHETIC_DRY_RUN_MANUAL_APPROVAL_HANDOFF_CONTRACT_BATCH,
  highestDetectedPhase:
    ATHENA_BACKEND_OWNED_SYNTHETIC_DRY_RUN_MANUAL_APPROVAL_HANDOFF_CONTRACT_PHASE,
  latestCompletedBatch:
    ATHENA_BACKEND_OWNED_SYNTHETIC_DRY_RUN_MANUAL_APPROVAL_HANDOFF_CONTRACT_BATCH,
  previousCompletedBatch:
    ATHENA_PREVIOUS_BACKEND_OWNED_SYNTHETIC_DRY_RUN_END_TO_END_PACKET_REVIEW_RECOVERY_PREVIEW_BATCH,
  nextLikelyBatch:
    ATHENA_NEXT_BACKEND_OWNED_SYNTHETIC_DRY_RUN_MANUAL_APPROVAL_HANDOFF_REVIEW_RECOVERY_PREVIEW_BATCH,
