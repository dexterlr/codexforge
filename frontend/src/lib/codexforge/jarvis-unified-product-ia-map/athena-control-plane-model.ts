...
export const ATHENA_PRODUCT_UX_POLISH_MODEL = {
  ...
  cockpitSummary:
    "Athena is the main Jarvis control layer. Athena is the main chat control layer. Athena can draft structured commands, prepare preview-only approval packets, review provider routing posture, inspect backend-owned run admission and dry-run runner layers, review backend-owned minimal synthetic execution previews, and now preview the backend-owned minimal manual-gated synthetic dry-run audit and approval join MVP. Athena can preview the backend-owned minimal manual-gated synthetic dry-run audit and approval join MVP. minimal synthetic audit and approval join MVP is backend-only. server-only synthetic audit and approval join helper exists. synthetic audit and approval join is produced in memory only. deterministic synthetic audit and approval join only. no frontend request is created. no API route is created. no real approval request. no real approval recording. approval fixture is preview-only. manual confirmation fixture is preview-only. approval token is not issued. approval lease is not created. provider response is not received. model output is not generated. No prompt sending. No model calls yet. No provider SDKs imported. no provider execution. no queue dispatch. no worker dispatch. no job execution. no result persistence. no audit persistence. no approval persistence. no database write. no file write. current readiness: minimal-synthetic-audit-approval-join-mvp-only / backend-only / in-memory-only / not provider-capable / not persistent. audit and approval join review and recovery preview comes next. Jarvis is the operating system / safety control plane, and specialist plugin pages stay preview-only tools Athena can route to and later control with approvals and backend gates.",
  ...
  nextOperatorActions: [
    ...
    "Review the backend-owned minimal manual-gated synthetic dry-run audit and approval join MVP, join input, join outputs, join envelope, join gates, join readiness matrix, and evidence preview before the backend-owned minimal manual-gated synthetic dry-run audit and approval join review and recovery preview batch.",
  ],
...
export const ATHENA_CHAT_PLACEHOLDER_MODEL = {
  ...
  helperText:
    "Chat input remains inert/local only. No prompt sending. No model calls yet. No provider SDKs imported. No frontend fetch/network call. Server-only adapter contracts are preview-only. Backend-owned run admission and dry-run runner layers are preview-only. Athena can preview the backend-owned minimal manual-gated synthetic dry-run audit and approval join MVP. minimal synthetic audit and approval join MVP is backend-only. server-only synthetic audit and approval join helper exists. synthetic audit and approval join is produced in memory only. deterministic synthetic audit and approval join only. no frontend request is created. no API route is created. approval fixture is preview-only. manual confirmation fixture is preview-only. approval token is not issued. approval lease is not created. provider response is not received. model output is not generated. database write is not implemented. file write is not implemented. current readiness: minimal-synthetic-audit-approval-join-mvp-only / backend-only / in-memory-only / not provider-capable / not persistent. audit and approval join review and recovery preview comes next. No browser storage. No persistent memory. No plugin execution from chat yet. Approval-gated handoffs only. Backend-only execution required. Audit required. Kill switch required. No queue dispatch. No worker dispatch. No job execution. No result persistence. No audit persistence. No approval persistence.",
  executionPosture:
    "Execution remains approval-gated, blocked by default, and backend-only. Command composer is preview-only. Approval drafts are preview-only. Adapter contracts are preview-only. Backend-owned run admission and dry-run runner layers are preview-only. Backend-owned minimal synthetic execution review is preview-only. Backend-owned minimal manual-gated synthetic dry-run result capture MVP is preview-only. Backend-owned minimal synthetic result capture review is preview-only. Backend-owned minimal manual-gated synthetic dry-run audit and approval join MVP is preview-only. Synthetic audit and approval join input, audit join output, approval join output, envelope, gates, readiness matrix, and evidence preview are preview-only. Result, audit, and approval references are not persisted. Current readiness is minimal-synthetic-audit-approval-join-mvp-only / backend-only / in-memory-only / not provider-capable / not persistent. Timeline is preview-only. Audit memory is static preview only. No autonomous execution.",
...
export const ATHENA_FUTURE_CAPABILITIES = [
  {
    id: "backend-owned-minimal-manual-gated-synthetic-dry-run-audit-and-approval-join-review-next",
    label:
      "Backend-owned minimal manual-gated synthetic dry-run audit and approval join review and recovery preview comes next",
    summary:
      "The next likely batch is 5546-5577 - Backend-Owned Minimal Manual-Gated Synthetic Dry-Run Audit and Approval Join Review and Recovery Preview.",
  },
  {
