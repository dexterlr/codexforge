...
} from "@/lib/codexforge/min-provider-admit/min-provider-admit-types";
...
export const ATHENA_PRODUCT_UX_POLISH_MODEL = {
  ...
  nextOperatorActions: [
    ...
    "Review the backend-owned minimal manual-gated provider adapter dry-run admission MVP, provider adapter dry-run admission input, provider adapter dry-run admission check, provider adapter dry-run admission output, provider adapter dry-run admission envelope, provider adapter dry-run intent preview, provider adapter dry-run blocked execution summary, provider adapter dry-run admission gates, provider adapter dry-run admission readiness matrix, and provider adapter dry-run admission evidence preview before the backend-owned minimal manual-gated provider adapter dry-run admission review and recovery preview batch.",
  ],
...
export const ATHENA_CURRENT_CAPABILITIES = [
  ...
  },
  {
    ...
    label:
      "Athena can now preview backend-owned provider dry-run admission",
    summary:
      "Athena can review deterministic fixture-only provider dry-run admission records with admitted-preview or blocked-preview posture while credential values, env vars, SDK imports, prompt sending, provider execution, and persistence all remain blocked.",
  },
  {
