...
export const ATHENA_PRODUCT_UX_POLISH_MODEL = {
  ...
  cockpitSummary:
    "Athena is the main Jarvis control layer. Athena is the main chat control layer. Athena can draft structured commands, prepare preview-only approval packets, review provider routing posture, inspect backend-owned run admission and dry-run runner layers, review backend-owned minimal synthetic execution previews, and now review the backend-owned minimal manual-gated synthetic dry-run result capture MVP. Athena can review the backend-owned minimal manual-gated synthetic dry-run result capture MVP. minimal synthetic result capture review is preview-only. server-only synthetic result capture helper exists. synthetic result capture is produced in memory only. deterministic synthetic capture only. no frontend request is created. no API route is created. no real approval request. no real approval recording. approval fixture is preview-only. manual confirmation fixture is preview-only. approval token is not issued. approval lease is not created. provider response is not received. model output is not generated. No prompt sending. No model calls yet. No provider SDKs imported. no provider execution. no queue dispatch. no worker dispatch. no job execution. no result persistence. no audit persistence. no approval persistence. no database write. no file write. current readiness: minimal-synthetic-result-capture-review-only / backend-only / in-memory-only / not provider-capable / not persistent. acceptance state: not accepted for live persistence / synthetic capture MVP accepted only. recovery is manual review only. retry disabled. fallback disabled. audit and approval join MVP comes next. Jarvis is the operating system / safety control plane, and specialist plugin pages stay preview-only tools Athena can route to and later control with approvals and backend gates.",
  ...
  nextOperatorActions: [
    ...
    "Review the backend-owned minimal synthetic result capture review, output review, gate failure review, recovery plan, recovery readiness, audit summary, and acceptance posture before the backend-owned minimal manual-gated synthetic dry-run audit and approval join MVP batch.",
  ],
...
export const ATHENA_CHAT_PLACEHOLDER_MODEL = {
  ...
  helperText:
    "Chat input remains inert/local only. No prompt sending. No model calls yet. No provider SDKs imported. No frontend fetch/network call. Server-only adapter contracts are preview-only. Backend-owned run admission and dry-run runner layers are preview-only. Athena can review the backend-owned minimal manual-gated synthetic dry-run result capture MVP. minimal synthetic result capture review is preview-only. server-only synthetic result capture helper exists. synthetic result capture is produced in memory only. deterministic synthetic capture only. no frontend request is created. no API route is created. approval fixture is preview-only. manual confirmation fixture is preview-only. approval token is not issued. approval lease is not created. provider response is not received. model output is not generated. database write is not implemented. file write is not implemented. current readiness: minimal-synthetic-result-capture-review-only / backend-only / in-memory-only / not provider-capable / not persistent. audit and approval join MVP comes next. No browser storage. No persistent memory. No plugin execution from chat yet. Approval-gated handoffs only. Backend-only execution required. Audit required. Kill switch required. No queue dispatch. No worker dispatch. No job execution. No result persistence. No audit persistence. No approval persistence.",
  executionPosture:
    "Execution remains approval-gated, blocked by default, and backend-only. Command composer is preview-only. Approval drafts are preview-only. Adapter contracts are preview-only. Backend-owned run admission and dry-run runner layers are preview-only. Backend-owned minimal synthetic execution review is preview-only. Backend-owned minimal manual-gated synthetic dry-run result capture MVP is preview-only. Backend-owned minimal synthetic result capture review is preview-only. Synthetic result capture output review, gate failure review, recovery plan, recovery readiness, review audit summary, and acceptance posture are preview-only. Result capture input, output, envelope, gates, readiness matrix, audit preview, approval preview, and evidence packet are preview-only. Result, audit, and approval references are not persisted. Current readiness is minimal-synthetic-result-capture-review-only / backend-only / in-memory-only / not provider-capable / not persistent. Timeline is preview-only. Audit memory is static preview only. No autonomous execution.",
...
export const ATHENA_FUTURE_CAPABILITIES = [
  {
    id: "backend-owned-minimal-manual-gated-synthetic-dry-run-audit-and-approval-join-mvp-next",
    label:
      "Backend-owned minimal manual-gated synthetic dry-run audit and approval join MVP comes next",
    summary:
      "The next likely batch is 5514-5545 - Backend-Owned Minimal Manual-Gated Synthetic Dry-Run Audit and Approval Join MVP.",
  },
  {
...
export const ATHENA_COMMAND_CENTER_MODEL = {
  batch:
    ATHENA_BACKEND_OWNED_MINIMAL_MANUAL_GATED_SYNTHETIC_DRY_RUN_RESULT_CAPTURE_REVIEW_RECOVERY_PREVIEW_BATCH,
  highestDetectedPhase:
    ATHENA_BACKEND_OWNED_MINIMAL_MANUAL_GATED_SYNTHETIC_DRY_RUN_RESULT_CAPTURE_REVIEW_RECOVERY_PREVIEW_PHASE,
  latestCompletedBatch:
    ATHENA_BACKEND_OWNED_MINIMAL_MANUAL_GATED_SYNTHETIC_DRY_RUN_RESULT_CAPTURE_REVIEW_RECOVERY_PREVIEW_BATCH,
  previousCompletedBatch:
    ATHENA_PREVIOUS_BACKEND_OWNED_MINIMAL_MANUAL_GATED_SYNTHETIC_DRY_RUN_RESULT_CAPTURE_MVP_BATCH,
  nextLikelyBatch:
    ATHENA_NEXT_BACKEND_OWNED_MINIMAL_MANUAL_GATED_SYNTHETIC_DRY_RUN_AUDIT_APPROVAL_JOIN_MVP_BATCH,
