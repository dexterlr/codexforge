...
} from "@/lib/codexforge/min-provider-select/min-provider-select-types";
...
export const ATHENA_PRODUCT_UX_POLISH_MODEL = {
  ...
  cockpitSummary:
    "Athena is the main Jarvis control layer. Athena is the main chat control layer. Athena can draft structured commands, prepare preview-only approval packets, review provider routing posture, inspect backend-owned run admission and dry-run runner layers, review backend-owned minimal synthetic execution previews, review backend-owned minimal manual-gated synthetic dry-run audit and approval join previews, review backend-owned minimal manual-gated synthetic dry-run end-to-end packet reviews, review backend-owned minimal manual-gated text model adapter audit and approval join MVP, and now preview the backend-owned minimal manual-gated provider adapter selection and credential reference MVP. Athena can now preview the backend-owned minimal manual-gated provider adapter selection and credential reference MVP. Athena can preview the backend-owned minimal manual-gated provider adapter selection and credential reference MVP. provider adapter selection is backend-only. server-only provider selection helper exists. provider slot selection is deterministic fixture-only. credential reference is opaque label only. credential value is not present. credential value is not read. env vars are not read. provider key is not read. selected provider slot is preview-only. backup provider slot is preview-only. local/private alternative is preview-only. provider adapter selection is not provider-capable yet. no frontend request is created. no API route is created. no real approval request. no real approval recording. approval fixture is preview-only. manual confirmation fixture is preview-only. approval token is not issued. approval lease is not created. provider response is not received. model output is not generated. No prompt sending. No model calls yet. No provider SDKs imported. no provider execution. no queue dispatch. no worker dispatch. no job execution. no result persistence. no audit persistence. no approval persistence. no database write. no file write. current readiness: minimal-provider-selection-credential-reference-mvp-only / backend-only / credential-reference-only / fixture-only / not provider-capable / not persistent. provider adapter selection and credential reference review and recovery preview comes next. Jarvis is the operating system / safety control plane, and specialist plugin pages stay preview-only tools Athena can route to and later control with approvals and backend gates.",
  ...
  nextOperatorActions: [
    ...
    "Review the backend-owned minimal manual-gated provider adapter selection and credential reference MVP, the provider adapter selection input, provider slot matrix, opaque credential reference, provider adapter selection output, provider adapter selection envelope, provider adapter selection gates, provider adapter selection readiness matrix, and provider adapter selection evidence preview before the backend-owned minimal manual-gated provider adapter selection and credential reference review and recovery preview batch.",
  ],
