...
} from "@/lib/codexforge/min-provider-capture/min-provider-capture-types";
...
export const ATHENA_PRODUCT_UX_POLISH_MODEL = {
  ...
  nextOperatorActions: [
    ...
    "Review the backend-owned minimal manual-gated provider adapter dry-run result capture MVP, provider adapter dry-run result capture input, provider adapter dry-run result capture check, provider adapter dry-run captured fixture result output, provider adapter dry-run result capture envelope, provider adapter dry-run result capture gates, provider adapter dry-run result capture readiness matrix, provider adapter dry-run result capture evidence preview, and provider adapter dry-run result capture blocked persistence summary before the backend-owned minimal manual-gated provider adapter dry-run result capture review and recovery preview batch.",
  ],
...
export const ATHENA_CHAT_PLACEHOLDER_MODEL = {
  ...
  helperText:
    "Chat input remains inert/local only. No prompt sending. No model calls yet. No provider SDKs imported. No frontend fetch/network call. Server-only adapter contracts are preview-only. Backend-owned run admission and dry-run runner layers are preview-only. Athena can preview the backend-owned minimal manual-gated provider adapter dry-run result capture MVP. provider adapter dry-run result capture is backend-only. server-only provider dry-run result capture helper exists. provider dry-run fixture response is captured in memory only. provider dry-run result capture is deterministic fixture-only. live provider execution is blocked. credential reference is opaque label only. credential value is not present. credential value is not read. env vars are not read. provider key is not read. selected provider slot is preview-only. backup provider slot is preview-only. local/private alternative is preview-only. no frontend request is created. no API route is created. no real approval request exists. no real approval recording exists. approval fixture is preview-only. manual confirmation fixture is preview-only. approval token is not issued. approval lease is not created. provider response is not received from provider. model output is not generated by provider/model. database write is not implemented. file write is not implemented. provider adapter dry-run result capture review and recovery preview comes next. current readiness: minimal-provider-dry-run-result-capture-mvp-only / backend-only / dry-run-fixture-capture-only / credential-reference-only / not-live-provider-executing / not persistent. retry disabled. fallback disabled. No browser storage. No persistent memory. No plugin execution from chat yet. Approval-gated handoffs only. Backend-only execution required. Audit required. Kill switch required. No queue dispatch. No worker dispatch. No job execution. No result persistence. No audit persistence. No approval persistence.",
  executionPosture:
    "Execution remains approval-gated, blocked by default, and backend-only. Command composer is preview-only. Approval drafts are preview-only. Adapter contracts are preview-only. Backend-owned run admission and dry-run runner layers are preview-only. Backend-owned minimal synthetic execution review is preview-only. Backend-owned minimal manual-gated synthetic dry-run result capture MVP is preview-only. Backend-owned minimal synthetic result capture review is preview-only. Backend-owned minimal manual-gated synthetic dry-run audit and approval join MVP is preview-only. Backend-owned minimal synthetic audit and approval join review is preview-only. Backend-owned minimal synthetic end-to-end packet review is preview-only. Backend-owned minimal manual-gated text model adapter MVP is preview-only. Backend-owned minimal text adapter review is preview-only. Backend-owned minimal manual-gated text model adapter result capture MVP is preview-only. Backend-owned minimal text adapter result capture review is preview-only. Backend-owned minimal manual-gated provider adapter dry-run result capture MVP is preview-only. Provider adapter dry-run result capture input, captured fixture result output, result capture envelope, result capture gates, result capture readiness matrix, evidence preview, and blocked persistence summary are preview-only. Provider dry-run fixture responses are captured in memory only, result, audit, and approval references are not persisted, and current readiness is minimal-provider-dry-run-result-capture-mvp-only / backend-only / dry-run-fixture-capture-only / credential-reference-only / not-live-provider-executing / not persistent. Timeline is preview-only. Audit memory is static preview only. No autonomous execution.",
