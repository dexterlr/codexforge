...
export const ATHENA_PRODUCT_UX_POLISH_MODEL = {
  ...
  cockpitSummary:
    "Athena is the main Jarvis control layer. Athena is the main chat control layer. Athena can draft structured commands, prepare preview-only approval packets, review provider routing posture, inspect backend-owned run admission and dry-run runner layers, review backend-owned minimal synthetic execution previews, review backend-owned minimal manual-gated synthetic dry-run audit and approval join previews, review backend-owned minimal manual-gated synthetic dry-run end-to-end packet reviews, and now preview the backend-owned minimal manual-gated text model adapter MVP. Athena can preview the backend-owned minimal manual-gated text model adapter MVP. minimal text model adapter MVP is backend-only. server-only text adapter helper exists. text adapter output is deterministic fixture output only. text adapter is not provider-capable yet. redacted prompt envelope is preview-only. prompt transmission state is not sent. no frontend request is created. no API route is created. no real approval request. no real approval recording. approval fixture is preview-only. manual confirmation fixture is preview-only. approval token is not issued. approval lease is not created. provider response is not received. model output is not generated. No prompt sending. No model calls yet. No provider SDKs imported. no provider execution. no queue dispatch. no worker dispatch. no job execution. no result persistence. no audit persistence. no approval persistence. no database write. no file write. current readiness: minimal-text-adapter-mvp-only / backend-only / fixture-only / not provider-capable / not persistent. recovery is manual review only. retry disabled. fallback disabled. text adapter review and recovery preview comes next. Jarvis is the operating system / safety control plane, and specialist plugin pages stay preview-only tools Athena can route to and later control with approvals and backend gates.",
...
export const ATHENA_CHAT_PLACEHOLDER_MODEL = {
  ...
  helperText:
    "Chat input remains inert/local only. No prompt sending. No model calls yet. No provider SDKs imported. No frontend fetch/network call. Server-only adapter contracts are preview-only. Backend-owned run admission and dry-run runner layers are preview-only. Athena can preview the backend-owned minimal manual-gated text model adapter MVP. minimal text model adapter MVP is backend-only. server-only text adapter helper exists. text adapter output is deterministic fixture output only. text adapter is not provider-capable yet. redacted prompt envelope is preview-only. prompt transmission state is not sent. no frontend request is created. no API route is created. approval fixture is preview-only. manual confirmation fixture is preview-only. approval token is not issued. approval lease is not created. provider response is not received. model output is not generated. database write is not implemented. file write is not implemented. current readiness: minimal-text-adapter-mvp-only / backend-only / fixture-only / not provider-capable / not persistent. text adapter review and recovery preview comes next. No browser storage. No persistent memory. No plugin execution from chat yet. Approval-gated handoffs only. Backend-only execution required. Audit required. Kill switch required. No queue dispatch. No worker dispatch. No job execution. No result persistence. No audit persistence. No approval persistence.",
  executionPosture:
    "Execution remains approval-gated, blocked by default, and backend-only. Command composer is preview-only. Approval drafts are preview-only. Adapter contracts are preview-only. Backend-owned run admission and dry-run runner layers are preview-only. Backend-owned minimal synthetic execution review is preview-only. Backend-owned minimal manual-gated synthetic dry-run result capture MVP is preview-only. Backend-owned minimal synthetic result capture review is preview-only. Backend-owned minimal manual-gated synthetic dry-run audit and approval join MVP is preview-only. Backend-owned minimal synthetic audit and approval join review is preview-only. Backend-owned minimal synthetic end-to-end packet review is preview-only. Backend-owned minimal manual-gated text model adapter MVP is preview-only. Text adapter input, redacted prompt envelope, deterministic fixture response, response envelope, gates, readiness matrix, evidence preview, audit preview, and approval preview are preview-only. Result, audit, and approval references are not persisted. Current readiness is minimal-text-adapter-mvp-only / backend-only / fixture-only / not provider-capable / not persistent. Timeline is preview-only. Audit memory is static preview only. No autonomous execution.",
