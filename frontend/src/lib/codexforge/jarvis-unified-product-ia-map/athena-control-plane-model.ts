...
export const ATHENA_PRODUCT_UX_POLISH_MODEL = {
  ...
  cockpitSummary:
    "Athena is the main Jarvis control layer. Athena is the main chat control layer. Athena can draft structured commands, prepare preview-only approval packets, review provider routing posture, inspect backend-owned run admission and dry-run runner layers, preview backend-owned synthetic dry-run result capture contracts, preview backend-owned synthetic dry-run audit and approval join contracts, preview backend-owned synthetic dry-run audit and approval join reviews, preview backend-owned synthetic dry-run end-to-end packet contracts, preview backend-owned synthetic dry-run end-to-end packet reviews, preview backend-owned synthetic dry-run manual approval handoff contracts and reviews, and now preview backend-owned synthetic dry-run manual approval decision contracts, decision packets, request/response contracts, approval outcome previews, decision gates, readiness matrices, and evidence summaries. Athena can preview backend-owned synthetic dry-run manual approval decision contracts. manual approval decision contract is preview-only. decision state: draft / preview-only / not evaluated. decision request is not created. decision invocation is not invoked. decision response is not received. decision error is not received. operator approval state: not requested. manual confirmation state: not captured. approval outcome state: not decided. approval token is not issued. approval lease is not created. approval reference is not persisted. audit reference is not persisted. result reference is not persisted. evidence packet is preview-only. database write is not implemented. file write is not implemented. No prompt sending. No model calls yet. No provider SDKs imported. manual approval decision review and recovery preview comes next. Provider execution is blocked. Queue dispatch is blocked. Worker dispatch is blocked. Job execution is blocked. Jarvis is the operating system / safety control plane, and specialist plugin pages stay preview-only tools Athena can route to and later control with approvals and backend gates.",
  ...
  safetyPostureSummary:
    "Athena conversational command composer, AI model provider registry, capability matrix, server-only model adapter contracts, adapter envelope previews, manual gated dry-run harness previews, model adapter dry-run result reviews, backend-owned run admission previews, backend-owned dry-run runner previews, backend-owned synthetic dry-run runner skeleton previews, backend-owned synthetic dry-run result capture contract previews, backend-owned synthetic dry-run result capture review and recovery previews, backend-owned synthetic dry-run audit and approval join contract previews, backend-owned synthetic dry-run audit and approval join review and recovery previews, backend-owned synthetic dry-run end-to-end packet contract previews, backend-owned synthetic dry-run end-to-end packet review and recovery previews, backend-owned synthetic dry-run manual approval handoff contract previews, backend-owned synthetic dry-run manual approval handoff review and recovery previews, and backend-owned synthetic dry-run manual approval decision contract previews remain frontend-safe and preview-only. manual approval decision contract is preview-only. decision state is draft / preview-only / not evaluated. decision request is not created. decision invocation is not invoked. decision response is not received. decision error is not received. operator approval state is not requested. manual confirmation state is not captured. approval outcome state is not decided. current readiness is manual-approval-decision-contract-only / not decided / not executable / not persistent. No prompt sending. No LLM/model calls. No frontend provider call. No frontend fetch/network call. No provider SDK imports. No provider execution. No plugin execution. No autonomous execution. No live video generation. No queue dispatch. No worker dispatch. No job execution. No browser storage. No persistent memory.",
  ...
  nextOperatorActions: [
    ...
    "Review the backend-owned synthetic dry-run manual approval decision contract, decision packet, request/response contract, approval outcome preview, decision gates, readiness matrix, and evidence summary before the backend-owned synthetic dry-run manual approval decision review and recovery preview batch.",
  ],
...
export const ATHENA_FUTURE_CAPABILITIES = [
  {
    id: "backend-owned-synthetic-dry-run-manual-approval-decision-review-next",
    label:
      "Backend-owned synthetic dry-run manual approval decision review and recovery preview comes next",
    summary:
      "The next likely batch is 5354-5385 - Backend-Owned Synthetic Dry-Run Manual Approval Decision Review and Recovery Preview.",
  },
  {
