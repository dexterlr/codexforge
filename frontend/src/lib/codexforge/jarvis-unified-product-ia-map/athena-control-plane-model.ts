...
} from "@/lib/codexforge/min-provider-exec/min-provider-exec-types";
...
export const ATHENA_PRODUCT_UX_POLISH_MODEL = {
  ...
  cockpitSummary:
    "Athena is the main Jarvis control layer. Athena can draft structured commands, prepare preview-only approval packets, review provider routing posture, inspect backend-owned run admission and dry-run runner layers, review provider adapter selection and credential reference previews, and now preview the backend-owned minimal manual-gated provider adapter dry-run execution MVP. provider adapter dry-run execution is backend-only. server-only provider dry-run execution helper exists. provider dry-run execution is deterministic fixture-only. dry-run fixture response is produced in memory only. live provider execution is blocked. credential reference is opaque label only. credential value is not present. credential value is not read. env vars are not read. provider key is not read. selected provider slot is preview-only. backup provider slot is preview-only. local/private alternative is preview-only. provider adapter dry-run execution is not live provider execution. no frontend request is created. no API route is created. no real approval request exists. no real approval recording exists. approval fixture is preview-only. manual confirmation fixture is preview-only. approval token is not issued. approval lease is not created. provider response is not received from provider. model output is not generated by provider/model. No prompt sending. No model calls yet. No provider SDKs imported. no live provider execution. no queue dispatch. no worker dispatch. no job execution. no result persistence. no audit persistence. no approval persistence. no database write. no file write. provider adapter dry-run execution review and recovery preview comes next. current readiness: minimal-provider-dry-run-execution-mvp-only / backend-only / dry-run-fixture-only / credential-reference-only / not-live-provider-executing / not persistent. Jarvis is the operating system / safety control plane, and specialist plugin pages stay preview-only tools Athena can route to and later control with approvals and backend gates.",
...
export const ATHENA_CHAT_PLACEHOLDER_MODEL = {
  ...
  helperText:
    "Chat input remains inert/local only. No prompt sending. No model calls yet. No provider SDKs imported. No frontend fetch/network call. Server-only adapter contracts are preview-only. Backend-owned run admission and dry-run runner layers are preview-only. Athena can now preview the backend-owned minimal manual-gated provider adapter dry-run execution MVP. provider adapter dry-run execution is backend-only. server-only provider dry-run execution helper exists. provider dry-run execution is deterministic fixture-only. dry-run fixture response is produced in memory only. live provider execution is blocked. credential reference is opaque label only. credential value is not present. credential value is not read. env vars are not read. provider key is not read. selected provider slot is preview-only. backup provider slot is preview-only. local/private alternative is preview-only. no frontend request is created. no API route is created. no real approval request exists. no real approval recording exists. approval fixture is preview-only. manual confirmation fixture is preview-only. approval token is not issued. approval lease is not created. provider response is not received from provider. model output is not generated by provider/model. database write is not implemented. file write is not implemented. provider adapter dry-run execution review and recovery preview comes next. current readiness: minimal-provider-dry-run-execution-mvp-only / backend-only / dry-run-fixture-only / credential-reference-only / not-live-provider-executing / not persistent. No browser storage. No persistent memory. No plugin execution from chat yet. Approval-gated handoffs only. Backend-only execution required. Audit required. Kill switch required. No queue dispatch. No worker dispatch. No job execution. No result persistence. No audit persistence. No approval persistence.",
  executionPosture:
