...
export const ATHENA_PRODUCT_UX_POLISH_MODEL = {
  ...
  cockpitSummary:
    "Athena is the main Jarvis control layer. Athena is the main chat control layer. Athena can draft structured commands, prepare preview-only approval packets, review provider routing posture, inspect backend-owned run admission and dry-run runner layers, preview backend-owned synthetic dry-run result capture contracts, preview backend-owned synthetic dry-run audit and approval join contracts, preview backend-owned synthetic dry-run audit and approval join reviews, preview backend-owned synthetic dry-run end-to-end packet contracts, preview backend-owned synthetic dry-run end-to-end packet reviews, preview backend-owned synthetic dry-run manual approval handoff contracts, handoff packets, request/response contracts, approval scopes, handoff gates, readiness matrices, and evidence summaries, and now preview backend-owned synthetic dry-run manual approval handoff reviews, decision reviews, gate failure reviews, recovery plans, recovery readiness, review audit summaries, and acceptance posture records. Athena can review why synthetic dry-run manual approval handoffs are held. manual approval handoff review is preview-only. handoff state: draft / preview-only. handoff request is not created. handoff invocation is not invoked. handoff response is not received. handoff error is not received. operator approval state: not requested. manual confirmation state: not captured. approval decision state: not evaluated. approval token is not issued. approval lease is not created. approval reference is not persisted. audit reference is not persisted. result reference is not persisted. evidence packet state: preview-only. database write is not implemented. file write is not implemented. No prompt sending. No model calls yet. No provider SDKs imported. manual approval decision contract comes next. Provider execution is blocked. Queue dispatch is blocked. Worker dispatch is blocked. Job execution is blocked. Jarvis is the operating system / safety control plane, and specialist plugin pages stay preview-only tools Athena can route to and later control with approvals and backend gates.",
  ...
  nextOperatorActions: [
    ...
    "Review the backend-owned synthetic dry-run manual approval handoff review, decision review, gate failure review, recovery plan, recovery readiness checklist, review audit summary, and acceptance posture before the backend-owned synthetic dry-run manual approval decision contract batch.",
  ],
...
export const ATHENA_CHAT_PLACEHOLDER_MODEL = {
  ...
  helperText:
    "Chat input remains inert/local only. No prompt sending. No model calls yet. No provider SDKs imported. No frontend fetch/network call. Server-only adapter contracts are preview-only. Backend-owned run admission and dry-run runner layers are preview-only. Backend-owned synthetic dry-run manual approval handoff review is preview-only. handoff state: draft / preview-only. handoff request is not created. handoff invocation is not invoked. handoff response is not received. handoff error is not received. operator approval state: not requested. manual confirmation state: not captured. approval decision state: not evaluated. approval token is not issued. approval lease is not created. approval reference is not persisted. audit reference is not persisted. result reference is not persisted. evidence packet is preview-only. database write is not implemented. file write is not implemented. current readiness: manual-approval-handoff-review-only / not approved / not executable / not persistent. No browser storage. No persistent memory. No plugin execution from chat yet. Approval-gated handoffs only. Backend-only execution required. Audit required. Kill switch required. No queue dispatch. No worker dispatch. No job execution. No result persistence. manual approval decision contract comes next.",
  executionPosture:
    "Execution remains approval-gated, blocked by default, and backend-only. Command composer is preview-only. Approval drafts are preview-only. Adapter contracts are preview-only. Backend-owned run admission and dry-run runner layers are preview-only. Backend-owned synthetic dry-run runner skeleton is preview-only. Backend-owned synthetic dry-run end-to-end packet contract is preview-only. Backend-owned synthetic dry-run end-to-end packet review is preview-only. Backend-owned synthetic dry-run manual approval handoff review is preview-only. Manual approval handoff decision review is preview-only. Manual approval handoff gate failure review is preview-only. Manual approval handoff recovery plan is preview-only. Manual approval handoff recovery readiness is preview-only. Manual approval handoff review audit summary is preview-only. Manual approval handoff acceptance posture is preview-only. Audit joins are not persisted. Approval joins are not persisted. Result references are not persisted. Current readiness is manual-approval-handoff-review-only / not approved / not executable / not persistent. Timeline is preview-only. Audit memory is static preview only. No autonomous execution.",
...
export const ATHENA_FUTURE_CAPABILITIES = [
  {
    id: "backend-owned-synthetic-dry-run-manual-approval-decision-contract-next",
    label:
      "Backend-owned synthetic dry-run manual approval decision contract comes next",
    summary:
      "The next likely batch is 5322-5353 - Backend-Owned Synthetic Dry-Run Manual Approval Decision Contract.",
  },
  {
...
export const ATHENA_COMMAND_CENTER_MODEL = {
  batch:
    ATHENA_BACKEND_OWNED_SYNTHETIC_DRY_RUN_MANUAL_APPROVAL_HANDOFF_REVIEW_RECOVERY_PREVIEW_BATCH,
  highestDetectedPhase:
    ATHENA_BACKEND_OWNED_SYNTHETIC_DRY_RUN_MANUAL_APPROVAL_HANDOFF_REVIEW_RECOVERY_PREVIEW_PHASE,
  latestCompletedBatch:
    ATHENA_BACKEND_OWNED_SYNTHETIC_DRY_RUN_MANUAL_APPROVAL_HANDOFF_REVIEW_RECOVERY_PREVIEW_BATCH,
  previousCompletedBatch:
    ATHENA_PREVIOUS_BACKEND_OWNED_SYNTHETIC_DRY_RUN_MANUAL_APPROVAL_HANDOFF_CONTRACT_BATCH,
  nextLikelyBatch:
    ATHENA_NEXT_BACKEND_OWNED_SYNTHETIC_DRY_RUN_MANUAL_APPROVAL_DECISION_CONTRACT_BATCH,
