...
export function buildBrainInspectionCommands(
  ...
  const commands: CodexForgeBrainCommand[] = [
    ...
    buildCommand({
      id: "brain.runtime-event-executor.review",
      label: "Review Runtime Event Executor",
      description: "Open the guarded runtime event executor review posture for approved memory.promoted requests; read-only command, no mutation.",
      category: "memory",
      kind: "inspect-memory",
      safety: "read-only",
      targetMode: "memory",
      keywords: ["runtime event executor", "memory.promoted", "approval", "no mutation"],
    }),
    buildCommand({
      id: "brain.runtime-event-executor.copy-approval-prompt",
      label: "Copy runtime event approval prompt",
      description: "Copy-only prompt pattern for reviewing runtime event request, policy, validation, approval, reducer preview, and append-only semantics.",
      category: "memory",
      kind: "inspect-memory",
      safety: "read-only",
      targetMode: "memory",
      keywords: ["copy", "runtime event approval prompt", "executor"],
    }),
    buildCommand({
      id: "brain.runtime-event-journal.open",
      label: "Go to Runtime Event Journal",
      description: "Open the read-only runtime event journal audit surface; no graph mutation and no event append from UI.",
      category: "memory",
      kind: "inspect-memory",
      safety: "read-only",
      targetMode: "memory",
      keywords: ["runtime event journal", "audit", "approval", "policy", "validation"],
    }),
    buildCommand({
      id: "brain.runtime-event-journal.copy-audit-prompt",
      label: "Copy runtime journal audit prompt",
      description: "Copy-only prompt pattern for reviewing Runtime Event Journal lifecycle visibility and append-only audit context.",
      category: "memory",
      kind: "inspect-memory",
      safety: "read-only",
      targetMode: "memory",
      keywords: ["copy", "runtime journal audit prompt", "append-only"],
    }),
    buildCommand({
