...
import {
  buildStrategyChangeControlWorkflowRouteModel,
  buildStrategyChangeControlWorkflowStableKey,
  type StrategyChangeControlWorkflowItem,
  type StrategyChangeControlWorkflowRouteSlug,
  type StrategyChangeControlWorkflowSection,
  type StrategyChangeControlWorkflowState,
} from "../strategy-change-control-workflow-model";

export function StrategyChangeControlWorkflowPageClientShell({
  routeSlug,
}: {
  routeSlug: StrategyChangeControlWorkflowRouteSlug;
}) {
  const model = buildStrategyChangeControlWorkflowRouteModel(routeSlug);
  ...
  return (
    ...
    >
      <StrategyChangeControlWorkflowRoutePanel routeSlug={routeSlug} />
    </CodexForgeAppShell>
  );
}

export function StrategyChangeControlWorkflowCockpitSummaryPanel() {
  const model = buildStrategyChangeControlWorkflowRouteModel("cockpit-strategy-change-control-summary");
  const workflow = model.strategyChangeControlWorkflow;

  return (
    <section
      style={cockpitPanel}
      aria-label="Strategy Change Control Workflow"
      data-codexforge-strategy-change-control-workflow={model.cockpitMarkers.join(" | ")}
    >
      <header style={hero}>
        <div style={eyebrowRow}>
          <span style={phaseBadge}>Strategy Change Control Workflow</span>
          <span style={surfaceBadge}>Trading Workspace</span>
          <span style={approvalBadge}>Review-only synthetic change control</span>
        </div>
        <h2 style={title}>Strategy Change Control Workflow</h2>
        <p style={summary}>
          Safe review-only strategy change control preview for proposed change intake, change rationale packet, linked
          evidence packet, risk impact assessment, mandate impact assessment, parameter change review, rule change review,
          strategy version draft, operator decision state, change rejection state, change approval boundary, no auto apply
          boundary, and denied paths. Synthetic data only. Review-only strategy change control. No financial advice from
          the cockpit, no personalised recommendations from the cockpit, no buy sell instructions from the cockpit, no
          strategy auto promotion from the cockpit, no strategy auto tuning from the cockpit, no automatic rule mutation
          from the cockpit, no frontend file mutation, no frontend approval persistence, no frontend evidence persistence,
          no real P&amp;L analysis from the cockpit, no live market data calls from the cockpit, no order placement from the
          cockpit, no order dispatch from the cockpit, no broker execution from the cockpit, no money movement from the
          cockpit, no trading automation from the cockpit, and no performance guarantees.
        </p>
      </header>

      <section style={summaryGrid} aria-label="Strategy change control workflow cockpit summary">
        {workflow.cockpitSummary.map((item, index) => (
          <CheckRow
            key={buildStrategyChangeControlWorkflowStableKey([
              "strategy-change-control-cockpit-summary",
              String(index),
              item.id,
            ])}
            item={item}
      ...
      <section style={sectionGrid} aria-label="Strategy change control workflow cockpit cards">
        <SummaryCard section={workflow.proposedChangeIntake} stateLabel="Intake" />
        <SummaryCard section={workflow.changeRationalePacket} stateLabel="Rationale" />
        <SummaryCard section={workflow.linkedEvidencePacket} stateLabel="Evidence" />
        <SummaryCard section={workflow.riskImpactAssessment} stateLabel="Risk" />
        <SummaryCard section={workflow.mandateImpactAssessment} stateLabel="Mandate" />
        <SummaryCard section={workflow.parameterChangeReview} stateLabel="Parameters" />
        <SummaryCard section={workflow.ruleChangeReview} stateLabel="Rules" />
        <SummaryCard section={workflow.strategyVersionDraft} stateLabel="Version" />
        <SummaryCard section={workflow.operatorDecisionState} stateLabel="Decision" />
        <SummaryCard section={workflow.changeRejectionState} stateLabel="Rejection" />
        <SummaryCard section={workflow.changeApprovalBoundary} stateLabel="Approval" />
        <SummaryCard section={workflow.noAutoApplyBoundary} stateLabel="No Auto Apply" />
      </section>

      <section style={noticeBand} aria-label="Strategy change control workflow safety limits">
        {workflow.explicitSafetyLimits.map((limit, index) => (
          <span
            key={buildStrategyChangeControlWorkflowStableKey([
              "strategy-change-control-cockpit-limit",
              String(index),
              limit,
            ])}
            style={dangerChip}
      ...
      <a style={safeLink} href="/cockpit-strategy-change-control-summary">
        Review Strategy Change Control Summary
      </a>
    </section>
  );
}

export function StrategyChangeControlWorkflowRoutePanel({
  routeSlug,
  embedded = false,
}: {
  routeSlug: StrategyChangeControlWorkflowRouteSlug;
  ...
  const model = buildStrategyChangeControlWorkflowRouteModel(routeSlug);
  const workflow = model.strategyChangeControlWorkflow;
  const titleText = embedded ? "Strategy Change Control Workflow" : model.route.title;

  return (
    <section
      style={embedded ? embeddedPage : page}
      data-codexforge-strategy-change-control-workflow-route={model.route.markerPhrases.join(" | ")}
    >
      <header style={hero}>
        <div style={eyebrowRow}>
          <span style={phaseBadge}>{embedded ? "Strategy Change Control" : model.route.phase}</span>
        ...
        <p style={bodyText}>
          Strategy Change Control Workflow v1 is review-only from the frontend. This is not financial advice,
          ...
          data, money movement, file mutation from the frontend, or approval persistence from the frontend.
        </p>
        <p style={bodyText}>
          Backend-owned change workflow remains required. Backend-owned evidence capture remains required.
          Backend-owned approval capture remains required. Operator review remains required. Risk governor approval
          remains required. Kill switch enforcement remains required. Explicit operator approval remains required.
        </p>
      </header>

      <section style={markerBand} aria-label="Strategy change control workflow page markers">
        {model.route.markerPhrases.map((marker, index) => (
          <span
            key={buildStrategyChangeControlWorkflowStableKey([
              "strategy-change-control-route-marker",
              model.route.slug,
              String(index),
              marker,
            ])}
      ...
      <section style={identityBand} aria-label="Strategy change control workflow model fields">
        <div>
          <p style={panelEyebrow}>Strategy change control workflow model</p>
          <h2 style={sectionTitle}>
            strategyChangeControlWorkflowId: {workflow.strategyChangeControlWorkflowId}
          </h2>
        </div>
        <p style={bodyText}>strategyChangeControlWorkflowKind: {workflow.strategyChangeControlWorkflowKind}</p>
        <div style={chipRow}>
          {[
            "strategyChangeControlWorkflowId",
            "strategyChangeControlWorkflowKind",
      ...
      <section style={sectionGrid} aria-label="Strategy change control workflow route sections">
        {model.sections.map((section, index) => (
          <SectionCard
            key={buildStrategyChangeControlWorkflowStableKey([
              "strategy-change-control-section",
              model.route.slug,
              String(index),
              section.sectionId,
            ])}
      ...
      <section style={splitBand} aria-label="Strategy change control workflow summary and safety limits">
        <article style={panel}>
          ...
          <div style={checklistGrid}>
            {workflow.cockpitSummary.map((item, index) => (
              <CheckRow
                key={buildStrategyChangeControlWorkflowStableKey([
                  "strategy-change-control-route-summary",
                  String(index),
                  item.id,
                ])}
                item={item}
        ...
        <article style={panel}>
          ...
          <div style={chipRow}>
            {workflow.explicitSafetyLimits.map((limit, index) => (
              <span
                key={buildStrategyChangeControlWorkflowStableKey([
                  "strategy-change-control-route-limit",
                  String(index),
                  limit,
                ])}
                style={dangerChip}
      ...
      <section style={continuityBand} aria-label="Strategy change control workflow continuity">
        <div style={panelHeader}>
          <div>
            <p style={panelEyebrow}>Continuity</p>
            <h3 style={sectionTitle}>
              Strategy Change Control Workflow Proposed Change Intake Change Rationale Packet Linked Evidence Packet Risk
              Impact Assessment Mandate Impact Assessment Parameter Change Review Rule Change Review Strategy Version
              Draft Operator Decision State Change Rejection State Change Approval Boundary No Auto Apply Boundary
            </h3>
          </div>
          <span style={stateStyle("backend-owned")}>Backend-owned change workflow required</span>
        </div>
        <p style={bodyText}>
          Controlled strategy change control workflow release candidate prepares CodexForge for backend-owned strategy
          change control workflows without frontend mutation, approval persistence, evidence persistence, auto tuning,
          strategy promotion, rule mutation, file writes, broker execution, order placement, order dispatch, live market
          data calls, real P&amp;L calculation, financial advice, personalised recommendations, buy sell instructions, or
          execution. Strategy change control remains review-only. Synthetic data only.
        </p>
      </section>
      ...
      <details style={diagnosticsDrawer} open={!embedded}>
        ...
        <p style={bodyText}>
          Normal users continue to work from /codexforge-cockpit. Strategy change control workflow phase pages remain
        ...
        <div style={routeGrid}>
          {model.diagnosticRoutes.map((route, index) => (
            <a
              key={buildStrategyChangeControlWorkflowStableKey([
                "strategy-change-control-diagnostic-route",
                String(index),
                route.slug,
              ])}
              style={routeLink}
...
function SummaryCard({ section, stateLabel }: { section: StrategyChangeControlWorkflowSection; stateLabel: string }) {
  return (
    <article style={panel}>
      <div style={panelHeader}>
        <div>
      ...
      <div style={chipRow}>
        {section.plannedOutputs.slice(0, 4).map((output, index) => (
          <span
            key={buildStrategyChangeControlWorkflowStableKey([
              "strategy-change-control-summary-output",
              section.sectionId,
              String(index),
              output,
            ])}
...
function SectionCard({ section }: { section: StrategyChangeControlWorkflowSection }) {
  return (
    <article style={panel}>
      <div style={panelHeader}>
        <div>
      ...
      <div style={chipRow}>
        {section.deniedActions.map((action, index) => (
          <span
            key={buildStrategyChangeControlWorkflowStableKey([
              "strategy-change-control-denied-action",
              section.sectionId,
              String(index),
              action,
            ])}
      ...
      <div style={checklistGrid}>
        {section.checklist.map((item, index) => (
          <CheckRow
            key={buildStrategyChangeControlWorkflowStableKey([
              "strategy-change-control-section-check",
              section.sectionId,
              String(index),
              item.id,
            ])}
  ...
  return (
    <div style={fieldRow}>
      ...
      {values.map((value, index) => (
        <span
          key={buildStrategyChangeControlWorkflowStableKey([
            "strategy-change-control-field-list",
            label,
            String(index),
            value,
          ])}
...
function CheckRow({ item }: { item: StrategyChangeControlWorkflowItem }) {
  return (
    <div style={checkRow}>
...
function formatState(state: StrategyChangeControlWorkflowState): string {
...
function stateStyle(state: StrategyChangeControlWorkflowState): CSSProperties {
  return {
    ...stateBadge,
    ...(state === "blocked"
      ? blockedBadge
...
function smallStateStyle(state: StrategyChangeControlWorkflowState): CSSProperties {
  return {
    ...smallStateBadge,
    ...(state === "blocked"
      ? blockedBadge
