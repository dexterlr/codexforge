...
type ExecuteToolRouteSuccess = {
  ...
  toolAdapter?: CodexForgeToolAdapterSummary;
...
function buildToolAdapterSummary(value: unknown) {
  const toolName = normalizeToolName(value);
  if (!toolName) return null;

  const toolAdapter = getCodexForgeToolAdapter(toolName);
  const toolAdapterSummary = toolAdapter
    ? {
        toolName: toolAdapter.toolName,
        kind: toolAdapter.kind,
        capability: toolAdapter.capability,
        executionMode: toolAdapter.executionMode,
        requiresApproval: toolAdapter.requiresApproval,
        blocksByDefault: toolAdapter.blocksByDefault,
        sideEffect: toolAdapter.sideEffect,
        adapter: toolAdapter.adapter,
      }
    : null;

  return toolAdapterSummary;
}

type CodexForgeToolAdapterSummary = ReturnType<typeof buildToolAdapterSummary>;
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const toolAdapterSummary = buildToolAdapterSummary(body.toolName ?? body.name);
    ...
    if (!toolPolicyDecision.allowed) {
      return json(toolPolicyDecision.status, {
        ...
        toolAdapter: toolAdapterSummary,
