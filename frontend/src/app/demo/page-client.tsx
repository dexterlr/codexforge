...
export default function DemoPageClient() {
  return <CodexForgeAppShell activePath="/demo" workspaceLabel="Demo the coding MVP" nextActionContext={{ hasRegressionOrFixWork: true, hasFileWork: true }} focusMode contentMaxWidth="wide" pageChrome="minimal" showRightRail={false} showSidebarBadges={false} showSidebarSafetyNotice={false} showHeroRouteChips={false}>
    <span hidden data-codexforge-demo-route="Operator Demo Mode route imports/renders OperatorDemoModePanel Demo the coding MVP Start demo What this proves What remains manual no auto-apply no auto-run approval required preserve latest-message authority Focus Mode UX calm workflow layout markers shell without duplicate route chip cloud route hero title does not vertically wrap no giant raw JSON above fold advanced details are collapsed or visually secondary no unsafe execution buttons" />
    <div style={linkRow}><Link href="/start" style={primaryHandoffLink}>Start</Link><Link href="/code-flow/mvp-path" style={handoffLink}>MVP path</Link><Link href="/code-flow/live-run" style={handoffLink}>Live run</Link><Link href="/code-flow/release-audit" style={handoffLink}>Release audit</Link></div>
