...
export default function DemoPageClient() {
  return <CodexForgeAppShell activePath="/demo" workspaceLabel="Demo the coding MVP" nextActionContext={{ hasRegressionOrFixWork: true, hasFileWork: true }} focusMode contentMaxWidth="wide" pageChrome="minimal" showRightRail={false} showSidebarBadges={false} showSidebarSafetyNotice={false} showHeroRouteChips={false}>
    <span hidden data-codexforge-demo-route="Operator Demo Mode route imports/renders OperatorDemoModePanel Demo the coding MVP Start demo Real Manual MVP Trial /code-flow/real-trial MVP Polish Demo Final /code-flow/final-polish What this proves What remains manual no auto-apply no auto-run approval required preserve latest-message authority Focus Mode UX calm workflow layout markers shell without duplicate route chip cloud route hero title does not vertically wrap no giant raw JSON above fold advanced details are collapsed or visually secondary no unsafe execution buttons" />
    <div style={linkRow}><Link href="/code-flow/real-trial" style={primaryHandoffLink}>Run real trial</Link><Link href="/code-flow/final-polish" style={handoffLink}>Final polish</Link><Link href="/code-flow/rc" style={handoffLink}>RC gate</Link><Link href="/code-flow/live-run" style={handoffLink}>Live run</Link></div>
