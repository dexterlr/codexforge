...
export default function DemoPageClient() {
  return <CodexForgeAppShell activePath="/demo" workspaceLabel="Demo the coding MVP" nextActionContext={{ hasRegressionOrFixWork: true, hasFileWork: true }} focusMode contentMaxWidth="wide" pageChrome="minimal" showRightRail={false} showSidebarBadges={false} showSidebarSafetyNotice={false} showHeroRouteChips={false}>
    <span hidden data-codexforge-demo-route="Operator Demo Mode route imports/renders OperatorDemoModePanel Demo the coding MVP Start demo Product Trial /product-trial First Task /first-task Operator Runbook /runbook MVP Experience Lock /mvp-experience Real Manual MVP Trial /code-flow/real-trial MVP Polish Demo Final /code-flow/final-polish What this proves What remains manual no demo result yet no auto-apply no auto-run approval required preserve latest-message authority Focus Mode UX calm workflow layout markers shell without duplicate route chip cloud route hero title does not vertically wrap no giant raw JSON above fold advanced details are collapsed or visually secondary no unsafe execution buttons" />
    <div style={linkRow}><Link href="/first-task" style={primaryHandoffLink}>First task path</Link><Link href="/product-trial" style={handoffLink}>Run product trial</Link><Link href="/onboarding" style={handoffLink}>Onboarding</Link><Link href="/runbook" style={handoffLink}>Runbook</Link><Link href="/mvp-experience" style={handoffLink}>MVP lock</Link><Link href="/code-flow/live-run" style={handoffLink}>Live run</Link></div>
