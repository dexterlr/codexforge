...
export default function GuardedApplyMvpPageClient() {
  return (
    <CodexForgeAppShell activePath="/guarded-apply-mvp" workspaceLabel="Guarded apply MVP" nextActionContext={{ hasRegressionOrFixWork: true, hasFileWork: true }} focusMode contentMaxWidth="wide" pageChrome="minimal" showRightRail={false} showSidebarBadges={false} showSidebarSafetyNotice={false} showHeroRouteChips={false}>
      <span hidden data-codexforge-guarded-apply-mvp-route="Guarded Apply MVP route imports/renders GuardedApplyMvpPanel Guarded apply MVP One file, one diff, one approval, then validate separately. Preview means nothing has changed yet. Apply means a file may change so approval is required. no patch preview yet no apply approval yet First Apply Trial Hardening /guarded-apply-mvp/trial-hardening blocked vs ready vs applied-supplied Guarded Apply MVP Hardening /guarded-apply-mvp/hardening blocked reasons approval messaging rollback checklist no fake apply success Review apply request no auto-apply no auto-run approval required preserve latest-message authority Focus Mode UX calm workflow layout markers shell without duplicate route chip cloud route hero title does not vertically wrap no giant raw JSON above fold advanced details are collapsed or visually secondary no unsafe execution buttons" />
      <div style={linkRow}>
        ...
        <Link href="/guarded-apply-mvp/trial-hardening" style={handoffLink}>Trial readiness</Link>
