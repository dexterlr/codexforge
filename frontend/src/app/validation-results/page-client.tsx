...
export default function ValidationResultsPageClient() {
  return <CodexForgeAppShell activePath="/validation-results" workspaceLabel="Capture validation result" nextActionContext={{ hasRegressionOrFixWork: true, hasFileWork: true }} focusMode contentMaxWidth="wide" pageChrome="minimal" showRightRail={false} showSidebarBadges={false} showSidebarSafetyNotice={false} showHeroRouteChips={false}>
    ...
    <div style={linkRow}><Link href="/validation" style={primaryHandoffLink}>Back to validation</Link><Link href="/workflow-results" style={handoffLink}>Workflow Results</Link><Link href="/run-history" style={handoffLink}>Run History</Link><Link href="/closed-loop" style={handoffLink}>Closed Loop</Link><Link href="/code-flow/friction-fixes" style={handoffLink}>Fix validation friction</Link><Link href="/code-flow/live-run" style={handoffLink}>Back to live run</Link></div>
