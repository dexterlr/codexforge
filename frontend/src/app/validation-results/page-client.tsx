...
export default function ValidationResultsPageClient() {
  return <CodexForgeAppShell activePath="/validation-results" workspaceLabel="Capture validation result" nextActionContext={{ hasRegressionOrFixWork: true, hasFileWork: true }} focusMode contentMaxWidth="wide" pageChrome="minimal" showRightRail={false} showSidebarBadges={false} showSidebarSafetyNotice={false} showHeroRouteChips={false}>
    ...
    <div style={linkRow}><Link href="/workflow-results" style={primaryHandoffLink}>Next: review result</Link><Link href="/review-inbox" style={handoffLink}>Review inbox</Link><Link href="/recovery" style={handoffLink}>Failed: recovery</Link><Link href="/runbook" style={handoffLink}>Recovery playbook</Link><Link href="/validation-results/hardening" style={handoffLink}>Capture guide</Link><Link href="/closed-loop" style={handoffLink}>Failed: closed loop</Link><Link href="/run-history" style={handoffLink}>Run history</Link></div>
