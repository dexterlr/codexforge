...
export default function ValidationResultsPageClient() {
  return <CodexForgeAppShell activePath="/validation-results" workspaceLabel="Capture validation result" nextActionContext={{ hasRegressionOrFixWork: true, hasFileWork: true }} focusMode contentMaxWidth="wide" pageChrome="minimal" showRightRail={false} showSidebarBadges={false} showSidebarSafetyNotice={false} showHeroRouteChips={false}>
    ...
    <div style={linkRow}><Link href="/workflow-results" style={primaryHandoffLink}>Next: review result</Link><Link href="/apply-evidence" style={handoffLink}>Back to apply evidence</Link><Link href="/run-history" style={handoffLink}>See run history</Link><Link href="/closed-loop" style={handoffLink}>Failed? Open Closed Loop</Link><Link href="/code-flow/live-run" style={handoffLink}>Back to live run</Link></div>
