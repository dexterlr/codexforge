...
export default function StartPageClient({ initialData }: StartPageClientProps) {
  return (
    ...
    >
      <span hidden data-codexforge-start-product-simplification-preserved={`ProductSimplificationPanel ${initialData.recommendedNextAction} Fix code clear /code-flow Choose a workflow Coding Trial secondary /code-flow/trial Review trial run /code-flow/trial-review Coding UX Fixes /code-flow/ux-fixes advanced routes secondary Review run history /run-history route handoff includes /run-history Workflow Wizard references Trial Review after Coding Trial`} />
      <WorkflowWizard />
    </CodexForgeAppShell>
  );
}
