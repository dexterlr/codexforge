...
export default function CodeFlowLiveRunPageClient() {
  return <CodexForgeAppShell activePath="/code-flow/live-run" workspaceLabel="Run the coding flow" nextActionContext={{ hasRegressionOrFixWork: true, hasFileWork: true }} focusMode contentMaxWidth="wide" pageChrome="minimal" showRightRail={false} showSidebarBadges={false} showSidebarSafetyNotice={false} showHeroRouteChips={false}>
    <span hidden data-codexforge-live-run-route="Coding Flow Live Run route imports/renders CodingFlowLiveRunPanel Run the coding flow Follow one safe path from file to validation result. no auto-apply no auto-run approval required preserve latest-message authority Focus Mode UX calm workflow layout markers shell without duplicate route chip cloud route hero title does not vertically wrap no giant raw JSON above fold advanced details are collapsed or visually secondary no unsafe execution buttons" />
    <div style={linkRow}><Link href="/code-flow/mvp-path" style={primaryHandoffLink}>MVP path</Link><Link href="/code-flow/manual-trial" style={primaryHandoffLink}>Run manual coding trial</Link><Link href="/code-flow" style={handoffLink}>Back to Code Flow</Link><Link href="/guarded-apply-mvp" style={handoffLink}>Guarded Apply MVP</Link><Link href="/apply-evidence" style={handoffLink}>Apply Evidence</Link><Link href="/validation-results" style={handoffLink}>Validation Result</Link><Link href="/code-flow/release-audit" style={handoffLink}>Release Audit</Link></div>
