...
export default function CodeFlowLiveRunPageClient() {
  return <CodexForgeAppShell activePath="/code-flow/live-run" workspaceLabel="Run the coding flow" nextActionContext={{ hasRegressionOrFixWork: true, hasFileWork: true }} focusMode contentMaxWidth="wide" pageChrome="minimal" showRightRail={false} showSidebarBadges={false} showSidebarSafetyNotice={false} showHeroRouteChips={false}>
    ...
    <div style={linkRow}><Link href="/files" style={primaryHandoffLink}>Next: pick a file</Link><Link href="/guarded-apply-mvp" style={handoffLink}>Review apply request</Link><Link href="/apply-evidence" style={handoffLink}>Capture apply evidence</Link><Link href="/validation-results" style={handoffLink}>Validate separately</Link><Link href="/workflow-results" style={handoffLink}>Review result</Link><Link href="/run-history" style={handoffLink}>See run history</Link><Link href="/code-flow/release-audit" style={handoffLink}>Release Audit</Link></div>
