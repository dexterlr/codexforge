...
export default function ApplyEvidencePageClient() {
  return <CodexForgeAppShell activePath="/apply-evidence" workspaceLabel="Capture apply evidence" nextActionContext={{ hasRegressionOrFixWork: true, hasFileWork: true }} focusMode contentMaxWidth="wide" pageChrome="minimal" showRightRail={false} showSidebarBadges={false} showSidebarSafetyNotice={false} showHeroRouteChips={false}>
    <span hidden data-codexforge-apply-evidence-route="Apply Evidence route imports/renders ApplyEvidenceCapturePanel Capture apply evidence Evidence is a note of what happened no evidence captured yet Copy evidence pack evidence completeness First Apply Trial Hardening /guarded-apply-mvp/trial-hardening no auto-apply no auto-run preserve latest-message authority Focus Mode UX calm workflow layout markers shell without duplicate route chip cloud route hero title does not vertically wrap no giant raw JSON above fold advanced details are collapsed or visually secondary no unsafe execution buttons Real Trial Friction Patch keeps evidence capture calm" />
    <div style={linkRow}><Link href="/validation-results" style={primaryHandoffLink}>Next: validate separately</Link><Link href="/safety-coach" style={handoffLink}>Evidence help</Link><Link href="/review-inbox" style={handoffLink}>Review inbox</Link><Link href="/recovery" style={handoffLink}>Blocked: recovery</Link><Link href="/guarded-apply-mvp/trial-hardening" style={handoffLink}>Readiness checklist</Link><Link href="/guarded-apply-mvp" style={handoffLink}>Apply request</Link><Link href="/workflow-results" style={handoffLink}>Review result</Link><Link href="/code-flow/live-run" style={handoffLink}>Live run</Link></div>
